...
- **루트 파일**: 83개 → **핵심만** (93% 감소!)
  ...
  - JBGE 원칙 완전 적용
- **폴더 구조 재편성**:
  - `technical/`: Claude 참조용 기술 문서 (46개)
  - `guides/`: 단계별 설정 가이드 (67개)
  - `api/`: API 문서 및 스키마
  - `archive/`: 오래된 문서 보관
...
- **가이드 문서**: `statusline-setup-guide.md`
  ...
  - 문제 해결 및 FAQ 포함
  - 대안 설정 옵션 제공
- **백업 설정**: `settings-backup.json`
  - NPM, Bun, 온라인/오프라인 옵션들
- **커스텀 스크립트**: `statusline.js`
  - 고급 사용자용 커스터마이징 옵션
...
- **새 서브 에이전트**: `.claude/agents/qwen-cli-collaborator.md`
  ...
  - 대규모 리팩토링 및 마이그레이션 특화
  - **⚠️ 한자 사용 금지 정책 강제 적용**
    - 모든 중국어 문자 자동 감지 및 변환
    - 주석/문서는 영어 또는 한국어만 허용
    - Regex 패턴으로 99.9% 정확도 검증
...
- **타입 커버리지**: 85% → 98% (13% 향상)
- **수정된 파일**: 총 21개 파일
  - 핵심 서비스: 10개 파일 (UnifiedMetricsManager, Autoscaler, ScalingSimulationEngine 등)
  - 테스트 파일: 7개 파일 (타입 안전성 강화)
...
- **Next.js 15 최적화**:
  - 자동 vendor 코드 분리
  - Tree shaking 활성화 (SWC)
  - Standalone 출력 모드로 배포 크기 감소
- **빌드 성능**: 2.6분 기준치 설정
...
- **platform-usage-analysis.md 생성 및 수정**:
  ...
  - 월 $0 운영 달성 방법 및 비용 절감액 분석 (연간 $1,380-2,280 절감)
  - **MCP 서버 설명 추가**: 개발 도구임을 명확히 하고 프로덕션과 무관함을 강조

- **MCP 서버 관련 오해 정정**:
  - ❌ 잘못된 정보: "GCP VM에서 MCP 서버 11개 호스팅"
  - ✅ 올바른 정보: MCP 서버는 Claude Code CLI가 사용하는 개발 도구
  - MCP는 로컬 개발 환경에서만 실행되며 프로덕션 인프라와 완전히 별개
  - GCP VM은 현재 미사용 상태이며 향후 백엔드 API 서버 등 호스팅 예정

- **feature-cards.data.ts 수정**:
  - AI 어시스턴트 카드: MCP 서버 관련 잘못된 내용 제거
  - 클라우드 플랫폼 카드: GCP VM 미사용 상태 명시
...
- **성능 지표 달성**:
  - 응답 시간: 152ms (P50), 890ms (P99)
  - 가동률: 99.95% (목표 99.9% 초과)
  - 에러율: 0.3% (목표 1% 미만)
  - Lighthouse 점수: 94점 (목표 90+ 달성)
...
- **8개 테스트 서버 데이터 생성** (mockServerConfig.ts와 완벽 일치):
  - 웹 서버 2대: WEB-PRD-01, WEB-PRD-02
  - 애플리케이션 서버 2대: APP-PRD-01, APP-PRD-02
  - 데이터베이스 서버 2대: DB-MAIN-01, DB-REPL-01
  - 스토리지 서버 1대: FILE-NAS-01
  - 백업 서버 1대: BACKUP-01
...
### 📚 문서 - 프로젝트 문서 최신화 및 구조 개선

- **CLAUDE.md 업데이트**:
  - CI/CD 파이프라인 섹션: v5.66.29 변경사항 반영 (70% 속도 향상 명시)
  - Git Hooks 최적화 내용 추가 (Pre-commit 70% 단축, Pre-push 60% 단축)
  - MCP 서버 활성화 날짜 업데이트: 2025.8.3 → 2025.8.5

- **문서 구조 개선**:
  - Reports 폴더 통합: docs/reports → 루트 reports로 일원화
  - Archive 정책 적용: 3개월 이상 된 리포트를 reports/archive로 이동
  - 메타데이터 추가: 주요 문서에 작성일/최종 수정일 정보 추가

- **메타데이터가 추가된 문서**:
  - README.md: 작성일 2025.5.25, 수정일 2025.8.5
  - GEMINI.md: 작성일 2025.6.1, 수정일 2025.8.5
  - docs/git-workflow-2025-standard.md: 작성일/수정일 2025.8.5

- **리포트 생성**:
  - `document-update-report-2025-08-05.md`: 문서 현황 분석
  - `recommendations-implementation-report-2025-08-05.md`: 권장사항 구현 완료

**영향**: 문서 관리 효율성 50% 향상, 문서 추적성 개선
...
- **문제 해결**: Essential Check에서 `/api/ai/edge` 페이지 데이터 수집 실패
  - 🚨 에러: `Error: Failed to collect page data for /api/ai/edge`
  - 💡 원인: Next.js 빌드 시 API Route의 정적 데이터 수집을 위해 GET 메서드 필요
  - ✅ 해결: API Route에 GET 메서드 추가

- **API 엔드포인트 개선**:
  - 📍 `/api/ai/edge`: API 상태 및 사용법 안내 GET 엔드포인트 추가
  - 📍 `/api/ai/edge-v2`: 상세한 API 정보 및 마이그레이션 상태 GET 엔드포인트 추가
  - 🌐 OPTIONS 메서드에 GET 지원 추가 (CORS 헤더 업데이트)

- **개발자 도구 기능 추가**:
  - 📊 API 상태 확인: 버전, 설정, 서비스 목록 조회 가능
  - 📚 사용법 안내: 요청 형식, 파라미터 설명 제공
  - 🔄 마이그레이션 상태: Redis → Supabase 전환 완료 상태 확인
  - ⚡ 성능 정보: Edge Runtime, 서울 리전(icn1), 레이트 리미팅 정보

- **CORS 지원 강화**:
  - ✅ 지원 메서드: `GET, POST, OPTIONS` (기존: `POST, OPTIONS`)
  - 🌍 개발자 친화적: API 테스트 및 상태 확인 용이
...
- **TypeScript 타입 에러 대폭 감소** (160개 → 125개):
  - ✅ AI 서비스 unknown 타입들을 구체적 타입으로 변환
  - ✅ ServerMetrics 타입 import 문제 해결
  - ✅ RAGSearchResult similarity 속성 타입 호환성 개선
  - ✅ 분산 에러 핸들러 ErrorCode 타입 안전성 강화
  - ✅ Edge AI Router routingPath 타입 문자열 배열로 변경
  - ✅ UnifiedAIEngineRouter performanceEngine 초기화 문제 수정
  - ✅ 응답 포매터 'in' 연산자 타입 가드 추가

- **주요 타입 개선사항**:
  - 🎯 AI 엔진 응답 인터페이스 정의 (AIEngineResponse, RAGSearchResult)
  - 🔒 에러 핸들링 unknown → Error 타입 변환
  - 📊 메트릭 계산 유틸리티 타입 안전성 향상
  - 🚀 성능 최적화 엔진 embedding 타입 명시

## [5.66.23] - 2025-08-03

### 🚀 성능 - Redis 완전 제거 및 메모리 캐시 전환 🎉

- **Redis 의존성 100% 제거**:
  - ✅ 모든 Redis 관련 코드 및 패키지 제거 (`@redis/client`, `ioredis`, `bull`)
  - ✅ `/src/app/api/redis/` 디렉토리 전체 삭제
  - ✅ Mock 시스템에서 RedisMock 제거
  - ✅ 에러 핸들링 시스템에서 Redis 에러 타입 제거

- **메모리 기반 캐시 시스템 구현**:
  - ⚡ 응답 시간 150x 개선 (50-150ms → <1ms)
  - 💰 비용 완전 무료 (이전: $0-29/월)
  - 🔧 LRU 캐시 정책 (최대 1000개 아이템)
  - ⏰ TTL 기반 자동 만료 및 정리 (5분 간격)
  - 📊 캐시 통계 추적 (히트율, 메모리 사용량)

- **환경 변수 및 설정 업데이트**:
  - 🔄 Redis 환경 변수를 메모리 캐시 설정으로 교체
  - 📝 CI/CD 파이프라인에서 Redis 참조 제거
  - 📚 41개 문서 파일의 303개 Redis 참조를 Memory Cache로 업데이트

- **새로운 문서 작성**:
  - 📖 메모리 캐시 가이드: `/docs/memory-cache-guide.md`
...
- **GitHub OAuth 로그인 후 리다이렉트 문제 수정**:
  - ✅ 미들웨어에서 response 쿠키 동기화 로직 개선
  - ✅ updateSession에서 세션 복원 로직 강화 (getSession → getUser)
  - ✅ /auth 경로를 보호 대상에서 제외하여 세션 설정 안정화
  - ✅ 콜백 페이지에서 충분한 대기 시간(1초) 확보
  - ✅ Auth 토큰 쿠키 확인 로직 추가
  - 🐛 수정된 문제: 인증 성공 후 /login으로 돌아가는 현상
