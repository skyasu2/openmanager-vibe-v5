...
1. **코드 분석가**: 대용량 파일 및 전체 코드베이스 분석
2. **품질 검토자**: SOLID 원칙, 타입 안전성, 중복 코드 검사
3. **문서 탐색자**: API 문서, 관련 파일 찾기
4. **빠른 답변자**: 간단한 질문에 즉시 응답

#### Claude에게 전달할 역할

1. **구현 주도자**: 복잡한 코드 작성 및 수정
2. **디버그 전문가**: 실시간 문제 해결
3. **아키텍트**: 시스템 설계 및 구조 결정
4. **Git 관리자**: 커밋, PR 생성 등
...
# Claude: 테스트 작성 (TDD - Red)
# Claude: 기능 구현 (TDD - Green)

# Gemini: 코드 리뷰
git diff | gemini -p "SOLID 원칙 관점에서 리뷰"

# Claude: 리팩토링 (TDD - Refactor)
```
...
```bash
# Gemini: 오류 원인 분석
echo "에러 메시지" | gemini -p "가능한 원인 3가지"

# Claude: 디버깅 및 수정

# Gemini: 사이드 이펙트 검토
git diff | gemini -p "다른 부분에 영향 확인"
```
...
```bash
# Gemini: 병목 현상 찾기
echo "성능 분석" | gemini -p "@src/ 느린 부분 찾기"

# Claude: 최적화 구현

# Gemini: 개선 효과 측정
echo "성능 비교" | gemini -p "개선 전후 분석"
```
