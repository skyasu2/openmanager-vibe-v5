...
export async function POST(req: NextRequest) {
  try {
    ...
    // 4. 생각중 상태 시작 (비동기)
    const thinkingPromise = redisCacheAdapter.addThinkingStep(
    ...
    thinkingPromise.then(() => 
      redisCacheAdapter.addThinkingStep(
        routerRequest.sessionId,
        {
          step: 'AI 처리 완료',
          status: 'completed',
    ...
    console.error('Edge AI Route Error:', error);
