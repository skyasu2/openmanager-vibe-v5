...
async function testAILogging() {
    ...
    try {
        ...
        // 로그 조회
        await DELAY(1000);
        const logsResponse = await fetch('http://localhost:3000/api/ai/logging?type=recent&limit=5');
        const logsData = await logsResponse.json();

        if (logsData.success && logsData.data.logs.length > 0) {
            console.log(chalk.green(`   ✅ 로그 조회 성공 (${logsData.data.logs.length}개)`));
        } else {
    ...
    try {
        ...
        if (aiResponse.ok) {
            ...
            console.log(chalk.gray(`   📊 응답 길이: ${aiData.answer ? aiData.answer.length : 0}자`));
            console.log(chalk.gray(`   ⏱️  처리 시간: ${aiData.metadata?.processing_time || 0}ms`));
        } else {
    ...
    try {
        const thinkingResponse = await fetch('http://localhost:3000/api/ai/logging?type=thinking&limit=3');
        const thinkingData = await thinkingResponse.json();

        if (thinkingData.success && thinkingData.data.logs.length > 0) {
            console.log(chalk.green(`   ✅ 사고 과정 로그 발견 (${thinkingData.data.logs.length}개)`));
            thinkingData.data.logs.forEach((log, index) => {
                console.log(chalk.gray(`   ${index + 1}. ${log.engine}: ${log.message.substring(0, 50)}...`));
            });
        } else {
    ...
    try {
        const metricsResponse = await fetch('http://localhost:3000/api/ai/logging?type=metrics');
        const metricsData = await metricsResponse.json();

        if (metricsData.success) {
            console.log(chalk.green('   ✅ 성능 메트릭 조회 성공'));
            console.log(chalk.gray(`   📈 총 엔진 수: ${metricsData.data.summary.totalEngines}`));
            console.log(chalk.gray(`   📝 총 로그 수: ${metricsData.data.summary.totalLogs}`));
        } else {
    ...
    console.log(chalk.gray('   • 로그 조회: GET /api/ai/logging?type=recent&limit=100'));
    console.log(chalk.gray('   • 실시간 스트리밍: GET /api/ai/logging/stream'));
    console.log(chalk.gray('   • 메트릭 조회: GET /api/ai/logging?type=metrics'));
    console.log(chalk.gray('   • AI 사고 과정: GET /api/ai/logging?type=thinking'));
}
