...
class UnifiedEnvManager {
    ...
    async decryptEnvFile() {
        try {
            const encryptedContent = await fs.readFile('.env.local.encrypted', 'utf8');
    ...
    try {
        switch (command) {
            ...
            case 'set':
                ...
                await envManager.setEnvVar(setKey, setValue, encryptFlag === '--encrypt');
                break;
            ...
            default:
                ...
                console.log('  node unified-env-manager.mjs list                    # 환경변수 목록');
                console.log('  node unified-env-manager.mjs get KEY [--decrypt]     # 환경변수 조회');
                console.log('  node unified-env-manager.mjs set KEY VALUE [--encrypt] # 환경변수 설정');
                console.log('  node unified-env-manager.mjs backup                  # 백업 생성');
                console.log('  node unified-env-manager.mjs encrypt                 # 파일 암호화');
                console.log('  node unified-env-manager.mjs decrypt                 # 파일 복호화');
                break;
        }
