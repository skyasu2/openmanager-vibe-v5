...
function getMcpServers() {
  try {
    ...
    console.error(`${colors.red}❌ MCP 서버 목록을 가져올 수 없습니다:${colors.reset}`, error.message);
    return null;
  }
}
...
async function testMcpServer(name, config, envVars) {
  return new Promise((resolve) => {
    ...
    if (name === 'github' && envVars.GITHUB_TOKEN) {
      ...
      if (envVars.SUPABASE_SERVICE_ROLE_KEY) serverEnv.SUPABASE_SERVICE_ROLE_KEY = envVars.SUPABASE_SERVICE_ROLE_KEY;
...
async function main() {
  ...
  if (!servers) {
    ...
    console.log('3. 설정 예시:');
    console.log(`   ${colors.cyan}claude mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem .${colors.reset}`);
    return;
  }
