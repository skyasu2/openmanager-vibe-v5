  ...
  async getLatestAnalysis() {
    ...
    try {
      ...
      if (error || !data || data.length === 0) {
        console.log(
          '⚠️ 저장된 분석 결과가 없어 시뮬레이션 데이터를 사용합니다.'
        );
        return this.getSimulationAnalysis();
      }
...
Next.js, TypeScript, Supabase, Redis 환경을 고려해주세요.
  ...
  async saveImprovementPlan(improvements) {
    try {
      const improvementsDir = path.join(
        process.cwd(),
...
`;
  ...
  async applyImprovements(improvements) {
    ...
    for (const change of simulatedChanges) {
      console.log(
        `   ${change.status === 'completed' ? '✅' : '🔄'} ${change.action}: ${change.file}`
      );
  ...
  async measureImprovements() {
    ...
    const improvements = {
      responseTime: {
        ...
        improvement: (
          ((beforeMetrics.avgResponseTime - afterMetrics.avgResponseTime) /
      ...
      userSatisfaction: {
        ...
        improvement: (
          ((afterMetrics.userSatisfaction - beforeMetrics.userSatisfaction) /
            beforeMetrics.userSatisfaction) *
          100
        ).toFixed(1),
      },
      throughput: {
  ...
  async runFullWorkflow() {
    ...
    try {
      ...
      console.log(
        `📊 측정된 개선 지표: ${Object.keys(effectMeasurement).length}개`
      );
  ...
  async showWorkflowSummary() {
    if (
      !this.workflowResults ||
      Object.keys(this.workflowResults).length === 0
    ) {
      console.log('⚠️ 실행된 워크플로우 결과가 없습니다.');
      console.log(
        '💡 먼저 "node scripts/data-driven-dev.js run" 명령을 실행해주세요.'
      );
      return;
    }
    ...
    const { analysisResults, improvements, appliedChanges, effectMeasurement } =
      this.workflowResults;
    ...
    appliedChanges.forEach(change => {
      console.log(
        `   ${change.status === 'completed' ? '✅' : '🔄'} ${change.file}: ${change.description}`
      );
    });
    ...
    Object.entries(effectMeasurement).forEach(([metric, data]) => {
      console.log(
        `   📊 ${metric}: ${data.improvement}% 개선 (${data.before} → ${data.after})`
      );
    });
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('사용법:');
      console.log(
        '   node scripts/data-driven-dev.js run      # 전체 워크플로우 실행'
      );
      console.log(
        '   node scripts/data-driven-dev.js summary  # 결과 요약 보기'
      );
      console.log('\n워크플로우:');
