...
async function setupSupabaseVector() {
  ...
  try {
    ...
    const sampleDocs = [
      {
        ...
        content:
          'CPU 사용률 모니터링을 위해 top 명령어를 사용하세요. top 명령어는 실시간으로 프로세스 상태를 확인할 수 있습니다.',
        embedding: Array(384)
          .fill(0)
          .map(() => Math.random() * 0.1), // 임시 임베딩
        metadata: { category: 'monitoring', type: 'cpu', priority: 'high' },
      },
      {
        id: 'sample-memory-check',
        content:
          '메모리 사용량 확인은 free -h 명령어로 가능합니다. 메모리 상태를 정기적으로 모니터링하는 것이 중요합니다.',
        embedding: Array(384)
          .fill(0)
          .map(() => Math.random() * 0.1), // 임시 임베딩
        metadata: {
      ...
      },
      {
        ...
        content:
          '디스크 사용량 확인은 df -h 명령어를 사용합니다. 디스크 공간이 부족하면 시스템 성능에 영향을 줍니다.',
        embedding: Array(384)
          .fill(0)
          .map(() => Math.random() * 0.1), // 임시 임베딩
    ...
    if (checkError) {
      ...
      console.log(
        `✅ 테이블 확인 완료: ${tableCheck?.length || 0}개 문서 존재`
      );
      if (tableCheck && tableCheck.length > 0) {
        tableCheck.forEach((doc, index) => {
          console.log(
            `  ${index + 1}. ${doc.id}: ${doc.content.substring(0, 50)}...`
          );
        });
      }
    }
