...
function getServerConfig() {
  try {
    ...
    // 설정값 추출
    const serverCountMatch = configContent.match(
      /DEFAULT_SERVER_COUNT = (\d+)/
    );
    const updateIntervalMatch = configContent.match(/updateInterval = (\d+)/);
    const warningPercentMatch = configContent.match(
      /warningPercent = (0\.\d+)/
    );
    ...
    return {
      ...
      updateInterval: updateIntervalMatch
        ? parseInt(updateIntervalMatch[1])
        : 30000,
      warningPercent: warningPercentMatch
        ? parseFloat(warningPercentMatch[1]) * 100
        : 30,
      criticalPercent: criticalPercentMatch
        ? parseInt(criticalPercentMatch[1])
        : 15,
    };
  } catch (error) {
...
# 서버 수: ${serverConfig.serverCount}개, 간격: ${serverConfig.updateInterval / 1000}초, 심각: ${serverConfig.criticalPercent}%, 경고: ${serverConfig.warningPercent}%`;
  ...
  content = content.replace(/# 데이터 생성기 최적화.*$/m, updatedComment);
...
async function main() {
  ...
  try {
    ...
    console.log(
      `🎯 서버 설정: ${serverConfig.serverCount}개, ${serverConfig.updateInterval / 1000}초 간격\n`
    );
