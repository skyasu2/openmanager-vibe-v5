...
class UnifiedFixTools {
    ...
    async fixMissingImports() {
        ...
        for (const file of files) {
            try {
                ...
                const hooksUsed = [];
                if (content.includes('useState') && !content.includes('import.*useState')) hooksUsed.push('useState');
                if (content.includes('useEffect') && !content.includes('import.*useEffect')) hooksUsed.push('useEffect');
                if (content.includes('useCallback') && !content.includes('import.*useCallback')) hooksUsed.push('useCallback');
                if (content.includes('useMemo') && !content.includes('import.*useMemo')) hooksUsed.push('useMemo');
                
                if (hooksUsed.length > 0) {
                    const hookImport = `import { ${hooksUsed.join(', ')} } from 'react';\n`;
                    const importMatch = updatedContent.match(/^(import.*?;?\n)*/m);
                    const importEnd = importMatch ? importMatch[0].length : 0;
                    updatedContent = updatedContent.slice(0, importEnd) + hookImport + updatedContent.slice(importEnd);
                    fileModified = true;
                    this.fixes.push(`${file}: React hooks import 추가 (${hooksUsed.join(', ')})`);
                }
        ...
        for (const file of files) {
            try {
                ...
                if (emptyArrayRegex.test(content)) {
                    updatedContent = updatedContent.replace(emptyArrayRegex, 'const $1: unknown[] = []');
                    fileModified = true;
                ...
                if (content.includes('[') && !content.includes('?.')) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        if (lines[i].includes('Cannot read property') || 
    ...
    async fixStorybookImports() {
        ...
        const storybookFiles = await this.findFiles(this.srcPath, ['.stories.ts', '.stories.tsx']);
        let fixCount = 0;
        ...
        for (const file of storybookFiles) {
            try {
                ...
                // Meta 타입 import 추가
                if (content.includes('Meta') && !content.includes("import type { Meta")) {
                    const importMatch = updatedContent.match(/^(import.*?;?\n)*/m);
                    const importEnd = importMatch ? importMatch[0].length : 0;
                    updatedContent = updatedContent.slice(0, importEnd) + 
    ...
    async fixVectorData() {
        ...
        try {
            // 벡터 데이터 관련 파일들 찾기
            const vectorFiles = await this.findFiles(this.srcPath, ['.ts', '.tsx'])
            ...
            for (const file of vectorFiles) {
                try {
                    ...
                    // 벡터 데이터베이스 연결 오류 수정
                    if (content.includes('pgvector') && !content.includes('vector_extension')) {
                        updatedContent = updatedContent.replace(
                            /pgvector/g,
    ...
    try {
        switch (command) {
            ...
            default:
                ...
                console.log('  node unified-fix-tools.mjs imports    # 누락된 import 수정');
                console.log('  node unified-fix-tools.mjs types      # TypeScript 타입 오류 수정');
                console.log('  node unified-fix-tools.mjs storybook  # Storybook import 수정');
                console.log('  node unified-fix-tools.mjs vector     # 벡터 데이터 구조 수정');
                console.log('  node unified-fix-tools.mjs report     # 수정 리포트 생성');
                break;
        }
