...
const fixes = [
  // RedisMetricsManager.ts - ServerMetric 필드 수정
  {
    ...
    fix: (content) => {
      ...
      content = content.replace(/metric\.systemMetrics\.cpuUsage/g, 'metric.cpu');
      content = content.replace(/metric\.systemMetrics\.memoryUsage/g, 'metric.memory');
      content = content.replace(/metric\.systemMetrics\.diskUsage/g, 'metric.disk');
      content = content.replace(/metric\.systemMetrics\.networkUsage/g, '(metric.network.in + metric.network.out) / 2');
      
      // applicationMetrics를 직접 필드로 변경
      content = content.replace(/metric\.applicationMetrics\.requestCount/g, 'metric.activeConnections');
      content = content.replace(/metric\.applicationMetrics\.errorRate/g, '0'); // 기본값
      content = content.replace(/metric\.applicationMetrics\.responseTime/g, 'metric.responseTime');
  ...
  // servers/all/route.ts - gcpDataService 수정
  {
    ...
    fix: (content) => {
      ...
      content = content.replace(/const servers = await gcpDataService\.getRealServerMetrics\(\);/g, 'const servers: any[] = []; // gcpDataService removed');
      return content;
    }
  },
  ...
  // servers/realtime/route.ts - GCPRealDataService 수정
  {
    ...
    fix: (content) => {
      content = content.replace(/const gcpService = GCPRealDataService\.getInstance\(\);/g, '// const gcpService = GCPRealDataService.getInstance(); // Removed');
      content = content.replace(/const gcpService = await GCPRealDataService\.getInstance\(\);/g, '// const gcpService = await GCPRealDataService.getInstance(); // Removed');
  ...
  // UnifiedProfileButton.tsx - aiToggle 수정
  {
    ...
    fix: (content) => {
      content = content.replace(/className=\{`\$\{false/g, 'className={`${false'); 
      return content;
    }
  },
  ...
  // TaskOrchestrator.ts - GCPRealDataService import 제거
  {
    ...
    fix: (content) => {
      content = content.replace(/import { GCPRealDataService } from '@\/services\/gcp\/GCPRealDataService';/g, '// GCPRealDataService import removed');
  ...
  // EnrichedMetricsGenerator.ts 수정
  {
    ...
    fix: (content) => {
      content = content.replace(/import { BaselineStorageService } from '@\/services\/gcp\/BaselineStorageService';/g, '// BaselineStorageService import removed');
