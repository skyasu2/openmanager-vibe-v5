...
class ProductionDataCollector {
  ...
  async initializeTables() {
    ...
    try {
      ...
      const { error: logsError } = await this.supabase.rpc(
        'create_production_logs_table'
      );

      // 성능 메트릭 테이블 생성
      const { error: metricsError } = await this.supabase.rpc(
        'create_performance_metrics_table'
      );

      // AI 분석 결과 테이블 생성
      const { error: analysisError } = await this.supabase.rpc(
  ...
  async saveToSupabase(data) {
    ...
    try {
      // 데이터 타입별로 분류하여 저장
      const logData = data.filter(item =>
        ['api_call', 'error'].includes(item.type)
      );
      const metricsData = data.filter(item =>
        ['performance', 'user_action'].includes(item.type)
      );
   ...
   * 📈 실시간 데이터 수집 시작
   */
  async startCollection(duration = 60000) {
    // 기본 1분
    ...
    const collectInterval = setInterval(async () => {
      try {
        ...
        console.log(
          `📊 데이터 수집: ${data.length}개 (총 ${this.dataBuffer.length}개)`
        );
  ...
  async generateCollectionSummary() {
    ...
    try {
      ...
      if (logs && logs.length > 0) {
        const errorRate =
          (logs.filter(log => log.status_code >= 400).length / logs.length) *
          100;
        const avgResponseTime =
          logs.reduce((sum, log) => sum + (log.response_time || 0), 0) /
          logs.length;
  ...
  async checkStatus() {
    ...
    console.log(
      `   Vercel: ${this.vercelToken ? '✅ 토큰 있음' : '⚠️ 토큰 없음 (시뮬레이션 모드)'}`
    );
    console.log(
      `   수집 상태: ${this.isCollecting ? '🔄 진행 중' : '⏸️ 대기 중'}`
    );
    ...
    console.log('\n🚀 사용법:');
    console.log(
      '   node scripts/production-data-collector.js start    # 데이터 수집 시작'
    );
    console.log(
      '   node scripts/production-data-collector.js status   # 상태 확인'
    );
    console.log(
      '   node scripts/production-data-collector.js demo     # 데모 데이터 생성'
    );
  }
    ...
    for (let hour = 0; hour < 24; hour++) {
      for (let i = 0; i < 10; i++) {
        const timestamp = new Date(
          now.getTime() - hour * 3600000 - Math.random() * 3600000
        );
