...
const requiredEnvVars = {
  ...
  UPSTASH_REDIS_REST_URL:
    process.env.UPSTASH_REDIS_REST_URL || 'https://your_redis_host_here',
  UPSTASH_REDIS_REST_TOKEN: process.env.UPSTASH_REDIS_REST_TOKEN || '',
  KV_REST_API_URL:
    process.env.KV_REST_API_URL || 'https://your_redis_host_here',
  ...
  // 🔄 MCP 서버
  MCP_REMOTE_URL:
    process.env.MCP_REMOTE_URL || 'https://openmanager-vibe-v5.gcp.run',
};
...
function createTestPage() {
  ...
  console.log(
    `📍 테스트 URL: https://your-vercel-domain.vercel.app/test-vercel-ai-natural-query.html`
  );
...
async function main() {
  console.log(
    '🎯 목표: 로컬에서 성공한 AI 자연어 질의 기능을 Vercel에서도 동작하게 하기\n'
  );
  ...
  if (shouldDeploy) {
    ...
    if (deploySuccess) {
      ...
      console.log(
        '2. https://your-vercel-domain.vercel.app/test-vercel-ai-natural-query.html 에서 테스트'
      );
