...
const fixes = [
  ...
  // import 문 수정
  {
    pattern: /import.*from\s*['"]@\/services\/data-generator\/RealServerDataGenerator['"];?\s*\n?/g,
    replacement: ""
  },
  {
    pattern: /import.*from\s*['"]\.\.\/data-generator\/RealServerDataGenerator['"];?\s*\n?/g,
    replacement: ""
  },
  {
  ...
  {
    pattern: /^(?!.*GCPRealDataService.*import)(.|\n)*?GCPRealDataService\.getInstance/m,
  ...
  // 존재하지 않는 함수/변수 이름 수정
  {
    ...
    replacement: "(() => { throw new Error('createServerDataGenerator deprecated - use GCPRealDataService.getInstance()'); })"
  },
  
  // 존재하지 않는 메서드 호출 수정
  {
    pattern: /\.getStatus\(\)/g,
    replacement: ".getRealServerMetrics().then(r => ({ status: r.success ? 'active' : 'error' }))"
  },
  {
  ...
  },
  {
    ...
    replacement: ".getRealServerMetrics().then(r => ({ summary: 'Available' }))"
  },
