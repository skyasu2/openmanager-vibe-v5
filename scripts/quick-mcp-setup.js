...
async function checkEnvironment() {
    ...
    try {
        ...
        colorLog('red', '❌ Node.js가 설치되지 않았습니다. Node.js 18+ 설치가 필요합니다.');
        process.exit(1);
    }
...
async function createCursorSettings() {
    ...
    try {
        ...
        await fs.writeFile(path.join('.cursor', 'settings.json'), settingsJson, 'utf8');
...
async function cachePackages() {
    ...
    for (const pkg of packages) {
        try {
            ...
            colorLog('yellow', `⚠️ ${pkg.split('/').pop() || pkg} 서버 캐시 실패 (첫 실행 시 다운로드됩니다)`);
        }
    }
}
...
function showCompletionMessage() {
    ...
    console.log('');
    colorLog('cyan', '❓ 문제가 있다면 docs/MCP_완벽_설정_가이드.md를 확인하세요.');
}
