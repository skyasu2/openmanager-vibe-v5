...
class ContextAnalyzer {
    ...
    async analyzeContextManagers() {
        ...
        console.log(`   📊 총 ${this.analysisResults.contextManagers.length}개 컨텍스트 관리자 발견\n`);
    }
    ...
    analyzeContextFile(content, fileName) {
        ...
        // 최적화 수준 평가
        if (analysis.cleanupMethods.length > 0 && analysis.memoryLimits.length > 0) {
            analysis.optimizationLevel = 'high';
        } else if (analysis.memoryLimits.length > 0 || analysis.cacheStrategies.length > 0) {
    ...
    async analyzeMemoryLimits() {
        ...
        console.log(`   📊 총 ${this.analysisResults.memoryLimits.length}개 메모리 제한 설정 발견\n`);
    }
    ...
    async identifyOptimizationPoints() {
        ...
        // 메모리 제한 분석
        const highMemoryLimits = this.analysisResults.memoryLimits.filter(limit => 
            limit.value > 100 * 1024 * 1024 // 100MB 이상
        );
        ...
        console.log(`   🎯 ${this.analysisResults.optimizationPoints.length}개 최적화 포인트 식별\n`);
    }
    ...
    async generateRecommendations() {
        ...
        this.analysisResults.recommendations = [
            {
                ...
                implementation: 'DevKeyManager의 Redis 설정을 활용하여 통합 캐싱 레이어 구축',
            ...
            },
            {
                ...
                implementation: '패턴 저장소 20개 → 10개, 결과 저장소 50개 → 25개로 축소',
        ...
        console.log(`   💡 ${this.analysisResults.recommendations.length}개 권장사항 생성\n`);
    }
    ...
    printAnalysisResults() {
        ...
        // 요약 정보
        console.log('\n📈 요약 정보:');
        console.log(`   • 컨텍스트 관리자: ${this.analysisResults.contextManagers.length}개`);
        console.log(`   • 메모리 제한 설정: ${this.analysisResults.memoryLimits.length}개`);
        console.log(`   • 최적화 포인트: ${this.analysisResults.optimizationPoints.length}개`);
        console.log(`   • 개선 권장사항: ${this.analysisResults.recommendations.length}개`);

        // 컨텍스트 관리자별 상태
        console.log('\n🔧 컨텍스트 관리자 상태:');
        this.analysisResults.contextManagers.forEach(manager => {
            const statusIcon = manager.optimizationLevel === 'high' ? '✅' : 
                              manager.optimizationLevel === 'medium' ? '⚠️' : '❌';
            console.log(`   ${statusIcon} ${manager.className} (${manager.optimizationLevel})`);
            console.log(`      📁 ${manager.file}`);
            console.log(`      🔒 메모리 제한: ${manager.memoryLimits.length}개`);
            console.log(`      💾 캐시 전략: ${manager.cacheStrategies.join(', ') || '없음'}`);
            console.log(`      🧹 정리 메서드: ${manager.cleanupMethods.join(', ') || '없음'}`);
        });

        // 우선순위별 권장사항
        console.log('\n💡 우선순위별 권장사항:');
        ['high', 'medium', 'low'].forEach(priority => {
            const recommendations = this.analysisResults.recommendations.filter(r => r.priority === priority);
