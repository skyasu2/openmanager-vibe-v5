...
async function saveSecureTokens(
...
async function updateMCPSettings(): Promise<void> {
  ...
  if (tokens.GITHUB_TOKEN && settings.mcpServers?.github) {
    ...
    settings.mcpServers.github.env.GITHUB_PERSONAL_ACCESS_TOKEN =
      tokens.GITHUB_TOKEN;
  ...
  try {
    switch (command) {
      case 'add':
        if (!args[0]) {
          console.error(
            '❌ 토큰 이름을 지정하세요: npm run secure:token add TOKEN_NAME'
          );
          break;
        }
      ...
      case 'get':
        if (!args[0]) {
          console.error(
            '❌ 토큰 이름을 지정하세요: npm run secure:token get TOKEN_NAME'
          );
          break;
        }
      ...
      case 'remove':
        if (!args[0]) {
          console.error(
            '❌ 토큰 이름을 지정하세요: npm run secure:token remove TOKEN_NAME'
          );
          break;
        }
