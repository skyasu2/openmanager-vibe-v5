...
/**
 * 🏢 Enterprise IDC 시스템 테스트 스크립트
 * 
 * 사용법:
 * - npm run test:enterprise
 * - node scripts/mockEnterpriseHit.ts
 */
...
async function testEndpoint(endpoint: string, method: 'GET' | 'POST' = 'GET', body?: any): Promise<TestResult> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`❌ ${endpoint} - ${response.status} (${duration}ms): ${result.error}`)
    }
    ...
    console.log(`💥 ${endpoint} - Network Error (${duration}ms): ${result.error}`)
    return result
  }
}
...
async function runEnterpriseTests() {
  ...
  // 8. 자연어 쿼리 시스템 (POST)
  results.push(await testEndpoint('/api/mcp/enterprise-query', 'POST', {
    query: '전체 인프라 상태 어때?'
  }))
  ...
  console.log(`📈 평균 응답시간: ${Math.round(totalDuration / results.length)}ms`)
  ...
  // 성공한 경우 주요 데이터 샘플 출력
  const enterpriseResult = results.find(r => r.endpoint === '/api/enterprise' && r.success)
