...
class UnifiedGCPMonitor {
  ...
  async checkGCPCredentials() {
    ...
    const checks = {
      projectId:
        !!this.config.projectId && this.config.projectId !== 'your-project-id',
  ...
  async checkQuotaThresholds(quotas) {
    ...
    if (criticals.length > 0) {
      await this.generateAlert(
        'critical',
        `${criticals.length}개 리소스가 임계 수준에 도달`,
        criticals
      );
    }
    ...
    if (warnings.length > 0) {
      await this.generateAlert(
        'warning',
        `${warnings.length}개 리소스가 경고 수준에 도달`,
        warnings
      );
    }

    console.log(
      `⚠️ 경고: ${warnings.length}개, 🚨 심각: ${criticals.length}개`
    );
  ...
  async generateAlert(level, message, details) {
    ...
    details.forEach(detail => {
      console.log(
        `  - ${detail.service}/${detail.resource}: ${detail.percentage}% (${detail.used}/${detail.limit})`
      );
    });
    ...
    try {
      ...
      await fs.appendFile('logs/gcp-alerts.log', JSON.stringify(alert) + '\n');
    } catch (error) {
  ...
  async runFullMonitoring() {
    ...
    console.log('\n📋 GCP 모니터링 완료:');
    console.log(
      `  인증 상태: ${Object.values(results.credentials).every(Boolean) ? '✅' : '❌'}`
    );
    console.log(`  할당량 체크: ✅`);
    console.log(
      `  함수 테스트: ${results.functions.filter(f => f.success).length}/${results.functions.length} 성공`
    );
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log(
            '  node unified-gcp-monitor.js monitor      # 전체 모니터링'
          );
          console.log(
            '  node unified-gcp-monitor.js quotas       # 할당량 체크'
          );
          console.log(
            '  node unified-gcp-monitor.js functions    # Cloud Functions 테스트'
          );
          console.log(
            '  node unified-gcp-monitor.js credentials  # 인증 정보 확인'
          );
          console.log(
            '  node unified-gcp-monitor.js report       # 상세 리포트 생성'
          );
          console.log(
            '  node unified-gcp-monitor.js alerts       # 최근 알림 조회'
          );
          break;
      }
