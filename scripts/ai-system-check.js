    ...
    async checkSystemStatus() {
        ...
        try {
            ...
            if (status.success) {
                ...
                console.log(`📊 사용 가능한 AI: ${status.data.availableProviders.length}개`);
                console.log(`🕐 확인 시간: ${new Date(status.data.timestamp).toLocaleString('ko-KR')}`);
...
실용적인 조언을 부탁드립니다.`
            ...
            const sessionInfo = await this.makeRequest('GET', `/api/ai-chat?action=session&sessionId=${this.sessionId}`);
            ...
            if (sessionInfo.success) {
                ...
                console.log(`   🕐 생성 시간: ${new Date(session.createdAt).toLocaleString('ko-KR')}`);
            }
