...
class UnifiedMonitoring {
  ...
  async analyzeHealth(health) {
    // API 상태 분석
    const failedAPIs = Object.entries(health.api).filter(
  ...
  async generateReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        healthyServices: Object.values(health.services).filter(s => s.running)
          .length,
    ...
    console.log(
      `  서비스 상태: ${report.summary.healthyServices}/${report.summary.totalServices} 정상`
    );
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log(
            '  node unified-monitoring.js start   # 실시간 모니터링 시작'
          );
          console.log('  node unified-monitoring.js check   # 현재 상태 점검');
          console.log(
            '  node unified-monitoring.js report  # 상세 리포트 생성'
          );
