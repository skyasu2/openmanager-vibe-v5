...
const fixes = [
  // RedisMetricsManager.ts
  {
    ...
    fix: (content) => {
      content = content.replace(/m\.active_connections/g, 'm.activeConnections');
  ...
  // SupabaseTimeSeriesManager.ts
  {
    ...
    fix: (content) => {
      content = content.replace(/metric\.active_connections/g, 'metric.activeConnections');
      content = content.replace(/metric\.response_time/g, 'metric.responseTime');
