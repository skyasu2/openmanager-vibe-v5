...
const {
  EnhancedEnvCryptoManager,
} = require('../src/lib/crypto/EnhancedEnvCryptoManager.ts');
...
async function getPassword(options) {
  ...
  throw new Error(
    '비밀번호가 필요합니다. --password, --password-file 또는 ENV_MASTER_PASSWORD 환경변수를 사용하세요.'
  );
}
...
async function encryptCommand(options) {
  ...
  try {
    ...
    log.warning(
      'Git에 커밋하기 전에 .env 파일은 반드시 .gitignore에 추가하세요!'
    );
  } catch (error) {
...
async function verifyCommand(options) {
  ...
  try {
    ...
    log.info(
      `생성 시간: ${new Date(encryptedConfig.variables[Object.keys(encryptedConfig.variables)[0]]?.timestamp).toLocaleString()}`
    );
  } catch (error) {
