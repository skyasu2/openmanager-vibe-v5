...
async function decryptEnvironmentVars() {
    try {
        ...
        // 암호화된 설정 파일 로드
        const configPath = path.join(__dirname, '..', 'config', 'encrypted-env-config.mjs');
        ...
        for (const [varName, encryptedData] of Object.entries(ENCRYPTED_ENV_CONFIG.variables)) {
            try {
...
async function decryptSpecificVar(varName) {
    try {
        ...
        const decryptedValue = decryptValue(ENCRYPTED_ENV_CONFIG.variables[varName], TEAM_PASSWORD);
