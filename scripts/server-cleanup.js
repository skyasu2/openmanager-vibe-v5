...
class SimpleServerCleanup {
  ...
  async checkPortUsage() {
    ...
    console.log(
      `🔍 [${await this.getCurrentTime()}] 포트 사용 현황 확인 중...`
    );
  ...
  async getNodeProcesses() {
    try {
      ...
      console.log(
        `📋 [${await this.getCurrentTime()}] Node.js 프로세스 ${processes.length}개 발견:`
      );
  ...
  async getMCPProcesses() {
    try {
        ...
        .map(line => {
          ...
          if (parts.length >= 3) {
            ...
            if (
              ...
              else if (commandLine.includes('postgres'))
                type = 'PostgreSQL MCP';
      ...
      if (mcpProcesses.length > 0) {
        console.log(
          `🤖 [${await this.getCurrentTime()}] MCP 서버 ${mcpProcesses.length}개 실행 중:`
        );
  ...
  async killProcessByPort(port) {
    try {
      console.log(
        `🔫 [${await this.getCurrentTime()}] 포트 ${port} 프로세스 종료 시도...`
      );
  ...
  async performHealthCheck() {
    ...
    if (busyPorts.length > 0) {
      console.log(
        `  - 사용 중인 포트: ${busyPorts.map(([port]) => port).join(', ')}`
      );
    } else {
