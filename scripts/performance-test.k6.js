...
export function aiQueryTest() {
  ...
  const response = http.post(`${BASE_URL}/api/ai/unified`, JSON.stringify(payload), params);
  ...
  if (!success) {
    systemErrors.add(1);
    console.log(`❌ AI 질의 실패: VU=${__VU}, Query="${query}", Status=${response.status}`);
  }
...
export function soakTest() {
  ...
  if (testType < 0.6) {
    ...
    // 10% - AI 질의 (가벼운 질의만)
    const lightQueries = ['시스템 상태가 어떤가요?', 'AI 에이전트 상태를 확인해주세요'];
    ...
    const response = http.post(`${BASE_URL}/api/ai/unified`, JSON.stringify(payload), {
