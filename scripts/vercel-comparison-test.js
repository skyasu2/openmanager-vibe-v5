...
class VercelComparisonTest {
    ...
    async testScenario(scenarioKey, scenario, testDurationMs = 60000) {
        ...
        while (Date.now() < endTime) {
            ...
            try {
                ...
                const statusPromise = this.makeRequest('/api/system/status', scenario.config);

                // 건강 상태 체크 (보조 요청)
                const healthPromise = this.makeRequest('/api/system/health', scenario.config);
            ...
            // 다음 폴링까지 대기
            const waitTime = scenario.config.pollingInterval - (Date.now() - batchStart);
            if (waitTime > 0) {
        ...
        const cacheHitRate = requests.length > 0 ?
            (cachedResponses.length / (requests.length * 2)) * 100 : 0;
        ...
        const result = {
            ...
            actualVsExpected: Math.round((projectedDailyRequests / scenario.expectedDaily) * 100),
    ...
    generateComparisonReport() {
        ...
        if (crisisResult && emergencyResult) {
            ...
            console.log('┌─────────────────────────┬─────────────┬──────────────┬─────────────┐');
            console.log('│ Metric                  │ Crisis      │ Emergency    │ Optimized   │');
            console.log('├─────────────────────────┼─────────────┼──────────────┼─────────────┤');
            console.log(`│ Daily Requests          │ ${crisisResult.projectedDailyRequests.toLocaleString().padEnd(11)} │ ${emergencyResult.projectedDailyRequests.toLocaleString().padEnd(12)} │ ${optimizedResult?.projectedDailyRequests.toLocaleString().padEnd(11) || 'N/A'.padEnd(11)} │`);
            console.log(`│ Avg Response Time       │ ${crisisResult.avgResponseTime}ms${' '.repeat(8 - crisisResult.avgResponseTime.toString().length)} │ ${emergencyResult.avgResponseTime}ms${' '.repeat(9 - emergencyResult.avgResponseTime.toString().length)} │ ${optimizedResult?.avgResponseTime || 'N/A'}ms${' '.repeat(8 - (optimizedResult?.avgResponseTime?.toString().length || 3))} │`);
            console.log(`│ Cache Hit Rate          │ ${crisisResult.cacheHitRate}%${' '.repeat(10 - crisisResult.cacheHitRate.toString().length)} │ ${emergencyResult.cacheHitRate}%${' '.repeat(11 - emergencyResult.cacheHitRate.toString().length)} │ ${optimizedResult?.cacheHitRate || 'N/A'}%${' '.repeat(10 - (optimizedResult?.cacheHitRate?.toString().length || 3))} │`);
            console.log('└─────────────────────────┴─────────────┴──────────────┴─────────────┘');

            // 개선 효과 계산
            const requestImprovement = ((crisisResult.projectedDailyRequests - emergencyResult.projectedDailyRequests) / crisisResult.projectedDailyRequests) * 100;
            const responseImprovement = ((crisisResult.avgResponseTime - emergencyResult.avgResponseTime) / crisisResult.avgResponseTime) * 100;

            console.log('\n📈 IMPROVEMENT ANALYSIS:');
            console.log(`🔥 Request Reduction: ${Math.round(requestImprovement)}% (${crisisResult.projectedDailyRequests.toLocaleString()} → ${emergencyResult.projectedDailyRequests.toLocaleString()})`);
            console.log(`⚡ Response Time: ${responseImprovement > 0 ? Math.round(responseImprovement) + '% faster' : Math.round(Math.abs(responseImprovement)) + '% slower'}`);
            console.log(`🗄️ Cache Effectiveness: ${emergencyResult.cacheHitRate}% hit rate (vs ${crisisResult.cacheHitRate}% in crisis)`);
            ...
            if (requestImprovement > 90) {
                ...
                console.log('⚠️  Consider gradual optimization to restore some real-time features');
            ...
            if (optimizedResult) {
                ...
                const optimizedImprovement = ((crisisResult.projectedDailyRequests - optimizedResult.projectedDailyRequests) / crisisResult.projectedDailyRequests) * 100;
                console.log(`📊 Optimized scenario: ${Math.round(optimizedImprovement)}% improvement vs crisis`);
                console.log(`⚖️  Balance: Better performance than emergency, still ${Math.round(((emergencyResult.projectedDailyRequests - optimizedResult.projectedDailyRequests) / emergencyResult.projectedDailyRequests) * 100)}% more efficient`);
            }
        }
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    tester.runFullComparison(duration)
        .then(() => {
