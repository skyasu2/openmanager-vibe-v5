...
import { getRedisClient, testRedisConnection, getRedisStats } from '../src/lib/redis';
...
async function main() {
  ...
  console.log(`  - FORCE_MOCK_REDIS: ${process.env.FORCE_MOCK_REDIS || 'false'}`);
  ...
  const redisEnvVars = {
    UPSTASH_REDIS_REST_URL: process.env.UPSTASH_REDIS_REST_URL ? '✅ 설정됨' : '❌ 없음',
    UPSTASH_REDIS_REST_TOKEN: process.env.UPSTASH_REDIS_REST_TOKEN ? '✅ 설정됨' : '❌ 없음',
  ...
  try {
    ...
    if (client) {
      ...
      console.log(`  - 테스트 쓰기/읽기: ${value === 'test-value' ? '✅ 성공' : '❌ 실패'}`);
  ...
  try {
    ...
    if (stats.mockRedis) {
      ...
      console.log(`    - 히트율: ${(stats.mockRedis.hitRate * 100).toFixed(1)}%`);
  ...
  if (process.env.NODE_ENV === 'development' && !process.env.USE_REAL_REDIS) {
    ...
    console.log('  - 실제 Redis를 사용하려면 USE_REAL_REDIS=true를 설정하세요.');
  }
  
  if (process.env.NODE_ENV === 'production' && process.env.FORCE_MOCK_REDIS === 'true') {
