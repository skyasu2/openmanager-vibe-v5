...
console.log('📁 파일 확인:');
const checks = [
  { path: join(__dirname, '../node_modules/tavily-mcp'), name: 'tavily-mcp 패키지' },
  { path: join(__dirname, 'tavily-mcp-wrapper.mjs'), name: 'Tavily 래퍼 스크립트' },
  { path: join(__dirname, '../config/tavily-encrypted.json'), name: 'API 키 설정' },
];
...
const wrapperProcess = spawn('node', [join(__dirname, 'tavily-mcp-wrapper.mjs')], {
  env: { ...process.env },
...
wrapperProcess.on('close', (code) => {
  ...
  if (errorOutput.includes('API 키 로드 성공') && errorOutput.includes('Tavily MCP 서버 시작 중')) {
