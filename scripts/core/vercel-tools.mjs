...
class UnifiedVercelTools {
  ...
  async checkVercelCLI() {
    ...
    try {
      const result = await this.executeCommand('vercel', ['--version'], {
        timeout: 10000,
      });
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const currentProject = projects.find(
          p =>
            p.name === this.config.projectName || p.name.includes('openmanager')
        );
        ...
        if (currentProject) {
          ...
          console.log(
            `  생성일: ${new Date(currentProject.createdAt).toLocaleString('ko-KR')}`
          );
          console.log(
            `  업데이트: ${new Date(currentProject.updatedAt).toLocaleString('ko-KR')}`
          );
  ...
  async testDeployedFunctions() {
    ...
    if (!project) {
      console.log(
        '❌ 프로젝트 정보를 가져올 수 없어 함수 테스트를 건너뜁니다.'
      );
      return [];
    }
  ...
  async runSystemTest() {
    ...
    console.log(
      `  함수 테스트: ${results.functionTests.length > 0 ? '✅' : '❌'}`
    );
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        console.log(
          '  node unified-vercel-tools.mjs test        # 시스템 전체 테스트'
        );
        console.log(
          '  node unified-vercel-tools.mjs info        # 프로젝트 정보'
        );
        console.log(
          '  node unified-vercel-tools.mjs deployments [개수] # 배포 내역'
        );
        console.log(
          '  node unified-vercel-tools.mjs functions   # 함수 테스트'
        );
        console.log(
          '  node unified-vercel-tools.mjs usage       # 사용량 모니터링'
        );
        break;
    }
