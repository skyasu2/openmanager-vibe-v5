    ...
    try {
      ...
      lines.forEach(line => {
        ...
        if (match) {
          ...
          analysis.providerUsage[provider] =
            (analysis.providerUsage[provider] || 0) + 1;
      ...
      analysis.averageOutputLength = Math.round(
        totalOutputLength / lines.length
      );
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        console.log(
          '  node unified-ai-tools.mjs chat [메시지] # AI 채팅 테스트'
        );
