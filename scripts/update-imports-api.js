#!/usr/bin/env node
/**
 * Phase 8: Update imports for api/ folder reorganization
 *
 * 이동된 파일 (2개):
 * - api-batcher.ts → api/api-batcher.ts
 * - api-config.ts → api/api-config.ts
 *
 * 검색 경로:
 * - src/app/
 * - src/components/
...
const importMappings = [
  { from: '@/lib/api-batcher', to: '@/lib/api/api-batcher' },
  { from: '@/lib/api-config', to: '@/lib/api/api-config' },
];
...
// Main execution
console.log('🔄 Phase 8: api/ 폴더 import 경로 업데이트 시작...\n');
