...
# 버전: 5.0.0
# 날짜: 2025-11-27
# 전략: 1:1:1:1 균등 분배 (Codex, Gemini, Claude, Qwen 각 25%) + 분할 리뷰 + 서브에이전트
#
...
# - 🔄 변경: 4:1 비율 → 1:1:1:1 균등 분배 (total % 5 → total % 4)
# - 🔄 변경: 폴백 체인 3단계 → 4단계 (Codex/Gemini/Claude/Qwen)
# - 📦 개선: 모듈화 구조 (lib/ 디렉토리로 분리)
# - 🎯 목표: 99.99% 가용성 (4개 AI 폴백)
# - 📊 효과: Codex 의존도 80% → 25% (Rate Limit 위험 감소)
#
...
main() {
    log_info "🚀 Auto AI Review 시작 (v5.0.0 - 1:1:1:1 비율 + 분할 리뷰 + 서브에이전트)"
    echo ""
