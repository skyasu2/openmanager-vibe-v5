...
# AI Review Core Functions - v7.2.0
...
**⛔ 리뷰 제외 (오탐 방지)**:
- hooks/, scripts/, config 파일에 테스트 부재 지적 금지
- \`catch { /* ignore */ }\`, \`git branch -D\` 등 의도적 단순화 존중
- 단순 코드에 과한 에러 핸들링/로깅 추가 권장 금지
- .md, .json, .yaml은 코드 품질 평가 대상 아님
