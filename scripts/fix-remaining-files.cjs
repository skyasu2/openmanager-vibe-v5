...
const filesToFix = [
  // components/unified-profile/UnifiedProfileButton.tsx
  {
    ...
    fix: (content) => {
      ...
      if (content.includes('adminStore.setAiToggle')) {
        content = content.replace(/adminStore\.setAiToggle\(/g, '// adminStore.setAiToggle(');
      }
      return content;
    }
  },
  ...
  // TaskOrchestrator.ts
  {
    ...
    fix: (content) => {
      ...
      if (content.includes("import { GCPRealDataService }")) {
        content = content.replace(/import { GCPRealDataService }[^;]+;/g, '// GCPRealDataService import removed');
      }
      // 사용 제거
  ...
  // EnrichedMetricsGenerator.ts
  {
    ...
    fix: (content) => {
      ...
      if (content.includes("import { BaselineStorageService }")) {
        content = content.replace(/import { BaselineStorageService }[^;]+;/g, '// BaselineStorageService import removed');
      }
  ...
  // SupabaseTimeSeriesManager.ts - generateMockTimeSeries 함수 전체 수정
  {
    ...
    fix: (content) => {
      ...
      content = content.replace(/response_time: number;/g, 'response_time: number;');
      
      // generateMockTimeSeries의 반환 타입 문제 해결
      if (content.includes('private generateMockTimeSeries')) {
        // ServerMetric[] 타입을 반환하도록 함수 전체를 수정
        const startIndex = content.indexOf('private generateMockTimeSeries(count: number = 100): ServerMetric[]');
        if (startIndex !== -1) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      activeConnections: Math.floor(Math.random() * 100)
          ...
          content = content.substring(0, startIndex) + newFunction + content.substring(functionEnd + 1);
        }
      }
