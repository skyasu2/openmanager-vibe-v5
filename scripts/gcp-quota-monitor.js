...
class GCPQuotaMonitor {
        ...
        try {
            ...
            return {
                ...
                invocations: {
                    ...
                    percentage: (totalInvocations / this.freeTerrLimits.cloudFunctions.invocations) * 100
                },
                gbSeconds: {
                    current: totalGbSeconds,
                    limit: this.freeTerrLimits.cloudFunctions.gbSeconds,
                    percentage: (totalGbSeconds / this.freeTerrLimits.cloudFunctions.gbSeconds) * 100
                },
                networkEgress: {
                    current: totalNetworkEgress,
                    limit: this.freeTerrLimits.cloudFunctions.networkEgress,
                    percentage: (totalNetworkEgress / this.freeTerrLimits.cloudFunctions.networkEgress) * 100
                }
            };
        } catch (error) {
            console.warn(chalk.yellow('⚠️ Cloud Functions 사용량 조회 실패'), error.message);
        ...
        try {
            ...
            return {
                instances: {
                    ...
                    percentage: (runningInstances.length / this.freeTerrLimits.computeEngine.instances) * 100
                },
            ...
            console.warn(chalk.yellow('⚠️ Compute Engine 사용량 조회 실패'), error.message);
        ...
        try {
            ...
            return {
                ...
                storage: {
                    ...
                    percentage: (totalStorage / this.freeTerrLimits.cloudStorage.storage) * 100
                },
                operations: {
                    current: totalOperations,
                    limit: this.freeTerrLimits.cloudStorage.classAOperations,
                    percentage: (totalOperations / this.freeTerrLimits.cloudStorage.classAOperations) * 100
                }
            };
        } catch (error) {
            console.warn(chalk.yellow('⚠️ Cloud Storage 사용량 조회 실패'), error.message);
    ...
    async getFirestoreUsage() {
        try {
            ...
            if (dbList.length === 0) {
                return {
                    enabled: false,
                    reads: { current: 0, limit: this.freeTerrLimits.firestore.reads, percentage: 0 },
                    writes: { current: 0, limit: this.freeTerrLimits.firestore.writes, percentage: 0 },
                    deletes: { current: 0, limit: this.freeTerrLimits.firestore.deletes, percentage: 0 },
                    storage: { current: 0, limit: this.freeTerrLimits.firestore.storage, percentage: 0 }
                };
            }
            ...
            console.warn(chalk.yellow('⚠️ Firestore 사용량 조회 실패'), error.message);
    ...
    displayUsage(usage) {
        ...
        console.log(chalk.gray(`📅 ${new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })} (KST)`));
        ...
        this.displayServiceUsage('네트워크 송신', usage.cloudFunctions.networkEgress);
        console.log('');
        ...
        if (usage.computeEngine.details) {
            usage.computeEngine.details.forEach(instance => {
                console.log(chalk.gray(`   └─ ${instance.name} (${instance.machineType}) - ${instance.status}`));
            });
        }
        console.log('');
    ...
    displayServiceUsage(name, usage) {
        ...
        console.log(`   ${name}: ${chalk[color](bar)} ${percentage.toFixed(1)}% (${usage.current}/${usage.limit})`);
    }
    ...
    async checkAlerts(usage) {
        ...
        if (usage.cloudFunctions.invocations.percentage > this.alertThreshold) {
            alerts.push({
                ...
                level: usage.cloudFunctions.invocations.percentage > this.criticalThreshold ? 'critical' : 'warning'
            });
        }
        ...
        if (usage.computeEngine.instances.percentage > this.alertThreshold) {
            alerts.push({
                ...
                level: usage.computeEngine.instances.percentage > this.criticalThreshold ? 'critical' : 'warning'
            });
        }
        ...
        if (alerts.length > 0) {
            ...
            alerts.forEach(alert => {
                ...
                console.log(chalk[color](`   ${alert.service} ${alert.metric}: ${alert.percentage.toFixed(1)}%`));
            });
        }
    }
    ...
    getSimulatedComputeEngineUsage() {
        return {
            ...
            details: [
                { name: 'context-api-vm', zone: 'asia-northeast3-a', machineType: 'e2-micro', status: 'RUNNING' }
            ]
        };
    }
