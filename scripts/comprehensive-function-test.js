...
class ComprehensiveFunctionTest {
    constructor() {
        this.baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://openmanager-vibe-v5.vercel.app';
    ...
    async testRealTimeFeatures(baseUrl, environment) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            await this.log(`    시도 ${i + 1}: ${result.responseTime}ms (캐시: ${result.headers?.['x-vercel-cache'] || 'N/A'})`);
        ...
        await this.log(`  ✅ 일관성 체크: ${analysis.consistencyCheck ? '통과' : '실패'}`);
    ...
    async testPerformanceMetrics(baseUrl, environment) {
        ...
        for (const test of performanceTests) {
            ...
            await this.log(`  ${test.name}: ${result.responseTime}ms (${result.success ? '성공' : '실패'})`);
        }
    ...
    async testErrorHandling(baseUrl, environment) {
        ...
        const errorTests = [
            ...
            { name: 'Invalid method', endpoint: '/api/system/status', method: 'POST' },
            { name: 'Malformed request', endpoint: '/api/system/status?invalid=<script>' }
        ];
        ...
        for (const test of errorTests) {
            ...
            const expected = result.status === 404 || result.status === 405 || result.status >= 400;
            await this.log(`  ${test.name}: ${result.status} (${expected ? '예상된 에러' : '예상치 못한 응답'})`);
        }
    ...
    async generateAnalysis() {
        ...
        const analysis = {
            ...
            coreAPIs: {
                production: production.coreAPIs ? Object.keys(production.coreAPIs).filter(k => production.coreAPIs[k].success).length : 0,
                local: local.coreAPIs ? Object.keys(local.coreAPIs).filter(k => local.coreAPIs[k].success).length : 0
            },
            performance: {
                production: production.realTime ? production.realTime.avgResponseTime : 0,
        ...
        if (analysis.performance.production > 500) {
            recommendations.push('⚠️ 프로덕션 응답시간이 느림 - 캐시 설정 재검토 필요');
        }
    ...
    async runFullTest() {
        try {
            ...
            this.testResults.production = {
                ...
                performance: await this.testPerformanceMetrics(this.baseUrl, 'Production'),
            ...
            try {
                ...
                const localCheck = await this.makeRequest(this.localUrl + '/api/system/status', 5000);
                ...
                if (localCheck.success) {
                    this.testResults.local = {
                        ...
                        performance: await this.testPerformanceMetrics(this.localUrl, 'Local')
                    };
                } else {
