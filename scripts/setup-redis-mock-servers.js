  ...
  return {
    cpu: {
      ...
      usage: Math.min(100, Math.max(0, 
        baseMetrics.cpu.usage + (Math.random() - 0.5) * baseMetrics.cpu.usage * variance
      ))
    },
    memory: {
      ...baseMetrics.memory,
      usage: Math.min(100, Math.max(0,
        baseMetrics.memory.usage + (Math.random() - 0.5) * baseMetrics.memory.usage * variance
      ))
    },
    disk: {
      ...baseMetrics.disk,
    ...
    network: {
      rx: Math.max(0, baseMetrics.network.rx + (Math.random() - 0.5) * baseMetrics.network.rx * variance),
      tx: Math.max(0, baseMetrics.network.tx + (Math.random() - 0.5) * baseMetrics.network.tx * variance),
      connections: Math.max(0, Math.floor(
        baseMetrics.network.connections + (Math.random() - 0.5) * baseMetrics.network.connections * variance
      ))
    }
  };
}
...
async function setupMockServers() {
  ...
  try {
    ...
    for (const server of MOCK_SERVERS) {
      ...
      // Redis에 저장 (30일 TTL)
      await redis.set(key, JSON.stringify(dynamicServer), { ex: 30 * 24 * 60 * 60 });
      savedCount++;
    ...
    for (const key of allKeys) {
      ...
      if (data) {
        ...
        console.log(`   - ${server.name}: ${server.status} (CPU: ${server.metrics.cpu.usage.toFixed(1)}%)`);
      }
    }
