...
class VercelMetricsMonitor {
  ...
  calculateStats() {
    ...
    this.metrics.summary = {
      ...
      errorRate:
        ((recentSamples.length - successfulSamples.length) /
  ...
  calculateRequestsPerMinute() {
    ...
    const recentRequests = this.metrics.samples.filter(
  ...
  displayDashboard() {
    ...
    console.log(
      `📈 Response Range: ${stats.minResponseTime}ms - ${stats.maxResponseTime}ms`
    );
    ...
    if (stats.requestsPerMinute > 0) {
      ...
      const improvement =
        ((previousCrisis - projectedDaily) / previousCrisis) * 100;

      console.log(
        `📅 Projected Daily: ${projectedDaily.toLocaleString()} requests`
      );
      console.log(
        `📉 vs Crisis (920K): ${Math.round(improvement)}% improvement`
      );
...
switch (command) {
  ...
  default:
    ...
    console.log(
      '  node vercel-metrics-monitor.js load [duration_minutes] [requests_per_second]'
    );
    break;
}
