...
class AIDataAnalyzer {
  ...
  async collectOperationalData() {
    ...
    try {
      ...
      console.log(
        `✅ 운영 로그 ${logs?.length || 0}개, 성능 메트릭 ${metrics?.length || 0}개 수집`
      );
  ...
  generateDataSummary(logs, metrics) {
    ...
    metrics.forEach(metric => {
      ...
      summary.performanceMetrics[name].avg =
        summary.performanceMetrics[name].total /
        summary.performanceMetrics[name].count;
    });
...
🔝 주요 엔드포인트:
${Object.entries(data.summary.topEndpoints)
  .map(
    ([endpoint, stats]) =>
...
📈 성능 메트릭:
${Object.entries(data.summary.performanceMetrics)
  .map(
    ([metric, stats]) =>
...
❌ 에러 발생 현황:
${Object.entries(data.summary.errorPatterns)
  .map(([error, count]) => `- ${error}: ${count}회 발생`)
  .join('\n')}
...
- 평균 응답시간: ${data.summary.avgResponseTime.toFixed(0)}ms
- 가장 많이 호출되는 API: ${
        Object.entries(data.summary.topEndpoints).sort(
          ([, a], [, b]) => b.count - a.count
        )[0]?.[0] || 'N/A'
      }
- 가장 느린 기능: ${
        Object.entries(data.summary.performanceMetrics).sort(
          ([, a], [, b]) => b.avg - a.avg
        )[0]?.[0] || 'N/A'
      }
...
특히 사용자 가치와 기술적 개선의 균형을 고려한 제안을 부탁드립니다.
  ...
  async runFullAnalysis() {
    ...
    console.log(
      `⏱️ 총 소요 시간: ${results.reduce((sum, r) => sum + r.processingTime, 0)}ms`
    );
  ...
  async showAnalysisSummary() {
      ...
      try {
        ...
        if (latestFile) {
          ...
          results.forEach(result => {
            console.log(
              `   - ${result.type}: ${result.analysis.substring(0, 100)}...`
            );
          });
        }
      } catch (error) {
    ...
    try {
      ...
      data.forEach((result, index) => {
        ...
        console.log(
          `   📅 분석 시간: ${new Date(result.created_at).toLocaleString('ko-KR')}`
        );
        console.log(`   ⏱️ 처리 시간: ${result.processing_time}ms`);
        console.log(
          `   📝 내용: ${result.analysis_content.substring(0, 150)}...`
        );
      });
    } catch (error) {
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('\n사용법:');
      console.log(
        '   node scripts/ai-data-analyzer.js analyze   # 전체 분석 실행'
      );
      console.log(
        '   node scripts/ai-data-analyzer.js summary   # 분석 결과 요약'
      );
      break;
  }
}
