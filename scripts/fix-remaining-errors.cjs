...
const fixes = [
  ...
  // servers/all/route.ts gcpDataService 수정
  {
    ...
    fix: (content) => {
      return content
        .replace(/const gcpDataService = [^;]+;/g, '// const gcpDataService removed')
        .replace(/const servers = await gcpDataService\.[^;]+;/g, 'const servers: any[] = []; // gcpDataService removed');
    }
  },
  ...
  // servers/realtime/route.ts GCPRealDataService 제거
  {
    ...
    fix: (content) => {
      return content
        .replace(/const gcpService = GCPRealDataService\.getInstance\(\);/g, '// const gcpService removed')
        .replace(/const gcpService = await GCPRealDataService\.getInstance\(\);/g, '// const gcpService removed')
        .replace(/\.map\(\(response\)/g, '.map((response: any)')
        .replace(/import { GCPRealDataService } from[^;]+;/g, '// GCPRealDataService import removed');
    }
  },
  ...
  // system/sync-data/route.ts 변수 수정
  {
    ...
    fix: (content) => {
      return content
        .replace(/const supabaseTime = Date\.now\(\) - supabaseStart;/g, 'const supabaseTime = 0; // supabaseStart removed')
        .replace(/const validationTime = Date\.now\(\) - validationStart;/g, 'const validationTime = 0; // validationStart removed');
    }
  },
  ...
  // MCPEngine.ts GCPRealDataService 제거
  {
    ...
    fix: (content) => {
      return content
        .replace(/const gcpService = GCPRealDataService\.getInstance\(\);/g, '// const gcpService removed')
        .replace(/const serverData = await GCPRealDataService\.getInstance\(\)\.[^;]+;/g, 'const serverData = { data: [] }; // GCPRealDataService removed')
        .replace(/GCPRealDataService\.getInstance\(\)/g, '{ getRealServerMetrics: async () => ({ data: [] }) }')
        .replace(/import { GCPRealDataService } from[^;]+;/g, '// GCPRealDataService import removed');
    }
  },
  ...
  // EnhancedDataAnalyzer.ts 수정
  {
    ...
    fix: (content) => {
      return content
        .replace(/const gcpService = GCPRealDataService\.getInstance\(\);/g, '// const gcpService removed')
        .replace(/const gcpDataService = GCPRealDataService\.getInstance\(\);/g, '// const gcpDataService removed')
        .replace(/GCPRealDataService\.getInstance\(\)/g, '{ getRealServerMetrics: async () => ({ data: [] }) }')
        .replace(/\.map\(\(response\)/g, '.map((response: any)')
  ...
  // ServerDataCache.ts generator 수정
  {
    ...
    fix: (content) => {
      return content
        ...
        .replace(/const data = await generator\.[^;]+;/g, 'const data = { data: [] }; // generator removed')
        .replace(/const mockData = await generator\.generateMockData\(\)\.map\(\(response\)/g, 'const mockData = [].map((response: any)');
    }
  },
  ...
  // UnifiedDataBroker.ts 수정
  {
    ...
    fix: (content) => {
      return content
        .replace(/const gcpDataService = [^;]+;/g, '// const gcpDataService removed')
        .replace(/const response = await gcpDataService\.[^;]+;/g, 'const response = { data: [] }; // gcpDataService removed')
        .replace(/await generator\.generateMockData\(\)\.map\(\(response\)/g, '[].map((response: any)')
        .replace(/generator\.generateMockData\(\)\.filter\(\(r\)/g, '[].filter((r: any)')
        .replace(/generator\.generateMockData\(\)\.forEach\(\(r\)/g, '[].forEach((r: any)')
        .replace(/generator\.generateMockData\(\)\.map\(\(r\)/g, '[].map((r: any)');
    }
  },
  ...
  // EnrichedMetricsGenerator.ts BaselineStorageService 제거
  {
    ...
    fix: (content) => {
      return content
        .replace(/const storageService = BaselineStorageService\.getInstance\(\);/g, '// const storageService removed')
        .replace(/import { BaselineStorageService } from[^;]+;/g, '// BaselineStorageService import removed');
    }
  },
  ...
  // server-metrics-adapter.ts GCPServerMetrics 제거
  {
    ...
    fix: (content) => {
      return content
        ...
        .replace(/GCPServerMetrics\[\]/g, 'any[] // GCPServerMetrics removed')
        .replace(/import { GCPServerMetrics } from[^;]+;/g, '// GCPServerMetrics import removed');
    }
  }
];
