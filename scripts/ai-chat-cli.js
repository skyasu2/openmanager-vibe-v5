...
class AIChatCLI {
    ...
    async start() {
        ...
        // 초기 상태 확인
        try {
            const status = await this.makeRequest('GET', '/api/ai-chat?action=status');
            if (status.success) {
                console.log(`✅ 서버 연결 성공! (${status.data.availableProviders.length}개 AI 제공자 사용 가능)`);
            }
        } catch (error) {
    ...
    async showProviders() {
        try {
            const result = await this.makeRequest('GET', '/api/ai-chat?action=providers');
            if (result.success) {
                console.log('\n🤖 사용 가능한 AI 제공자:');
                result.data.providers.forEach((provider, index) => {
                    console.log(`  ${index + 1}. ${provider.name} (${provider.model})`);
                    console.log(`     키워드: ${provider.name.toLowerCase().split(' ')[0]}`);
                });
                console.log('');
            } else {
    ...
    async sendMessage(message) {
        if (!this.currentSessionId) {
            console.log('❌ 활성 세션이 없습니다. /start 명령어로 대화를 시작하세요.');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                if (response.metadata) {
                    console.log(`📊 토큰: ${response.metadata.tokensUsed || 0}, 시간: ${responseTime}ms, 신뢰도: ${Math.round((response.metadata.confidence || 0) * 100)}%`);
                }
                console.log('');
            } else {
    ...
    async showSessions() {
        try {
            const result = await this.makeRequest('GET', '/api/ai-chat?action=sessions');
            if (result.success) {
                ...
                sessions.forEach((session, index) => {
                    ...
                    console.log(`     수정: ${new Date(session.updatedAt).toLocaleString('ko-KR')}`);
                    console.log('');
                });
            } else {
    ...
    async switchSession(sessionId) {
        try {
            ...
            if (result.success) {
                ...
                console.log(`✅ 세션을 전환했습니다: ${result.data.currentSession.title}`);
            } else {
