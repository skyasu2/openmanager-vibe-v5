...
class VercelSystemTest {
  ...
  async testVercelServerDataGenerator() {
    ...
    try {
      const response = await this.makeRequest(
        'GET',
        `${this.vercelUrl}/api/servers/realtime?limit=5`
      );
      ...
      this.log(
        `❌ Vercel 서버 데이터 생성기 테스트 실패: ${error.message}`,
        'error'
      );
      this.results.serverDataGenerator = {
        status: 'failed',
  ...
  async testDashboardDataRefresh() {
    this.log(
      '📊 2. Vercel 대시보드 데이터 갱신 및 24시간 데이터 테스트',
      'info'
    );

    try {
      // 실시간 메트릭 API 테스트
      const realtimeResponse = await this.makeRequest(
        'GET',
        `${this.vercelUrl}/api/metrics/realtime`
      );

      // 서버 상태 요약 API 테스트
      const summaryResponse = await this.makeRequest(
        'GET',
        `${this.vercelUrl}/api/servers/summary`
      );
      ...
      this.log(
        `   - 서버 요약: ${summaryResponse.status} (총 ${this.results.dashboardData.summary.totalServers}개)`,
        'info'
      );
      ...
      this.log(
        `❌ Vercel 대시보드 데이터 갱신 테스트 실패: ${error.message}`,
        'error'
      );
      this.results.dashboardData = {
        status: 'failed',
  ...
  async testServerCardModal() {
    ...
    try {
      // 서버 목록 가져오기
      const serversResponse = await this.makeRequest(
        'GET',
        `${this.vercelUrl}/api/servers/realtime?limit=3`
      );
      ...
      try {
        serverDetailResponse = await this.makeRequest(
          'GET',
          `${this.vercelUrl}/api/servers/${firstServer.id}`
        );
      } catch (detailError) {
      ...
      this.log(
        `   - 서버 목록: ${this.results.serverCards.serverList.count}개`,
        'info'
      );
      this.log(
        `   - 첫 번째 서버: ${firstServer.name} (${firstServer.status})`,
        'info'
      );
      this.log(
        `   - CPU: ${firstServer.cpu}%, Memory: ${firstServer.memory}%`,
        'info'
      );
      ...
      this.log(
        `❌ Vercel 서버 카드 및 모달 테스트 실패: ${error.message}`,
        'error'
      );
      this.results.serverCards = {
        status: 'failed',
  ...
  async testAIAssistantDataAnalysis() {
    ...
    try {
      // AI 어시스턴트 상태 확인
      const aiStatusResponse = await this.makeRequest(
        'GET',
        `${this.vercelUrl}/api/ai/status`
      );

      // AI 통합 쿼리 테스트 (서버 데이터 분석)
      const aiQueryResponse = await this.makeRequest(
        'POST',
        `${this.vercelUrl}/api/ai/unified-query`,
        {
          query:
            '현재 서버 상태를 분석해주세요. CPU와 메모리 사용률이 높은 서버가 있나요?',
          mode: 'AUTO',
      ...
      this.log(
        `   - AI 쿼리: ${aiQueryResponse.status} (${this.results.aiAssistant.aiQuery.processingTime}ms)`,
        'info'
      );
      this.log(
        `   - 신뢰도: ${this.results.aiAssistant.aiQuery.confidence}`,
        'info'
      );
      this.log(
        `   - 사용된 엔진: ${this.results.aiAssistant.aiQuery.enginesUsed.join(', ')}`,
        'info'
      );
      ...
      this.log(
        `❌ Vercel AI 어시스턴트 테스트 실패: ${error.message}`,
        'error'
      );
      this.results.aiAssistant = {
        status: 'failed',
  ...
  async testMCPServerIntegration() {
    ...
    try {
      ...
      // MCP 서버 파일 시스템 테스트
      const fileSystemResponse = await this.makeRequest(
        'POST',
        `${this.mcpServerUrl}/mcp/tools/list_directory`,
        {
          path: '.',
        }
      );
      ...
      this.log(
        `   - 파일시스템: ${fileSystemResponse.status} (${this.results.mcpServer.fileSystem.filesCount}개 파일)`,
        'info'
      );
  ...
  generateReport() {
    ...
    const testResults = [
      {
        name: 'Vercel 서버 데이터 생성기',
        result: this.results.serverDataGenerator,
      },
    ...
    testResults.forEach(test => {
      if (test.result?.status === 'success') {
        ...
        this.log(
          `❌ ${test.name}: 실패 - ${test.result?.error || '알 수 없는 오류'}`,
          'error'
        );
        failedTests++;
      }
    ...
    this.log(
      `\n📊 테스트 결과: ${passedTests}개 성공, ${failedTests}개 실패`,
      'info'
    );
    ...
    if (this.results.serverDataGenerator?.status === 'success') {
      this.log(
        `   - Vercel 서버 데이터: ${this.results.serverDataGenerator.totalServers}개 서버 운영 중`,
        'info'
      );
      this.log(
        `   - Mock 모드: ${this.results.serverDataGenerator.isMockMode ? 'Yes (주의 필요)' : 'No (실제 데이터)'}`,
        this.results.serverDataGenerator.isMockMode ? 'warning' : 'success'
      );
    }

    if (this.results.aiAssistant?.status === 'success') {
      this.log(
        `   - AI 어시스턴트 응답시간: ${this.results.aiAssistant.aiQuery.processingTime}ms`,
        'info'
      );
      this.log(
        `   - AI 신뢰도: ${this.results.aiAssistant.aiQuery.confidence}`,
        'info'
      );
    }
    ...
    if (this.results.mcpServer?.status === 'success') {
      this.log(
        `   - MCP 서버 상태: 정상 (${this.results.mcpServer.fileSystem.filesCount}개 파일 접근 가능)`,
        'success'
      );
    }
