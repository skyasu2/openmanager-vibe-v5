...
async function testMonitoringProcessing() {
    // 동적 import 사용
    const { UnifiedDataProcessor } = await import('../../src/services/data-generator/UnifiedDataProcessor.ts');
...
async function testAIProcessing() {
    ...
    if (normalized.cpu < 0 || normalized.cpu > 1 ||
        normalized.memory < 0 || normalized.memory > 1 ||
        normalized.disk < 0 || normalized.disk > 1 ||
        normalized.network < 0 || normalized.network > 1) {
...
async function testBothProcessing() {
    ...
    return {
        ...
        processingTime,
        dataConsistency: result.monitoring.servers.length === result.ai.metrics.length,
        completeness: result.metadata.completeness
    };
}
...
async function testAIFeatures() {
    ...
    metrics.forEach(metric => {
        ...
        // 패턴 시그니처 검증
        if (features.patternSignature && features.patternSignature.startsWith('pattern_')) {
            featuresValidation.patternSignatures.push(features.patternSignature);
        }

        // 컨텍스트 검증
        if (context.serverRole && context.environment && context.businessCriticality) {
            validContextCount++;
        }
    });
    ...
    return {
        ...
        avgAnomalyScore: featuresValidation.anomalyScores.reduce((a, b) => a + b, 0) / featuresValidation.anomalyScores.length,
...
function generateReport() {
    ...
    log(`   실패: ${testResults.failedTests}개`, testResults.failedTests > 0 ? 'red' : 'white');
    log(`   성공률: ${successRate.toFixed(1)}%`, successRate >= 90 ? 'green' : 'yellow');

    log(`\n⚡ 성능 분석:`, 'cyan');
    log(`   모니터링 처리: ${testResults.performance.monitoringProcessingTime}ms`);
    log(`   AI 처리: ${testResults.performance.aiProcessingTime}ms`);
    log(`   통합 처리: ${testResults.performance.bothProcessingTime}ms`);
    log(`   캐시 히트율: ${(testResults.performance.cacheHitRate * 100).toFixed(1)}%`);

    log(`\n📈 데이터 품질:`, 'cyan');
    log(`   모니터링 데이터 품질: ${(testResults.dataQuality.monitoringDataQuality * 100).toFixed(1)}%`);
    log(`   AI 데이터 품질: ${(testResults.dataQuality.aiDataQuality * 100).toFixed(1)}%`);
    log(`   완전성: ${(testResults.dataQuality.completeness * 100).toFixed(1)}%`);

    log(`\n🎯 종합 평가:`, 'cyan');
    if (successRate >= 95 && testResults.performance.monitoringProcessingTime < 200) {
    ...
    const reportPath = path.join(__dirname, '../docs/unified-processor-test-report.json');
