...
class IdleSystemUsageAnalyzer {
  ...
  async checkSystemStatus() {
    try {
      ...
      if (status.isRunning) {
        console.log(
          '⚠️ 시스템이 실행 중입니다. 중지 후 테스트하는 것을 권장합니다.'
        );
      }
    } catch (error) {
  ...
  async monitorApiCalls() {
    ...
    const endpoints = ['/api/system/status', '/api/metrics', '/api/health'];
  ...
  async analyzeResults() {
    ...
    const totalCalls = this.apiCalls.reduce(
      (sum, ep) => sum + ep.totalCalls,
      0
    );
    ...
    console.log(
      `📊 하루 예상 호출 수: ${Math.round(callsPerMinute * 60 * 24)}회/일`
    );
