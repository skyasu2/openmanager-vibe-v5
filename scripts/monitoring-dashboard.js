...
class MonitoringDashboard {
  ...
  calculateStats() {
    ...
    const recentCacheHits = this.metrics.cacheHits.filter(
      c => new Date(c.timestamp).getTime() > last24h
    );
  ...
  checkSafetyLimits(stats) {
    ...
    if (stats.dailyRequests > this.safetyLimits.dailyRequests) {
      alerts.push(
        `🚨 일일 요청 한도 초과: ${stats.dailyRequests} > ${this.safetyLimits.dailyRequests}`
      );
    }

    if (stats.avgResponseTime > this.safetyLimits.avgResponseTime) {
      alerts.push(
        `⚠️ 평균 응답 시간 초과: ${Math.round(stats.avgResponseTime)}ms > ${this.safetyLimits.avgResponseTime}ms`
      );
    }

    if (stats.errorRate > this.safetyLimits.errorRate) {
      alerts.push(
        `❌ 에러율 초과: ${Math.round(stats.errorRate)}% > ${this.safetyLimits.errorRate}%`
      );
    }

    if (stats.cacheHitRate < this.safetyLimits.cacheHitRate) {
      alerts.push(
        `🗄️ 캐시 히트율 부족: ${Math.round(stats.cacheHitRate)}% < ${this.safetyLimits.cacheHitRate}%`
      );
    }
  ...
  displayDashboard() {
    ...
    console.log('📈 핵심 지표:');
    console.log(
      `  일일 요청: ${stats.dailyRequests.toLocaleString()} / ${this.safetyLimits.dailyRequests.toLocaleString()}`
    );
    console.log(
      `  평균 응답: ${Math.round(stats.avgResponseTime)}ms / ${this.safetyLimits.avgResponseTime}ms`
    );
    console.log(
      `  에러율: ${Math.round(stats.errorRate)}% / ${this.safetyLimits.errorRate}%`
    );
    console.log(
      `  캐시 히트율: ${Math.round(stats.cacheHitRate)}% / ${this.safetyLimits.cacheHitRate}%`
    );
    ...
    console.log(
      `  위기 대비: ${Math.round(((920000 - projectedDaily) / 920000) * 100)}% 감소`
    );
    console.log('');
