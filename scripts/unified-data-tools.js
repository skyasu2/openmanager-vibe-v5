...
class UnifiedDataTools {
  ...
  async analyzeDataPatterns() {
    ...
    try {
      ...
      console.log(
        `  활성 서버: ${analysis.serverAnalysis.activeServers}/${analysis.serverAnalysis.totalServers}`
      );
      console.log(`  평균 CPU: ${analysis.serverAnalysis.avgCPU}%`);
      console.log(
        `  평균 응답시간: ${analysis.metricsAnalysis.avgResponseTime}ms`
      );
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log(
            '  node unified-data-tools.js collect     # 프로덕션 데이터 수집'
          );
          console.log(
            '  node unified-data-tools.js validate    # 데이터 통합 검증'
          );
          console.log(
            '  node unified-data-tools.js analyze     # 데이터 패턴 분석'
          );
          console.log(
            '  node unified-data-tools.js consistency # 데이터 일관성 검사'
          );
          break;
      }
