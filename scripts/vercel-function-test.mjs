  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      recordTest(
        '헬스체크 API',
        true,
        `상태: ${data.status}, 버전: ${data.version}`
      );
      return data;
    } else {
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      recordTest(
        '대시보드 페이지',
        hasContent,
        `페이지 크기: ${response.body.length} bytes`
      );
    } else {
...
async function testAIFeatures() {
  const aiEndpoints = [
    {
      path: '/api/ai-agent/integrated?action=status',
      name: 'AI 에이전트 상태',
    },
    {
      path: '/api/ai-agent/integrated?action=health',
      name: 'AI 에이전트 헬스',
    },
...
async function testDataGenerators() {
  const dataEndpoints = [
    ...
    {
      path: '/api/data-generator/unified-preprocessing',
      name: '통합 전처리 엔진',
    },
  ];
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      if (data.services && data.services.memory) {
        recordTest(
          '메모리 상태',
          data.services.memory === 'healthy',
          `상태: ${data.services.memory}`
        );
      }
    }
  } catch (error) {
