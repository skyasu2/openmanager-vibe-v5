...
analyze_with_qwen() {
    ...
    log_info "🤖 Qwen CLI (성능+알고리즘) 분석 중... (30초 타임아웃)"
    ...
    if [ -f "$file_path" ]; then
        ...
        timeout 30s qwen -p "TypeScript $(basename "$file_path") 성능 점수(10점)와 최적화 제안 1개" 2>/dev/null || {
            log_warning "⚠️ Qwen CLI 타임아웃 (30초 초과)"
