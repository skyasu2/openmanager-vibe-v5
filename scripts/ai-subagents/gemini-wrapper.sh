...
# 버전: 3.0.0
# 날짜: 2025-11-21 (범용 도구로 개선 + auto-ai-review.sh 개선사항 적용)
...
$query"
    ...
    # Gemini 실행 (stderr 분리) - auto-ai-review.sh와 동일한 패턴
    if timeout "${TIMEOUT_SECONDS}s" bash -c "echo \"$query\" | gemini --model \"$model\"" > "$temp_stdout" 2> "$temp_stderr"; then
        exit_code=0
    else
        exit_code=$?
    fi
...
usage() {
    cat << EOF
${CYAN}🟢 Gemini CLI Wrapper v3.0.0 - Claude Code 내부 도구${NC}
...
main() {
    ...
    echo ""
    log_info "🚀 Gemini Wrapper v3.0.0 시작"
    echo ""
