...
class ServerManager {
        ...
        .map(line => {
          ...
          if (parts.length >= 3) {
            ...
            if (
              ...
              commandLine.includes('duckduckgo') ||
              commandLine.includes('tavily')
            ) {
              return {
                pid,
  ...
  async cleanupDeadProcesses() {
    console.log(
      `🧹 [${await this.getCurrentTime()}] 죽은 프로세스 정리 시작...`
    );
    ...
    for (const lock of locks) {
      try {
        ...
        if (!stdout.includes(lock.pid)) {
          ...
          cleanedCount++;
          console.log(
            `  ❌ 죽은 프로세스 락 제거: ${lock.serverType} (PID: ${lock.pid})`
          );
        }
      } catch (error) {
        await this.removeServerLock(lock.serverType);
    ...
    console.log(
      `✅ [${await this.getCurrentTime()}] ${cleanedCount}개 죽은 프로세스 락 정리 완료`
    );
    return cleanedCount;
  }
  ...
  async startServer(serverType, command, options = {}) {
    ...
    try {
      ...
      console.log(
        `✅ [${time}] ${serverType} 서버 시작 완료 (PID: ${child.pid}, Port: ${port})`
      );
      return child;
    } catch (error) {
      console.error(
        `❌ [${time}] ${serverType} 서버 시작 실패:`,
        error.message
      );
      return false;
    }
  ...
  async stopServer(serverType) {
    ...
    if (killed) {
      await this.removeServerLock(serverType);
      console.log(
        `✅ [${time}] ${serverType} 서버 중지 완료 (PID: ${lock.pid})`
      );
      return true;
    } else {
      console.log(
        `❌ [${time}] ${serverType} 서버 중지 실패 (PID: ${lock.pid})`
      );
      return false;
    }
  }
  ...
  async cleanup() {
    ...
    locks.forEach((lock, index) => {
      console.log(
        `  ${index + 1}. ${lock.serverType} (PID: ${lock.pid}, Port: ${lock.port})`
      );
    });
  ...
  switch (command) {
    ...
    case 'start':
      ...
      if (serverType === 'nextjs') {
        await serverManager.startServer('NextJS', 'npm run dev', {
          port: 3000,
        });
      } else if (serverType === 'storybook') {
        await serverManager.startServer('Storybook', 'npm run storybook', {
          port: 6006,
        });
      }
      break;
