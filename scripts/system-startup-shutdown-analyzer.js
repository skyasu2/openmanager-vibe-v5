...
class SystemAnalyzer {
  ...
  async monitorIdleState() {
    ...
    while (Date.now() < endTime) {
      try {
        ...
        console.log(
          `📊 상태 체크: ${responseTime}ms (실행: ${response.isRunning})`
        );
      } catch (error) {
        calls.push({
          timestamp: new Date(),
    ...
    this.results.idleMonitoring = {
      ...
      avgResponseTime:
        calls
          ...
          calls.filter(c => c.success).length || 0,
      calls: calls,
    };
  ...
  async analyzeResults() {
    ...
    // 활성 백그라운드 프로세스 분석
    const activeProcesses = this.results.backgroundProcesses.filter(
      p => p.active
    );
  ...
  async generateReport() {
    ...
    if (this.results.improvements.length > 0) {
      ...
      this.results.improvements.forEach((improvement, index) => {
        console.log(
          `  ${index + 1}. [${improvement.priority.toUpperCase()}] ${improvement.description}`
        );
      });
    }
  ...
  async saveReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        activeBackgroundProcesses: this.results.backgroundProcesses.filter(
          p => p.active
        ).length,
      },
      ...this.results,
