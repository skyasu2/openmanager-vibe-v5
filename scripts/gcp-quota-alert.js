...
class GCPQuotaAlert {
  ...
  async startAlertMonitoring() {
    ...
    console.log(
      chalk.gray(
        `⚠️ 경고: ${this.thresholds.warning}% | 위험: ${this.thresholds.critical}% | 긴급: ${this.thresholds.emergency}%\n`
      )
    );
  ...
  async checkAndAlert() {
    ...
    await this.checkServiceAlerts(
      'Cloud Functions - 호출',
      usage.cloudFunctions.invocations
    );
    await this.checkServiceAlerts(
      'Cloud Functions - GB초',
      usage.cloudFunctions.gbSeconds
    );
    await this.checkServiceAlerts(
      'Cloud Functions - 네트워크',
      usage.cloudFunctions.networkEgress
    );
    await this.checkServiceAlerts(
      'Compute Engine - 인스턴스',
      usage.computeEngine.instances
    );
    await this.checkServiceAlerts(
      'Cloud Storage - 저장소',
      usage.cloudStorage.storage
    );
    await this.checkServiceAlerts(
      'Cloud Storage - 작업',
      usage.cloudStorage.operations
    );
  ...
  displayAlert(alert) {
    ...
    console.log(
      chalk.gray(
        `📊 ${alert.usage.current}/${alert.usage.limit} (${alert.usage.percentage.toFixed(1)}%)`
      )
    );
