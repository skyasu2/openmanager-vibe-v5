...
class GCPQuotaReport {
  ...
  async getProjectedUsage() {
    ...
    const projections = {
      cloudFunctions: {
        invocations: this.calculateProjection(
          current.cloudFunctions.invocations,
          projectionPeriod
        ),
        gbSeconds: this.calculateProjection(
          current.cloudFunctions.gbSeconds,
          projectionPeriod
        ),
        networkEgress: this.calculateProjection(
          current.cloudFunctions.networkEgress,
          projectionPeriod
        ),
      },
      computeEngine: {
        instances: this.calculateProjection(
          current.computeEngine.instances,
          projectionPeriod
        ),
        storage: this.calculateProjection(
          current.computeEngine.storage,
          projectionPeriod
        ),
        networkEgress: this.calculateProjection(
          current.computeEngine.networkEgress,
          projectionPeriod
        ),
      },
      cloudStorage: {
        storage: this.calculateProjection(
          current.cloudStorage.storage,
          projectionPeriod
        ),
        classAOperations: this.calculateProjection(
          current.cloudStorage.classAOperations,
          projectionPeriod
        ),
        classBOperations: this.calculateProjection(
          current.cloudStorage.classBOperations,
          projectionPeriod
        ),
        networkEgress: this.calculateProjection(
          current.cloudStorage.networkEgress,
          projectionPeriod
        ),
      },
      firestore: {
        enabled: false,
        reads: this.calculateProjection(
          current.firestore.reads,
          projectionPeriod
        ),
        writes: this.calculateProjection(
          current.firestore.writes,
          projectionPeriod
        ),
        deletes: this.calculateProjection(
          current.firestore.deletes,
          projectionPeriod
        ),
        storage: this.calculateProjection(
          current.firestore.storage,
          projectionPeriod
        ),
      },
    };
  ...
  async getActionItems() {
    ...
    const actionItems = [
      ...
      },
      {
        ...
        commands: ['npm run gcp:monitor', 'npm run gcp:alert'],
        estimatedTime: '10분',
        impact: '사전 경고 시스템 구축',
      },
      {
      ...
      },
      {
        ...
        commands: ['npm run gcp:quota-report', 'crontab -e # 주간 스케줄 설정'],
        estimatedTime: '20분',
        impact: '지속적인 모니터링',
      },
    ];
...
`;
  ...
  generateProjectionTable(projected) {
    const services = [
      {
        name: 'Cloud Functions 호출',
        data: projected.cloudFunctions.invocations,
      },
      {
        name: 'Cloud Functions GB-초',
        data: projected.cloudFunctions.gbSeconds,
      },
      {
        name: 'Compute Engine 인스턴스',
        data: projected.computeEngine.instances,
      },
      { name: 'Cloud Storage 저장소', data: projected.cloudStorage.storage },
    ];

    let table =
      '| 서비스 | 현재 사용량 | 예상 사용량 | 예상 사용률 | 위험도 |\n';
    table += '|--------|-------------|-------------|-------------|--------|\n';
  ...
  displayReportSummary(data) {
    ...
    console.log(
      chalk.white(`   전체 위험 항목: ${data.riskAnalysis.totalRisks}개`)
    );
    console.log(
      chalk.white(
        `   고위험: ${data.riskAnalysis.highRisks}개 | 중위험: ${data.riskAnalysis.mediumRisks}개`
      )
    );

    console.log(chalk.cyan('\n💡 주요 제안'));
    const highPrioritySuggestions = data.optimizationSuggestions.filter(
      s => s.priority === 'high'
    );
...
switch (command) {
  ...
  default:
    ...
    console.log(
      chalk.gray('  npm run gcp:quota-report          # 종합 보고서 생성')
    );
    console.log(
      chalk.gray('  npm run gcp:quota-report generate # 종합 보고서 생성')
    );
    break;
}
