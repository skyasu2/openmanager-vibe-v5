...
class ServerMonitor {
  ...
  async detectDuplicateProcesses() {
    ...
    Object.entries(processNames).forEach(([name, count]) => {
      if (count > 2 && name === 'node.exe') {
        // node.exe는 2개 이상부터 의심
        duplicates.push({
          type: 'process',
          name: name,
          count: count,
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      await this.log(
        `📋 Node.js 프로세스 ${status.nodeProcesses.length}개, MCP 서버 ${status.mcpProcesses.length}개 실행 중`,
        'INFO'
      );
  ...
  async sendAlert(duplicates) {
    const now = new Date();
    const timeSinceLastAlert = this.lastAlert
      ? (now - this.lastAlert) / 1000 / 60
      : Infinity; // 분 단위

    // 30분 이내에 알림을 보낸 경우 스킵
    if (timeSinceLastAlert < 30) {
      await this.log(
        `🔕 최근 알림 발송으로 인해 알림 스킵 (${Math.round(timeSinceLastAlert)}분 전)`,
        'INFO'
      );
      return;
    }
    ...
    this.lastAlert = now;
    const alertMessage =
      `🚨 서버 중복 실행 알림 (${this.duplicateCount}번 연속 감지)\n` +
  ...
  async startMonitoring() {
    ...
    await this.log(
      `⚠️ 알림 임계값: ${this.alertThreshold}번 연속 중복 감지시`,
      'INFO'
    );
  ...
  async generateReport() {
    ...
    const reportFile = path.join(
      process.cwd(),
      'logs',
      `server-monitor-report-${new Date().toISOString().substring(0, 10)}.json`
    );
