...
console.log('📅 현재 한국시간:', new Date().toLocaleString('ko-KR', {
    timeZone: 'Asia/Seoul',
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
...
function analyzeUsage(usage, quota) {
    ...
    if (dailyUsageRate > 90) {
        ...
        analysis.warnings.push(`🚨 일일 할당량 90% 초과 (${dailyUsageRate.toFixed(1)}%)`);
        analysis.recommendations.push('즉시 사용량 제한 또는 유료 플랜 고려');
    } else if (dailyUsageRate > 70) {
        analysis.status = 'warning';
        analysis.warnings.push(`⚠️ 일일 할당량 70% 초과 (${dailyUsageRate.toFixed(1)}%)`);
    ...
    if (minuteUsageRate > 80) {
        analysis.warnings.push(`⚠️ 분당 할당량 80% 초과 (${minuteUsageRate.toFixed(1)}%)`);
    ...
    if (testRatio > 30) {
        analysis.warnings.push(`📊 테스트 사용량 비율 높음 (${testRatio.toFixed(1)}%)`);
...
function displayUsageReport() {
    ...
    console.log(`   일일 사용량: ${usage.daily.total}회 / ${modelQuota.rpd}회 (${((usage.daily.total / modelQuota.rpd) * 100).toFixed(1)}%)`);
    ...
    console.log(`🚦 상태: ${getStatusEmoji(analysis.status)} ${analysis.status.toUpperCase()}`);
    console.log();
    ...
    console.log();
    console.log('='.repeat(70));
    console.log('🔄 다음 업데이트:', new Date(Date.now() + 60000).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }) + ' (KST)');
}
...
function displayQuotaComparison() {
    ...
    console.log('모델'.padEnd(25) + 'RPM'.padStart(5) + 'TPM'.padStart(10) + 'RPD'.padStart(6) + ' 추천용도');
    console.log('-'.repeat(70));

    Object.entries(QUOTAS).forEach(([key, quota]) => {
        const isCurrentModel = key === currentModel ? '👉 ' : '   ';
        const tpmFormatted = quota.tpm >= 1000000 ? `${quota.tpm / 1000000}M` : `${quota.tpm / 1000}K`;
...
console.log('\n✅ 모니터링 완료:', new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }) + ' (KST)'); 