...
try {
  ...
  cliUtils = {
    ...
    validateAPIKey: key =>
      key && key.startsWith('AIza') && key.length >= 20 && key.length <= 50,
  ...
  async encryptGoogleAI() {
    ...
    try {
      ...
      const apiKey = await new Promise(resolve => {
        rl.question(
          `${cliUtils.colors.yellow}Google AI Studio API 키를 입력하세요:${cliUtils.colors.reset} `,
          resolve
        );
      });
      ...
      if (!cliUtils.validateAPIKey(apiKey.trim())) {
        cliUtils.errorLog(
          '올바르지 않은 API 키 형식입니다. (AIza로 시작해야 하며, 20-50자 사이)'
        );
        return false;
      }
      ...
      backup.entries.forEach(entry => {
        try {
          const value = entry.encrypted
            ? this.decryptLegacy(entry.value)
            : entry.value;
