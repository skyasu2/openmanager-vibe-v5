...
| --------------- | ----------- | -------------------- | -------------------- |
| **일반 개발**   | 4GB         | `npm run dev:stable` | 대부분의 개발 작업   |
| **네트워크**    | 4GB         | `npm run dev:network`| WSL 외부 접속 필요시 |
| **안전 모드**   | 4GB         | `npm run wsl:dev`    | WSL 환경 권장        |

### 🏗️ 빌드 환경

| 환경            | 메모리 설정 | 명령어                | 용도                      |
| --------------- | ----------- | --------------------- | ------------------------- |
| **프로덕션**    | 2GB         | `npm run build:prod`  | 프로덕션 빌드             |
| **CI 환경**     | 2GB         | `npm run build:ci`    | GitHub Actions 최적화     |
| **Vercel 빌드** | 2GB         | `npm run build:vercel`| Vercel 배포용             |

### 💻 Claude Code 실행

| 용도          | 메모리 설정 | 명령어          | 권장 상황                |
| ------------- | ----------- | --------------- | ------------------------ |
| **일반 사용** | 기본        | `claude`        | 표준 AI 개발 작업        |

> **Note**: Claude Code는 자체 메모리 관리를 수행합니다. 시스템 메모리가 충분하면(16GB+) 별도 설정 불필요.
...
# 2단계: 안전 모드로 시작
npm run wsl:dev          # WSL 안전 모드 (4GB)

# 3단계: 또는 표준 모드
npm run dev:stable       # 표준 개발 모드 (4GB)
...
```bash
# 개발
npm run dev:stable       # 4GB (표준)
npm run dev:network      # 4GB (네트워크 접속)
npm run wsl:dev          # 4GB (WSL 안전 모드)

# 빌드
npm run build:prod       # 2GB (프로덕션)
npm run build:ci         # 2GB (CI)
npm run build:vercel     # 2GB (Vercel 배포)
