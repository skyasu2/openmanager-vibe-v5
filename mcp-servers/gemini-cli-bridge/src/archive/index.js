...
async function main() {
  console.error(
    '[PowerShell Gemini CLI Bridge] PowerShell 전용 서버 시작 중...'
  );
  ...
  try {
    ...
    console.error(
      `[PowerShell Gemini CLI Bridge] 호출 컨텍스트: ${context.caller} → ${context.target}`
    );
    console.error(
      `[PowerShell Gemini CLI Bridge] 실행 전략: ${context.executionStrategy}`
    );
    ...
    console.error(
      '[PowerShell Gemini CLI Bridge] ✅ 초기화 완료 - PowerShell 최적화 모드 활성화'
    );
  } catch (error) {
    console.error(
      '[PowerShell Gemini CLI Bridge] ⚠️ 초기화 경고:',
      error.message
    );
    console.error(
      '[PowerShell Gemini CLI Bridge] 계속 진행하지만 일부 기능이 제한될 수 있습니다.'
    );
  }
  ...
  console.error(
    '[PowerShell Gemini CLI Bridge] MCP 서버가 stdio 모드로 실행 중입니다.'
  );
  console.error(
    '[PowerShell Gemini CLI Bridge] Claude ↔ Gemini PowerShell 양방향 호출 지원'
  );
}
