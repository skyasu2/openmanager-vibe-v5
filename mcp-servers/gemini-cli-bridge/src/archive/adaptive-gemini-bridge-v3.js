...
export class AdaptiveGeminiBridge {
  ...
  async initialize() {
    ...
    console.error(
      `[GeminiBridge v3] 초기화 완료 - 전략: ${this.context.executionStrategy}`
    );
    return this.context;
  }
  ...
  _buildChatCommand(prompt, options = {}) {
    ...
    console.error(
      `[GeminiBridge v3] 선택된 모델: ${strategy.selectedModel || options.model || 'default'}`
    );
    console.error(
      `[GeminiBridge v3] 전략 특성: ${JSON.stringify(strategy.characteristics || {})}`
    );
  ...
  async chat(prompt, options = {}) {
    ...
    const fallbackChain = buildFallbackChain(
      strategy.selectedModel || options.model
    );
    ...
    for (let i = 0; i < fallbackChain.length; i++) {
      ...
      try {
        ...
        const modelCommand = command.replace(/-m \S+/, `-m ${currentModel}`);
  ...
  async getStats() {
    try {
      ...
      const rawStats = await this.executeCommand(statsCommand, {
        timeout: 5000,
      });
  ...
  async executeCommand(command, options = {}) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.error(
          `[GeminiBridge v3] 시도 ${attempt}/${retries} 실패:`,
          error.message
        );
  ...
  async _executeWithStrategy(command, timeout) {
    ...
    if (!strategy) {
      console.warn(
        `[GeminiBridge v3] 알 수 없는 전략 '${strategyName}', powershell 사용`
      );
