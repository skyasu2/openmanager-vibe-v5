...
export class CachedContextDetector extends ContextDetector {
  ...
  async detectContext() {
    ...
    if (cachedResult) {
      this.cacheHits++;
      console.error(
        `[CachedContextDetector] 캐시 히트! (히트율: ${this.getCacheHitRate()}%)`
      );
      return cachedResult;
    }
  ...
  getCachedContext(key) {
    ...
    if (age > this.cacheTimeout) {
      console.error(
        `[CachedContextDetector] 캐시 만료 (나이: ${Math.round(age / 1000)}초)`
      );
  ...
  setCachedContext(key, context) {
    ...
    if (this.cache.size >= this.maxCacheSize) {
      ...
      console.error(
        '[CachedContextDetector] 캐시 크기 제한 - 오래된 항목 제거'
      );
    }
    ...
    console.error(
      `[CachedContextDetector] 컨텍스트 캐싱 완료 (캐시 크기: ${this.cache.size})`
    );
  }
  ...
  clearCache() {
    ...
    this.cacheMisses = 0;
    console.error(
      `[CachedContextDetector] 캐시 초기화 완료 (${size}개 항목 제거)`
    );
  }
  ...
  invalidateCache(key) {
    if (this.cache.delete(key)) {
      console.error(
        `[CachedContextDetector] 캐시 항목 무효화: ${key.substring(0, 8)}...`
      );
      return true;
    }
    return false;
  ...
  watchForChanges() {
    ...
    const envWatcher = setInterval(() => {
      ...
      if (hasChanges && this.cache.size > 0) {
        console.error(
          '[CachedContextDetector] 환경 변경 감지 - 캐시 자동 무효화'
        );
        this.clearCache();
      }
    }, 10000); // 10초마다 확인
  ...
  printDebugInfo() {
    ...
    if (this.cache.size > 0) {
      ...
      this.getCacheStats().entries.forEach(entry => {
        console.error(
          `  - ${entry.key} (${entry.age}초 전): ${entry.context.caller} → ${entry.context.executionStrategy}`
        );
      });
    }
  ...
  async warmupCache() {
    ...
    for (const scenario of scenarios) {
      ...
      try {
        ...
        console.error(
          '[CachedContextDetector] 캐시 예열 중 오류:',
          error.message
        );
      }
    ...
    console.error(
      `[CachedContextDetector] PowerShell 캐시 예열 완료 (${this.cache.size}개 항목)`
    );
  }
}
