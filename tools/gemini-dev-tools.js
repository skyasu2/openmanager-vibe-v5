...
class GeminiDevTools {
  ...
  async executeGemini(args = [], options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (hasStdinInput) {
        ...
        if (this.debug) {
          console.error(
            `[GeminiDevTools] stdin 입력 전달: ${input.substring(0, 100)}...`
          );
        }
      ...
      child.on('close', code => {
        ...
        if (this.debug) {
          console.error(
            `[GeminiDevTools] 완료 (${duration}ms) - 코드: ${code}`
          );
        }
  ...
  async handleSystemCommand(command, args) {
    switch (command) {
      ...
      case '/memory':
        ...
        return await this.systemCommands.memoryCommand(
          subCommand,
          ...memoryArgs
        );
      case '/help':
  ...
  async quickChat(prompt, options = {}) {
    ...
    try {
      ...
      const estimatedTokens = Math.floor(
        (prompt.length + result.stdout.length) / 4
      );
  ...
  /**
   * 💬 대화형 모드 (gemini CLI 직접 실행)
   * 복잡한 분석이나 깊이 있는 대화에 사용
   */
  async interactiveMode() {
    console.log(`
💬 Gemini CLI 대화형 모드로 전환합니다...
📝 복잡한 분석이나 깊이 있는 대화에 적합합니다
🚪 종료하려면 Ctrl+C 또는 exit 입력
    `);

    // gemini를 대화형 모드로 직접 실행
  ...
  async saveContext(name) {
    ...
    const contextFile = join(contextDir, `${name}.json`);
    const context = {
      name,
      savedAt: new Date().toISOString(),
      // 실제 컨텍스트는 gemini CLI가 관리하므로 메타데이터만 저장
      metadata: {
    ...
    try {
      ...
      switch (command) {
        ...
        case 'memory':
          ...
          const memoryResult = await tool.systemCommands.memoryCommand(
            ...memoryArgs
          );
        ...
        case 'save-context':
          const contextName = args[0] || 'default';
          const saveResult = await tool.saveContext(contextName);
          console.log(saveResult);
          break;

        case 'list-contexts':
          const listResult = await tool.listContexts();
        ...
        default:
          ...
          if (stdinInput) {
            ...
            console.log(`
🚀 Gemini 개발 도구 v5.2 사용법

기본 명령어:
  node tools/gemini-dev-tools.js chat "질문내용"     빠른 채팅
  node tools/gemini-dev-tools.js interactive         대화형 모드 (복잡한 분석용)
  ...
  node tools/gemini-dev-tools.js save-context       컨텍스트 저장
  node tools/gemini-dev-tools.js list-contexts      저장된 컨텍스트 목록
...
사용 시나리오:
  1. 간단한 질문: ./tools/g "에러 해결법" (캐싱 활용)
  2. 복잡한 분석: ./tools/g i → 대화형 모드에서 깊이 있는 분석
  3. 코드 리뷰: git diff | ./tools/g "SOLID 원칙 검토"
            `);
          }
      }
    } catch (error) {
