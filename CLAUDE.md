...
- ✅ 타임아웃 시 분할/간소화 제안
- 🚀 Qwen YOLO Mode (완전 무인 동작, 모든 도구 자동 승인)|### Bash Wrapper (v2.3.0)
...
**1️⃣ 개별 AI 사용 (직접 wrapper 호출)**

```bash
# ✅ Codex만 사용
Bash: ./scripts/ai-subagents/codex-wrapper.sh "버그 근본 원인 분석"

# ✅ Gemini만 사용
Bash: ./scripts/ai-subagents/gemini-wrapper.sh "SOLID 원칙 검토"

# ✅ Qwen만 사용
Bash: ./scripts/ai-subagents/qwen-wrapper.sh "성능 병목점 분석"
```

**언제**: 특정 AI의 전문성만 필요할 때, 빠른 1회성 질문

---

**2️⃣ AI 교차검증 (서브에이전트 위임)** ⭐

```bash
# ✅ 사용자가 "AI 교차검증" 요청 시
Task multi-ai-verification-specialist "useState vs useReducer 선택 기준"

# ❌ 교차검증인데 직접 호출 (자동화 이점 상실)
Bash: ./scripts/ai-subagents/codex-wrapper.sh "..." &
Bash: ./scripts/ai-subagents/gemini-wrapper.sh "..." &
Bash: ./scripts/ai-subagents/qwen-wrapper.sh "..." &
```

**언제**: 사용자가 "AI 교차검증" 명시할 때
