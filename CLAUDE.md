...
| --------------------- | ------- | ------------------- | --------------------------- | -------------------------- | -------------------------- | ------------- |
| **Claude Code**       | v1.0.108 | Max ($200/월) | 🏆 **메인 개발 환경**       | WSL 직접 실행 | ✅ 완벽 지원                | 2025-09-12 최신 |
| **OpenAI CLI (Codex)** | v0.29.0 | Plus ($20/월)       | 🤝 **계정 로그인 전용** ⚠️ | codex exec (WSL)  | ✅ **API 사용 금지**           | 2025-09-12 최신 |
| **Google Gemini CLI** | **v0.4.1** 🆕 | 무료 (1K/day 한도)   | 👨‍💻 **계정 로그인 전용** ⚠️ | gemini (WSL)           | ✅ 무료 한도 준수                | **2025-09-12 업그레이드** |
| **Qwen Code**         | v0.0.10  | 무료 (OAuth 2K/day)   | 🔷 **계정 인증 전용** ⚠️ | qwen (WSL)             | ✅ OAuth 전용                | 2025-09-12 최신 |
...
**현재 성능**: gemini-wrapper 9.33/10 최고 성능 (안정적 운영)
...
```bash
# 🎯 스마트 사용량 관리 시스템 활용
./scripts/ai-development/smart-ai-usage.sh auto typescript medium "코드 리팩토링"
./scripts/ai-development/smart-ai-usage.sh auto architecture high "SOLID 원칙 검증" 
./scripts/ai-development/smart-ai-usage.sh status  # 일일 사용량 현황

# 📊 사용량 기반 자동 AI 선택
# Codex (30-150/5h) → Gemini (1K/day) → Qwen (2K/day) → Claude Code 무제한
```
...
**3. 직접 AI CLI 호출** (한도 내 전문 활용)
```bash
# 전문 분야별 직접 활용 (사용량 추적 포함)
codex exec "Next.js 15 App Router 성능 최적화"                   # GPT-5 TypeScript 전문
gemini -p "대규모 리팩토링 전략 및 기술부채 해결 방안"              # Google AI 아키텍처  
qwen -p "알고리즘 복잡도 분석 및 병렬 처리 최적화"                 # Qwen 성능 전문

# 🚀 통합 교차검증 시스템
./scripts/ai-cross-validation.sh src/path/to/file.ts             # 4-AI 8.5/10 품질 보장
```
