...
**✅ Claude Code v2.0.8 업그레이드 완료** (2025-10-05):
- **v2.0.1 → v2.0.8**: 7단계 버전 업그레이드
- **v2.0.5에서 수정됨**: "OAuth expiration 401 API errors" 해결 ✅
- **효과**: Vercel MCP 재인증 반복 문제 근본 해결
- **업그레이드 명령어**: `npm install -g @anthropic-ai/claude-code@latest`
...
**파일/폴더 드래그앤드롭** ✅ (v2.0.8 신규)
- 채팅 입력창에 파일/폴더 직접 드래그 가능
- 여러 파일 동시 업로드 지원
- 프로젝트 컨텍스트에 즉시 추가
- 빠른 코드 분석 및 수정 작업 가능
...
### Multi-AI MCP 서버 (우선 사용) ⭐

**위치**: `packages/multi-ai-mcp/` (프로젝트 전용 MCP)
**현재 버전**: v1.4.0 (2025-10-05 최신)
...
// 기본 도구
mcp__multi_ai__queryAllAIs({ query: "코드 품질 분석" })
mcp__multi_ai__queryWithPriority({ query: "성능 분석", includeQwen: true })
...
**📚 상세 워크플로우**: [Multi-AI Verification Specialist 에이전트](.claude/agents/multi-ai-verification-specialist.md)
- Phase 1-3: 3-AI 교차검증 프로세스
- **Phase 4: Claude Code 최종 결정** ⭐
  - 합의 항목 검토 및 즉시 적용
  - 충돌 항목 프로젝트 컨텍스트 기반 판단
  - 선택적 AI 제안 적용
  - 실전 시나리오별 예시
...
**⚙️ 타임아웃 설정** (2025-10-06 v1.6.0 대폭 증가):
...
```bash
# .env.local 설정 (선택적)
MCP_TIMEOUT=360000  # 360초 (6분, 3-AI 병렬 실행 안정)
```

**개별 AI 타임아웃** (MCP 서버 내부, v1.6.0):
- **Codex**: 60s (simple) / 90s (medium) / **180s (complex, 3분)** ⬆️ 50% 증가
- **Gemini**: **300s (5분, 복잡한 분석)** ⬆️ 150% 증가
- **Qwen**: **120s (normal, 2분)** ⬆️ 100% / **300s (plan mode, 5분)** ⬆️ 150%
- **MCP 전체**: **360s (6분)** ⬆️ 100% 증가

**최대 한도**: Gemini/Qwen **600s (10분)**
...
|------|------------|------|----------|----------|
| **Claude Code** | `claude --version` | 메인 IDE/Task | **v2.0.8** ✅ | 2025-10-05 |
