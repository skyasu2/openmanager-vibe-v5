...
- MCP 활용도: 65% → 90% 목표
- 코드 리뷰: **Codex → Gemini → Claude Code** (auto-ai-review.sh v4.2.0) 🆕
  - 4:1 비율 선택 (Codex 4회, Gemini 1회)
...
1. **중복 기능 검색** 🔎
   - 유사한 기능이 이미 존재하는지 확인 (@serena 코드 검색)
   - 기존 유틸리티/헬퍼 함수 재사용 가능성 검토
   - 같은 목적의 다른 구현체 확인 (난개발 방지)

2. **레거시 코드 정리** 🧹
   - 사용하지 않는 코드 제거 우선
   - Deprecated 함수/컴포넌트 식별 및 마이그레이션
   - 주석 처리된 코드 블록 제거 (Git history 활용)
...
1. **문서 업데이트** 📝
   - README, API 문서, JSDoc/TSDoc 동기화
   - 변경된 인터페이스/타입 문서화
   - 예제 코드 업데이트

2. **테스트 관리** 🧪
   - **수정**: 기존 테스트 케이스 업데이트
   - **생성**: 새 기능에 대한 테스트 추가 (unit + integration)
   - **제거**: 삭제된 기능의 테스트 제거
   - Vercel E2E 우선 검증 (실제 환경)

3. **Side-Effect 처리** ⚡
   - 의존성 변경 시 package.json 업데이트
   - 환경변수 추가/제거 시 .env.example 동기화
   - 타입 변경 시 영향받는 모든 파일 수정
...
**AI 모드 선택 UI 완전 제거** - 자동 라우팅으로 전환

- **변경**: AIMode 타입 단순화 (`UNIFIED` 단일 값)
- **제거**: 4개 UI 컴포넌트 (~1,196줄)
- **효과**: 코드베이스 단순화, 일관된 사용자 경험
- **하위 호환**: 완전 유지 (Breaking Changes 없음)
- **상세**: @docs/ai/MODE-SELECTION-REMOVAL.md

**영향**:

- ✅ `/api/ai/query` 호출 시 mode 파라미터 생략 가능
- ✅ useAIEngine Hook 단순화 (UNIFIED 고정값 반환)
- ✅ localStorage 자동 마이그레이션 (`ai-mode-cleanup.ts`)
