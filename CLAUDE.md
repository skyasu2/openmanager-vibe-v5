...
1. **🤖 Gemini CLI 협업 전문가** (`gemini-cli-collaborator`) - AI 협업 전문가
   - WSL 터미널에서 Gemini CLI와 직접 대화 및 지시
   - Claude가 혼자 해결하기 어려운 복잡한 문제 해결
   - 대량의 코드 분석 및 병렬 처리 작업 수행
   - 무료 티어 Gemini를 활용한 AI 모델 간 시너지 창출
   - 두 번째 의견(second opinion) 제공 및 다각도 분석
...
5. **🎭 Sub Agent Orchestrator** (`planner-spec`) - 서브 에이전트 오케스트레이터
   - 사용자 요청에 가장 먼저 반응하여 작업 분석
   - 작업 복잡도 평가 및 실행 계획 수립
   - 서브 에이전트와 MCP 도구 최적 배치
   - 순차/병렬 작업 전략 수립 및 조율
   - 전체 작업 흐름 모니터링 및 최적화
...
```
"Gemini CLI 협업 전문가를 사용해서 이 복잡한 버그 해결을 도와줘"
"Gemini CLI 협업 전문가로 대량의 파일을 병렬 분석해줘"
```
...
### 🎭 Sub Agent Orchestrator 활용 예시

1. **복잡한 기능 구현**

   ```
   "사용자 인증 시스템을 OAuth2.0으로 마이그레이션해줘"
   ```

   → 복잡도: 🔴 복잡 | 병렬: 문서 분석 + 코드 검토 | 순차: 구현 → 테스트 → 배포

2. **다중 파일 리팩토링**

   ```
   "모든 API 엔드포인트에 에러 핸들링과 로깅을 추가해줘"
   ```

   → 복잡도: 🟡 중간 | 병렬: 파일별 수정 | 순차: 수정 → 테스트 → 검증

3. **성능 최적화 작업**

   ```
   "전체 앱의 로딩 속도를 50% 개선해줘"
   ```

   → 복잡도: 🔴 복잡 | 병렬: 분석 + 측정 | 순차: 최적화 → 검증 → 문서화

### 📊 Gemini CLI 협업 전문가 활용 예시

1. **복잡한 문제 해결**

   ```
   "이 버그를 3시간째 해결 못하고 있는데, Gemini CLI와 함께 다른 접근 방법을 찾아줘"
   ```

2. **병렬 코드 분석**

   ```
   "src 폴더의 모든 컴포넌트를 Gemini CLI로 동시에 분석해서 패턴 위반 사항을 찾아줘"
   ```

3. **두 번째 의견 구하기**
   ```
   "내가 작성한 아키텍처 설계를 Gemini에게도 검토받고 싶어, 다른 관점에서 피드백 받아줘"
   ```
