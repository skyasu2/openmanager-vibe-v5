...
import { BasicContextManager, BasicContextCache } from '../../context/basic-context-manager';
import { AdvancedContextManager, DocumentEmbedding } from '../../context/advanced-context-manager';
import { CustomContextManager, OrganizationSettings, CustomRule } from '../../context/custom-context-manager';
...
export interface QueryIntent {
  category: 'status' | 'troubleshooting' | 'configuration' | 'analysis' | 'general';
  keywords: string[];
...
export class MCPOrchestrator {
  ...
  async processQuery(query: MCPQuery): Promise<MCPResponse> {
    ...
    try {
      // 1단계: 질의 분석 및 분류
      const analysisStep = this.createProcessingStep('질의 분석', '자연어 질의를 분석하고 의도를 파악합니다');
      ...
      // 2단계: 컨텍스트 수집
      const contextStep = this.createProcessingStep('컨텍스트 수집', '관련 컨텍스트 정보를 수집합니다');
      steps.push(contextStep);
      
      const contextData = await this.gatherContext(queryIntent, query.organizationId);
      this.completeProcessingStep(contextStep, contextData);
      
      // 3단계: 응답 생성
      const responseStep = this.createProcessingStep('응답 생성', '수집된 정보를 바탕으로 응답을 생성합니다');
      steps.push(responseStep);
      
      const response = await this.generateResponse(query, queryIntent, contextData);
      this.completeProcessingStep(responseStep, response);
      
      // 4단계: 액션 및 추천사항 생성
      const actionStep = this.createProcessingStep('액션 생성', '추가 액션과 추천사항을 생성합니다');
      steps.push(actionStep);
      
      const actions = await this.generateActions(queryIntent, contextData, query.organizationId);
      response.actions = actions;
      this.completeProcessingStep(actionStep, actions);
      
      response.processingTime = Date.now() - startTime;
      
      console.log(`✅ [MCPOrchestrator] 질의 처리 완료 (${response.processingTime}ms)`);
      return response;
      ...
      return {
        ...
        queryId: query.id,
        answer: '죄송합니다. 질의 처리 중 오류가 발생했습니다. 다시 시도해 주세요.',
        confidence: 0,
        sources: [],
  ...
  private async analyzeQueryIntent(question: string): Promise<QueryIntent> {
    ...
    if (this.matchesPatterns(questionLower, ['상태', '현재', '어떻게', 'status', 'health'])) {
      category = 'status';
    } else if (this.matchesPatterns(questionLower, ['문제', '오류', 'error', '해결', '고장', '안됨'])) {
      category = 'troubleshooting';
    } else if (this.matchesPatterns(questionLower, ['설정', '구성', 'config', '변경', '수정'])) {
      category = 'configuration';
    } else if (this.matchesPatterns(questionLower, ['분석', '통계', '추세', 'analyze', '리포트'])) {
      category = 'analysis';
    }
    
    // 긴급도 판단
    let urgency: QueryIntent['urgency'] = 'low';
    if (this.matchesPatterns(questionLower, ['긴급', '중요', '즉시', 'urgent', 'critical'])) {
      urgency = 'high';
    } else if (this.matchesPatterns(questionLower, ['빨리', '빠르게', 'quickly', 'asap'])) {
      urgency = 'medium';
    }
    
    // 액션 필요 여부
    const requiresAction = this.matchesPatterns(questionLower, [
      '시작', '중지', '재시작', '실행', '실행해', '설정해', '변경해'
    ]);
  ...
  private extractKeywords(text: string): string[] {
    ...
    const stopWords = new Set(['은', '는', '이', '가', '을', '를', '에', '의', 'is', 'the', 'a', 'an']);
  ...
  private extractEntities(text: string): { type: string; value: string }[] {
    ...
    // 시간 엔티티
    const timeMatches = text.match(/\d+(?:시간?|분|초|hour|minute|second)/g) || [];
    timeMatches.forEach(match => {
      entities.push({ type: 'time', value: match });
    });
    
    // 시스템 컴포넌트
    const componentMatches = text.match(/(cpu|memory|disk|서버|메모리|디스크)/gi) || [];
  ...
  private async gatherContext(
    ...
    // 병렬로 모든 컨텍스트 수집
    const [basicContext, advancedDocs, customSettings, customRules] = await Promise.all([
      this.basicContext.getCurrentContext(),
      this.advancedContext.searchDocuments(queryIntent.keywords.join(' '), 5),
      organizationId ? this.customContext.getOrganizationSettings(organizationId) : Promise.resolve(null),
      this.customContext.getCustomRules(queryIntent.category === 'configuration' ? 'threshold' : undefined)
    ]);
  ...
  private async generateResponse(
    ...
    if (contextData.custom.rules.length > 0) {
      const ruleResponse = this.generateRuleResponse(contextData.custom.rules, queryIntent);
      if (ruleResponse) {
        answer += ruleResponse;
        confidence += 0.3;
      }
    ...
    return {
      ...
      contextUsed: {
        ...
        custom: !!(contextData.custom.settings || contextData.custom.rules.length > 0)
      },
      timestamp: Date.now()
    };
  }
   ...
   * 📚 문서 기반 응답 생성
   */
  private generateDocumentResponse(doc: DocumentEmbedding, question: string): string {
   ...
   * 📏 규칙 기반 응답 생성
   */
  private generateRuleResponse(rules: CustomRule[], queryIntent: QueryIntent): string {
  ...
  private generateFallbackResponse(queryIntent: QueryIntent): string {
    const fallbackResponses: Record<string, string> = {
      ...
      troubleshooting: '문제 해결을 위해 로그를 분석 중입니다. 구체적인 오류 메시지를 알려주시면 더 정확한 도움을 드릴 수 있습니다.',
      configuration: '설정 변경과 관련된 질문이군요. 어떤 부분의 설정을 변경하고 싶으신지 구체적으로 알려주세요.',
      analysis: '분석을 진행하고 있습니다. 어떤 지표나 기간에 대한 분석을 원하시는지 명시해 주세요.',
      general: '질문을 이해하고 있습니다. 더 구체적인 정보를 제공해 주시면 더 정확한 답변을 드릴 수 있습니다.'
    };
   ...
   * 💡 추천사항 생성
   */
  private generateRecommendations(queryIntent: QueryIntent, contextData: any): string[] {
    ...
    switch (queryIntent.category) {
      case 'status':
        recommendations.push('시스템 알림을 설정하여 자동으로 상태 변화를 추적하세요');
        break;
      case 'troubleshooting':
   ...
   * 📊 처리 단계 생성
   */
  private createProcessingStep(step: string, description: string): ProcessingStep {
    return {
      step,
      description,
      startTime: Date.now(),
