...
import type { ServerInstance, ServerCluster, ApplicationMetrics } from '@/services/data-generator/RealServerDataGenerator';
...
export class ServerMonitoringAgent {
  ...
  private knowledgeBase = {
    ...
    patterns: {
      ...
      'memory-leak': {
        ...
        causes: ['unclosed connections', 'cache overflow', 'circular references'],
    ...
    // 서버별 특성
    serverTypes: {
      web: { typical_cpu: 30, typical_memory: 40, critical_services: ['nginx', 'frontend'] },
      api: { typical_cpu: 50, typical_memory: 60, critical_services: ['api-server', 'middleware'] },
      database: { typical_cpu: 40, typical_memory: 70, critical_services: ['postgres', 'redis'] },
      cache: { typical_cpu: 20, typical_memory: 80, critical_services: ['redis', 'memcached'] }
    }
  };
  ...
  public async processQuery(request: QueryRequest): Promise<QueryResponse> {
    ...
    try {
      ...
      if (thinkingSteps.length > 0) {
        ...
        lastStep.result = error instanceof Error ? error.message : 'Unknown error';
      }
   ...
   * 📊 현재 데이터 수집
   */
  private async gatherCurrentData(context?: QueryRequest['context']): Promise<any> {
  ...
  private async analyzePatterns(data: any, intent: any): Promise<any> {
    ...
    for (const server of data.servers) {
      ...
      if (server.metrics.cpu > this.knowledgeBase.thresholds.cpu.critical) {
        ...
        analysis.issues.push(`${server.name}: CPU 사용률 위험 (${server.metrics.cpu.toFixed(1)}%)`);
        analysis.recommendations.push(`${server.name} 서버의 CPU 부하를 줄이거나 스케일링을 고려하세요`);
      } else if (server.metrics.cpu > this.knowledgeBase.thresholds.cpu.warning) {
        analysis.pattern = 'cpu-warning';
        analysis.severity = analysis.severity === 'critical' ? 'critical' : 'warning';
        analysis.issues.push(`${server.name}: CPU 사용률 주의 (${server.metrics.cpu.toFixed(1)}%)`);
      }
      
      // 메모리 이상
      if (server.metrics.memory > this.knowledgeBase.thresholds.memory.critical) {
        analysis.pattern = 'memory-issue';
        analysis.severity = 'critical';
        analysis.issues.push(`${server.name}: 메모리 사용률 위험 (${server.metrics.memory.toFixed(1)}%)`);
        analysis.recommendations.push(`${server.name}에서 메모리 누수 확인이 필요합니다`);
      }
      ...
      if (server.metrics.disk > this.knowledgeBase.thresholds.disk.critical) {
        ...
        analysis.severity = 'critical';
        analysis.issues.push(`${server.name}: 디스크 공간 부족 (${server.metrics.disk.toFixed(1)}%)`);
        analysis.recommendations.push(`${server.name}의 로그 정리 및 디스크 확장을 검토하세요`);
      }
      ...
      if (server.health.score < this.knowledgeBase.thresholds.health.critical) {
        ...
        analysis.severity = 'critical';
        analysis.issues.push(`${server.name}: 시스템 건강도 저하 (${server.health.score.toFixed(1)}점)`);
        analysis.recommendations.push(`${server.name}의 상세 진단이 필요합니다`);
      }
    }
   ...
   * 💬 답변 생성 (의도별 핸들러)
   */
  private async generateAnswer(intent: any, data: any, analysis: any): Promise<string> {
  ...
  private handleServerStatusQuery(data: any, analysis: any): string {
    ...
    const runningServers = servers.filter((s: ServerInstance) => s.status === 'running').length;
  ...
  private handlePerformanceQuery(data: any, analysis: any): string {
    ...
    // 성능 지표 분석
    const cpuStatus = summary.performance.avgCpu > 70 ? '높음 ⚠️' : summary.performance.avgCpu > 50 ? '보통' : '낮음 ✅';
    const memoryStatus = summary.performance.avgMemory > 80 ? '높음 ⚠️' : summary.performance.avgMemory > 60 ? '보통' : '낮음 ✅';
  ...
  private handleCostQuery(data: any, analysis: any): string {
    ...
    // 비용 절약 제안
    response += `**비용 최적화 제안:**\n`;
    const lowUtilizationServers = data.servers.filter((s: ServerInstance) => 
      s.metrics.cpu < 30 && s.metrics.memory < 40
    );
   ...
   * 💡 스마트 인사이트 생성
   */
  private async generateInsights(data: any, analysis: any): Promise<MonitoringInsight[]> {
    await new Promise(resolve => setTimeout(resolve, 400));
    
    const insights: MonitoringInsight[] = [];
    
    // 성능 인사이트
    const highCpuServers = data.servers.filter((s: ServerInstance) => s.metrics.cpu > 70);
    ...
    // 비용 인사이트
    const underutilizedServers = data.servers.filter((s: ServerInstance) => 
      s.metrics.cpu < 30 && s.metrics.memory < 40
    );
    ...
    // 가용성 인사이트
    const unhealthyServers = data.servers.filter((s: ServerInstance) => s.health.score < 80);
   ...
   * 📊 자동 장애 보고서 생성
   */
  public async generateIncidentReport(serverId: string): Promise<IncidentReport> {
    ...
    const report: IncidentReport = {
      ...
      severity: server.status === 'error' ? 'critical' : server.status === 'warning' ? 'major' : 'minor',
      ...
      metadata: {
        ...
        escalationLevel: server.status === 'error' ? 3 : server.status === 'warning' ? 2 : 1,
        reportGenerated: now
      }
    };
   ...
   * 🎭 생각과정 단계 생성
   */
  private createThinkingStep(step: number, title: string, description: string): ThinkingStep {
    return {
