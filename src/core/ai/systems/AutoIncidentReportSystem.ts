...
export class AutoIncidentReportSystem implements IAutoIncidentReportSystem {
  ...
  constructor(
    ...
    console.log(
      `🚨 AutoIncidentReportSystem 초기화 - 모드: ${this.currentMode}, ML: ${this.learningConfig.enableMLOptimization}`
    );
  }
  ...
  private async initializeConnections(): Promise<void> {
    try {
      ...
      if (
        cached &&
        Date.now() - cached.timestamp < this.HEALTH_CHECK_CACHE_TTL
      ) {
        if (cached.result) {
          console.log(
            '✅ AutoIncidentReportSystem: RuleBasedMainEngine 연결 (캐시됨)'
          );
          return;
        }
      }
      ...
      const { RuleBasedMainEngine } = await import(
        '@/core/ai/engines/RuleBasedMainEngine'
      );
      ...
      // 실패 결과도 캐싱 (재시도 방지)
      this.healthCheckCache.set('ruleBasedEngine_health', {
        result: false,
        timestamp: Date.now(),
      });
      console.warn(
        '⚠️ RuleBasedMainEngine 연결 실패, 기본 모드로 동작:',
        error
      );
    }
  ...
  private initializeLearningSystem(): void {
    ...
    console.log(
      `🧠 AI 학습 시스템 초기화 완료 (패턴: ${this.learningPatterns.size}개)`
    );
  }
  ...
  async predictIncidentWithML(metrics: ServerMetrics): Promise<{
    ...
    try {
      // ML 엔진을 통한 예측
      const mlPrediction = await this.mlEngine.predictPerformanceIssues(
        [metrics],
        []
      );
  ...
  private async learnFromIncidentReport(report: IncidentReport): Promise<void> {
    try {
      ...
      if (
        this.ruleBasedEngine &&
        (await this.shouldUpdateRuleEngine(pattern))
      ) {
        try {
      ...
      console.log(
        `🧠 패턴 학습 완료: ${pattern.category} (신뢰도: ${Math.round(pattern.confidence * 100)}%)`
      );
    } catch (error) {
   ...
   */
  private async shouldUpdateRuleEngine(
    pattern: LearningPattern
  ): Promise<boolean> {
    // 같은 카테고리의 기존 패턴 수 확인
    const existingPatterns = Array.from(this.learningPatterns.values()).filter(
    ...
    const similarPattern = existingPatterns.find(
      p => this.calculatePatternSimilarity(p, pattern) > 0.8
    );
      ...
      avgSuccessRate:
        patterns.length > 0
          ? patterns.reduce((sum, p) => sum + p.successRate, 0) /
            patterns.length
          : 0,
      recentLearnings: patterns.filter(
        p => Date.now() - p.learnedAt < 24 * 60 * 60 * 1000
      ).length,
  ...
  setLearningEnabled(enabled: boolean): void {
    ...
    console.log(
      `🧠 AI 학습 모드: ${this.learningEnabled ? '활성화' : '비활성화'}`
    );
  }
  ...
  async detectIncident(metrics: ServerMetrics): Promise<Incident | null> {
    try {
      ...
      // 심각도 재계산
      incident.severity =
        await this.detectionEngine.calculateSeverity(incident);

      console.log(
        `🔍 장애 감지: ${incident.type} (${incident.severity}) - ${incident.affectedServer}`
      );
  ...
  async detectMemoryLeak(trend: ServerMetrics[]): Promise<Incident | null> {
    ...
    try {
      ...
      const incident: Incident = {
        ...
        type: 'memory_leak',
        severity:
          memoryValues[memoryValues.length - 1] > 90 ? 'critical' : 'high',
      ...
      console.log(
        `🧠 메모리 누수 감지: ${incident.affectedServer} (예상 고갈: ${Math.round(predictedTime / 60)}분 후)`
      );
  ...
  async detectCascadeFailure(
    ...
    try {
      ...
      // 연쇄 장애 패턴 확인
      const criticalServers = sortedMetrics.filter(
        m =>
          m.status === 'critical' || m.cpu > 95 || m.memory > 95 || m.disk > 95
      );

      if (criticalServers.length < 2) return null;

      // 시간 간격 분석 (5분 이내 연속 장애)
      const timeDiffs = criticalServers
        .slice(1)
        .map((m, i) => m.timestamp - criticalServers[i].timestamp);
  ...
  async generateReport(incident: Incident): Promise<IncidentReport> {
    try {
      ...
      if (this.learningEnabled && this.learningConfig.enabled) {
        ...
        console.log(
          `🧠 학습 큐에 보고서 추가: ${report.id} (큐 크기: ${this.learningQueue.length})`
        );
      }

      const processingTime = Date.now() - startTime;
      console.log(
        `📋 장애 보고서 생성 완료: ${report.id} (${processingTime}ms)`
      );
  ...
  async generateKoreanReport(incident: Incident): Promise<IncidentReport> {
    ...
    console.log(
      `🇰🇷 [AutoIncidentReport] 한국어 처리 - 모드: ${this.currentMode}, 전략:`,
      strategy
    );
        ...
        switch (engineType) {
          case 'korean':
            if (strategy.useKorean && this.ruleBasedEngine) {
              ...
              const koreanResult =
                await this.ruleBasedEngine.processQuery(koreanQuery);
   ...
   * 🏠 로컬 AI 한국어 처리
   */
  private enhanceWithLocalAI(
    incident: Incident,
    report: IncidentReport
  ): string {
    const baseDescription =
      report.description || this.generateDefaultDescription(incident);
  ...
  async generateSolutions(incident: Incident): Promise<Solution[]> {
    try {
      ...
      console.log(
        `💡 ${prioritizedSolutions.length}개 해결방안 생성: ${incident.type}`
      );
  ...
  async predictFailureTime(
    ...
    try {
      ...
      // 예측 시점 계산
      const predictedTime = this.calculatePredictedFailureTime(
        historicalData,
        trendAnalysis
      );
      ...
      console.log(
        `🔮 장애 예측 완료: ${Math.round((predictedTime - Date.now()) / 60000)}분 후 (신뢰도: ${Math.round(confidence * 100)}%)`
      );
  ...
  async analyzeImpact(incident: Incident): Promise<IncidentImpact> {
    try {
      ...
      const affectedServers = incident.affectedServers || [
        incident.affectedServer,
      ];
      ...
      console.log(
        `📊 영향도 분석 완료: ${affectedServices.length}개 서비스, ${estimatedDowntime}분 예상 다운타임`
      );
   ...
   * 🔄 실시간 장애 전체 프로세스 처리
   */
  async processRealTimeIncident(
    metrics: ServerMetrics
  ): Promise<FullIncidentReport> {
    const startTime = Date.now();
  ...
  private generateReportSummary(
    ...
    return (
      `${incident.affectedServer} 서버에서 ${incident.type} 장애가 감지되었습니다. ` +
  ...
  private generateDefaultDescription(incident: Incident): string {
    return (
      `${incident.affectedServer} 서버에서 ${incident.type} 유형의 장애가 발생했습니다. ` +
  ...
  private prioritizeSolutions(
    ...
    return solutions.sort((a, b) => {
      ...
      if (incident.severity === 'critical') {
        if (
          a.category === 'immediate_action' &&
          b.category !== 'immediate_action'
        )
          return -1;
        if (
          b.category === 'immediate_action' &&
  ...
  private analyzeTrend(data: ServerMetrics[]): TrendAnalysis {
    ...
    return {
      direction,
      rate:
        Math.abs(cpuValues[cpuValues.length - 1] - cpuValues[0]) / data.length,
      pattern: 'linear',
      anomalies: [],
    };
  }
  ...
  private analyzeUserImpact(
    ...
    return {
      affectedUsers:
        (severityMultiplier[incident.severity] || 10) * affectedServers.length,
  ...
  private makeCompatibleWithLegacyFormat(
    report: IncidentReport
  ): IncidentReport {
  ...
  getMLEnhancedLearningMetrics(): LearningMetrics {
    ...
    // ML 패턴 개수 계산
    const mlPatterns = Array.from(this.learningPatterns.values()).filter(
      p => p.source === 'ml_optimization'
    );
