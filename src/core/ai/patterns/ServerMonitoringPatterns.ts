...
export class ServerMonitoringPatterns implements IServerMonitoringPatterns {
    ...
    private initializePatterns(): void {
        ...
        this.patterns.set('server_status', {
            ...
            patterns: [
                ...
                /서버.*상태/i, /상태.*확인/i, /서버.*어떤/i, /현재.*서버/i,
                /서버.*체크/i, /헬스.*체크/i, /온라인.*서버/i, /오프라인.*서버/i,

                // 새로 추가된 7개 패턴
                /서버.*살아있/i, /서버.*죽었/i, /서버.*다운/i, /서버.*정상/i,
                /서버.*문제/i, /서버.*이상/i, /가동.*상태/i
            ]
        });
        ...
        this.patterns.set('performance_analysis', {
            ...
            patterns: [
                // 기존 패턴 유지
                /성능.*분석/i, /리소스.*사용/i, /cpu.*메모리/i, /느린.*서버/i,
                /최적화/i, /부하.*분석/i, /응답.*시간/i, /처리.*속도/i,

                // 새로 추가된 4개 패턴
                /병목.*현상/i, /지연.*시간/i, /처리량.*분석/i, /대역폭.*사용/i
            ]
        });
        ...
        this.patterns.set('log_analysis', {
            ...
            patterns: [
                /에러.*로그/i, /로그.*분석/i, /오류.*메시지/i, /예외.*상황/i,
                /장애.*로그/i, /로그.*확인/i, /에러.*추적/i, /로그.*검색/i,
                /시스템.*로그/i, /액세스.*로그/i
            ]
        });
        ...
        this.patterns.set('troubleshooting', {
            ...
            patterns: [
                ...
                /장애.*해결/i, /문제.*해결/i, /복구.*방안/i, /긴급.*대응/i,
                /해결.*방법/i, /조치.*방안/i, /대응.*절차/i, /복구.*계획/i
            ],
    ...
    async getPracticalGuide(
        ...
        if (queryLower.includes('문제') || queryLower.includes('장애') || queryLower.includes('오류')) {
