...
export class ServerMonitoringPatterns implements IServerMonitoringPatterns {
  ...
  private initializePracticalGuides(): void {
    ...
    // ��️ 가상머신 (VM) 실무 가이드
    this.practicalGuides.set('vm', {
      serverType: 'vm',
      commonCommands: {
        '상태 확인': {
          command: 'virsh list --all',
          description: 'VM 상태 확인',
          example: 'virsh list --state-running',
        },
        '로그 확인': {
          command: 'virsh console [vm_name]',
          description: 'VM 콘솔 접근',
          example: 'virsh console vm-web-01',
        },
        'VM 접속': {
          command: 'virsh console [vm_name]',
          description: 'VM 콘솔 연결',
          example: 'virsh console vm-db-01',
        },
        '리소스 확인': {
          command: 'virsh domstats',
          description: 'VM 리소스 사용량',
          example: 'virsh domstats --cpu-total --balloon',
        },
        '정리 작업': {
          command: 'virsh snapshot-delete',
          description: '사용하지 않는 스냅샷 정리',
          example: 'virsh snapshot-delete vm-01 old-snapshot',
        },
      },
      troubleshooting: [
        {
          ...
          solution: [
            '1. VM 설정 파일 확인',
            '2. 하이퍼바이저 상태 확인 → systemctl status libvirtd',
            '3. 리소스 사용량 확인',
            '4. 스토리지 볼륨 접근 권한 확인',
          ],
        },
