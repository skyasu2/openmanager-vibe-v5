...
export class SystemWatchdog {
  ...
  private alertsHistory: Array<{ timestamp: Date; type: string; message: string }> = [];
  private readonly maxHistoryLength = 100;
  private readonly monitoringIntervalMs = 10000; // 10초
   ...
   * 오류율 계산
   */
  private calculateErrorRate(systemStatus: any): number {
    if (!systemStatus.processes || systemStatus.processes.length === 0) {
      return 0;
    }
  ...
  private analyzeStability(): void {
    ...
    if (this.metrics.performanceScore < 60) {
      this.addAlert('performance', `시스템 성능 저하 감지 (${this.metrics.performanceScore.toFixed(1)}%)`);
    }

    // 안정성 문제 감지
    if (this.metrics.stabilityScore < 70) {
      this.addAlert('stability', `시스템 안정성 문제 감지 (${this.metrics.stabilityScore.toFixed(1)}%)`);
    }
  }
  ...
  private calculatePerformanceScore(): number {
    ...
    if (this.metrics.memory.length > 0) {
      const avgMemory = this.metrics.memory.reduce((sum, m) => sum + m.value, 0) / this.metrics.memory.length;
    ...
    if (this.metrics.cpu.length > 0) {
      const avgCPU = this.metrics.cpu.reduce((sum, c) => sum + c.value, 0) / this.metrics.cpu.length;
  ...
  private checkAlerts(): void {
    ...
    if (alerts.highErrorRate) {
      this.addAlert('high-error-rate', `높은 오류율 감지 (${this.metrics.errorRate.toFixed(1)}%)`);
      this.processManager?.emit('system:high-error-rate', { errorRate: this.metrics.errorRate });
    }

    // 성능 저하 알림
    if (alerts.performanceDegradation) {
      this.addAlert('performance-degradation', '시스템 성능 저하 감지');
      this.processManager?.emit('system:performance-degradation', { score: this.metrics.performanceScore });
    }

    // 빈번한 재시작 알림
    if (alerts.frequentRestarts) {
      this.addAlert('frequent-restarts', `빈번한 프로세스 재시작 감지 (${this.metrics.restartCount}회)`);
      this.processManager?.emit('system:frequent-restarts', { count: this.metrics.restartCount });
    }
  }
   ...
   * 추세 계산 (선형 회귀)
   */
  private calculateTrend(data: Array<{ timestamp: number; value: number }>): { slope: number; intercept: number; correlation: number } {
   ...
   * 최근 알림 조회
   */
  private getRecentAlerts(timeWindowMs: number): Array<{ timestamp: Date; type: string; message: string }> {
    const cutoffTime = Date.now() - timeWindowMs;
    return this.alertsHistory.filter(alert => alert.timestamp.getTime() > cutoffTime);
  }
   ...
   * 알림 히스토리 조회
   */
  getAlertsHistory(): Array<{ timestamp: Date; type: string; message: string }> {
  ...
  getHealthSummary(): {
    ...
    if (this.metrics.performanceScore < 70 || this.metrics.stabilityScore < 70) {
      overall = 'warning';
    }
    
    if (this.metrics.performanceScore < 50 || this.metrics.stabilityScore < 50) {
      overall = 'critical';
    }
