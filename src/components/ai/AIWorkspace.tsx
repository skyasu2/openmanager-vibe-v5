...
export default function AIWorkspace({ mode, onClose }: AIWorkspaceProps) {
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'history' | 'tools'>('chat');
  const [isRightPanelOpen, setIsRightPanelOpen] = useState(true);

  const handleFunctionClick = (type: 'chat' | 'report' | 'prediction' | 'advanced') => {
    ...
    switch (type) {
      case 'chat':
        ...
        break;
      case 'report':
        store.setInputValue('Please analyze the current server logs and metrics to generate a comprehensive failure root cause report. Identify any critical anomalies.');
        break;
      case 'prediction':
        store.setInputValue('Based on the historical data and current trends, predict any potential system anomalies or load spikes for the next 24 hours.');
        break;
      case 'advanced':
    ...
    return (
      <div className="flex h-full bg-[#1e1e1e]">
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {activeTab === 'tools' && <ToolsView onRunTool={(prompt) => {
            setActiveTab('chat');
        ...
        <div className="flex w-[60px] flex-col items-center gap-4 border-l border-gray-800 bg-[#252526] py-4">
          ...
          <button
            ...
            className={`group relative flex h-10 w-10 items-center justify-center rounded-xl transition-all ${
              activeTab === 'chat' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
            }`}
          ...
          <button
            ...
            className={`group relative flex h-10 w-10 items-center justify-center rounded-xl transition-all ${
              activeTab === 'tools' ? 'bg-purple-500 text-white shadow-lg shadow-purple-500/20' : 'bg-purple-500/10 text-purple-400 hover:bg-purple-500 hover:text-white hover:shadow-lg hover:shadow-purple-500/20'
            }`}
          ...
          <div className="my-2 h-px w-8 bg-gray-700" />
             ...
             <button
                ...
                className={`group relative flex h-10 w-10 items-center justify-center rounded-xl transition-all ${
                activeTab === 'history' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700'
                }`}
                title="History"
            >
        ...
        <div className="flex flex-1 flex-col relative min-w-0">
            ...
            <div className="flex h-14 items-center justify-between border-b border-gray-800 bg-[#1e1e1e] px-4">
                    <div className="flex items-center gap-2 text-gray-400 text-sm">
                        <span className="font-medium text-gray-200">OpenManager Vibe</span>
                        <span>/</span>
            ...
            <div className="flex-1 overflow-hidden">
                {activeTab === 'chat' && <AIChatInterface mode="fullscreen" embedded />}
                {activeTab === 'history' && (
                    <div className="p-8"><HistoryView /></div>
                )}
                {activeTab === 'tools' && (
                     <div className="p-8">
                        <ToolsView onRunTool={(prompt) => {
                            setActiveTab('chat');
                            useAIChatStore.getState().setInputValue(prompt);
                        }} />
                     </div>
             ...
             <div className="w-[320px] border-l border-gray-800 bg-[#18181b] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    ...
                    <div className="space-y-3">
                        <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Live Status</h4>
                    ...
                    {/* Active Tools */}
                    <div className="space-y-3">
                        <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Active Tools</h4>
