...
const generateMockPatterns = (): PatternData[] => [
  {
    ...
    trend: 'increasing',
    confidence: 0.87,
    description: '최근 3일간 오후 2-4시에 CPU 사용량이 지속적으로 증가하는 패턴이 감지되었습니다.',
  ...
  },
  {
    ...
    trend: 'stable',
    confidence: 0.92,
    description: 'Server-07에서 24시간 주기로 메모리 사용량이 점진적으로 증가하다 갑자기 감소하는 패턴이 반복됩니다.',
  ...
  },
  {
    ...
    trend: 'decreasing',
    confidence: 0.65,
    description: '오전 시간대 디스크 I/O 대기시간이 높아지는 패턴이 감소하는 추세입니다.',
...
const PatternAnalysisPanel: React.FC<PatternAnalysisPanelProps> = ({ className = '' }) => {
  ...
  return (
    <BasePanelLayout
      ...
      onRefresh={reload}
      isLoading={isLoading}
      adminPath="/admin/ai-agent/patterns"
      adminLabel="상세분석"
        ...
        <div className="space-y-3">
          {filteredPatterns.map((pattern: PatternData) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-white font-medium text-sm">{pattern.title}</h4>
                    <span className={`px-2 py-1 rounded-full text-xs border ${getSeverityColor(pattern.severity)}`}>
              ...
              {/* 패턴 설명 */}
              <p className="text-gray-200 text-sm mb-3">{pattern.description}</p>
