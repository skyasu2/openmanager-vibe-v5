...
export default function AgentThinkingPanel({
  ...
  const processingProgress =
    logs.length > 0
      ? (logs.filter(log => (log as any).progress === 1.0).length /
          logs.length) *
        100
      : 0;
  ...
  if (!isThinking && logs.length === 0) {
    return (
      <div
        className={`bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-slate-700/50 rounded-xl backdrop-blur-sm p-4 ${className}`}
      >
