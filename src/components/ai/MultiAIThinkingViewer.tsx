...
export const MultiAIThinkingViewer: React.FC<MultiAIThinkingProps> = ({
  ...
  const getModeDisplay = () => {
    switch (mode) {
      case 'AUTO':
        return '🔄 AUTO 모드 (MCP+RAG → Google AI 백업)';
      case 'LOCAL':
        return '🏠 LOCAL 모드 (MCP+RAG 전용)';
      case 'GOOGLE_ONLY':
        return '🤖 GOOGLE_ONLY 모드 (Google AI 전용)';
      default:
        return '🤔 알 수 없는 모드';
    }
  };
                    ...
                    >
                      <div className='flex items-center space-x-3'>
                        ...
                        <div>
                          <div className='flex items-center space-x-2'>
                            <span
                              className={`text-sm font-medium ${engine.color}`}
                            >
                              {engine.name}
                            </span>
                              ...
                              {step.details && step.details.length > 0 && (
                                <div className='space-y-1'>
                                  ...
                                  {step.details.map((detail, idx) => (
                                    <div
                                      key={idx}
                                      className='text-xs text-gray-500 pl-2 border-l-2 border-gray-300'
                                    >
                                      • {detail}
                                    </div>
                                  ))}
