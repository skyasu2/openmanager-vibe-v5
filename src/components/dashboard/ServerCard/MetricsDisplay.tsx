          ...
          <div key={metric.key}>
            ...
            {(showLabels || showValues) && (
              <div className={`flex justify-between items-center ${classes.spacing}`}>
                ...
                {showValues && (
                  <span className={`${classes.textSize} font-medium text-gray-900`}>
            ...
            <div className="relative">
              <div className={`w-full bg-gray-200 rounded-full ${classes.progressHeight} overflow-hidden`}>
                <div 
            ...
            {variant === 'detailed' && (
              <div className="flex justify-between items-center mt-1">
                ...
                <span className="text-xs text-gray-400">
                  {status === 'critical' ? '즉시 조치 필요' :
                   status === 'warning' ? '모니터링 필요' : '안정'}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="mt-4 p-2 bg-gray-50 rounded-lg">
          <div className="flex justify-between items-center">
            ...
            <span className={`text-sm font-medium ${
              getMetricStatus((server.cpu + server.memory + server.disk) / 3) === 'critical' ? 'text-red-600' :
              getMetricStatus((server.cpu + server.memory + server.disk) / 3) === 'warning' ? 'text-yellow-600' : 'text-green-600'
            }`}>
              {Math.round(100 - (server.cpu + server.memory + server.disk) / 3)}%
            </span>
          </div>
        </div>
      )}
