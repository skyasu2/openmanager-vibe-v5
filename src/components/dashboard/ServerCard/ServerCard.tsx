...
const ServerCard: React.FC<ServerCardProps> = memo(
  ({ server, onClick, variant = 'default', showActions = true, onAction }) => {
    ...
    const handleAction = useCallback(
      (action: string, targetServer: Server) => {
        if (onAction) {
    ...
    return (
      ...
      >
        {/* 헤더: 서버 아이콘 + 이름 + 상태 */}
        <div
          className={`flex justify-between items-start ${styles.headerSpacing}`}
        >
        ...
        {!styles.showDetailedInfo && (
          <div
            className={`space-y-1 ${styles.infoSpacing} text-xs text-gray-600`}
          >
