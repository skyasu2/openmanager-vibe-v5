...
import { ServerData, RealtimeData, NetworkStatus, NetworkData } from './EnhancedServerModal.types';
...
const getNetworkStatusInfo = (status?: NetworkStatus) => {
  switch (status) {
    case 'excellent':
      return { color: 'bg-green-400 shadow-green-400/50', text: '최상', textColor: 'text-green-300' };
    case 'good':
      return { color: 'bg-yellow-400 shadow-yellow-400/50', text: '양호', textColor: 'text-yellow-300' };
    case 'poor':
      return { color: 'bg-red-400 shadow-red-400/50', text: '부족', textColor: 'text-red-300' };
    case 'offline':
      return { color: 'bg-blue-400 shadow-blue-400/50', text: '오프라인', textColor: 'text-blue-300' };
    default:
      return { color: 'bg-gray-400 shadow-gray-400/50', text: '알수없음', textColor: 'text-gray-300' };
  }
};
...
export const NetworkTab: React.FC<NetworkTabProps> = ({
  ...
  const latestNetwork = realtimeData.network[realtimeData.network.length - 1] || { in: 0, out: 0 };
  const latestLatency = realtimeData.latency[realtimeData.latency.length - 1] || 0;
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <span className="font-mono font-bold text-gray-900">{server.ip || '192.168.1.100'}</span>
            </MotionDiv>
            <MotionDiv
              whileHover={{ x: 5 }}
              className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow"
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <span className="font-medium text-gray-900">{server.provider}</span>
            </MotionDiv>
            <MotionDiv
              whileHover={{ x: 5 }}
              className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <span className="font-medium text-gray-900 capitalize">{server.environment}</span>
            </MotionDiv>
            <MotionDiv
              whileHover={{ x: 5 }}
              className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <span className="font-medium text-gray-900 capitalize">{server.type || 'Unknown'}</span>
            </MotionDiv>
          </div>
        </div>
      </MotionDiv>
