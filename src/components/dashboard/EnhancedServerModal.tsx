...
import { 
  X, Activity, Cpu, HardDrive, Wifi, Users, Clock, 
  AlertTriangle, CheckCircle, TrendingUp, TrendingDown,
  Server, Database, Shield, BarChart3, Terminal, 
  Network, Zap, Globe, Settings, RefreshCw, Play, Pause,
  Eye, Download, Share, Maximize2, Monitor, FileText,
  Brain, Target, Layers, GitBranch
} from 'lucide-react';
...
export default function EnhancedServerModal({ server, onClose }: EnhancedServerModalProps) {
  const [selectedTab, setSelectedTab] = useState<'overview' | 'metrics' | 'processes' | 'logs' | 'network' | 'ai'>('overview');
  ...
  const [realtimeData, setRealtimeData] = useState<{
    ...
    latency: number[];
    processes: Array<{name: string; cpu: number; memory: number; pid: number}>;
    logs: Array<{timestamp: string; level: string; message: string; source: string}>;
    insights: Array<{type: string; message: string; severity: string; timestamp: string}>;
  }>({
    cpu: [],
    memory: [],
    disk: [],
  ...
  useEffect(() => {
    ...
    const generateRealtimeData = () => {
      ...
      setRealtimeData(prev => ({
        cpu: [...prev.cpu.slice(-29), server.cpu + (Math.random() - 0.5) * 10].slice(-30),
        memory: [...prev.memory.slice(-29), server.memory + (Math.random() - 0.5) * 8].slice(-30),
        disk: [...prev.disk.slice(-29), server.disk + (Math.random() - 0.5) * 3].slice(-30),
        network: [...prev.network.slice(-29), {
          in: Math.random() * 1000 + 500,
          out: Math.random() * 800 + 300
        }].slice(-30),
        latency: [...prev.latency.slice(-29), Math.random() * 100 + 50].slice(-30),
        processes: Array.from({length: 8}, (_, i) => ({
          name: ['nodejs', 'nginx', 'postgres', 'redis', 'docker', 'systemd', 'ssh', 'cron'][i],
          cpu: Math.random() * 20,
        ...
        logs: [
          ...prev.logs.slice(-19),
          {
            ...
            source: ['nginx', 'app', 'db', 'cache'][Math.floor(Math.random() * 4)]
          }
        ].slice(-20),
        insights: [
          ...prev.insights.slice(-4),
          ...(Math.random() > 0.8 ? [{
            type: ['performance', 'security', 'capacity'][Math.floor(Math.random() * 3)],
            message: [
  ...
  // 3D 게이지 컴포넌트
  const CircularGauge3D = ({ value, max = 100, label, color, size = 120 }: { 
    value: number; 
    max?: number; 
    label: string; 
    color: string; 
  ...
  // 실시간 차트 컴포넌트
  const RealtimeChart = ({ data, color, label, height = 100 }: {
    data: number[];
    color: string;
    label: string;
    height?: number;
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100">{server.type} • {server.location}</p>
                </div>
              </div>
              ...
              >
                {selectedTab === 'overview' && (
                  <div className="space-y-6">
                    {/* 3D 게이지들 */}
                    <div>
                      <h3 className="text-xl font-bold text-gray-900 mb-4">실시간 리소스 모니터링</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 bg-white rounded-xl p-6 shadow-sm">
                        <CircularGauge3D value={server.cpu} label="CPU" color="#ef4444" size={140} />
                        <CircularGauge3D value={server.memory} label="메모리" color="#3b82f6" size={140} />
                        <CircularGauge3D value={server.disk} label="디스크" color="#8b5cf6" size={140} />
                      </div>
                    </div>
                      ...
                      <div className="bg-white rounded-xl p-6 shadow-sm">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{server.os || 'Ubuntu 20.04 LTS'}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{server.specs?.cpu_cores || 8}개</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">메모리</span>
                            <span className="font-medium">{server.specs?.memory_gb || 16}GB</span>
                          </div>
                        </div>
                      </div>

                      <div className="bg-white rounded-xl p-6 shadow-sm">
                        <h4 className="text-lg font-semibold text-gray-900 mb-4">서비스 상태</h4>
                        <div className="space-y-3">
                          {server.services.map((service, idx) => (
                            <div key={idx} className="flex items-center justify-between">
                              ...
                              <span className="text-sm text-gray-600">:{service.port}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                {selectedTab === 'metrics' && (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xl font-bold text-gray-900">성능 메트릭</h3>
                      <select
                        value={timeRange}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <RealtimeChart data={realtimeData.cpu} color="#ef4444" label="CPU 사용률" />
                      <RealtimeChart data={realtimeData.memory} color="#3b82f6" label="메모리 사용률" />
                      <RealtimeChart data={realtimeData.disk} color="#8b5cf6" label="디스크 사용률" />
                      <RealtimeChart 
                ...
                {selectedTab === 'processes' && (
                  <div className="space-y-6">
                    <h3 className="text-xl font-bold text-gray-900">실행 중인 프로세스</h3>
                ...
                {selectedTab === 'logs' && (
                  <div className="space-y-6">
                    <h3 className="text-xl font-bold text-gray-900">실시간 로그</h3>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                {insight.type} • {new Date(insight.timestamp).toLocaleString()}
                              </p>
                            </div>
                          </div>
                        </motion.div>
