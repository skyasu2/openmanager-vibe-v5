...
export default function UsageMonitor() {
  ...
  const toggleService = (service: 'supabase' | 'redis') => {
    ...
    const isEnabled =
      service === 'supabase' ? status.supabase.enabled : status.redis.enabled;
  ...
  const toggleKeepAlive = (service: 'supabase' | 'redis') => {
    ...
    const isActive =
      service === 'supabase' ? status.isActive.supabase : status.isActive.redis;
    ...
    <div className='p-6 bg-white rounded-lg shadow-lg'>
      ...
      <div className='grid md:grid-cols-2 gap-6'>
        ...
        <div className='border rounded-lg p-4'>
          <div className='flex items-center justify-between mb-4'>
            <div className='flex items-center space-x-2'>
              <div
                className={`w-3 h-3 rounded-full ${usageStatus.supabase.enabled ? 'bg-green-500' : 'bg-red-500'}`}
              ></div>
          ...
          <div className='space-y-3 mb-4'>
            <div>
              <div className='flex justify-between text-sm mb-1'>
                <span>월간 전송량</span>
                <span>
                  {usageStatus.supabase.usage.transferMB.toFixed(1)}MB /{' '}
                  {usageStatus.supabase.limits.monthlyTransferMB}MB
                </span>
              </div>
              <ProgressBar
                percentage={usageStatus.supabase.percentage.transfer}
                color={getStatusColor(
                  usageStatus.supabase.percentage.transfer,
            ...
            <div>
              <div className='flex justify-between text-sm mb-1'>
                <span>월간 요청수</span>
                <span>
                  {usageStatus.supabase.usage.requests.toLocaleString()} /{' '}
                  {usageStatus.supabase.limits.requests.toLocaleString()}
                </span>
              </div>
              <ProgressBar
                percentage={usageStatus.supabase.percentage.requests}
                color={getStatusColor(
                  usageStatus.supabase.percentage.requests,
                  usageStatus.supabase.enabled
                )}
              />
            </div>
          ...
          <div className='border-t pt-3 mb-4'>
            <div className='flex items-center justify-between mb-2'>
              ...
              <div
                className={`w-2 h-2 rounded-full ${keepAliveStatus.isActive.supabase ? 'bg-green-500' : 'bg-gray-400'}`}
              ></div>
            </div>
            ...
            <div className='text-xs text-gray-600 space-y-1'>
              <div>
                마지막 핑:{' '}
                {keepAliveStatus.lastPing.supabase
                  ? new Date(keepAliveStatus.lastPing.supabase).toLocaleString()
                  : '없음'}
              </div>
              <div>
                다음 핑: {formatTime(keepAliveStatus.timeToNext.supabase)}
              </div>
              {dangerStatus.supabase.danger && (
        ...
        <div className='border rounded-lg p-4'>
          <div className='flex items-center justify-between mb-4'>
            <div className='flex items-center space-x-2'>
              <div
                className={`w-3 h-3 rounded-full ${usageStatus.redis.enabled ? 'bg-green-500' : 'bg-red-500'}`}
              ></div>
          ...
          <div className='space-y-3 mb-4'>
            <div>
              <div className='flex justify-between text-sm mb-1'>
                <span>일일 명령수</span>
                <span>
                  {usageStatus.redis.usage.commands.toLocaleString()} /{' '}
                  {usageStatus.redis.limits.dailyCommands.toLocaleString()}
                </span>
              </div>
              <ProgressBar
                percentage={usageStatus.redis.percentage.commands}
                color={getStatusColor(
                  usageStatus.redis.percentage.commands,
          ...
          <div className='border-t pt-3 mb-4'>
            <div className='flex items-center justify-between mb-2'>
              <span className='text-sm font-medium'>Keep-Alive 상태</span>
              <div
                className={`w-2 h-2 rounded-full ${keepAliveStatus.isActive.redis ? 'bg-green-500' : 'bg-gray-400'}`}
              ></div>
            </div>
            ...
            <div className='text-xs text-gray-600 space-y-1'>
              <div>
                마지막 핑:{' '}
                {keepAliveStatus.lastPing.redis
                  ? new Date(keepAliveStatus.lastPing.redis).toLocaleString()
                  : '없음'}
              </div>
        ...
        {(dangerStatus.supabase.danger || dangerStatus.redis.danger) && (
          <div className='p-3 bg-red-50 border border-red-200 rounded-lg'>
            ...
            <div className='text-sm text-red-700 space-y-1'>
              {dangerStatus.supabase.danger && (
                <div>
                  • Supabase: {dangerStatus.supabase.daysLeft}일 후 휴면 위험
                </div>
              )}
              {dangerStatus.redis.danger && (
          ...
          <div className='p-3 bg-blue-50 border border-blue-200 rounded-lg'>
            <div className='flex items-center space-x-2'>
              ...
              <span className='text-sm text-blue-800'>
                일부 서비스가 비활성화되어 있습니다. 캐시된 데이터를 사용
                중입니다.
              </span>
            </div>
          </div>
      ...
      <div className='mt-6 p-4 bg-green-50 border border-green-200 rounded-lg'>
        ...
        <ul className='text-sm text-green-700 space-y-1'>
          <li>
            • <strong>Supabase</strong>: 4시간마다 자동 핑 (7일 휴면 방지)
          </li>
          <li>
            • <strong>Redis</strong>: 12시간마다 자동 핑 (30일 삭제 방지)
          </li>
