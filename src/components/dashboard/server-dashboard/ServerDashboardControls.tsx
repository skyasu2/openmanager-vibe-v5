    ...
    <div className='flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between'>
      ...
      <div className='flex items-center space-x-2'>
        <button
          ...
          className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
        >
          <LayoutGrid size={20} />
        </button>
        <button
          onClick={() => onViewModeChange('list')}
          className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
        >
          <List size={20} />
        </button>
      </div>
