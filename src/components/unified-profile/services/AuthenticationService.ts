...
// 개발 환경 설정
const DEVELOPMENT_MODE = process.env.NODE_ENV === 'development' || typeof window !== 'undefined';
...
export class AuthenticationService {
     ...
     * AI 에이전트 인증
     */
    async authenticateAIAgent(password: string, store: any): Promise<ApiResponse> {
        try {
            // 개발 모드에서 비밀번호 우회
            if (DEVELOPMENT_MODE && BYPASS_PASSWORD) {
     ...
     * 자동 잠금 해제 타이머 설정
     */
    setAutoUnlockTimer(userId: string, duration: number, callback: () => void): void {
        // 기존 타이머가 있다면 제거
