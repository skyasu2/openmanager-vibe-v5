...
import { SettingsData, GeneratorConfig, ApiResponse } from '../types/ProfileTypes';
...
export function useSettingsData(): UseSettingsDataReturn {
    ...
    const loadGeneratorConfig = useCallback(async () => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : '제너레이터 설정 로드 실패';
     ...
     * 서버 개수 업데이트
     */
    const updateServerCount = useCallback(async (count: number): Promise<ApiResponse> => {
        setGeneratorError(null);
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : '서버 개수 변경 중 오류';
            setGeneratorError(errorMessage);
     ...
     * 아키텍처 업데이트
     */
    const updateArchitecture = useCallback(async (arch: string): Promise<ApiResponse> => {
        setGeneratorError(null);
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : '아키텍처 변경 중 오류';
            setGeneratorError(errorMessage);
            return { success: false, error: errorMessage };
        }
    }, [settingsService, loadGeneratorConfig]);
