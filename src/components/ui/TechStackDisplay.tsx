...
import { TechCategory, TechItem, generateTechStackSummary } from '../../utils/TechStackAnalyzer';
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            ...
            {summary.topCategories.map((category, index) => (
              <span key={category} className="text-blue-300">
                {category}{index < summary.topCategories.length - 1 && ', '}
              </span>
            ))}
          </div>
        </motion.div>
          ...
          >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex-shrink-0">
                      <div className={`
                        ...
                        ${tech.importance === 'high' ? 'bg-red-400' : 
                          tech.importance === 'medium' ? 'bg-yellow-400' : 'bg-green-400'}
            ...
            <div className="mt-4 pt-3 border-t border-gray-700/30 flex items-center justify-between text-xs text-gray-400">
              <div className="flex gap-4">
                ...
                <span>
                  고중요도: {category.items.filter(item => item.importance === 'high').length}개
                </span>
              </div>
              <div className="flex items-center gap-1">
                ...
                <div className="flex gap-0.5">
                  {['high', 'medium', 'low'].map(level => {
                    const count = category.items.filter(item => item.importance === level).length;
                    return count > 0 ? (
                      <div
                        key={level}
        ...
        >
          <div className="text-center text-xs text-gray-300">
            ...
            <div className="flex justify-center gap-6">
              <span>총 <span className="text-blue-300 font-medium">{summary.totalTechs}개</span> 기술</span>
              <span>•</span>
              <span><span className="text-amber-300 font-medium">{summary.coreCount}개</span> 핵심 기술</span>
              <span>•</span>
              <span><span className="text-green-300 font-medium">{summary.categoryCount}개</span> 기술 분야</span>
            </div>
          </div>
        </motion.div>
      )}
