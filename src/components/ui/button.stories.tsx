...
import { createStoryMeta, storyTitles, a11yLabels } from '../../stories/templates/StoryTemplate';
...
export const Sizes: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: '다양한 크기의 버튼들입니다. 화면 공간과 중요도에 따라 선택할 수 있습니다.',
      },
    },
  },
};
...
export const ServerControlButtons: Story = {
  render: () => {
    const [serverState, setServerState] = useState<'online' | 'starting' | 'stopped'>('stopped');
      ...
      <Card className="w-96">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            서버 제어 패널
            <Badge variant={serverState === 'online' ? 'default' : serverState === 'starting' ? 'secondary' : 'destructive'}>
              {serverState === 'online' ? '온라인' : serverState === 'starting' ? '시작 중' : '중단됨'}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button 
              ...
              className="flex-1"
            >
              <RefreshCw className={`h-4 w-4 mr-1 ${serverState === 'starting' ? 'animate-spin' : ''}`} />
              재시작
            </Button>
          </div>
  ...
  parameters: {
    docs: {
      description: {
        story: '서버 모니터링 플랫폼의 핵심인 서버 제어 버튼들입니다. 상태에 따른 버튼 활성화/비활성화와 아이콘 애니메이션을 포함합니다.',
      },
    },
  },
};
...
export const AIInteractionButtons: Story = {
  render: () => {
    const [aiState, setAiState] = useState<'ready' | 'thinking' | 'error'>('ready');
    ...
    const handleAIAction = (action: string) => {
      ...
      setTimeout(() => {
        setAiState('ready');
        setConversation(prev => [...prev, `AI: ${action} 작업을 완료했습니다.`]);
      }, 2000);
    };
      ...
      <Card className="w-96">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            AI 어시스턴트
            <Badge variant={aiState === 'ready' ? 'default' : aiState === 'thinking' ? 'secondary' : 'destructive'}>
              {aiState === 'ready' ? '준비됨' : aiState === 'thinking' ? '처리 중' : '오류'}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Button 
              ...
              size="sm"
            >
              <RefreshCw className={`h-4 w-4 mr-1 ${aiState === 'thinking' ? 'animate-spin' : ''}`} />
              최적화
            </Button>
            <Button 
              variant="outline"
  ...
  parameters: {
    docs: {
      description: {
        story: 'Multi-AI 엔진과의 상호작용을 위한 버튼들입니다. AI 상태에 따른 버튼 동작과 실시간 피드백을 제공합니다.',
      },
    },
  },
};
...
export const ActionButtonGroups: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: '실제 사용되는 다양한 액션 버튼 그룹들입니다. 파일 관리, 인증, 네비게이션 등 용도별로 구분됩니다.',
      },
    },
  },
};
...
export const IconButtons: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: '아이콘이 포함된 버튼들입니다. 아이콘만 있는 버튼과 텍스트+아이콘 조합 버튼을 보여줍니다.',
      },
    },
  },
};
...
export const LoadingAndStates: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: '로딩 상태와 비활성화 상태의 버튼들입니다. 사용자에게 현재 작업 진행 상황을 명확히 전달합니다.',
      },
    },
  },
};
...
export const MobileOptimized: Story = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story: '모바일 화면에 최적화된 버튼들입니다. 터치 인터페이스와 화면 크기를 고려한 디자인입니다.',
      },
    },
  },
};
...
export const DarkMode: Story = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story: '다크모드에서의 버튼 모양을 확인할 수 있습니다. 색상 대비와 가독성을 검증합니다.',
      },
    },
  },
};
...
export const AllVariantsGrid: Story = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story: '모든 variant와 size 조합을 한눈에 비교할 수 있는 그리드입니다. 디자인 시스템의 일관성을 확인할 수 있습니다.',
      },
    },
  },
};
...
export const AccessibilityTest: Story = {
  render: () => (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Button variant="destructive" aria-label="서버를 완전히 종료합니다. 이 작업은 되돌릴 수 없습니다.">
          <Square className="h-4 w-4 mr-2" />
          종료
        </Button>
      </div>
  ...
  parameters: {
    docs: {
      description: {
        story: '접근성을 고려한 버튼들입니다. 스크린 리더, 키보드 네비게이션, 상태 전달 등을 테스트할 수 있습니다.',
      },
    },
  },
};