...
class InlineFeedbackManager {
    ...
    private listeners: Set<(feedbacks: Map<string, InlineFeedback>) => void> = new Set();
    private idCounter = 0;
    ...
    subscribe(listener: (feedbacks: Map<string, InlineFeedback>) => void): () => void {
    ...
    // 특정 영역에 피드백 표시
    showFeedback(area: string, type: InlineFeedback['type'], message: string, options?: {
        duration?: number;
        persistent?: boolean;
    }): string {
    ...
    // 편의 메서드들
    success(area: string, message: string, options?: { duration?: number; persistent?: boolean }): string {
        return this.showFeedback(area, 'success', message, options);
    }

    error(area: string, message: string, options?: { duration?: number; persistent?: boolean }): string {
        return this.showFeedback(area, 'error', message, options);
    }

    warning(area: string, message: string, options?: { duration?: number; persistent?: boolean }): string {
        return this.showFeedback(area, 'warning', message, options);
    }

    info(area: string, message: string, options?: { duration?: number; persistent?: boolean }): string {
        return this.showFeedback(area, 'info', message, options);
    }

    loading(area: string, message: string): string {
        return this.showFeedback(area, 'loading', message, { duration: 0, persistent: true });
    }
}
        ...
        switch (feedback.type) {
            case 'success':
                return {
                    ...
                    icon: <CheckCircle className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-green-600`} />
                };
            case 'error':
                return {
                    container: `${baseStyles} bg-red-50 text-red-800 border border-red-200`,
                    icon: <XCircle className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-red-600`} />
                };
            case 'warning':
                return {
                    container: `${baseStyles} bg-yellow-50 text-yellow-800 border border-yellow-200`,
                    icon: <AlertTriangle className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-yellow-600`} />
                };
            case 'loading':
                return {
                    container: `${baseStyles} bg-blue-50 text-blue-800 border border-blue-200`,
                    icon: <Loader2 className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-blue-600 animate-spin`} />
                };
            case 'info':
            default:
                return {
                    container: `${baseStyles} bg-blue-50 text-blue-800 border border-blue-200`,
                    icon: <Info className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} text-blue-600`} />
                };
        }
    };
...
export function InlineFeedbackContainer({
    ...
    const [feedbacks, setFeedbacks] = useState<Map<string, InlineFeedback>>(new Map());
    ...
    return (
        <div className="space-y-2">
            <motion.button
                ...
                className={`${className} ${status === 'loading' ? 'opacity-75 cursor-not-allowed' : ''
                    }`}
