...
import { RefreshCw, CheckCircle, XCircle, AlertCircle, User as UserIcon, Github, Clock } from 'lucide-react';
...
export default function AuthStatusChecker() {
  ...
  const getAuthStatusBadge = () => {
    ...
    if (supabaseSession.active) {
      return <Badge variant="default" className="bg-green-600"><Github className="w-3 h-3 mr-1" />GitHub 인증됨</Badge>;
    }
    
    if (guestSession.active) {
      return <Badge variant="secondary" className="bg-blue-600"><UserIcon className="w-3 h-3 mr-1" />게스트 활성</Badge>;
    }
    
    return <Badge variant="destructive"><XCircle className="w-3 h-3 mr-1" />인증 없음</Badge>;
  };
            ...
            {authStatus.supabaseSession.user && (
              <>
                <div className="text-sm">
                  <strong>사용자 ID:</strong> {authStatus.supabaseSession.user.id}
                </div>
                <div className="text-sm">
                  <strong>이메일:</strong> {authStatus.supabaseSession.user.email}
                </div>
                <div className="text-sm">
                  <strong>이름:</strong> {authStatus.supabaseSession.user.user_metadata?.full_name || 'N/A'}
                </div>
                <div className="text-sm">
                  <strong>Provider:</strong> {authStatus.supabaseSession.provider || 'N/A'}
                </div>
              </>
            )}
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {authStatus.guestSession.active ? (
                <Badge variant="default" className="bg-blue-600">활성</Badge>
              ) : (
                <Badge variant="secondary">비활성</Badge>
              )}
            </div>
            ...
            {authStatus.guestSession.sessionId && (
              <div className="text-sm">
                <strong>세션 ID:</strong> {authStatus.guestSession.sessionId.substring(0, 20)}...
              </div>
            )}
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                {authStatus.guestSession.cookies.sessionId ? (
                  <Badge variant="outline" className="bg-green-50">존재</Badge>
                ) : (
                  <Badge variant="outline" className="bg-red-50">없음</Badge>
                )}
              </div>
              <div className="flex items-center justify-between text-xs">
                <span>쿠키 Auth Type:</span>
                {authStatus.guestSession.cookies.authType ? (
                  <Badge variant="outline" className="bg-green-50">게스트</Badge>
                ) : (
                  <Badge variant="outline" className="bg-red-50">없음</Badge>
                )}
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div className="flex items-center justify-between">
              ...
              {authStatus.environment.hasSupabaseUrl ? (
                <Badge variant="outline" className="bg-green-50">설정됨</Badge>
              ) : (
            ...
            <div className="flex items-center justify-between">
              ...
              {authStatus.environment.isClient ? (
                <Badge variant="outline" className="bg-green-50">브라우저</Badge>
              ) : (
                <Badge variant="outline" className="bg-blue-50">서버</Badge>
              )}
            </div>
          </div>
        </CardContent>
