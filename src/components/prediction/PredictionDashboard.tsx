...
export const PredictionDashboard: React.FC<PredictionDashboardProps> = ({
  ...
  // 장애 확률 게이지
  const ProbabilityGauge: React.FC<{ probability: number; severity: string }> = ({ probability, severity }) => {
    ...
    return (
      <div className="relative w-32 h-32 mx-auto">
        ...
        <div className="absolute inset-0 flex items-center justify-center flex-col">
          <span className="text-2xl font-bold text-white">{Math.round(probability)}%</span>
  ...
  // 예측 시간 카운트다운
  const TimeCountdown: React.FC<{ predictedTime: Date }> = ({ predictedTime }) => {
  ...
  if (error) {
    return (
      <div className={`bg-gray-900/95 backdrop-blur-lg border border-red-500/30 rounded-xl p-6 ${className}`}>
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="text-center">
                ...
                />
                <div className="mt-4">
                  <div className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-xs ${getSeverityConfig(predictionData.prediction.severity).bgColor} ${getSeverityConfig(predictionData.prediction.severity).color}`}>
                    {React.createElement(getSeverityConfig(predictionData.prediction.severity).icon, { className: 'w-3 h-3' })}
                    <span>{getSeverityConfig(predictionData.prediction.severity).label} 위험도</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-2">예측 정보</h4>
                  <div className="space-y-2">
                    <TimeCountdown predictedTime={predictionData.prediction.predictedTime} />
                    <div className="flex items-center space-x-2">
                      <Target className="w-4 h-4 text-green-400" />
                      <span className="text-sm text-gray-300">신뢰도:</span>
                      <span className="text-green-400">{Math.round(predictionData.prediction.confidence)}%</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Activity className="w-4 h-4 text-blue-400" />
                      <span className="text-sm text-gray-300">분석 타입:</span>
                      <span className="text-blue-400 capitalize">{predictionData.prediction.analysisType}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* 주요 원인 메트릭 */}
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-2">주요 원인</h4>
                  <div className="flex flex-wrap gap-2">
                    {predictionData.prediction.triggerMetrics.map((metric, index) => (
                      <span
                        key={index}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {predictionData.prediction.preventiveActions.slice(0, 4).map((action, index) => (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-white mb-2">예측 데이터 없음</h3>
