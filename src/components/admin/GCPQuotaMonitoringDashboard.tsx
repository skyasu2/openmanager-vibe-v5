...
import { Activity, AlertTriangle, CheckCircle, Cloud, TrendingDown, TrendingUp } from 'lucide-react';
...
export const GCPQuotaMonitoringDashboard: React.FC = () => {
    ...
    const [threeTierStats, setThreeTierStats] = useState<ThreeTierStats | null>(null);
    ...
    return (
        <div className="container mx-auto p-6 space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold">GCP Functions 모니터링 대시보드</h1>
                    <p className="text-gray-600">베르셀 부하 75% 감소 + AI 성능 50% 향상 추적</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">베르셀 부하 감소</CardTitle>
                        <TrendingDown className="h-4 w-4 text-green-600" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-green-600">
                            {threeTierStats?.performanceMetrics.vercelLoadReduction.toFixed(1)}%
                        </div>
                        <p className="text-xs text-gray-500">목표: 75%</p>
                        <Progress
                            value={threeTierStats?.performanceMetrics.vercelLoadReduction || 0}
                            className="mt-2"
                        />
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-blue-600">
                            {threeTierStats?.performanceMetrics.aiPerformanceImprovement.toFixed(1)}%
                        </div>
                        <p className="text-xs text-gray-500">목표: 50%</p>
                        <Progress
                            value={threeTierStats?.performanceMetrics.aiPerformanceImprovement || 0}
                            className="mt-2"
                        />
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">GCP 호출 사용량</CardTitle>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <div className="flex justify-between items-center mb-2">
                                            ...
                                            <span className="text-sm">{gcpStats?.freeQuotaUsage.calls.toLocaleString()}</span>
                                        </div>
                                        <Progress value={gcpStats?.freeQuotaUsage.callsPercent || 0} />
                                        <p className="text-xs text-gray-500 mt-1">
                                            {gcpStats?.freeQuotaUsage.callsPercent.toFixed(2)}% of 2M/month
                                        </p>
                                    </div>

                                    <div>
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-sm font-medium">컴퓨팅 (GB-초)</span>
                                            <span className="text-sm">{gcpStats?.freeQuotaUsage.compute.toFixed(2)}</span>
                                        </div>
                                        <Progress value={gcpStats?.freeQuotaUsage.computePercent || 0} />
                                        <p className="text-xs text-gray-500 mt-1">
                                            {gcpStats?.freeQuotaUsage.computePercent.toFixed(2)}% of 400K/month
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-2">
                                            ...
                                            <span className="text-sm">{gcpStats?.freeQuotaUsage.network.toFixed(2)}</span>
                                        </div>
                                        <Progress value={gcpStats?.freeQuotaUsage.networkPercent || 0} />
                                        <p className="text-xs text-gray-500 mt-1">
                                            {gcpStats?.freeQuotaUsage.networkPercent.toFixed(2)}% of 25GB/month
                                        </p>
                                    </div>
                                </div>
                            </CardContent>
                            ...
                            <CardContent>
                                <ResponsiveContainer width="100%" height={300}>
                                    <PieChart>
                                        <Pie
                                            data={[
                                                { name: 'Korean NLP', value: gcpStats?.engineUsage.korean || 0 },
                                                { name: 'Rule Engine', value: gcpStats?.engineUsage.rule || 0 },
                                                { name: 'Basic ML', value: gcpStats?.engineUsage.ml || 0 },
                                                { name: 'Fallback', value: gcpStats?.engineUsage.fallback || 0 },
                                            ]}
                                            cx="50%"
                                            cy="50%"
                                            labelLine={false}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <Card>
                            ...
                            <CardContent>
                                <ResponsiveContainer width="100%" height={300}>
                                    <BarChart data={[
                                        { name: 'Local', value: threeTierStats?.routingDecisions.local || 0 },
                                        { name: 'GCP', value: threeTierStats?.routingDecisions.gcp || 0 },
                                        { name: 'Google', value: threeTierStats?.routingDecisions.google || 0 },
                                    ]}>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <ResponsiveContainer width="100%" height={300}>
                                    <BarChart data={[
                                        { name: 'Local', time: threeTierStats?.averageResponseTimes.local || 0 },
                                        { name: 'GCP', time: threeTierStats?.averageResponseTimes.gcp || 0 },
                                        { name: 'Google', time: threeTierStats?.averageResponseTimes.google || 0 },
                                    ]}>
                                        <CartesianGrid strokeDasharray="3 3" />
                                        <XAxis dataKey="name" />
                                        <YAxis />
                            ...
                            <CardContent>
                                <ResponsiveContainer width="100%" height={300}>
                                    <LineChart data={historicalData}>
                                        ...
                                        <YAxis />
                                        <Tooltip />
                                        <Line type="monotone" dataKey="vercelLoadReduction" stroke="#8884d8" strokeWidth={2} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                            ...
                            <CardContent>
                                <ResponsiveContainer width="100%" height={300}>
                                    <LineChart data={historicalData}>
                                        ...
                                        <YAxis />
                                        <Tooltip />
                                        <Line type="monotone" dataKey="aiPerformanceImprovement" stroke="#82ca9d" strokeWidth={2} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div className="text-center">
                                    <div className="text-4xl font-bold text-green-600 mb-2">
                                        {((threeTierStats?.performanceMetrics.vercelLoadReduction || 0) / 75 * 100).toFixed(0)}%
                                    </div>
                                    <p className="text-sm text-gray-600">베르셀 부하 감소 목표 달성률</p>
                                    <p className="text-xs text-gray-500">(목표: 75%)</p>
                                </div>
                                <div className="text-center">
                                    <div className="text-4xl font-bold text-blue-600 mb-2">
                                        {((threeTierStats?.performanceMetrics.aiPerformanceImprovement || 0) / 50 * 100).toFixed(0)}%
                                    </div>
                                    <p className="text-sm text-gray-600">AI 성능 향상 목표 달성률</p>
                                    <p className="text-xs text-gray-500">(목표: 50%)</p>
                                </div>
                            </div>
                        </CardContent>
                        ...
                        <CardContent>
                            <ResponsiveContainer width="100%" height={400}>
                                <LineChart data={historicalData}>
                                    ...
                                    <YAxis />
                                    <Tooltip />
                                    <Line type="monotone" dataKey="totalRequests" stroke="#8884d8" strokeWidth={2} name="Total Requests" />
                                    <Line type="monotone" dataKey="gcpRequests" stroke="#82ca9d" strokeWidth={2} name="GCP Requests" />
                                    <Line type="monotone" dataKey="localRequests" stroke="#ffc658" strokeWidth={2} name="Local Requests" />
                                </LineChart>
                            </ResponsiveContainer>
                        </CardContent>
                    </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-2 text-sm">
                            ...
                            <div className="flex items-start gap-2">
                                <CheckCircle className="h-4 w-4 mt-0.5 text-green-600" />
                                <span>GCP Functions을 우선 사용하여 베르셀 부하를 감소시키세요.</span>
                            </div>
                            <div className="flex items-start gap-2">
                                <CheckCircle className="h-4 w-4 mt-0.5 text-green-600" />
