...
export default function LogAnalyticsDashboard() {
    ...
    // 로그 분석 데이터 조회
    const { data: analyticsData, isLoading, refetch } = useQuery<LogAnalyticsData>({
        queryKey: ['logAnalytics', selectedTimeRange],
        queryFn: async () => {
            const response = await fetch(`/api/admin/logs/analytics?timeRange=${selectedTimeRange}`);
            if (!response.ok) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900">📊 로그 분석 대시보드</h2>
                                    ...
                                    <tbody>
                                        {analyticsData.enginePerformance.map((engine, index) => (
                                            <tr key={index} className="border-b">
                                                ...
                                                <td className="p-2 text-right">{engine.requests.toLocaleString()}</td>
                                                <td className="p-2 text-right">{engine.avgResponseTime}</td>
                                                <td className="p-2 text-right">
                                                    <Badge variant={engine.successRate >= 95 ? 'default' : 'secondary'}>
                                                        {engine.successRate}%
                                                    </Badge>
                                                </td>
                                                <td className="p-2 text-right">
                                                    <Badge variant={engine.reliability >= 0.9 ? 'default' : 'secondary'}>
                ...
                <TabsContent value="quality" className="space-y-4">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>신뢰도, 사용자 만족도, 정확도 추이</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <ResponsiveContainer width="100%" height={400}>
