...
export type AIAgentState = 'disabled' | 'enabled' | 'processing' | 'idle' | 'learning';
...
export interface AIState {
  ...
  stopProcessing: () => void;
  updateMetrics: (query: string, responseTime: number, success: boolean) => void;
  ...
  subscribeWithSelector(
    persist(
      (set, get) => ({
        ...
        // 메트릭 업데이트
        updateMetrics: (query: string, responseTime: number, success: boolean) => {
          try {
            const { metrics } = get();
            
            const newTotalQueries = metrics.totalQueries + 1;
            const newSuccessfulQueries = success ? metrics.successfulQueries + 1 : metrics.successfulQueries;
            
            // 평균 응답 시간 계산
            const newAverageResponseTime = 
              (metrics.averageResponseTime * metrics.totalQueries + responseTime) / newTotalQueries;
            ...
            console.log(`📊 [AI] 메트릭 업데이트 - Query: ${query}, Success: ${success}`);
        ...
        addImprovement: (improvement: string) => {
          try {
            ...
            set({
              learningData: {
                ...learningData,
                improvements: [...learningData.improvements, improvement].slice(-20) // 최근 20개만 유지
              }
            });
        ...
        logError: (error: string) => {
          try {
            ...
            set({
              learningData: {
                ...learningData,
                errorLog: [...learningData.errorLog, `${new Date().toISOString()}: ${error}`].slice(-30) // 최근 30개만 유지
              }
            });
