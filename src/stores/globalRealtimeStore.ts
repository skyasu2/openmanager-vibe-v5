...
export const useGlobalRealtimeStore = create<GlobalRealtimeState>()(
    subscribeWithSelector((set, get) => ({
        ...
        fetchAllData: async () => {
            ...
            try {
                console.log('🔄 통합 실시간 데이터 업데이트 시작 (KST):', new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }));

                // 병렬 API 호출로 성능 최적화
                const [systemResponse, performanceResponse, serversResponse] = await Promise.allSettled([
                ...
                if (currentRetryCount < MAX_RETRY_COUNT) {
                    ...
                    console.log(`🔄 ${delay}ms 후 재시도 (${currentRetryCount}/${MAX_RETRY_COUNT})`);
        ...
        startPolling: () => {
            ...
            console.log(`🚀 통합 실시간 폴링 시작 (${UNIFIED_POLLING_INTERVAL / 1000}초 간격)`);
...
export const usePerformanceData = () => {
    const performanceData = useGlobalRealtimeStore(state => state.performanceData);
    const lastUpdate = useGlobalRealtimeStore(state => state.lastUpdate);
    const error = useGlobalRealtimeStore(state => state.error);
