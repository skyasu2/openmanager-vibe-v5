 ...
 *
 * OpenManager v5.44.3 - 데이터 보존 관리 훅 (2025-01-04 18:15 KST)
...
export function useDataRetention(): UseDataRetentionReturn {
    ...
    // 수동 정리 실행
    const runManualCleanup = useCallback(async (dataType?: string): Promise<CleanupResult[]> => {
        const scheduler = getScheduler();
        if (!scheduler) return [];
    ...
    // 정책 추가
    const addPolicy = useCallback((policyData: Omit<RetentionPolicy, 'id'>): string | null => {
        const scheduler = getScheduler();
    ...
    // 정책 업데이트
    const updatePolicy = useCallback((id: string, updates: Partial<RetentionPolicy>): boolean => {
        const scheduler = getScheduler();
    ...
    // 정책 삭제
    const deletePolicy = useCallback((id: string): boolean => {
        const scheduler = getScheduler();
        if (!scheduler) return false;
...
// 개별 데이터 타입 정리 전용 훅
export function useDataRetentionByType(dataType: 'metrics' | 'alerts' | 'connections' | 'logs' | 'cache' | 'sse') {
