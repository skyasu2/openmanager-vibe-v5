...
export const useRealtimeServersSSE = (config: SSEConfig = {}) => {
  ...
  const {
    ...
    reconnectInterval = 30000, // 5초 → 30초로 개선
  ...
  const [lastMessage, setLastMessage] = useState<SSERealtimeMessage | null>(
    null
  );
    ...
    try {
      ...
      eventSource.onmessage = event => {
        try {
            ...
            case 'server-metrics':
              ...
              if (message.serverId && message.data) {
                queryClient.setQueryData(
                  ...
                  (oldData: any[]) => {
                    if (!oldData)
                      return [{ id: message.serverId, ...message.data }];
                ...
                queryClient.setQueryData(
                  ...
                  (oldData: any) =>
                    oldData ? { ...oldData, ...message.data } : message.data
                );
              }
              break;
          ...
          if (
            ...
            console.error(
              `🚨 ${message.data?.serverName || '시스템'}: ${message.data?.prediction || message.message}`
            );
          } else if (message.priority === 'high') {
            console.warn(
              `⚠️ ${message.data?.serverName || '시스템'}: ${message.data?.prediction || message.message}`
            );
          }
        } catch (error) {
      ...
      eventSource.onerror = error => {
        ...
        if (
          ...
          console.log(
            `🔄 서버 SSE 재연결 시도 ${reconnectAttemptsRef.current}/${maxReconnectAttempts}`
          );
...
export const useRealtimePredictionsSSE = (config: SSEConfig = {}) => {
  ...
  const [lastPrediction, setLastPrediction] =
    useState<SSERealtimeMessage | null>(null);
    ...
    try {
      ...
      eventSource.onmessage = event => {
        try {
          ...
          if (
            ...
            console.error(
              `🚨 AI 예측: ${message.data?.serverName} - ${message.data?.prediction}`
            );
          }
        } catch (error) {
      ...
      eventSource.onerror = error => {
        ...
        if (
          ...
          reconnectAttemptsRef.current++;
          console.log(
            `🔄 AI 예측 SSE 재연결 시도 ${reconnectAttemptsRef.current}/${maxReconnectAttempts}`
          );
