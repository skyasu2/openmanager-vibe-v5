...
export const systemKeys = {
    ...
    metrics: (timeRange: string) => [...systemKeys.all, 'metrics', timeRange] as const,
...
const fetchSystemAlerts = async (filters?: {
    ...
    if (dashboardData.servers) {
        dashboardData.servers.forEach((server: any) => {
            if (server.status === 'critical' && (!filters?.level || filters.level === 'error')) {
                alerts.push({
            ...
            } else if (server.status === 'warning' && (!filters?.level || filters.level === 'warning')) {
                alerts.push({
