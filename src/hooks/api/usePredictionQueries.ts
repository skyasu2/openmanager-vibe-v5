...
const fetchPredictionHistory = async (
  ...
  const response = await fetch(
    `/api/ai/prediction/history?metric=${metric}&limit=${limit}`
  );
  if (!response.ok) {
...
const fetchPredictionAnalytics = async (
  ...
  const response = await fetch(
    `/api/ai/prediction/analytics?range=${timeRange}`
  );
  if (!response.ok) {
...
export const predictionKeys = {
  ...
  history: (metric: string) =>
    [...predictionKeys.all, 'history', metric] as const,
  analytics: (timeRange: string) =>
    [...predictionKeys.all, 'analytics', timeRange] as const,
      ...
      const withAccuracy = data.map((item) => ({
        ...
        accuracy:
          item.actual_value && item.predicted_value
            ? 1 -
              Math.abs(item.actual_value - item.predicted_value) /
                item.actual_value
            : undefined,
      }));
    ...
    select: (data) => ({
      ...
      performanceGrade:
        data.avgAccuracy >= 0.9
          ? 'A'
          : data.avgAccuracy >= 0.8
            ? 'B'
            : data.avgAccuracy >= 0.7
              ? 'C'
              : 'D',
      recommendedActions:
        data.avgAccuracy < 0.7
          ...
          : data.avgAccuracy < 0.9
            ? ['모델 성능 개선 여지 있음', '추가 데이터 수집 권장']
            : ['우수한 예측 성능', '현재 설정 유지 권장'],
    }),
    meta: {
...
export const useRealtimePrediction = (
  ...
  return useQuery({
    ...
    select: (data) => ({
      ...
      confidence_level:
        data.accuracy_score >= 0.9
          ? 'high'
          : data.accuracy_score >= 0.7
            ? 'medium'
            : 'low',
    }),
    meta: {
    ...
    ? {
        ...
        byMetric: predictions.reduce(
          (acc, pred) => {
            ...
            acc[metric].avgAccuracy =
              acc[metric].predictions.reduce(
                (sum: number, p: PredictionResult) => sum + p.accuracy_score,
                0
              ) / acc[metric].count;
            acc[metric].latestTrend =
