  ...
  return {
    data: servers
      ? {
          ...
          online: servers.filter((s: unknown) => s.status === 'online').length,
          offline: servers.filter((s: unknown) => s.status === 'offline').length,
          warning: servers.filter((s: unknown) => s.status === 'warning').length,
          avgCpu:
