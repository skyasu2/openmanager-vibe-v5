...
export function useSequentialServerGeneration(options: UseSequentialServerGenerationOptions = {}) {
  const {
    autoStart = false,
    intervalMs = 1000,
    onServerAdded,
  ...
  const generateNextServer = useCallback(async (): Promise<{
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        return { success: false, currentCount: servers.length, isComplete: false };
      }
  ...
  }, [status.isGenerating, generateNextServer, intervalMs, onServerAdded, onComplete, onError, servers]);
  ...
  // 자동 시작
  useEffect(() => {
    if (autoStart && !status.isGenerating && !status.isComplete && servers.length === 0) {
      startGeneration();
    }
  }, [autoStart, status.isGenerating, status.isComplete, servers.length, startGeneration]);
