...
interface SystemEvent {
  id: string;
  type: 'pattern_detected' | 'notification_sent' | 'data_cleaned' | 'connection_change' | 'error' | 'prediction';
...
export const useSystemIntegration = () => {
  ...
  // 🧠 예측 분석 상태
  const [predictionResults, setPredictionResults] = useState<{ [serverId: string]: any }>({});
      ...
      setState(prev => {
        const newState = {
          ...
          patternMatcher: {
            ...
            lastAnalysis: patternData.data?.stats?.lastAnalysis ? new Date(patternData.data.stats.lastAnalysis) : null,
            detectedAnomalies: patternData.data?.stats?.totalAlerts || 0,
            averageProcessingTime: patternData.data?.stats?.averageProcessingTime || 0
          },
          dataRetention: {
            isRunning: retentionData.success && retentionData.data?.isActive,
            lastCleanup: retentionData.data?.stats?.lastCleanup ? new Date(retentionData.data.stats.lastCleanup) : null,
          ...
          notifications: {
            ...
            channels: (notificationData.data as any)?.channels || { slack: false, discord: false, email: false },
            pendingNotifications: (notificationData.data as any)?.pending || 0,
            lastNotification: (notificationData.data as any)?.lastSent ? new Date((notificationData.data as any).lastSent) : null
          },
          lastUpdate: new Date()
        };
        ...
        newState.overallHealth = 
          averageHealth >= 80 ? 'healthy' :
          averageHealth >= 60 ? 'warning' :
          averageHealth >= 30 ? 'critical' : 'offline';
  ...
  const initializeSystem = useCallback(async (): Promise<boolean> => {
    try {
      ...
      emitEvent('connection_change', 'info', '시스템 초기화 시작...');
      ...
      // PatternMatcher 활성화
      const patternTest = await fetch('/api/metrics/pattern-check', { method: 'POST' });
      if (!patternTest.ok) throw new Error('PatternMatcher 초기화 실패');
      
      setState(prev => ({ ...prev, initializationProgress: 75 }));
      
      // DataRetention 시작
      const retentionTest = await fetch('/api/cron/cleanup', { method: 'POST' });
      ...
      emitEvent('connection_change', 'info', '✅ 시스템 초기화 완료!');
      return true;

    } catch (error) {
      emitEvent('error', 'critical', `❌ 시스템 초기화 실패: ${error}`);
      setState(prev => ({ ...prev, isInitialized: false, initializationProgress: 0 }));
      return false;
    }
  ...
  const triggerPatternAnalysis = useCallback(async (): Promise<any> => {
    try {
      ...
      if (result.success) {
        emitEvent('pattern_detected', 'info', `✅ 패턴 분석 완료: ${result.data?.detectedPatterns || 0}개 패턴 발견`);
        return result.data;
      } else {
  ...
  const forceDataCleanup = useCallback(async (): Promise<any> => {
    try {
      ...
      if (result.success) {
        emitEvent('data_cleaned', 'info', `✅ 데이터 정리 완료: ${result.data?.cleanedCount || 0}개 항목 정리`);
        return result.data;
      } else {
  ...
  const sendTestNotification = useCallback(async (channel?: string): Promise<boolean> => {
    try {
      ...
      if (result.success) {
        emitEvent('notification_sent', 'info', `✅ 테스트 알림 발송 성공 (${channel || 'all'})`);
        return true;
      } else {
  ...
  }, [testRealTimeHub, triggerPatternAnalysis, forceDataCleanup, sendTestNotification, emitEvent]);
  ...
  const exportSystemReport = useCallback(async (): Promise<any> => {
    const report = {
      ...
      summary: {
        ...
        criticalEvents: state.recentEvents.filter(e => e.severity === 'critical').length,
        warningEvents: state.recentEvents.filter(e => e.severity === 'warning').length,
        moduleStatus: {
  ...
  const subscribeToEvents = useCallback((callback: (event: SystemEvent) => void) => {
   ...
   * 🧠 예측 분석 엔진 연동
   */
  const runPredictionAnalysis = useCallback(async (serverId?: string) => {
    try {
      ...
      const prediction = await predictiveAnalysisEngine.addDataPoint(targetServerId, currentMetrics);
      ...
      if (prediction) {
        ...
        if (prediction.failureProbability > 70) {
          emitEvent('prediction', prediction.severity === 'critical' ? 'critical' : 'warning', `${targetServerId}에서 ${Math.round(prediction.failureProbability)}% 장애 확률 예측됨`, {
      ...
      emitEvent('error', 'warning', '예측 분석 엔진에서 오류가 발생했습니다', { error: error instanceof Error ? error.message : String(error) });
    }
  }, [emitEvent]);
  ...
  return {
    ...
    eventStats: {
      ...
      critical: state.recentEvents.filter(e => e.severity === 'critical').length,
