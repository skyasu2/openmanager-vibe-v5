...
export class OptimizedDataGenerator {
  ...
  async generateBaselineData(servers: EnhancedServerMetrics[]): Promise<void> {
    ...
    console.log(`✅ ${servers.length}개 서버의 24시간 베이스라인 데이터 생성 완료`);
   ...
   * 📊 개별 서버 베이스라인 생성
   */
  private async createServerBaseline(server: EnhancedServerMetrics): Promise<ServerBaselineData> {
   ...
   * 🏗️ 서버 역할별 기본 부하 설정
   */
  private getServerBaseLoad(role: ServerRole, status: ServerStatus): BaseLoadConfig {
    const statusMultiplier = status === 'critical' ? 1.5 : status === 'warning' ? 1.2 : 1.0;
    
    const baseLoads: Record<ServerRole, BaseLoadConfig> = {
      web: { cpu: 45, memory: 50, disk: 40, networkIn: 150, networkOut: 200, responseTime: 120 },
  ...
  private generateInitialVariation(): RealTimeVariation {
    return {
      cpu_variation: (Math.random() - 0.5) * this.config.realTimeVariationIntensity,
      memory_variation: (Math.random() - 0.5) * this.config.realTimeVariationIntensity,
      disk_variation: (Math.random() - 0.5) * (this.config.realTimeVariationIntensity * 0.5), // 디스크는 변동 적음
      network_variation: (Math.random() - 0.5) * this.config.realTimeVariationIntensity * 2, // 네트워크는 변동 큼
      response_variation: (Math.random() - 0.5) * this.config.realTimeVariationIntensity,
  ...
  async generateRealTimeData(): Promise<EnhancedServerMetrics[]> {
    ...
    for (const [serverId, baseline] of this.baselineStorage) {
      const currentMinute = Math.floor((currentTime - baseline.last_generated) / 60000) % 1440;
      const baselinePoint = baseline.daily_pattern[currentMinute];
      const variation = this.currentVariations.get(serverId) || this.generateInitialVariation();
      ...
      const server: EnhancedServerMetrics = {
        ...
        cpu_usage: this.applyVariation(baselinePoint.cpu_baseline, variation.cpu_variation, variation.anomaly_factor),
        memory_usage: this.applyVariation(baselinePoint.memory_baseline, variation.memory_variation, variation.anomaly_factor),
        disk_usage: this.applyVariation(baselinePoint.disk_baseline, variation.disk_variation, 0), // 디스크는 이상치 없음
        network_in: this.applyVariation(baselinePoint.network_in_baseline, variation.network_variation, variation.anomaly_factor),
        network_out: this.applyVariation(baselinePoint.network_out_baseline, variation.network_variation, variation.anomaly_factor),
        response_time: this.applyVariation(baselinePoint.response_time_baseline, variation.response_variation, variation.anomaly_factor),
   ...
   * 📊 베이스라인 값에 변동 적용
   */
  private applyVariation(baseline: number, variation: number, anomaly: number): number {
   ...
   * 📈 현재 상태 계산
   */
  private calculateCurrentStatus(baseline: BaselineDataPoint, variation: RealTimeVariation): ServerStatus {
    const avgLoad = (baseline.cpu_baseline + baseline.memory_baseline) / 2;
    const variationImpact = Math.abs(variation.cpu_variation) + Math.abs(variation.memory_variation);
    
    if (avgLoad > 80 || variationImpact > 0.3 || variation.anomaly_factor > 0.2) {
      return 'critical';
    }
   ...
   * 🔄 변동값 부드럽게 업데이트
   */
  private updateVariation(serverId: string, currentVariation: RealTimeVariation): void {
    ...
    const smoothedVariation: RealTimeVariation = {
      cpu_variation: currentVariation.cpu_variation * smoothingFactor + newVariation.cpu_variation * (1 - smoothingFactor),
      memory_variation: currentVariation.memory_variation * smoothingFactor + newVariation.memory_variation * (1 - smoothingFactor),
      disk_variation: currentVariation.disk_variation * smoothingFactor + newVariation.disk_variation * (1 - smoothingFactor),
      network_variation: currentVariation.network_variation * smoothingFactor + newVariation.network_variation * (1 - smoothingFactor),
      response_variation: currentVariation.response_variation * smoothingFactor + newVariation.response_variation * (1 - smoothingFactor),
  ...
  private calculateUptime(status: ServerStatus): number {
    switch (status) {
      case 'critical': return Math.random() * 72 + 24; // 1-3일
      case 'warning': return Math.random() * 168 + 72; // 3일-1주
      default: return Math.random() * 8760 + 168; // 1주-1년
    }
  }
  ...
  async start(initialServers: EnhancedServerMetrics[]): Promise<void> {
    ...
    timerManager.register({
      ...
      callback: async () => {
        ...
        // 메모리 최적화 (100회마다)
        if (this.config.memoryOptimizationEnabled && this.updateCounter % 100 === 0) {
          await this.optimizeMemory();
        }
        
        // 패턴 업데이트 체크
        if (Date.now() - this.lastPatternUpdate > this.config.patternUpdateInterval) {
  ...
  private async refreshPatterns(): Promise<void> {
    ...
    for (const baseline of this.baselineStorage.values()) {
      serverList.push({
        ...
        network_in: 0, network_out: 0, response_time: 0,
        uptime: 0, last_updated: '', alerts: []
      });
    }
  ...
  private async compressBaselineData(): Promise<void> {
    for (const [serverId, baseline] of this.baselineStorage) {
      ...
      for (let hour = 0; hour < 24; hour++) {
        const hourlyPoints = baseline.daily_pattern.slice(hour * 60, (hour + 1) * 60);
        const avgPoint: BaselineDataPoint = {
          timestamp: hourlyPoints[0].timestamp,
          cpu_baseline: hourlyPoints.reduce((sum, p) => sum + p.cpu_baseline, 0) / hourlyPoints.length,
          memory_baseline: hourlyPoints.reduce((sum, p) => sum + p.memory_baseline, 0) / hourlyPoints.length,
          disk_baseline: hourlyPoints.reduce((sum, p) => sum + p.disk_baseline, 0) / hourlyPoints.length,
          network_in_baseline: hourlyPoints.reduce((sum, p) => sum + p.network_in_baseline, 0) / hourlyPoints.length,
          network_out_baseline: hourlyPoints.reduce((sum, p) => sum + p.network_out_baseline, 0) / hourlyPoints.length,
          response_time_baseline: hourlyPoints.reduce((sum, p) => sum + p.response_time_baseline, 0) / hourlyPoints.length,
          pattern_multiplier: hourlyPoints.reduce((sum, p) => sum + p.pattern_multiplier, 0) / hourlyPoints.length
        };
