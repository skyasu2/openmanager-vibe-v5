...
export class RedisMetricsManager {
     ...
     */
    async saveRealtimeMetrics(sessionId: string, metrics: ServerMetric[]): Promise<void> {
    ...
    async calculateRealtimeStats(sessionId: string): Promise<RealtimeStats> {
        ...
        return {
            ...
            averageDiskUsage: diskUsages.reduce((a, b) => a + b, 0) / diskUsages.length,
            averageNetworkUsage: networkUsages.reduce((a, b) => a + b, 0) / networkUsages.length,
     ...
     */
    async getMultipleServerLatestMetrics(serverIds: string[]): Promise<(ServerMetric | null)[]> {
