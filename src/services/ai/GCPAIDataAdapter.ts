...
export class GCPAIDataAdapter {
  ...
  async getAIAnalysisDataset(sessionId?: string): Promise<ProcessedAIData> {
    ...
    try {
      ...
      console.log(
        `✅ GCP → AI 변환 완료: ${dataset.metrics.length}개 메트릭, ${processingTime}ms`
      );
      return result;
    } catch (error) {
  ...
  private async fetchGCPData(sessionId?: string): Promise<GCPDataResponse> {
    ...
    try {
      const response = await fetch(
        `/api/gcp/server-data?sessionId=${sessionId}&limit=50`
      );
    ...
    metrics.forEach(metric => {
      ...
      if (metric.application.requests.errors > 0) {
        logs.push({
          ...
          metadata: {
            errorRate:
              metric.application.requests.errors /
              metric.application.requests.total,
    ...
    metrics.forEach(metric => {
      ...
      if (metric.application.requests.total > 0) {
        traces.push({
          ...
          metadata: {
            ...
            successRate:
              metric.application.requests.success /
              metric.application.requests.total,
      ...
      if (metric.application.database.queries.total > 0) {
        traces.push({
          ...
          status:
            metric.application.database.deadlocks > 0 ? 'error' : 'success',
          metadata: {
  ...
  private async analyzePatterns(metrics: TimeSeriesMetrics[]): Promise<any> {
    ...
    metrics.forEach(metric => {
      ...
      const memoryUsage =
        ...
        100;
      const errorRate =
        metric.application.requests.total > 0
          ? metric.application.requests.errors /
            metric.application.requests.total
          : 0;
    ...
    Object.entries(serverGroups).forEach(([serverId, serverMetrics]) => {
      if (serverMetrics.length > 1) {
        const avgCpu =
          serverMetrics.reduce((sum, m) => sum + m.system.cpu.usage, 0) /
          serverMetrics.length;
        const avgErrors =
          serverMetrics.reduce(
            (sum, m) => sum + m.application.requests.errors,
            0
          ) / serverMetrics.length;
    ...
    Object.entries(serverGroups).forEach(([serverId, serverMetrics]) => {
      if (serverMetrics.length >= 3) {
        const sortedMetrics = serverMetrics.sort(
          (a, b) => a.timestamp.getTime() - b.timestamp.getTime()
        );
        const cpuTrend = this.calculateTrend(
          sortedMetrics.map(m => m.system.cpu.usage)
        );
  ...
  private generateInsights(dataset: AIAnalysisDataset): any {
    ...
    // 전체 시스템 상태 분석
    const avgCpu =
      metrics.reduce((sum, m) => sum + m.system.cpu.usage, 0) / metrics.length;
    const avgMemory =
    ...
    const totalRequests = metrics.reduce(
      (sum, m) => sum + m.application.requests.total,
      0
    );
    const errorRate = totalRequests > 0 ? totalErrors / totalRequests : 0;

    // 심각도 분류
    const criticalServers = metrics.filter(
      m =>
    ...
    const warningServers = metrics.filter(
      m =>
    ...
    return {
      ...
      recommendations: [
        ...
        avgMemory > 85
          ? '🚨 메모리 사용률이 높습니다. 메모리 최적화가 필요합니다.'
          : null,
        errorRate > 0.05
          ? '🚨 에러율이 높습니다. 애플리케이션 로그를 확인하세요.'
          : null,
        criticalServers > 0
          ? `🔥 ${criticalServers}개 서버가 임계 상태입니다.`
          : null,
      ...
      criticalAlerts: [
        ...
        dataset.patterns?.anomalies && dataset.patterns.anomalies.length > 5
          ? '다수 이상 패턴 감지'
          : null,
      ]
        .filter(Boolean)
        .concat(
          ...
          criticalServers === 0 &&
            ...
            (!dataset.patterns?.anomalies ||
              dataset.patterns.anomalies.length <= 5)
            ? ['정상 운영 상태']
            : []
        ),
  ...
  private generateAIRecommendations(dataset: AIAnalysisDataset): string[] {
    ...
    if ((dataset.patterns?.anomalies?.length || 0) > 5) {
      recommendations.push(
        '다수의 이상 패턴이 감지되었습니다. 시스템 점검이 필요합니다.'
      );
    }

    if (dataset.metrics.length < 10) {
      recommendations.push(
        '더 많은 데이터 포인트가 필요합니다. 모니터링 주기를 늘려보세요.'
      );
    }
