...
export class AIStateManager {
        ...
        try {
            ...
            switch (state.type) {
                case 'master':
                    ...
                    responseTime = masterStatus.reduce((sum, s) => sum + s.avg_response_time, 0) / masterStatus.length;
                    errorRate = 1 - masterStatus.reduce((sum, s) => sum + s.success_rate, 0) / masterStatus.length;
                    break;
            ...
            state.health = {
                ...
                consecutiveFailures: isHealthy ? 0 : state.health.consecutiveFailures + 1,
            };
            ...
            if (isHealthy) {
                state.status = 'active';
            } else if (state.health.consecutiveFailures >= this.healthCheckConfig.maxConsecutiveFailures) {
                state.status = 'error';
            } else {
            ...
            console.log(`🔍 ${state.name} 헬스 체크: ${isHealthy ? '✅' : '❌'} (${responseTime.toFixed(0)}ms)`);
    ...
    private updateSystemState(): void {
        ...
        // 성능 메트릭 계산
        const totalResponseTime = engines.reduce((sum, e) => sum + e.health.responseTime, 0);
        const averageResponseTime = totalEngines > 0 ? totalResponseTime / totalEngines : 0;

        const totalRequests = engines.reduce((sum, e) => sum + e.performance.totalRequests, 0);
        const successfulRequests = engines.reduce((sum, e) => sum + e.performance.successfulRequests, 0);
        const overallSuccessRate = totalRequests > 0 ? successfulRequests / totalRequests : 0;

        const totalMemoryUsage = engines.reduce((sum, e) => sum + (e.performance.memoryUsage || 0), 0);
        const systemThroughput = engines.reduce((sum, e) => sum + e.performance.throughput, 0);
    ...
    private async processHealthAlerts(): Promise<void> {
        ...
        for (const engine of engines) {
            // 엔진 장애 알림
            if (engine.status === 'error' && engine.health.consecutiveFailures === this.healthCheckConfig.maxConsecutiveFailures) {
            ...
            // 성능 저하 알림
            if (engine.health.responseTime > this.healthCheckConfig.alertThresholds.responseTimeMs) {
                await unifiedNotificationService.sendAIAlert(
                    `${engine.name} 성능 저하`,
            ...
            // 메모리 사용량 알림
            if (engine.performance.memoryUsage &&
                engine.performance.memoryUsage > this.healthCheckConfig.alertThresholds.memoryUsageMB) {
                await unifiedNotificationService.sendAIAlert(
            ...
            // 복구 알림
            if (engine.status === 'active' && engine.health.consecutiveFailures === 0) {
    ...
    private initializeEngineStates(): void {
        ...
        this.engineStates.set('opensource', {
            ...
            capabilities: ['anomaly', 'prediction', 'autoscaling', 'korean-nlp', 'enhanced-search'],
            version: '1.0.0',
            uptime: 0,
        });
    }
    ...
    async restartEngine(engineId: string): Promise<boolean> {
        ...
        try {
            ...
            switch (state.type) {
                case 'master':
                    await this.masterEngine.restart();
                    break;
                case 'unified':
                    await this.unifiedEngine.restart();
                    break;
                case 'opensource':
                    await this.openSourceEngines.restart();
                    break;
            }
