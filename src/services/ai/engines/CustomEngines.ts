...
export class CustomEngines {
  ...
  async mcpQuery(query: string, context?: any): Promise<MCPQueryResult> {
    const reasoning_steps = ['질의 분석', '컨텍스트 로드', '추론 적용', '응답 생성'];
    
    return {
      answer: `"${query}"에 대한 MCP 분석이 완료되었습니다.`,
      confidence: 0.85,
      reasoning_steps,
      related_servers: context?.servers?.slice(0, 3).map(s => s.id) || [],
   ...
   */
  async hybridAnalysis(query: string, data: any): Promise<HybridAnalysisResult> {
  ...
  private analyzeContext(query: string, context?: any): boolean {
    return context && (context.servers || context.metrics || context.user_session);
  }
  ...
  private async generateMCPResponse(query: string, context?: any, steps?: string[]): Promise<string> {
    // MCP 기반 응답 생성 시뮬레이션
  ...
  private generateRecommendations(query: string, context?: any): string[] {
    ...
    return recommendations.length > 0 ? recommendations : ['시스템 상태 정기 점검'];
  }
  ...
  private calculateMCPConfidence(query: string, context?: any, steps?: string[]): number {
  ...
  private async fallbackMCPResponse(query: string, context?: any): Promise<MCPQueryResult> {
  ...
  private async runTestQueries(): Promise<Array<{ query: string; success: boolean; response_time: number }>> {
    const testQueries = [
      '서버 상태 확인',
      '성능 지표 조회',
      '최근 알림 내역'
  ...
  private async analyzeWithOpenSource(query: string, data: any): Promise<any> {
    // 오픈소스 엔진 조합으로 분석
    const nlpResult = await this.openSourceEngines.advancedNLP(query);
    const searchResult = await this.openSourceEngines.hybridSearch([data], query);
  ...
  private calculateHybridConfidence(mcpAnalysis: any, opensourceAnalysis: any): number {
  ...
  private generateHybridRecommendation(mcpAnalysis: any, opensourceAnalysis: any): string {
    if (mcpAnalysis && opensourceAnalysis) {
      return 'MCP와 오픈소스 분석 결과가 일치합니다. 높은 신뢰도로 권장사항을 제공합니다.';
    }
    if (mcpAnalysis) return 'MCP 분석 결과를 기반으로 권장사항을 제공합니다.';
    if (opensourceAnalysis) return '오픈소스 분석 결과를 기반으로 권장사항을 제공합니다.';
  ...
  private async analyzeServers(servers: any[]): Promise<any> {
    ...
    return {
      ...
      critical_servers: servers.filter(s => s.cpu_usage > 90 || s.memory_usage > 90)
    };
  }
  ...
  private async analyzeAlerts(alerts: any[]): Promise<any> {
    ...
    const recentAlerts = alerts.filter(a => Date.now() - new Date(a.timestamp).getTime() < 3600000);
  ...
  private calculateUnifiedScore(serverAnalysis: any, logAnalysis: any, metricAnalysis: any, alertAnalysis: any): number {
    let score = 100;
  ...
  private generatePriorityActions(serverAnalysis: any, logAnalysis: any, metricAnalysis: any, alertAnalysis: any): string[] {
    const actions = [];
  ...
  private classifyIntent(query: string): string {
    ...
    if (lowerQuery.includes('상태') || lowerQuery.includes('status')) return 'status_check';
    if (lowerQuery.includes('성능') || lowerQuery.includes('performance')) return 'performance_analysis';
    if (lowerQuery.includes('문제') || lowerQuery.includes('error')) return 'troubleshooting';
    if (lowerQuery.includes('추천') || lowerQuery.includes('recommend')) return 'recommendation';
    if (lowerQuery.includes('예측') || lowerQuery.includes('predict')) return 'prediction';
    
    return 'general_inquiry';
  }

  private extractOpenManagerEntities(query: string): Array<{ type: string; value: string; confidence: number }> {
    const entities = [];
    
    // 서버 엔티티
    const serverMatches = query.match(/server-?\w+/gi) || [];
    serverMatches.forEach(match => {
      entities.push({ type: 'server', value: match, confidence: 0.9 });
    });
    
    // 메트릭 엔티티
    const metricKeywords = ['cpu', 'memory', 'disk', 'network', 'response_time'];
    metricKeywords.forEach(keyword => {
      if (query.toLowerCase().includes(keyword)) {
        entities.push({ type: 'metric', value: keyword, confidence: 0.8 });
      }
    });
    ...
    actionKeywords.forEach(keyword => {
      if (query.toLowerCase().includes(keyword)) {
        entities.push({ type: 'action', value: keyword, confidence: 0.85 });
      }
    });
  ...
  private selectResponseTemplate(intent: string, entities: any[]): string {
    ...
    return templates[intent] || templates.general_inquiry;
  }
  ...
  getEngineStatus() {
    return {
      ...
      engines: {
        mcp_query: { status: this.mcpConnected ? 'active' : 'disconnected', core: true },
