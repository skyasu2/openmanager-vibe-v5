...
export class IncidentReportService {
   ...
   * 장애 분석 및 보고서 생성
   */
  async analyzeIncident(data: ServerMetric | ServerMetric[]): Promise<IncidentReport> {
    ...
    // 영향 범위 분석
    const affectedServers = this.analyzeAffectedServers(metrics, primaryIncident);
    ...
    const report: IncidentReport = {
      ...
      recommendations: this.generateRecommendations(primaryIncident, cascadeImpact),
      ...
      impact: {
        ...
        businessImpact: this.assessBusinessImpact(primaryIncident, affectedServers),
      },
   ...
   * 주요 장애 선택
   */
  private selectPrimaryIncident(incidents: IncidentPattern[]): IncidentPattern | null {
  ...
  private generateDescription(
    ...
    return `${incident.title}가 감지되었습니다. ` +
   ...
   * 상태별 그룹화
   */
  private groupByStatus(metrics: ServerMetric[]): Record<string, ServerMetric[]> {
