...
import { DocumentIndexManager, DocumentContext } from './hybrid/managers/DocumentIndexManager';
...
export class HybridAIEngine {
  ...
  async processHybridQuery(
    ...
    try {
      ...
      console.log(`🎯 하이브리드 쿼리 처리 완료 (${totalTime}ms, 신뢰도: ${response.confidence}%)`);
      return result;
  ...
  private generateDocumentBasedResponse(
    ...
    switch (smartQuery.intent) {
      ...
      case 'search':
        response = `검색 결과:\n${topDocs.map(doc =>
          `• ${doc.path}: ${doc.keywords.slice(0, 5).join(', ')}`
        ).join('\n')}`;
        break;

      case 'prediction':
        response = `예측 분석:\n${topDocs.map(doc =>
          `• ${doc.path}에서 추출한 트렌드 패턴`
        ).join('\n')}`;
        break;

      case 'optimization':
        response = `최적화 권장사항:\n${topDocs.map(doc =>
          `• ${doc.path}에서 확인된 개선 포인트`
        ).join('\n')}`;
        break;
   ...
   * 📄 관련 콘텐츠 추출
   */
  private extractRelevantContent(doc: DocumentContext, keywords: string[]): string {
  ...
  async healthCheck(): Promise<EngineHealth> {
    ...
    try {
      // 각 모듈 헬스 체크
      components.documentIndex = this.documentIndexManager.getDocumentIndex().size > 0;
      ...
      console.log(`🏥 헬스 체크 완료: ${overall} (${healthyComponents}/${totalComponents} 모듈 정상)`);
      return health;
  ...
  getPerformanceStats(): any {
    return {
      ...
      session: {
        ...
        totalQueries: Array.from(this.sessionStats.values()).reduce((sum, stat) => sum + stat.queryCount, 0)
      }
    };
  }
  ...
  private updateSessionContext(sessionId: string | undefined, smartQuery: SmartQuery, response: any): void {
    if (!sessionId) return;
  ...
  private determineEngineUsed(analysisResults: any): HybridAnalysisResult['engineUsed'] {
    if (analysisResults.korean?.success && analysisResults.transformers?.success && analysisResults.tensorflow?.predictions?.length > 0) {
      return 'hybrid';
...
• 문제 해결: "연결 오류를 해결하는 방법은?"`;
  ...
  private logInitializationStatus(): void {
    ...
    console.log(`📄 문서 인덱스: ${this.documentIndexManager.getDocumentIndex().size}개 문서`);
