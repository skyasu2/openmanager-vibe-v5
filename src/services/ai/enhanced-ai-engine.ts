...
export class EnhancedAIEngine {
  ...
  private async initializeTensorFlowInBackground(): Promise<void> {
    try {
      ...
      console.warn(
        '⚠️ TensorFlow.js 백그라운드 초기화 실패 (기본 모드로 동작):',
        error
      );
    }
  }
  ...
  private async ensureTensorFlowInitialized(): Promise<void> {
    if (
      ...
      !(this.tensorflowEngine as any).isInitialized
    ) {
    ...
    try {
      ...
      if (mcpResult.success && mcpResult.results.length > 0) {
        ...
        for (const doc of mcpResult.results) {
          try {
            const docContext = await this.analyzeDocument(
              doc.path,
              doc.content
            );
      ...
      console.log(
        `✅ 문서 인덱싱 완료: ${documentCount}개 문서, ${processingTime}ms`
      );
    } catch (error) {
   ...
   */
  private async createFallbackDocumentContext(
    path: string
  ...
  private findContextLinks(content: string): string[] {
    ...
    pathRefs.forEach((path: string) => links.add(path));
  ...
  private calculateRelevanceScore(path: string, content: string): number {
    ...
    if (contentLower.includes('중요') || contentLower.includes('important'))
      score += 0.5;
  ...
  async processSmartQuery(
    ...
    try {
      ...
      if (
        ...
        smartQuery.intent === 'analysis'
      ) {
        await this.ensureTensorFlowInitialized();
        tensorflowPredictions = await this.runTensorFlowAnalysis(
          smartQuery,
          relevantDocs
        );
      }
      ...
      if (
        ...
        if (smartQuery.tensorflowModels.includes('failure_prediction')) {
          predictions.failurePrediction =
            await this.tensorflowEngine.predictFailure(mockMetrics);
        }

        if (smartQuery.tensorflowModels.includes('anomaly_detection')) {
          predictions.anomalyDetection =
            await this.tensorflowEngine.detectAnomalies(mockMetrics);
        }

        if (smartQuery.tensorflowModels.includes('timeseries')) {
          const analysisResult =
            await this.tensorflowEngine.analyzeMetricsWithAI({
              cpu: mockMetrics,
              memory: mockMetrics,
              disk: mockMetrics,
            });
  ...
  private async generateContextualAnswer(
    ...
    // 1. 쿼리 이해 확인
    answerParts.push(
      `**"${smartQuery.originalQuery}"**에 대한 분석 결과입니다.\n`
    );
    ...
    if (
      tensorflowPredictions &&
      Object.keys(tensorflowPredictions).length > 0
    ) {
      ...
      if (tensorflowPredictions.failurePrediction) {
        ...
        answerParts.push(
          `- **장애 예측**: ${(pred.prediction[0] * 100).toFixed(1)}% 확률\n`
        );
        confidence += 0.15;
      }
      ...
      if (tensorflowPredictions.anomalyDetection) {
        ...
        answerParts.push(
          `- **이상 탐지**: ${anomaly.is_anomaly ? '⚠️ 이상 감지' : '✅ 정상'}\n`
        );
        confidence += 0.15;
      }
   ...
   * 💡 의도별 맞춤 조언 생성
   */
  private generateIntentBasedAdvice(
