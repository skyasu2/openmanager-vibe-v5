...
export class RealTimeMetricsCollector {
    ...
    recordAPICall(metric: APICallMetric): void {
        ...
        console.log(`📊 API 호출 기록: ${metric.endpoint} (${metric.responseTime}ms)`);
    }
    ...
    getEngineMetrics(): EngineMetrics[] {
        ...
        const aiCalls = this.apiCalls.filter(call =>
        ...
        engineMap.forEach((calls, engineName) => {
            ...
            const avgResponseTime = calls.reduce((sum, c) => sum + c.responseTime, 0) / totalCalls;
     ...
     * 엔진 상태 판단
     */
    private determineEngineStatus(calls: APICallMetric[]): 'active' | 'inactive' | 'error' {
        if (calls.length === 0) return 'inactive';

        const recentCalls = calls.filter(c =>
            Date.now() - c.timestamp < 5 * 60 * 1000 // 최근 5분
        );
