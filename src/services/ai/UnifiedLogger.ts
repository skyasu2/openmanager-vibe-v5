...
export class UnifiedLogger {
    ...
    private config = {
        ...
        enabledCategories: ['ai-engine', 'fallback', 'performance', 'mcp', 'google-ai', 'rag', 'system', 'user', 'security'] as LogCategory[],
     ...
     * 📝 레벨별 편의 메서드들
     */
    public debug(category: LogCategory, source: string, message: string, data?: any, metadata?: LogEntry['metadata']): void {
        this.log('debug', category, source, message, data, metadata, ['debug']);
    }

    public info(category: LogCategory, source: string, message: string, data?: any, metadata?: LogEntry['metadata']): void {
        this.log('info', category, source, message, data, metadata, ['info']);
    }

    public warn(category: LogCategory, source: string, message: string, data?: any, metadata?: LogEntry['metadata']): void {
        this.log('warn', category, source, message, data, metadata, ['warning']);
    }

    public error(category: LogCategory, source: string, message: string, error?: Error, metadata?: LogEntry['metadata']): void {
        const logEntry: LogEntry = {
            id: this.generateLogId(),
            timestamp: new Date().toISOString(),
            level: 'error',
    ...
    public critical(category: LogCategory, source: string, message: string, error?: Error, metadata?: LogEntry['metadata']): void {
        const logEntry: LogEntry = {
            id: this.generateLogId(),
            timestamp: new Date().toISOString(),
            level: 'critical',
    ...
    public logPerformance(
        ...
        const level: LogLevel = responseTime > 10000 ? 'warn' :
            responseTime > 5000 ? 'info' : 'debug';
        ...
        this.log(
            ...
            `${operation} completed in ${responseTime}ms`,
            {
                ...
                responseTime,
                success,
                performanceCategory: responseTime > 10000 ? 'slow' :
                    responseTime > 5000 ? 'moderate' : 'fast'
            },
            {
                ...metadata,
                responseTime,
    ...
    public getLogStats(): LogStats {
        if (this.logs.length === 0) {
            return {
                ...
                categoryBreakdown: { 'ai-engine': 0, fallback: 0, performance: 0, mcp: 0, 'google-ai': 0, rag: 0, system: 0, user: 0, security: 0 },
                errorRate: 0,
                recentErrors: [],
        ...
        const categoryBreakdown: Record<LogCategory, number> = {
            'ai-engine': 0, fallback: 0, performance: 0, mcp: 0, 'google-ai': 0,
            rag: 0, system: 0, user: 0, security: 0
        };
    ...
    private startCleanupTimer(): void {
        this.cleanupInterval = setInterval(() => {
            ...
            if (removedCount > 0) {
                console.log(`🧹 로그 정리 완료: ${removedCount}개 로그 삭제, ${this.logs.length}개 로그 보관`);
            }
