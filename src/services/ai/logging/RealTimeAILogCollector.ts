...
export class RealTimeAILogCollector extends EventEmitter {
  ...
  private constructor() {
    ...
    try {
      ...
      console.warn(
        '⚠️ UniversalAILogger 초기화 실패, 기본 모드로 동작:',
        error
      );
  ...
  private setupUniversalLoggerListeners(): void {
    if (!this.universalLogger) {
      console.warn(
        '⚠️ UniversalAILogger가 없어 이벤트 리스너 설정을 건너뜁니다'
      );
      return;
    }
  ...
  private detectTechnology(message: string, engineId: string): string[] {
    ...
    Object.keys(this.techStackMapping).forEach(tech => {
      if (
        ...
        lowerMessage.includes(this.techStackMapping[tech].toLowerCase())
      ) {
        detected.push(tech);
      }
    });
  ...
  private mapToAILogLevel(level: string): LogLevel {
    switch (level) {
      ...
      case 'PROCESSING':
        return LogLevel.AI_THINKING;
      case 'ANALYSIS':
        return LogLevel.AI_ANALYSIS;
      default:
   ...
   * 🛠️ 기술 스택 사용 통계
   */
  private getTechStackUsage(
    processes: AIEngineProcess[]
   ...
   * 📊 최근 로그 조회
   */
  getRecentLogs(limit: number = 50): RealTimeAILog[] {
