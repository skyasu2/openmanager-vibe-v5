...
export class HybridAIEngine {
  ...
  private async buildHybridDocumentIndex(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ 하이브리드 인덱싱 완료: ${documentCount}개 문서, ${processingTime}ms`
      );
    } catch (error) {
  ...
  async processHybridQuery(
    ...
    try {
      ...
      console.log(
        `🎯 쿼리 분석 완료 - 언어: ${smartQuery.isKorean ? '한국어' : '영어'}, 의도: ${smartQuery.intent}`
      );
      ...
      const analysisResults = await this.runHybridAnalysis(
        smartQuery,
        documents
      );
      ...
      const finalAnswer = await this.generateHybridResponse(
        smartQuery,
        documents,
        analysisResults
      );
  ...
  private async hybridDocumentSearch(
    ...
    try {
              ...
              vectorResults.forEach(result => {
                ...
                if (doc) {
                  results.set(result.id, {
                    ...doc,
                    relevanceScore: doc.relevanceScore * result.similarity,
                  });
                }
              });
        ...
        Promise.resolve().then(() => {
          ...
          for (const [path, doc] of this.documentIndex) {
            ...
            if (matchScore > 0.3) {
              ...
              if (
                !existing ||
                existing.relevanceScore < doc.relevanceScore * matchScore
              ) {
                results.set(path, {
                  ...doc,
      ...
      if (smartQuery.requiredDocs.length > 0) {
        searchPromises.push(
          this.mcpClient
            .searchDocuments(smartQuery.keywords.join(' '))
            .then(mcpResult => {
              if (mcpResult.success) {
                console.log(
                  `📚 MCP 실시간 검색: ${mcpResult.results.length}개 결과`
                );
                mcpResult.results.forEach(doc => {
                  if (!results.has(doc.path)) {
                    results.set(
                      doc.path,
                      this.createDocumentContext(doc.path, doc.content)
                    );
                  }
                });
              }
  ...
  private async runHybridAnalysis(
    ...
    if (smartQuery.isKorean && this.engineStats.korean.initialized) {
      analysisPromises.push(
        this.koreanEngine
          .processQuery(smartQuery.originalQuery)
          .then(result => {
    ...
    if (
      ...
      analysisPromises.push(
        this.transformersEngine
          .analyzeText(smartQuery.originalQuery, {
  ...
  private async generateHybridResponse(
    ...
    try {
      ...
      if (smartQuery.isKorean && analysisResults.korean) {
        ...
        confidence = Math.max(
          confidence,
          analysisResults.korean.confidence || 0.5
        );
      ...
      if (analysisResults.transformers) {
        const transformersConfidence =
          analysisResults.transformers.confidence || 0.5;
        ...
        if (analysisResults.transformers.classification?.interpreted) {
          const severity =
            analysisResults.transformers.classification.interpreted.severity;
      ...
      // 6. 실행 가능한 액션 추가
      const actionAdvice = this.generateActionAdvice(
        smartQuery.intent,
        analysisResults
      );
      if (actionAdvice) {
  ...
  private async runTensorFlowAnalysis(
    ...
    try {
      ...
      // 문서에서 숫자 데이터 추출하여 실제 메트릭으로 사용
      docs.slice(0, 3).forEach((doc, idx) => {
        const numbers = doc.content.match(/[-+]?\d*\.?\d+/g)?.map(Number) || [];
        if (numbers.length > 0) {
          metrics[`doc_${idx}`] = numbers.slice(0, 20);
        }
      });

      // 문서에서 메트릭을 추출할 수 없는 경우 기본 메트릭 생성
      if (Object.keys(metrics).length === 0) {
        metrics['cpu'] = this.generateMockMetrics();
        metrics['memory'] = this.generateMockMetrics();
        metrics['disk'] = this.generateMockMetrics();
      }

      return await this.tensorflowEngine.analyzeMetricsWithAI(metrics);
    } catch (error: any) {
  ...
  private async executeMCPActions(smartQuery: SmartQuery): Promise<string[]> {
    ...
    try {
      if (smartQuery.mcpActions.includes('search_docs')) {
        const result = await this.mcpClient.searchDocuments(
  ...
  private generateFallbackResponse(smartQuery: SmartQuery): string {
    const responses = {
      analysis:
        '요청하신 분석을 수행했습니다. 현재 시스템 상태는 정상적으로 보입니다.',
      search: '검색 결과를 정리했습니다. 관련 문서와 정보를 확인해 주세요.',
      prediction:
        '예측 분석을 완료했습니다. 향후 트렌드와 권장사항을 참고하시기 바랍니다.',
      optimization:
        '최적화 방안을 제안드립니다. 성능 개선을 위한 다음 단계를 고려해 보세요.',
      troubleshooting:
        '문제 해결 방안을 분석했습니다. 단계별 해결 방법을 확인해 주세요.',
    };

    return responses[smartQuery.intent] || '요청하신 작업을 처리했습니다.';
  }

  private generateActionAdvice(
    ...
    const advice = {
      ...
      prediction:
        '📈 예측 정확도 향상을 위해 더 많은 데이터를 수집하시기 바랍니다.',
      optimization:
        '⚡ 성능 최적화를 위해 시스템 리소스 모니터링을 강화하세요.',
  ...
  private updateEngineStats(
    ...
    if (smartQuery.isKorean) {
      ...
      this.engineStats.korean.avgTime =
        (this.engineStats.korean.avgTime + processingTime) / 2;
    }

    if (smartQuery.useTransformers) {
      this.engineStats.transformers.successCount++;
      this.engineStats.transformers.avgTime =
        (this.engineStats.transformers.avgTime + processingTime) / 2;
    }
  }
  ...
  private logEngineStatus(): void {
    ...
    console.log(
      `  🇰🇷 한국어: ${this.engineStats.korean.initialized ? '✅' : '❌'}`
    );
    console.log(
    ...
    console.log(
      `  🗄️ Vector DB: ${this.engineStats.vector.initialized ? '✅' : '❌'} (${this.engineStats.vector.documentCount}개 문서)`
    );
  }
  ...
  dispose(): void {
    ...
    if (
      this.tensorflowEngine &&
      typeof this.tensorflowEngine.dispose === 'function'
    ) {
    ...
    if (
      this.transformersEngine &&
      typeof this.transformersEngine.dispose === 'function'
    ) {
