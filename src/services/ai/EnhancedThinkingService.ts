...
export class EnhancedThinkingService extends EventEmitter {
    ...
    startThinkingSession(question: string): string {
        ...
        // 초기 분석 단계 추가
        this.addThinkingStep(sessionId, 'Unified', 'analyzing',
            '사용자 질문을 분석하고 있습니다...');
     ...
     */
    private simulateNextStep(sessionId: string, engine: string, currentType: string): void {
        const session = this.activeSessions.get(sessionId);
        if (!session || !session.isActive) return;
        ...
        setTimeout(() => {
            ...
            if (nextSteps.length > 0) {
                ...
                this.addThinkingStep(sessionId, engine, nextStep.type, nextStep.content, nextStep.progress);
            }

            // MCP 엔진 완료 후 RAG 엔진 시작
            if (engine === 'MCP' && currentType === 'completed') {
                setTimeout(() => {
                    this.addThinkingStep(sessionId, 'RAG', 'searching',
                        '관련 문서를 검색하고 있습니다...');
                }, 800);
            }
            ...
            if (engine === 'RAG' && currentType === 'completed') {
                setTimeout(() => {
                    this.addThinkingStep(sessionId, 'Google-AI', 'generating',
                        'Google AI로 최종 답변을 생성하고 있습니다...');
                }, 1200);
            }

        }, Math.random() * 1500 + 500); // 0.5~2초 랜덤 지연
    }

    /**
     * 🎯 엔진별 다음 단계 정의
     */
    private getNextStepsByEngine(engine: string, currentType: string): Array<{
        type: EnhancedThinkingStep['type'];
        content: string;
        progress?: number;
    }> {
        const stepMap: Record<string, Record<string, any[]>> = {
            'Unified': {
                'analyzing': [
                    { type: 'processing', content: '질문 유형을 분류하고 적절한 AI 엔진을 선택하고 있습니다...', progress: 0.3 }
                ],
                'processing': [
                    { type: 'completed', content: 'MCP 엔진으로 전환합니다.', progress: 1.0 }
                ]
            },
            'MCP': {
                'analyzing': [
                    { type: 'processing', content: '서버 컨텍스트와 시스템 상태를 분석하고 있습니다...', progress: 0.4 }
                ],
                'processing': [
                    { type: 'generating', content: '서버 데이터를 기반으로 초기 분석을 수행하고 있습니다...', progress: 0.8 }
                ],
                'generating': [
                    { type: 'completed', content: 'MCP 엔진 분석 완료. 추가 정보 검색이 필요합니다.', progress: 1.0 }
                ]
            },
            'RAG': {
                'searching': [
                    { type: 'processing', content: '벡터 데이터베이스에서 관련 문서를 찾았습니다...', progress: 0.6 }
                ],
                'processing': [
                    { type: 'generating', content: '찾은 문서들을 분석하여 컨텍스트를 구성하고 있습니다...', progress: 0.9 }
                ],
                'generating': [
                    { type: 'completed', content: 'RAG 검색 완료. Google AI로 최종 답변을 생성합니다.', progress: 1.0 }
                ]
            },
            'Google-AI': {
                'generating': [
                    { type: 'processing', content: 'Google AI가 종합적인 답변을 작성하고 있습니다...', progress: 0.7 }
                ],
                'processing': [
                    { type: 'completed', content: '모든 AI 엔진의 분석이 완료되었습니다.', progress: 1.0 }
                ]
            }
        };
    ...
    onEngineStart(sessionId: string, engine: string, operation: string): void {
        this.addThinkingStep(sessionId, engine, 'analyzing',
            `${engine} 엔진이 ${operation} 작업을 시작합니다...`);
    }

    onEngineProgress(sessionId: string, engine: string, message: string, progress: number): void {
    ...
    convertToAgentLogs(sessionId: string): AgentLog[] {
        ...
        return session.steps.map(step => {
            // 타입 매핑
            const mappedType: AgentLog['type'] = step.type === 'analyzing' ? 'analysis' :
                step.type === 'processing' ? 'data_processing' :
                    step.type === 'searching' ? 'pattern_matching' :
                        step.type === 'generating' ? 'response_generation' :
                            'info';
