...
import { FastAIEngineRouter, getFastAIEngineRouter } from './FastAIEngineRouter';
...
export class AIPerformanceBenchmark {
   ...
   * 📊 개별 엔진 테스트
   */
  private async testEngine(engineType: EngineType): Promise<PerformanceMetrics> {
    ...
    for (const [complexity, times] of Object.entries(complexityTimes)) {
      if (times.length > 0) {
        metrics.complexityBreakdown[complexity as keyof typeof metrics.complexityBreakdown].avgTime = 
   ...
   * 🚀 엔진 실행 (타입별 적응)
   */
  private async executeWithEngine(engine: any, request: QueryRequest): Promise<QueryResponse> {
   ...
   * 📊 결과 분석
   */
  private analyzeResults(results: PerformanceMetrics[]): BenchmarkResult['overallPerformance'] {
    ...
    const improvementPercentage = Math.round(((baseline - bestTime) / baseline) * 100);
   ...
   * 🔍 상세 분석
   */
  private generateDetailedAnalysis(results: PerformanceMetrics[]): BenchmarkResult['detailedAnalysis'] {
    ...
    results.forEach(result => {
      if (result.avgResponseTime > 100) {
        bottlenecks.push(`${result.engineName}: 평균 응답시간 ${result.avgResponseTime.toFixed(1)}ms`);
      }
      
      if (result.cacheHitRate < 0.5) {
        bottlenecks.push(`${result.engineName}: 낮은 캐시 히트율 ${(result.cacheHitRate * 100).toFixed(1)}%`);
      }
      
      if (result.successRate < 0.95) {
        bottlenecks.push(`${result.engineName}: 낮은 성공률 ${(result.successRate * 100).toFixed(1)}%`);
      }
    });
  ...
  private generateTestCases(): TestCase[] {
    return [
      ...
      { query: '서버 상태', expectedComplexity: 'simple', category: 'monitoring', expectedResponseTime: 60 },
      { query: 'CPU 사용률', expectedComplexity: 'simple', category: 'metrics', expectedResponseTime: 55 },
      { query: '메모리 확인', expectedComplexity: 'simple', category: 'metrics', expectedResponseTime: 50 },
      { query: '디스크 공간', expectedComplexity: 'simple', category: 'storage', expectedResponseTime: 65 },
      
      // Medium queries (목표: 70-90ms)
      { query: '지난 1시간 CPU 트렌드 분석', expectedComplexity: 'medium', category: 'analysis', expectedResponseTime: 85 },
      { query: '메모리 사용량 서버별 비교', expectedComplexity: 'medium', category: 'comparison', expectedResponseTime: 80 },
      { query: '네트워크 트래픽 이상 감지', expectedComplexity: 'medium', category: 'anomaly', expectedResponseTime: 90 },
      
      // Complex queries (목표: 90-120ms, Google AI 사용)
      { query: '전체 시스템 성능 최적화 방안 제시', expectedComplexity: 'complex', category: 'optimization', expectedResponseTime: 110 },
      { query: '서버 클러스터 부하 분산 전략 분석', expectedComplexity: 'complex', category: 'strategy', expectedResponseTime: 120 },
      { query: '장애 예측 및 예방 조치 권고', expectedComplexity: 'complex', category: 'prediction', expectedResponseTime: 100 },
      
      // 추가 실제 사용자 쿼리들
      { query: 'top 명령어 사용법', expectedComplexity: 'simple', category: 'command', expectedResponseTime: 45 },
      { query: '로그 파일 분석 방법', expectedComplexity: 'medium', category: 'troubleshooting', expectedResponseTime: 75 },
      { query: '데이터베이스 연결 상태', expectedComplexity: 'simple', category: 'database', expectedResponseTime: 55 },
    ];
  }
...
export function getAIPerformanceBenchmark(config?: Partial<BenchmarkConfig>): AIPerformanceBenchmark {
