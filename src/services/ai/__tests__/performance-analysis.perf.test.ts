...
describe('SimplifiedQueryEngine 성능 분석', () => {
  ...
  describe('1. 초기화 성능 분석', () => {
    ...
    it('최적화된 엔진 워밍업 시간 측정', async () => {
      ...
      console.log(
        `📊 예열된 임베딩: ${stats.optimization.preloadedEmbeddings}개`
      );
  ...
  describe('2. 쿼리 처리 성능 비교', () => {
    it('단일 쿼리 성능 비교 - 로컬 모드', async () => {
      ...
      console.log(
        `   개선율: ${(((originalTime - optimizedTime) / originalTime) * 100).toFixed(1)}%`
      );
  ...
  describe('3. 다중 쿼리 성능 분석', () => {
    it('연속 쿼리 처리 성능 측정', async () => {
      ...
      // 기본 엔진 테스트
      const originalResults = await performBatchTest(
        originalEngine,
        testQueries.slice(0, iterations)
      );

      // 최적화된 엔진 테스트
      const optimizedResults = await performBatchTest(
        optimizedEngine,
        testQueries.slice(0, iterations)
      );
      ...
      const improvement = (
        ((originalResults.avgResponseTime - optimizedResults.avgResponseTime) /
          originalResults.avgResponseTime) *
        100
      ).toFixed(1);
      console.log(`   전체 개선율: ${improvement}%`);

      expect(optimizedResults.successRate).toBeGreaterThanOrEqual(
        originalResults.successRate
      );
      expect(optimizedResults.avgResponseTime).toBeLessThanOrEqual(
        originalResults.avgResponseTime * 1.1
      ); // 10% 오차 허용
    });
    ...
    it('캐시 효과 분석', async () => {
      ...
      responses.forEach((r) => {
        console.log(
          `   ${r.iteration}회차: ${r.responseTime}ms (캐시: ${r.cached ? 'HIT' : 'MISS'})`
        );
      });
  ...
  describe('4. 병목 지점 분석', () => {
    it('MCP 컨텍스트 수집 오버헤드 측정', async () => {
      ...
      const overheadPercentage =
        withoutMCPTime > 0
          ? ((mcpOverhead / withoutMCPTime) * 100).toFixed(1)
          : '0.0';
  ...
  describe('5. 회로 차단기 동작 확인', () => {
    it('연속 실패 시 회로 차단기 활성화', async () => {
      ...
      results.forEach((r) => {
        console.log(
          `   ${r.attempt}회차: ${r.success ? 'SUCCESS' : 'FAIL'} (${r.responseTime}ms) ${r.isFallback ? '[FALLBACK]' : ''}`
        );
      });
      ...
      if (fallbackResponses.length > 0) {
        const avgFallbackTime =
          fallbackResponses.reduce((sum, r) => sum + r.responseTime, 0) /
