...
export class GCPFunctionsService {
  ...
  constructor(customConfig?: Partial<GCPFunctionsConfig>) {
    this.config = {
      ...
      vmContext: {
        ...
        endpoint:
          process.env.GCP_VM_CONTEXT_URL || 'http://34.64.213.108:10001',
      },
  ...
  public async processQuery(request: AIRequest): Promise<AIResponse> {
    ...
    try {
      ...
      if (this.config.fallbackToLocal) {
        ...
        throw new Error(
          `GCP Functions 실패 (폴백 필요): ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
  ...
  private updateStats(response: AIResponse, startTime: number): void {
    ...
    this.stats.freeQuotaUsage.network += this.estimateNetworkUsage(
      response as any,
      response
    );
    this.stats.freeQuotaUsage.compute +=
      this.estimateComputeUsage(responseTime);

    // 백분율 계산 (무료 한도 기준)
    this.stats.freeQuotaUsage.callsPercent =
      (this.stats.freeQuotaUsage.calls / 2000000) * 100;
    this.stats.freeQuotaUsage.computePercent =
      (this.stats.freeQuotaUsage.compute / 400000) * 100;
    this.stats.freeQuotaUsage.networkPercent =
      (this.stats.freeQuotaUsage.network / (5 * 1024 * 1024 * 1024)) * 100;
  }
