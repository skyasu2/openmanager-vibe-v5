...
export class VectorSearchService {
  ...
  constructor(
    ...
    documentIndex: Map<string, DocumentContext>
  ) {
  ...
  async performVectorSearch(
    ...
    try {
      // LocalRAGEngine의 search 메서드 사용
      const ragResponse = await this.vectorDB.search({
        query: query,
        maxResults: options.maxResults || 10,
        threshold: options.minSimilarity || 0.1,
      });
      ...
      if (ragResponse.success && ragResponse.results) {
        for (const result of ragResponse.results) {
          const vectorResult: VectorSearchResult = {
            ...
            matchedKeywords: result.document.keywords || [],
            searchType: 'vector',
          };
      ...
      console.log(
        `🔍 벡터 검색 완료: ${results.length}개 결과 (${searchTime}ms)`
      );
  ...
  async performKeywordSearch(
    ...
    try {
      ...
      console.log(
        `🔤 키워드 검색 완료: ${limitedResults.length}개 결과 (${searchTime}ms)`
      );
  ...
  async hybridDocumentSearch(
    ...
    try {
      ...
      console.log(
        `✅ 하이브리드 검색 완료: ${documents.length}개 문서 (${searchTime}ms)`
      );
  ...
  private calculateKeywordMatchScore(
    ...
    for (const keyword of queryKeywords) {
      ...
      // 부분 매칭
      const partialMatches = document.keywords.filter(
        docKeyword =>
    ...
    const finalScore =
      (weightedScore / (queryKeywords.length * 2)) * matchRatio;
  ...
  private applyHybridRanking(
    ...
    return results
      .map(result => {
        // 정규화된 점수들
        const vectorScore =
          result.searchType === 'vector' ? result.similarity : 0;
        const keywordScore =
          result.searchType === 'keyword' ? result.similarity : 0;
