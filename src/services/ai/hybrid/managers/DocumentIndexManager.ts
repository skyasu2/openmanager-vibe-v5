...
export class DocumentIndexManager {
  ...
  private vectorDB: LocalRAGEngine;
  private documentIndex: Map<string, DocumentContext> = new Map();
  private lastIndexUpdate: number = 0;

  constructor(mcpClient: RealMCPClient, vectorDB: LocalRAGEngine) {
  ...
  async buildHybridDocumentIndex(): Promise<void> {
    ...
    try {
      ...
      const analysisPromises = documents.map(async (doc, index) => {
        try {
          const context = await this.analyzeAndVectorizeDocument(
            doc.path,
            doc.content
          );
          ...
          if (index % 10 === 0) {
            console.log(
              `📊 진행률: ${index + 1}/${documents.length} (${Math.round(((index + 1) / documents.length) * 100)}%)`
            );
          }
        } catch (error) {
      ...
      console.log(
        `   - 평균 처리 시간: ${Math.round(processingTime / documents.length)}ms/문서`
      );
    } catch (error) {
  ...
  private findContextLinks(content: string): string[] {
    const linkRegex =
      /(?:import|from|require)\s+['"](.*?)['"]|href\s*=\s*['"](.*?)['"]|src\s*=\s*['"](.*?)['"]/g;
    const links: string[] = [];
    let match;
    ...
    const fallbackDocs = [
      {
        path: 'README.md',
        content:
          'OpenManager는 통합 서버 관리 플랫폼입니다. AI 기반 모니터링과 예측 기능을 제공합니다.',
        keywords: ['openmanager', '서버', '관리', 'ai', '모니터링', '예측'],
      },
      {
        path: 'docs/architecture.md',
        content:
          '시스템 아키텍처는 마이크로서비스 기반으로 설계되었습니다. React, Next.js, TypeScript를 사용합니다.',
        keywords: [
          '아키텍처',
          '마이크로서비스',
          'react',
      ...
      },
      {
        path: 'docs/api.md',
        content:
          'REST API를 통해 서버 메트릭, 알림, 설정을 관리할 수 있습니다.',
  ...
  getIndexStats(): {
    ...
    const averageRelevanceScore =
      documents.length > 0
        ? documents.reduce((sum, doc) => sum + doc.relevanceScore, 0) /
          documents.length
        : 0;
    const totalKeywords = documents.reduce(
