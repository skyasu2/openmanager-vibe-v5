...
export class TransformersEngine {
  ...
  async analyzeText(
    ...
    try {
      console.log('🔍 Transformers.js 텍스트 분석 시작:', text.substring(0, 100));
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    try {
      ...
      console.warn('⚠️ 임베딩 생성 실패:', error);
      return [];
    }
  }
   ...
   * 🔍 분류 결과 해석 (서버 모니터링 관점)
   */
  private interpretClassification(result: any): { severity: string; category: string; action: string } {
