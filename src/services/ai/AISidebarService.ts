...
import { PredictionResult, predictiveAnalysisEngine } from '@/engines/PredictiveAnalysisEngine';
...
export class AISidebarService {
   ...
   * 🖥️ 현재 서버 상태 조회
   */
  async getServerStatus(): Promise<AISidebarServiceResponse<ServerStatusSummary>> {
    try {
      ...
      servers.forEach((server: any) => {
        ...
        if (status === 'running' && cpu < 70 && memory < 80) {
          ...
          warningServers++;
          if (cpu >= 90) topIssues.push(`${server.name}: 높은 CPU 사용률 (${cpu}%)`);
          if (memory >= 90) topIssues.push(`${server.name}: 높은 메모리 사용률 (${memory}%)`);
        } else {
          criticalServers++;
      ...
      const overallStatus = criticalServers > 0 ? 'critical' : 
                           warningServers > 0 ? 'warning' : 'healthy';
   ...
   * 📊 서버 성능 분석
   */
  async getPerformanceAnalysis(): Promise<AISidebarServiceResponse<PerformanceAnalysis>> {
    try {
   ...
   * 🔮 장애 예측 결과 조회
   */
  async getFailurePrediction(): Promise<AISidebarServiceResponse<PredictionSummary>> {
    try {
      ...
      if (data.success && data.data) {
        ...
        const accuracyData = accuracyResponse.ok ? await accuracyResponse.json() : null;
        ...
        const summary: PredictionSummary = {
          ...
          nextPredictedFailure: prediction.failureProbability > 50 ? {
            serverId: prediction.serverId,
            probability: prediction.failureProbability,
            timeToFailure: this.formatTimeToFailure(prediction.predictedTime)
          } : undefined,
  ...
  async getHighMemoryServers(): Promise<AISidebarServiceResponse<any[]>> {
    try {
      ...
      console.log('✅ 고메모리 서버 조회 완료:', highMemoryServers.length + '개');
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '고메모리 서버 조회 실패',
        timestamp: Date.now()
      };
    }
  }
  ...
  async getLowDiskServers(): Promise<AISidebarServiceResponse<any[]>> {
    try {
      ...
      console.log('✅ 저용량 디스크 서버 조회 완료:', lowDiskServers.length + '개');
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '저용량 디스크 서버 조회 실패',
        timestamp: Date.now()
      };
    }
  }
  ...
  async getNetworkLatency(): Promise<AISidebarServiceResponse<any>> {
    try {
      ...
      const networkStatus = {
        ...
        overallStatus: highLatencyCount === 0 ? 'good' : 
                      highLatencyCount < 3 ? 'warning' : 'critical'
      };
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '네트워크 지연 조회 실패',
        timestamp: Date.now()
      };
    }
  }
  ...
  async getLoadBalancingStatus(): Promise<AISidebarServiceResponse<any>> {
    try {
      ...
      const loadBalancingStatus = {
        cpuBalance: cpuVariance < 15 ? 'good' : cpuVariance < 30 ? 'fair' : 'poor',
        memoryBalance: memoryVariance < 15 ? 'good' : memoryVariance < 30 ? 'fair' : 'poor',
        averageCpu: Math.round(cpuUsages.reduce((a: number, b: number) => a + b, 0) / cpuUsages.length),
        averageMemory: Math.round(memoryUsages.reduce((a: number, b: number) => a + b, 0) / memoryUsages.length),
        recommendations: this.generateLoadBalancingRecommendations(cpuVariance, memoryVariance)
      };
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '로드 밸런싱 상태 조회 실패',
        timestamp: Date.now()
      };
    }
  }
  ...
  private generateLoadBalancingRecommendations(cpuVariance: number, memoryVariance: number): string[] {
    const recommendations: string[] = [];
    
    if (cpuVariance > 25) {
      recommendations.push('CPU 부하 분산이 불균등합니다. 로드 밸런서 설정을 검토하세요');
    }
    
    if (memoryVariance > 25) {
      recommendations.push('메모리 사용량이 불균등합니다. 애플리케이션 배치를 재검토하세요');
    }
