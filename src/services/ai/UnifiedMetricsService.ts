...
export class UnifiedMetricsService {
  ...
  private cache = new Map<
    string,
    { data: UnifiedMetricsResponse; fetchedAt: number }
  >();
  ...
  async getMetricTrends(serverId: string): Promise<{
    ...
    // 기준값과 비교하여 트렌드 분석
    const baseline = server.metadata?.baseline || {
      cpu: 50,
      memory: 50,
      network: 30,
    };
    ...
    const trends = {
      cpu: {
        ...
        trend: analyzeTrend(cpuValue, baseline.cpu),
      },
      memory: {
        current: memoryValue,
        baseline: baseline.memory,
        trend: analyzeTrend(memoryValue, baseline.memory),
      },
      network: {
        current: networkValue,
        baseline: baseline.network,
        trend: analyzeTrend(networkValue, baseline.network),
      },
    } as const;
  ...
  private generateRecommendations(
    ...
    if (cpuValue > 80) {
      recommendations.push(
        'CPU 사용률이 높습니다. 프로세스 최적화를 고려해보세요.'
      );
    }

    if (memoryValue > 85) {
      recommendations.push(
        '메모리 사용률이 높습니다. 메모리 누수 확인이 필요합니다.'
      );
    }

    if (server.responseTime > 200) {
      recommendations.push(
        '응답 시간이 느립니다. 데이터베이스 쿼리를 최적화해보세요.'
      );
    }

    if (trends.cpu.trend === 'rising' && cpuValue > 70) {
      recommendations.push(
        'CPU 사용률이 지속적으로 증가하고 있습니다. 모니터링을 강화하세요.'
      );
    }
