...
export class NaturalLanguageErrorHandler {
  ...
  private initializeErrorPatterns(): void {
    ...
    this.errorPatterns.set(
      'api.*key.*missing',
      NLErrorCode.GOOGLE_AI_API_KEY_MISSING
    );
    this.errorPatterns.set(
      'unauthorized',
      NLErrorCode.GOOGLE_AI_API_KEY_MISSING
    ...
    this.errorPatterns.set(
      'quota.*exceeded',
      NLErrorCode.GOOGLE_AI_QUOTA_EXCEEDED
    );
    ...
    this.errorPatterns.set(
      'network.*error',
      NLErrorCode.GOOGLE_AI_NETWORK_ERROR
    );
    this.errorPatterns.set(
      'connection.*failed',
      NLErrorCode.MCP_CONNECTION_FAILED
    );
    ...
    // 한국어 AI 관련
    this.errorPatterns.set(
      'korean.*ai.*unavailable',
      NLErrorCode.KOREAN_AI_UNAVAILABLE
    );
    this.errorPatterns.set(
      'korean.*engine.*failed',
      NLErrorCode.KOREAN_AI_UNAVAILABLE
    );
  ...
  private analyzeLocalModeError(
    ...
    if (lowerMessage.includes('korean ai') || lowerMessage.includes('한국어')) {
      return {
        ...
        userMessage:
          '한국어 AI 엔진에 일시적인 문제가 발생했습니다. MCP 또는 RAG 엔진으로 처리를 시도합니다.',
        suggestions: [
          'MCP 엔진을 사용해보세요',
    ...
    if (lowerMessage.includes('mcp') || lowerMessage.includes('연결')) {
      return {
        ...
        userMessage:
          'MCP 시스템 컨텍스트 엔진에 연결할 수 없습니다. RAG 엔진으로 처리를 시도합니다.',
        suggestions: [
    ...
    if (lowerMessage.includes('모든') && lowerMessage.includes('로컬')) {
      return {
        ...
        userMessage:
          '현재 모든 로컬 AI 엔진에 문제가 발생했습니다. 시스템 관리자에게 문의하세요.',
        suggestions: [
   ...
   * 🌐 Google AI 모드 에러 분석
   */
  private analyzeGoogleAIModeError(
    errorMessage: string,
    context?: any
  ): NLErrorInfo {
    const lowerMessage = errorMessage.toLowerCase();
    ...
    if (
      ...
      return {
        ...
        userMessage:
          'Google AI 서비스 인증에 실패했습니다. 관리자가 API 키를 설정해야 합니다.',
        suggestions: [
          'LOCAL 모드를 사용해보세요',
    ...
    if (
      ...
      return {
        ...
        userMessage:
          'Google AI 서비스의 일일 사용 한도를 초과했습니다. 내일 다시 시도하거나 로컬 모드를 사용하세요.',
        suggestions: [
          'LOCAL 모드로 전환하세요',
          '내일 다시 시도해보세요',
          '사용량 모니터링을 확인하세요',
    ...
    if (
      ...
      return {
        ...
        userMessage:
          'Google AI 서비스에 연결할 수 없습니다. 네트워크 상태를 확인하고 다시 시도하세요.',
        suggestions: [
          '네트워크 연결을 확인해주세요',
          'LOCAL 모드를 시도해보세요',
          '잠시 후 다시 시도해주세요',
    ...
    // Google AI 일반 사용 불가
    if (
      lowerMessage.includes('google ai') ||
      lowerMessage.includes('처리 실패')
    ) {
      return {
        ...
        userMessage:
          'Google AI 서비스에 일시적인 문제가 발생했습니다. 로컬 모드로 폴백하거나 잠시 후 다시 시도하세요.',
        suggestions: [
          'LOCAL 모드로 전환하세요',
          '폴백 옵션을 활성화하세요',
          '잠시 후 다시 시도해주세요',
  ...
  private handleValidationError(errorMessage: string): NLErrorInfo {
    ...
    if (
      ...
      lowerMessage.includes('질의') ||
      lowerMessage.includes('빈')
    ) {
      return {
  ...
  public analyzeFallbackScenario(scenario: FallbackScenario): NLErrorInfo {
    const { mode, primaryEngine, fallbackEngines, failedEngines, finalError } =
      scenario;

    // 폴백이 완전히 실패한 경우
    if (failedEngines.length === fallbackEngines.length + 1) {
      // +1 for primary
      return {
  ...
  public analyzeErrorPattern(errors: NLErrorInfo[]): {
    ...
    // 심각도 분포
    const severityDistribution = errors.reduce(
      (acc, error) => {
        acc[error.severity]++;
        return acc;
      },
  ...
  private generateRecommendations(
    ...
    if (
      severityDistribution[ErrorSeverity.HIGH] >
      severityDistribution[ErrorSeverity.LOW]
    ) {
      recommendations.push(
        '높은 심각도 에러가 많습니다. 시스템 점검이 필요합니다'
      );
    }
  ...
  private normalizeMode(
    mode: string | NaturalLanguageMode
  ): NaturalLanguageMode {
