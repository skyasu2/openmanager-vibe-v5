...
export class NaturalLanguageModeProcessor {
    ...
    public static getInstance(): NaturalLanguageModeProcessor {
        if (!NaturalLanguageModeProcessor.instance) {
            NaturalLanguageModeProcessor.instance = new NaturalLanguageModeProcessor();
        }
    ...
    public async processQuery(request: NLModeRequest): Promise<NLModeResponse> {
        ...
        try {
            ...
            return this.createErrorResponse(query, mode, error, errorInfo, startTime, fallbacksUsed);
        }
        ...
        try {
            ...
            switch (mode) {
                case 'LOCAL':
                    result = await this.processLocalMode(query, context, processOptions, fallbacksUsed);
                    break;
                case 'GOOGLE_AI':
                    result = await this.processGoogleAIMode(query, context, processOptions, fallbacksUsed);
                    break;
                default:
            ...
            console.log(`✅ 자연어 질의 처리 완료: ${mode} 모드, ${result.engine} 엔진`);
            return result;
            ...
            const errorInfo = this.errorHandler.analyzeFallbackScenario(fallbackScenario);
            return this.createErrorResponse(query, mode, error, errorInfo, startTime, fallbacksUsed);
        }
    }
    ...
    private async processLocalMode(
        ...
        // 모든 로컬 엔진 실패
        throw new Error('모든 로컬 AI 엔진에서 처리 실패했습니다. 시스템 관리자에게 문의하세요.');
    }
    ...
    private async processGoogleAIMode(
        ...
        throw new Error('Google AI 및 폴백 엔진에서 처리 실패했습니다. API 키 또는 네트워크 상태를 확인하세요.');
    }

    /**
     * 🇰🇷 Korean AI 엔진 시도
     */
    private async tryKoreanAI(query: string, context: any, options: any): Promise<EngineResult> {
        try {
     ...
     * 🔧 MCP 엔진 시도
     */
    private async tryMCP(query: string, context: any, options: any): Promise<EngineResult> {
        try {
     ...
     * 📚 RAG 엔진 시도
     */
    private async tryRAG(query: string, context: any, options: any): Promise<EngineResult> {
        try {
            const aiRequest: AIRequest = {
                query,
                type: 'natural_language',
     ...
     * 🌐 Google AI 엔진 시도
     */
    private async tryGoogleAI(query: string, context: any, options: any): Promise<EngineResult> {
        try {
            const aiRequest: AIRequest = {
                query,
                type: 'natural_language',
