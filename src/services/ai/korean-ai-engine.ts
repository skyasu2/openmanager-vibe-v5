...
export class KoreanServerNLU {
  ...
  constructor() {
    ...
    this.entities = {
      '서버타입': ['웹서버', '데이터베이스', 'API서버', '캐시서버', 'DB서버', '앱서버'],
      '메트릭': ['CPU', '메모리', '디스크', '네트워크', '응답시간', '처리량', 'RAM'],
      '환경': ['개발', '스테이징', '프로덕션', '테스트', 'dev', 'prod', 'staging'],
  ...
  private calculateConfidence(text: string, intent: string, entities: Record<string, string[]>): number {
...
export class KoreanResponseGenerator {
  ...
  generate(status: string, server?: string, metric?: string, value?: number) {
    const template = this.getRandomTemplate(status);
  ...
  private getRandomTemplate(status: string): string {
  ...
  private adjustKoreanParticles(text: string, metric: string): string {
    // 한국어 조사 자동 처리 (간단한 버전)
    const vowels = ['a', 'e', 'i', 'o', 'u', 'ㅏ', 'ㅓ', 'ㅗ', 'ㅜ', 'ㅡ', 'ㅣ'];
...
export class KoreanAIEngine {
  ...
  async processQuery(query: string, serverData?: any): Promise<any> {
    ...
    try {
      ...
      return {
        success: false,
        error: error.message,
        fallbackResponse: '죄송합니다. 요청을 처리하는 중 문제가 발생했습니다. 다시 시도해 주세요.',
        engine: 'korean-ai'
      };
    }
  }

  private async analyzeServerMetrics(nluResult: any, serverData?: any): Promise<any> {
    // 기본 시뮬레이션 데이터 (실제 서버 데이터로 교체 가능)
    const mockMetrics = {
      CPU: Math.random() * 100,
      메모리: Math.random() * 100,
      디스크: Math.random() * 100,
      네트워크: Math.random() * 100
    };
    ...
    const value = Math.round(mockMetrics[targetMetric as keyof typeof mockMetrics] || mockMetrics.CPU);
  ...
  private generateAdditionalInfo(nluResult: any, analysis: any): any {
    ...
    switch (nluResult.intent) {
      case '분석':
        tips.push('💡 더 자세한 분석을 원하시면 "상세 분석해줘"라고 말씀해 주세요.');
        break;
      case '최적화':
        tips.push('⚡ 최적화 방법을 알려드릴까요? "최적화 방법 알려줘"라고 말씀해 주세요.');
        break;
      case '모니터링':
    ...
    if (analysis.status === '위험상태') {
      tips.push('🚨 즉시 조치가 필요한 상황입니다. 담당자에게 알림을 보내드릴까요?');
    }
