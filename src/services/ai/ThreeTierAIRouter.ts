...
export class ThreeTierAIRouter {
    ...
    private constructor() {
        this.config = {
            ...
            fallbackPolicy: (process.env.THREE_TIER_FALLBACK_POLICY as any) || 'adaptive',
            loadBalancing: {
    ...
    public async processQuery(request: AIRequest): Promise<AIResponse> {
        ...
        try {
            ...
            // 3. 폴백 전략 실행
            const fallbackResult = await this.executeFallbackStrategy(request, primaryTier);
    ...
    private selectReliabilityTier(request: AIRequest): TierType {
        ...
        // 가장 높은 성공률의 Tier 선택
        const bestTier = Object.entries(recentSuccessRates).reduce((best, [tier, rate]) =>
     ...
     * 🎲 특정 Tier 시도
     */
    private async tryTier(tier: TierType, request: AIRequest): Promise<TierResult> {
        const startTime = Date.now();
     ...
     * 🔄 폴백 전략 실행
     */
    private async executeFallbackStrategy(request: AIRequest, failedTier: TierType): Promise<TierResult> {
    ...
    private analyzeQueryType(request: AIRequest) {
        ...
        return {
            ...
            isNaturalLanguage: query.includes('?') || query.includes('어떻게') || query.includes('무엇'),
            isSystemRelated: query.includes('서버') || query.includes('시스템') || query.includes('모니터링'),
        };
    }
     ...
     * 🏁 Tier 응답 완료 처리
     */
    private finalizeTierResponse(result: TierResult, startTime: number): AIResponse {
    ...
    private updatePerformanceMetrics(tier: TierType, responseTime: number): void {
        ...
        const improvementRate = Math.max(0,
            ((this.performanceBaseline - currentPerformance) / this.performanceBaseline) * 100
        );
    ...
    private calculateRecentSuccessRates(): Record<TierType, number> {
        ...
        return {
            local: totalRequests > 0 ? (this.stats.routingDecisions.local / totalRequests) * 100 : 90,
            gcp: totalRequests > 0 ? (this.stats.routingDecisions.gcp / totalRequests) * 100 : 95,
            google: totalRequests > 0 ? (this.stats.routingDecisions.google / totalRequests) * 100 : 85,
        };
    }
    ...
    public getStats(): RouterStats {
        return {
            ...this.stats,
            performanceMetrics: {
                vercelLoadReduction: Math.min(this.stats.performanceMetrics.vercelLoadReduction, 75), // 목표: 75%
                aiPerformanceImprovement: Math.min(this.stats.performanceMetrics.aiPerformanceImprovement, 50), // 목표: 50%
            },
        };
    }
