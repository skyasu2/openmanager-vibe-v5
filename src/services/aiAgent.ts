...
export class SmartAIAgent {
  ...
  classifyQuery(query: string): QueryType {
    ...
    if (lowerQuery.includes('상태') || lowerQuery.includes('현황') || lowerQuery.includes('status')) {
      return 'status';
    }
    if (lowerQuery.includes('성능') || lowerQuery.includes('느림') || lowerQuery.includes('performance')) {
      return 'performance';
    }
    if (lowerQuery.includes('문제') || lowerQuery.includes('오류') || lowerQuery.includes('장애')) {
      return 'troubleshooting';
    }
    if (lowerQuery.includes('최적화') || lowerQuery.includes('개선') || lowerQuery.includes('튜닝')) {
      return 'optimization';
    }
  ...
  generateAutoReport(): void {
    ...
    // 정적 서버 데이터 생성
    const staticServers = [
      { id: '1', status: 'healthy', metrics: { cpu: 25, memory: 40, disk: 30 } },
      { id: '2', status: 'healthy', metrics: { cpu: 30, memory: 45, disk: 35 } },
      { id: '3', status: 'healthy', metrics: { cpu: 20, memory: 35, disk: 25 } },
      { id: '4', status: 'warning', metrics: { cpu: 75, memory: 80, disk: 60 } },
      { id: '5', status: 'warning', metrics: { cpu: 70, memory: 75, disk: 55 } },
      { id: '6', status: 'warning', metrics: { cpu: 65, memory: 70, disk: 50 } },
      { id: '7', status: 'warning', metrics: { cpu: 80, memory: 85, disk: 65 } },
      { id: '8', status: 'warning', metrics: { cpu: 72, memory: 78, disk: 58 } },
      { id: '9', status: 'critical', metrics: { cpu: 95, memory: 95, disk: 90 } },
      { id: '10', status: 'critical', metrics: { cpu: 90, memory: 90, disk: 85 } }
    ];
    
    const details = this.generateDetailedAnalysis(staticServers, activeAlerts, condition);
    const recommendations = this.generateRecommendations(condition, staticServers);
  ...
  private identifyMainIssues(servers: any[]): string {
    ...
    if (highCpuServers > 0) issues.push(`CPU 사용률 높음 (${highCpuServers}대)`);
    if (highMemoryServers > 0) issues.push(`메모리 사용률 높음 (${highMemoryServers}대)`);
    if (highDiskServers > 0) issues.push(`디스크 사용률 높음 (${highDiskServers}대)`);
  ...
  private generateFallbackResponse(query: string, condition: SystemCondition): SmartResponse {
    return {
      ...
      followUpQuestions: ['어떤 부분이 궁금하신가요?', '특정 서버를 확인하시겠습니까?']
    };
  }
