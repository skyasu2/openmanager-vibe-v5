...
export class MonitoringFocusStrategy implements ProcessingStrategy {
  ...
  async execute(request: any): Promise<any> {
    ...
    try {
      ...
      console.log(
        `✅ [${request.requestId}] 모니터링 전략 완료: ${processingTime}ms`
      );
      return result;
    } catch (error) {
  ...
  getMetadata(): StrategyMetadata {
    return {
      ...
      avgProcessingTime:
        this.usageCount > 0 ? this.totalProcessingTime / this.usageCount : 0,
      successRate:
        this.usageCount > 0 ? this.successCount / this.usageCount : 0,
...
export class AIAnalysisStrategy implements ProcessingStrategy {
  ...
  async execute(request: any): Promise<any> {
    ...
    try {
      ...
      console.log(
        `✅ [${request.requestId}] AI 분석 전략 완료: ${processingTime}ms`
      );
      return result;
    } catch (error) {
...
export class HybridBalancedStrategy implements ProcessingStrategy {
  ...
  async execute(request: any): Promise<any> {
    ...
    try {
      ...
      console.log(
        `✅ [${request.requestId}] 하이브리드 전략 완료: ${processingTime}ms`
      );
      return result;
    } catch (error) {
...
export class StrategyFactory {
  ...
  async selectStrategy(request: any): Promise<ProcessingStrategy> {
    ...
    if (strategyName === 'auto_select') {
      ...
      if (!strategy) {
        console.warn(
          `⚠️ 알 수 없는 전략: ${selectedStrategyName}, hybrid_balanced로 대체`
        );
    ...
    if (!strategy) {
      console.warn(
        `⚠️ 알 수 없는 전략: ${strategyName}, hybrid_balanced로 대체`
      );
      return this.strategies.get('hybrid_balanced')!;
    }
  ...
  async getStatus(): Promise<any> {
    const strategyStats = Array.from(this.strategies.entries()).map(
      ([name, strategy]) => ({
        name,
