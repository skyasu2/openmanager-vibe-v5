...
import {
  FailureAnalyzer,
  QuestionGroup,
} from '@/modules/ai-agent/analytics/FailureAnalyzer';
import {
  PatternSuggester,
  RegexPattern,
} from '@/modules/ai-agent/improvement/PatternSuggester';
import {
  ABTestManager,
  TestResult,
  Comparison,
} from '@/modules/ai-agent/testing/ABTestManager';
import {
  UserInteractionLog,
  PatternSuggestion,
  AnalysisResult,
...
export class PatternAnalysisService {
  ...
  async runFullAnalysis(): Promise<AnalysisReport> {
    ...
    try {
      ...
      const analysisResult =
        await this.failureAnalyzer.analyzeLowConfidenceResponses(interactions);
      const patternAnalysis =
        await this.failureAnalyzer.analyzeNegativeFeedbackPatterns(
          interactions
        );
      const questionTypes =
        await this.failureAnalyzer.identifyUnhandledQuestionTypes(interactions);

      // 3. 패턴 제안 생성
      const questionGroups = await this.extractQuestionGroups(interactions);
      const suggestions =
        await this.patternSuggester.generatePatternSuggestions(
          interactions,
          questionGroups
        );
      ...
      console.log(
        `✅ [PatternAnalysisService] 전체 분석 완료 (${processingTime}ms):`,
        {
  ...
  async approvePatternSuggestion(suggestionId: string): Promise<boolean> {
    try {
      // 실제 패턴 저장소에 적용
      console.log(
        `✅ [PatternAnalysisService] 패턴 제안 승인 처리: ${suggestionId}`
      );
      ...
      const newPattern: RegexPattern = {
        ...
        description: `승인된 패턴 제안 (ID: ${suggestionId})`,
        category: 'approved_suggestion',
        confidence: suggestion.confidenceScore,
        testCases: [],
        expectedMatches: suggestion.estimatedImprovement || 0,
      };
      ...
      console.log(
        `✅ [PatternAnalysisService] 패턴 제안 승인 완료: ${suggestionId} -> 패턴 ID: ${newPattern.id}`
      );
      return true;
    } catch (error) {
      console.error(
        `❌ [PatternAnalysisService] 패턴 승인 실패: ${suggestionId}`,
        error
      );
      return false;
    }
   ...
   * 패턴 제안 거부
   */
  async rejectPatternSuggestion(
    suggestionId: string,
    reason?: string
  ): Promise<boolean> {
    try {
      console.log(
        `❌ [PatternAnalysisService] 패턴 제안 거부 처리: ${suggestionId}`,
        reason
      );
      ...
      console.log(
        `✅ [PatternAnalysisService] 패턴 제안 거부 완료: ${suggestionId}`
      );
      return true;
    } catch (error) {
      console.error(
        `❌ [PatternAnalysisService] 패턴 거부 실패: ${suggestionId}`,
        error
      );
      return false;
    }
  ...
  async startPatternTest(
    ...
    try {
      ...
      if (activeTests.length >= this.config.maxConcurrentTests) {
        console.log(
          `⚠️ [PatternAnalysisService] 최대 동시 테스트 수 초과 (${activeTests.length}/${this.config.maxConcurrentTests})`
        );
        return null;
      }
   ...
   * 질문 그룹 추출
   */
  private async extractQuestionGroups(
  ...
  private generateRecommendations(
    ...
    if (
      ...
      analysisResult.totalInteractions * 0.3
    ) {
      recommendations.push(
        '낮은 신뢰도 응답이 30% 이상입니다. 패턴 매칭 알고리즘 개선이 시급합니다.'
      );
    }
    ...
    if (patternAnalysis.priorityLevel === 'high') {
      recommendations.push(
        '높은 우선순위의 실패 패턴이 발견되었습니다. 즉시 개선 작업을 시작하세요.'
      );
    }
    ...
    if (highConfidenceSuggestions.length > 0) {
      recommendations.push(
        `${highConfidenceSuggestions.length}개의 고신뢰도 패턴 제안이 있습니다. 검토 후 적용을 고려하세요.`
      );
    }
    ...
    if (activeTests.length === 0 && suggestions.length > 0) {
      recommendations.push(
        '새로운 패턴 제안이 있지만 활성 테스트가 없습니다. A/B 테스트를 시작하세요.'
      );
    }
    ...
    if (autoApprovable.length > 0) {
      recommendations.push(
        `${autoApprovable.length}개의 제안이 자동 승인 기준을 만족합니다.`
      );
    }
  ...
  private async startAutoTests(
    ...
    if (sortedSuggestions.length > 0) {
      console.log(
        `🤖 [PatternAnalysisService] 자동 테스트 시작: ${sortedSuggestions.length}개 패턴`
      );
    }
  }
  ...
  private createEmptyReport(): AnalysisReport {
    return {
      ...
      analysisResult: {
        ...
        recommendedActions: [
          '데이터가 충분하지 않습니다. 더 많은 상호작용 데이터를 수집하세요.',
        ],
      ...
      activeTests: [],
      recommendations: [
        '데이터 수집을 계속하여 의미 있는 분석을 위한 충분한 샘플을 확보하세요.',
      ],
      priority: 'low',
    };
  }
   ...
   * 제안 ID로 제안 찾기
   */
  private async findSuggestionById(
    suggestionId: string
  ...
  private async storePattern(pattern: RegexPattern): Promise<boolean> {
    try {
      ...
      if (typeof globalThis !== 'undefined') {
        (globalThis as any).__pattern_store =
          (globalThis as any).__pattern_store || new Map();
  ...
  private async updateSuggestionStatus(
    ...
    try {
      // 로컬 메모리 업데이트
      if (typeof globalThis !== 'undefined') {
        (globalThis as any).__suggestion_status =
          (globalThis as any).__suggestion_status || new Map();
   ...
   * AI 엔진에 패턴 업데이트 알림
   */
  private async notifyAIEnginePatternUpdate(
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('ai-pattern-updated', {
            detail: { pattern },
          })
        );
      }
  ...
  private async learnFromRejection(
    ...
    try {
      ...
      // PatternSuggester에 피드백 제공 (향후 구현 예정)
