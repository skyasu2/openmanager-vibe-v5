...
export class RecoveryService {
    ...
    async attemptRecovery(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                ...
                error: recoveryError instanceof Error ? recoveryError.message : '알 수 없는 복구 에러'
            };
        }
    }
    ...
    private async applyBackoffDelay(attempts: number): Promise<void> {
        ...
        const delay = Math.min(
            this.defaultRecoveryConfig.baseDelay * Math.pow(this.defaultRecoveryConfig.backoffFactor, attempts - 1),
     ...
     * 에러 타입별 복구 전략 실행
     */
    private async executeRecoveryStrategy(error: ServiceError): Promise<RecoveryResult> {
        switch (error.code) {
            case 'NETWORK_ERROR':
     ...
     * 네트워크 에러 복구
     */
    private async recoverFromNetworkError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: testError instanceof Error ? testError.message : '네트워크 테스트 실패'
            };
        }
    }
     ...
     * 데이터베이스 에러 복구
     */
    private async recoverFromDatabaseError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: testError instanceof Error ? testError.message : '데이터베이스 상태 확인 실패'
            };
        }
    }
     ...
     * 타임아웃 에러 복구
     */
    private async recoverFromTimeoutError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: retryError instanceof Error ? retryError.message : '타임아웃 복구 실패'
            };
        }
    }
     ...
     * Redis 에러 복구
     */
    private async recoverFromRedisError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: testError instanceof Error ? testError.message : 'Redis 상태 확인 실패'
            };
        }
    }
     ...
     * 외부 API 에러 복구
     */
    private async recoverFromExternalAPIError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: testError instanceof Error ? testError.message : '외부 API 테스트 실패'
            };
        }
    }
     ...
     * WebSocket 에러 복구
     */
    private async recoverFromWebSocketError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: reconnectError instanceof Error ? reconnectError.message : 'WebSocket 재연결 실패'
            };
        }
    }
     ...
     * AI 에이전트 에러 복구
     */
    private async recoverFromAIAgentError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: testError instanceof Error ? testError.message : 'AI 에이전트 상태 확인 실패'
            };
        }
    }
     ...
     * 메모리 에러 복구
     */
    private async recoverFromMemoryError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: cleanupError instanceof Error ? cleanupError.message : '메모리 정리 실패'
            };
        }
    }
     ...
     * 디스크 공간 에러 복구
     */
    private async recoverFromDiskSpaceError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: cleanupError instanceof Error ? cleanupError.message : '디스크 정리 실패'
            };
        }
    }
     ...
     * 시스템 과부하 에러 복구
     */
    private async recoverFromSystemOverloadError(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: throttleError instanceof Error ? throttleError.message : '스로틀링 활성화 실패'
            };
        }
    }
     ...
     * 일반적인 복구 시도
     */
    private async attemptGenericRecovery(error: ServiceError): Promise<RecoveryResult> {
        ...
        try {
            ...
            return {
                success: false,
                attempts: 1,
                error: genericError instanceof Error ? genericError.message : '일반 복구 실패'
            };
        }
    }
    ...
    getRecoveryStats(): {
        ...
        return {
            ...
            averageAttempts: activeRecoveries > 0 ? totalAttempts / activeRecoveries : 0
        };
    }
