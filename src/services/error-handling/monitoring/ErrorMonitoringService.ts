...
export class ErrorMonitoringService {
    ...
    generateMonitoringReport(): {
        ...
        const criticalErrorsLast24h = this.criticalErrors.filter(
            error => error.timestamp && (Date.now() - error.timestamp.getTime()) < 24 * 60 * 60 * 1000
        ).length;

        const overallHealth = this.assessOverallHealth(stats, criticalErrorsLast24h);

        // 복구 성공률 계산
        const recoveryEvents = this.monitoringEvents.filter(event => event.type === 'recovery');
        const recoverySuccessRate = recoveryEvents.length > 0 ?
            (recoveryEvents.length / stats.total) * 100 : 0;

        // 권장사항 생성
        const recommendations = this.generateRecommendations(stats, topErrorCodes, criticalErrorsLast24h);
    ...
    checkForAlerts(): Array<{
        ...
        const alerts: Array<{ type: 'warning' | 'critical'; message: string; data: any }> = [];

        // 심각한 에러 급증 감지
        const recentCritical = this.criticalErrors.filter(
            error => error.timestamp && (Date.now() - error.timestamp.getTime()) < 60 * 60 * 1000 // 1시간
        );
        ...
        // 에러율 급증 감지
        const stats = this.getErrorStats();
        if (stats.errorRate > 50) { // 시간당 50개 이상
            alerts.push({
                type: 'warning',
        ...
        this.serviceStats.forEach((count, service) => {
            ...
            if (serviceErrorRate > 30) { // 전체 에러의 30% 이상이 한 서비스에서 발생
                alerts.push({
                    type: 'warning',
    ...
    exportMetrics(): string {
        ...
        Object.entries(stats.byService).forEach(([service, count]) => {
            metrics.push(`error_handling_errors_by_service{service="${service}"} ${count}`);
        });
        ...
        metrics.push(`error_handling_critical_errors ${this.criticalErrors.length}`);
     ...
     * 모니터링 이벤트 기록
     */
    private recordMonitoringEvent(type: MonitoringEvent['type'], data: Record<string, any>): void {
    ...
    private isCriticalError(error: ServiceError): boolean {
        return error.severity === 'critical' ||
            ['SECURITY_BREACH', 'SYSTEM_FAILURE', 'MEMORY_EXHAUSTED', 'DISK_FULL'].includes(error.code);
    }

    /**
     * 전체 건강 상태 평가
     */
    private assessOverallHealth(stats: ErrorStats, criticalErrorsLast24h: number): 'healthy' | 'warning' | 'critical' {
    ...
    private calculateHourlyErrorCount(): number[] {
        ...
        this.monitoringEvents
            ...
            .forEach(event => {
                const hoursDiff = Math.floor((now.getTime() - event.timestamp.getTime()) / (1000 * 60 * 60));
    ...
    private generateRecommendations(
        ...
        if (criticalErrorsLast24h > 0) {
            recommendations.push('심각한 에러가 발생했습니다. 즉시 시스템 점검이 필요합니다.');
        }

        // 에러율 기반 권장사항
        if (stats.errorRate > 50) {
            recommendations.push('에러율이 높습니다. 시스템 부하를 줄이거나 복구 전략을 개선하세요.');
        }
        ...
        topErrorCodes.forEach(({ code, percentage }) => {
            if (percentage > 20) {
                switch (code) {
                    case 'NETWORK_ERROR':
                        recommendations.push('네트워크 에러가 빈번합니다. 네트워크 인프라를 점검하세요.');
                        break;
                    case 'DATABASE_ERROR':
                        recommendations.push('데이터베이스 에러가 많습니다. DB 성능 및 연결 상태를 확인하세요.');
                        break;
                    case 'EXTERNAL_API_ERROR':
                        recommendations.push('외부 API 에러가 빈번합니다. Circuit Breaker 패턴 적용을 고려하세요.');
                        break;
                    default:
                        recommendations.push(`${code} 에러가 ${percentage}%를 차지합니다. 해당 영역의 안정성을 개선하세요.`);
                }
            }
        });
