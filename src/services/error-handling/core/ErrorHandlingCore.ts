...
export class ErrorHandlingCore implements IErrorHandler {
    ...
    private logError(error: ServiceError): void {
        ...
        switch (error.severity) {
            case 'critical':
                this.logger.error(`CRITICAL [${error.service}]: ${error.message}`, error, logData);
                break;
            case 'high':
                this.logger.error(`HIGH [${error.service}]: ${error.message}`, error, logData);
                break;
            case 'medium':
                this.logger.warn(`MEDIUM [${error.service}]: ${error.message}`, logData);
                break;
            case 'low':
                this.logger.info(`LOW [${error.service}]: ${error.message}`, logData);
                break;
            default:
                this.logger.error(`[${error.service}]: ${error.message}`, error, logData);
        }
    }
     ...
     * 개별 핸들러 실행
     */
    private executeHandler(handler: ErrorHandlerFunction, error: ServiceError): void {
        try {
