...
export class OfficialMCPClient {
   ...
   * 🎯 실제 요청 처리
   */
  private async handleRealRequest(
    serverName: string,
    request: any
  ): Promise<any> {
      ...
      system: [
        {
          ...
          inputSchema: {
            ...
            properties: {
              type: {
                type: 'string',
                description: '메트릭 타입 (cpu, memory, disk)',
              },
            },
          },
        },
  ...
  private async loadAllTools(): Promise<void> {
    for (const [serverName, client] of this.clients) {
      try {
        ...
        console.log(
          `✅ ${serverName} 도구 ${result.tools?.length || 0}개 로드됨`
        );
      } catch (error) {
  ...
  getStats(): {
    ...
    const totalTools = Array.from(this.tools.values()).reduce(
...
export async function testMCPConnection(): Promise<boolean> {
  try {
    ...
    const healthyServers = Object.values(health).filter(
      h => h.status === 'healthy'
    ).length;

    console.log(
      `🏥 MCP 상태 확인: ${healthyServers}/${Object.keys(health).length} 서버 정상`
    );
