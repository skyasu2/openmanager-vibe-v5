...
export class MCPProcessor {
  ...
  async processQuery(query: string, serverData?: any): Promise<MCPResponse> {
    ...
    switch (intent.intent) {
      ...
      case 'performance_analysis':
        response = this.generatePerformanceAnalysisResponse(
          entities,
          serverData
        );
      ...
      case 'specific_server_analysis':
        response = this.generateSpecificServerAnalysisResponse(
          entities,
          serverData
        );
  ...
  private extractEntities(query: string): Record<string, any> {
    ...
    // 메트릭 타입 추출
    const metricMatches = query.match(
    ...
    // 시간 범위 추출
    const timeMatches = query.match(
      /\b(24시간|1주일|1개월|어제|오늘|최근)\b/gi
    );
    if (timeMatches) {
  ...
  private generateServerStatusResponse(
    entities: any,
    serverData?: any
  ): string {
...
**권장 조치:** 리소스 최적화 및 재시작 검토`;
  ...
  private generatePerformanceAnalysisResponse(
    entities: any,
    serverData?: any
  ): string {
    return `⚡ **성능 분석 결과**
...
- 네트워크 모니터링 강화`;
  ...
  private generateAlertManagementResponse(
    entities: any,
    serverData?: any
  ): string {
    return `🚨 **알림 관리 현황**
...
**권장 조치:** 임계값 조정 및 자동 대응 설정`;
  ...
  private generateSpecificServerAnalysisResponse(
    entities: any,
    serverData?: any
  ): string {
