...
export class MCPToolHandler {
    ...
    async getAvailableTools(): Promise<{ tools: any[] }> {
        ...
        console.log(`📋 사용 가능한 도구 ${toolsList.length}개:`,
    ...
    private async realSearchFiles(args: {
        ...
        try {
            ...
            console.log(`🔍 파일 검색 완료: ${results.length}개 결과 (${responseTime}ms)`);
    ...
    private isTextFile(filename: string): boolean {
        const textExtensions = ['.ts', '.tsx', '.js', '.jsx', '.md', '.txt', '.json', '.css', '.html'];
    ...
    private async realListDirectory(dirPath: string): Promise<any> {
        ...
        try {
            ...
            console.log(`📁 디렉토리 나열 완료: ${dirPath} (${items.length}개 항목, ${responseTime}ms)`);
     ...
     * 🔧 서버별 도구 목록 조회
     */
    async listTools(serverName: string, clients: Map<string, MCPClient>): Promise<any[]> {
        try {
            ...
            try {
                ...
                console.log(`📋 ${serverName} 서버 도구 목록:`, response.tools?.length || 0);
                return response.tools || [];
            } catch (error) {
                console.warn(`⚠️ ${serverName} 서버 도구 목록 조회 실패, 기본 도구 반환`);
                return Array.from(this.tools.values());
            }
        } catch (error) {
    ...
    async searchWeb(query: string): Promise<MCPSearchResult> {
        ...
        try {
            ...
            console.log(`✅ 웹 검색 완료: ${mockResults.length}개 결과 (${responseTime}ms)`);
