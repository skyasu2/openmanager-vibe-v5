...
export class MCPContextManager {
    ...
    private cleanupExpiredContexts(): void {
        ...
        for (const [sessionId, context] of this.contexts.entries()) {
            const expiresAt = context.expiresAt || (context.timestamp + this.defaultTTL);
    ...
    async storeContext(sessionId: string, context: any): Promise<boolean> {
        try {
            ...
            console.log(`💾 컨텍스트 저장됨: ${sessionId} (크기: ${JSON.stringify(context).length}바이트)`);
    ...
    async retrieveContext(sessionId: string): Promise<any> {
        try {
            ...
            // 만료 확인
            const now = Date.now();
            const expiresAt = sessionContext.expiresAt || (sessionContext.timestamp + this.defaultTTL);
    ...
    searchContexts(query: string): SessionContext[] {
        ...
        for (const context of this.contexts.values()) {
            ...
            if (dataStr.includes(searchTerm) || context.sessionId.toLowerCase().includes(searchTerm)) {
    ...
    exportContexts(sessionIds?: string[]): any {
        ...
        console.log(`📤 ${Object.keys(exportData).length}개 컨텍스트 내보내기 완료`);
        return exportData;
    }
