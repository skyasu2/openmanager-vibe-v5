...
export class MCPPerformanceMonitor {
    ...
    private logPerformanceStats(): void {
        const { totalRequests, totalResponseTime, serverLoadBalance } = this.performanceMetrics;
    ...
    selectOptimalServer(
        ...
        // 로드 밸런싱 통계 업데이트
        const currentLoad = this.performanceMetrics.serverLoadBalance.get(selectedServer) || 0;
        this.performanceMetrics.serverLoadBalance.set(selectedServer, currentLoad + 1);
        ...
        const report = {
            ...
            globalMetrics: {
                ...
                averageResponseTime: this.performanceMetrics.totalRequests > 0
                    ...
                    : 0,
                lastOptimized: new Date(this.performanceMetrics.lastOptimized).toISOString()
            },
            serverMetrics: {} as Record<string, any>,
            loadBalancing: Object.fromEntries(this.performanceMetrics.serverLoadBalance)
        };
        ...
        for (const [name, config] of servers.entries()) {
            if (config.stats) {
                const successRate = config.stats.totalRequests > 0
                    ? (config.stats.successfulRequests / config.stats.totalRequests) * 100
                    : 0;
                ...
                report.serverMetrics[name] = {
                    ...
                    averageResponseTime: config.stats.averageResponseTime.toFixed(2) + 'ms',
    ...
    getPerformanceStatus(): any {
        return {
            ...
            averageResponseTime: this.performanceMetrics.totalRequests > 0
                ? this.performanceMetrics.totalResponseTime / this.performanceMetrics.totalRequests
                : 0,
            serverLoadDistribution: Object.fromEntries(this.performanceMetrics.serverLoadBalance),
            lastOptimized: new Date(this.performanceMetrics.lastOptimized).toISOString(),
