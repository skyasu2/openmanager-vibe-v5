...
export class SSEHealthMonitor {
    ...
    async performHealthCheck(): Promise<boolean> {
        ...
        // 건강 체크 로직 (시뮬레이션)
        const isHealthy = this.healthStatus.consecutiveErrors < this.config.errorThreshold;
    ...
    recordError(errorMessage: string): void {
        ...
        console.warn(`🚨 SSE 건강 모니터 오류 기록: ${errorMessage} (연속 ${this.healthStatus.consecutiveErrors}회)`);
    }
    ...
    getStatistics(): {
        ...
        const errorRate = uptimeInMinutes > 0 ? this.healthStatus.errorCount / uptimeInMinutes : 0;
