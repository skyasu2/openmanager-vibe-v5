...
export class VMMultiServiceOrchestrator extends EventEmitter {
    ...
    async startAllServices(): Promise<{
        ...
        if (this.isRunning) {
            return {
                ...
                services: {
                    ...
                    dataGenerator: this.getServiceStatus('dataGenerator')?.status === 'running',
                    scenarioEngine: this.getServiceStatus('scenarioEngine')?.status === 'running',
                },
                totalTime: 0,
            };
        }
        ...
        try {
            ...
            if (this.config.enableMCP) {
                ...
                systemLogger.info(`📡 MCP 서버: ${mcpStarted ? '시작 완료' : '이미 실행 중 또는 실패'}`);
            }
            ...
            if (this.config.enableDataGenerator || this.config.enableScenarioEngine) {
                ...
                systemLogger.info(`📊 데이터 생성기: ${dataGeneratorStarted ? '시작 완료' : '실패'}`);
                systemLogger.info(`🎭 시나리오 엔진: ${scenarioEngineStarted ? '시작 완료' : '실패'}`);
            }
    ...
    async stopAllServices(): Promise<{
        ...
        try {
            ...
            systemLogger.info(`📊 데이터 생성기: ${dataGeneratorStopped ? '정지 완료' : '실패'}`);
            systemLogger.info(`🎭 시나리오 엔진: ${scenarioEngineStopped ? '정지 완료' : '실패'}`);

            // 3️⃣ MCP 서버는 별도 VM 프로세스이므로 상태만 확인
            mcpStopped = await this.checkMCPServerStatus();
            systemLogger.info(`📡 MCP 서버: ${mcpStopped ? '정상 운영 중' : '상태 확인 필요'}`);
    ...
    private async startMCPServer(): Promise<boolean> {
        try {
            ...
            if (isRunning) {
                ...
                systemLogger.info('📡 MCP 서버가 이미 실행 중입니다 (VM 외부 프로세스)');
                return true;
            }

            // MCP 서버가 없다면 경고만 표시 (VM에서 별도 관리되므로)
            this.updateServiceStatus('mcp', 'error');
            systemLogger.warn('⚠️ MCP 서버가 감지되지 않습니다. VM에서 별도로 시작해야 합니다.');
            return false;
     ...
     * 🔄 서비스 상태 업데이트
     */
    private updateServiceStatus(serviceName: string, status: ServiceStatus['status'], additionalData?: Partial<ServiceStatus>): void {
    ...
    private startHealthCheck(): void {
        ...
        this.healthCheckInterval = setInterval(async () => {
            try {
                ...
                this.updateServiceStatus('dataGenerator', vmStatus.isRunning ? 'running' : 'error');
                this.updateServiceStatus('scenarioEngine', vmStatus.isRunning ? 'running' : 'error');
    ...
    private logServicesSummary(): void {
        ...
        systemLogger.info(`📊 서비스 현황: 실행 중 ${summary.running}개, 정지 ${summary.stopped}개, 오류 ${summary.error}개`);
