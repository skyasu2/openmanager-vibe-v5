...
export class DistributedDataManager {
  ...
  async saveDistributedMetrics(
    ...
    try {
      ...
      this.supabaseManager
        .batchInsertMetrics(sessionId, metrics)
  ...
  async handleFailover(
    ...
    console.warn(
      `Primary storage failed: ${error.message}. Initiating failover...`
    );
  ...
  async getLoadBalancingStatus(): Promise<{
    ...
    if (hotLayer.currentUsage > hotLayer.maxCapacity * 0.8) {
      recommendations.push(
        'Hot Layer 사용률이 80%를 초과했습니다. Warm Layer로 일부 데이터 이동을 고려하세요.'
      );
    }

    if (warmLayer.currentUsage > warmLayer.maxCapacity * 0.8) {
      recommendations.push(
        'Warm Layer 사용률이 80%를 초과했습니다. Cold Layer 아카이빙을 고려하세요.'
      );
    }

    if (this.performanceMetrics.errorRate > 5) {
      recommendations.push(
        '에러율이 5%를 초과했습니다. 시스템 상태를 확인하세요.'
      );
    }
  ...
  async executeUnifiedQuery(query: {
    ...
    switch (query.type) {
      ...
      case 'aggregated':
        if (query.sessionId) {
          return this.supabaseManager.calculateSessionAggregates(
            query.sessionId
          );
        }
  ...
  async validateDataConsistency(
    ...
    try {
      ...
      // Warm Layer 카운트
      const warmMetrics =
        await this.supabaseManager.getSessionMetricsHistory(sessionId);
      ...
      if (Math.abs(hotCount - warmCount) > hotCount * 0.1) {
        inconsistencies.push(
          `Hot-Warm Layer 불일치: Hot(${hotCount}) vs Warm(${warmCount})`
        );
      }
  ...
  private updateLayerUsage(layerName: string, metricCount: number): void {
    ...
    if (layer) {
      ...
      layer.currentUsage = Math.min(
        layer.maxCapacity,
        layer.currentUsage + metricCount * 0.01
      );
    }
  }
