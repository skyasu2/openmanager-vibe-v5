...
export class RealisticVariationGenerator {
  ...
  static generateNaturalVariance(baseValue: number, serverId: string): number {
    ...
    if (dayOfWeek === 1)
      dayMultiplier = 1.1; // 월요일: 높은 부하
    else if (dayOfWeek === 5)
      dayMultiplier = 1.2; // 금요일: 배포 등으로 높은 부하
  ...
  static checkRandomEvent(serverId: string): {
    ...
    if (adjustedRoll < 0.015) {
      const severEvents = [
        {
          impact: 45,
          type: '메모리 누수 감지',
          description: '점진적 메모리 사용량 증가 패턴 발견',
        },
        {
          impact: 40,
          type: 'DB 커넥션 풀 고갈',
          description: '동시 연결 수 한계 도달',
        },
        {
          impact: 50,
          type: 'CPU 과부하',
          description: '비정상적인 CPU 사용률 급증',
        },
        {
          impact: 35,
          type: '디스크 I/O 병목',
          description: '디스크 읽기/쓰기 지연 발생',
        },
      ];
      const selected =
        severEvents[
      ...
      return {
        hasEvent: true,
        impact: selected?.impact ?? 50,
        type: selected?.type ?? 'Unknown Event',
        description: selected?.description,
      };
    }

    // 중간 이벤트 (8-12% 확률)
    else if (adjustedRoll < 0.1) {
      const mediumEvents = [
        {
          impact: 22,
          type: '트래픽 스파이크',
          description: '예상보다 높은 사용자 요청',
        },
        {
          impact: 18,
          type: '캐시 미스 증가',
          description: '캐시 효율성 일시적 저하',
        },
        {
          impact: 25,
          type: '네트워크 지연',
          description: '외부 API 응답 시간 증가',
        },
        {
          impact: 20,
          type: '가비지 컬렉션',
          description: 'GC 실행으로 인한 일시적 부하',
        },
      ];
      const selected =
        mediumEvents[
      ...
      const minorEvents = [
        {
          impact: 8,
          type: '일반적 부하 변동',
          description: '정상 범위 내 사용량 변화',
        },
        {
          impact: 12,
          type: '백그라운드 작업',
          description: '스케줄된 작업 실행 중',
        },
        { impact: 10, type: '세션 정리', description: '만료된 세션 정리 작업' },
        {
          impact: 6,
          type: '로그 로테이션',
          description: '로그 파일 순환 처리',
        },
      ];
      const selected =
        minorEvents[
  ...
  static calculateCascadeEffect(
    ...
    if (serverType === 'api') {
      ...
      const avgWebLoad =
        webServers.reduce((sum, s) => sum + s.cpu, 0) / webServers.length;
      if (avgWebLoad > 70) {
    ...
    if (serverType !== 'database') {
      ...
      const avgDbLoad =
        dbServers.reduce((sum, s) => sum + s.cpu, 0) / dbServers.length;
      if (avgDbLoad > 80) {
  ...
  static generateBatchMetrics(
    ...
    const results = serverInfos.map(({ id, type: _type, baseMetrics }) => {
      const serverSeed = id.charCodeAt(0) * 7;
      const _combinedSeed = timeSeed + serverSeed;
    ...
    results.forEach((result) => {
      ...
      if (cascadeImpact > 0) {
        ...
        result.metrics.memory = Math.min(
          95,
          result.metrics.memory + cascadeImpact * 0.7
        );
      }
    });
