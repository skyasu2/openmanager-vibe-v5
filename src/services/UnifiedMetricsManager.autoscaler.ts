...
export class Autoscaler {
  ...
  static async performAutoscaling(
    ...
    try {
      ...
      await this.simulateAutoscaling(servers, serverList, config, createServerFn);
  ...
  static calculateScalingRecommendation(
    ...
    const avgCpu = servers.reduce((sum, s) => sum + s.node_cpu_usage_percent, 0) / currentCount;
    const avgMemory = servers.reduce((sum, s) => sum + s.node_memory_usage_percent, 0) / currentCount;
    ...
    if (avgCpu > config.autoscaling.target_cpu_percent) {
      if (currentCount < config.autoscaling.max_servers) {
        ...
        recommendedCount = Math.min(currentCount + Math.ceil(avgCpu / 50), config.autoscaling.max_servers);
      } else {
    ...
    // Scale in logic
      ...
      if (currentCount > config.autoscaling.min_servers) {
        ...
        recommendedCount = Math.max(currentCount - 1, config.autoscaling.min_servers);
      } else {
  ...
  static predictScalingNeeds(
    ...
    const currentCpu = servers.reduce((sum, s) => sum + s.node_cpu_usage_percent, 0) / servers.length;
    ...
    if (predictedCpu > config.autoscaling.target_cpu_percent * 1.1) {
      ...
      confidence = Math.min(90, 60 + (predictedCpu - config.autoscaling.target_cpu_percent));
      estimatedTimeToAction = Math.max(5, timeHorizonMinutes - ((predictedCpu - config.autoscaling.target_cpu_percent) * 2));
      ...
      prediction = 'scale_in';
      confidence = Math.min(85, 60 + (config.autoscaling.target_cpu_percent - predictedCpu));
      estimatedTimeToAction = Math.max(10, timeHorizonMinutes - ((config.autoscaling.target_cpu_percent - predictedCpu) * 3));
