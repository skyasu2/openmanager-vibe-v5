...
export class UserSessionService {
  ...
  private sessionCallbacks: Set<(session: UserSession | null) => void> =
    new Set();
  ...
  private updateSession(): void {
    ...
    this.currentSession.remainingTime = Math.max(
      0,
      this.currentSession.remainingTime - this.UPDATE_INTERVAL
    );

    // 진행률 계산
    this.currentSession.progress =
      1 - this.currentSession.remainingTime / this.SESSION_DURATION;
    ...
    const remainingMinutes = Math.floor(
      this.currentSession.remainingTime / 60000
    );
    if (
      remainingMinutes === 5 ||
      remainingMinutes === 3 ||
   ...
   * ⚠️ 경고 알림 구독
   */
  public subscribeToWarnings(
    callback: (remainingMinutes: number) => void
  ): () => void {
