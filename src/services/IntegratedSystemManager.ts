...
export class IntegratedSystemManager {
  ...
  async initializeSystem(): Promise<void> {
    ...
    try {
      ...
      if (this.config.system.startupDelay > 0) {
        console.log(
          `⏰ 시스템 시작 지연: ${this.config.system.startupDelay}ms`
        );
        await new Promise(resolve =>
          setTimeout(resolve, this.config.system.startupDelay)
        );
      }
  ...
  private startSystemHealthCheck(): void {
    ...
    console.log(
      `⏰ 시스템 헬스 체크 시작 (${this.config.system.healthCheckInterval}ms 간격)`
    );
  }
  ...
  private calculateSystemHealthScore(): number {
    ...
    if (this.config.notifications.enabled) {
      const notificationScore =
        this.systemState.services.notifications.status === 'active' ? 100 : 0;
    ...
    if (this.config.aiStateManager.enabled) {
      const aiScore =
        this.systemState.services.aiStateManager.status === 'active' ? 100 : 0;
      totalScore += aiScore;
      serviceCount++;
    }
    ...
    if (this.config.performanceMonitor.enabled) {
      const performanceScore =
        this.systemState.services.performanceMonitor.status === 'active'
          ? 100
          : 0;
  ...
  private identifySystemIssues(): string[] {
    ...
    const performanceMetrics =
      centralizedPerformanceMonitor.getCurrentMetrics();
    if (performanceMetrics) {
      if (performanceMetrics.system.memory.usage > 85) {
        issues.push(
          `메모리 사용량이 높습니다: ${performanceMetrics.system.memory.usage}%`
        );
      }

      if (performanceMetrics.system.cpu.usage > 80) {
        issues.push(
          `CPU 사용량이 높습니다: ${performanceMetrics.system.cpu.usage.toFixed(1)}%`
        );
      }
    }
  ...
  private generateSystemRecommendations(): string[] {
    ...
    if (issues.some(issue => issue.includes('메모리 사용량'))) {
      recommendations.push(
        '메모리 사용량을 줄이기 위해 불필요한 서비스를 중지하거나 메모리 최적화를 고려하세요.'
      );
    }

    if (issues.some(issue => issue.includes('CPU 사용량'))) {
      recommendations.push(
        'CPU 사용량을 줄이기 위해 부하 분산이나 프로세스 최적화를 고려하세요.'
      );
    }
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          ...
          this.config.system.gracefulShutdownTimeout
        );
      });
