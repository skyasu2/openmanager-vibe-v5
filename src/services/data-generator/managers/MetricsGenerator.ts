...
export class GCPMetricsCollector {
  ...
  async updateServerMetrics(server: ServerInstance): Promise<void> {
    try {
      ...
      if (!gcpMetrics) {
        throw new Error(
          `GCP에서 서버 ${server.id}의 메트릭을 찾을 수 없습니다`
        );
      }
      ...
      throw new Error(
        `GCP 메트릭 수집 실패: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private async collectGCPMetrics(
    serverId: string
  ): Promise<GCPMetricData | null> {
    try {
      ...
      if (
        cached &&
        now - this.lastCollectionTime < this.config.refreshInterval
      ) {
      ...
      const response = await fetch(
        `/api/gcp/metrics?serverId=${serverId}&sessionId=${this.config.sessionId}`
      );

      if (!response.ok) {
        throw new Error(
          `GCP API 호출 실패: ${response.status} ${response.statusText}`
        );
      }
   ...
   */
  private applyGCPMetrics(
    server: ServerInstance,
    gcpMetrics: GCPMetricData
  ): void {
    // GCP 메트릭을 서버 메트릭에 적용
    if (server.metrics) {
    ...
    if (
      ['warning', 'error', 'stopped', 'running', 'maintenance'].includes(
        statusValue
      )
    ) {
      server.status = statusValue as
        | 'warning'
        | 'error'
        | 'stopped'
        | 'running'
  ...
  simulateIncidents(): void {
    throw new Error(
      '시뮬레이션 기능이 제거되었습니다. GCP에서 실제 메트릭을 사용하세요.'
    );
  }
  ...
  getActiveIncidents(): never {
    throw new Error(
      '시뮬레이션 기능이 제거되었습니다. GCP에서 실제 알림을 사용하세요.'
    );
  }
}

// 🔧 GCP 메트릭 수집기 팩토리 함수
export function createGCPMetricsCollector(
  config: GCPMetricsConfig
): GCPMetricsCollector {
