...
export class StateManager {
  ...
  public updateServerState(
    serverId: string,
    newMetrics: Partial<ServerState>
  ): void {
  ...
  private analyzeTrends(
    ...
    if (newMetrics.memory !== undefined) {
      ...
      const direction =
        memoryDiff > 5 ? 'up' : memoryDiff < -5 ? 'down' : 'stable';
  ...
  public getPatternStats() {
    ...
    stats.recentPatterns = allPatterns.filter(
      p => p.startTime > oneHourAgo
    ).length;
  ...
  public async healthCheck() {
    ...
    return {
      ...
      lastUpdate:
        states.length > 0 ? Math.max(...states.map(s => s.lastUpdate)) : 0,
    };
  }
