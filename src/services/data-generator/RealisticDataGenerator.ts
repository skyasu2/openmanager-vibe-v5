 ...
 * 🎭 시연용 현실적 데이터 생성기
 * 
 * 경쟁 프로젝트 시연을 위한 다양한 서버 시나리오 시뮬레이션
 * - 실제 서버 패턴 기반 데이터 생성
 * - 5가지 주요 시나리오 지원
 * - 시간대별 트래픽 패턴 반영
 */
...
export type DemoScenario = 'normal' | 'spike' | 'memory_leak' | 'ddos' | 'performance_degradation';
...
export class RealisticDataGenerator {
   ...
   * 🔥 단일 데이터 포인트 생성
   */
  private generateDataPoint(timestamp: Date, index: number, total: number): ServerMetrics {
    // 시간대별 기본 패턴
    ...
    let memory = this.generateMemoryPattern(timePattern, scenarioModifier, index);
    let disk = this.generateDiskPattern(timePattern, scenarioModifier, index);
    let networkIn = this.generateNetworkInPattern(timePattern, scenarioModifier, index);
    let networkOut = this.generateNetworkOutPattern(timePattern, scenarioModifier, index);
    let responseTime = this.generateResponseTimePattern(timePattern, scenarioModifier, index);
    let activeConnections = this.generateConnectionsPattern(timePattern, scenarioModifier, index);
   ...
   * 🎭 시나리오별 수정자
   */
  private getScenarioModifier(index: number, total: number): Record<string, number> {
   ...
   * 🖥️ CPU 패턴 생성
   */
  private generateCPUPattern(timePattern: number, modifier: Record<string, number>, index: number): number {
   ...
   * 💾 메모리 패턴 생성 (누수 포함)
   */
  private generateMemoryPattern(timePattern: number, modifier: Record<string, number>, index: number): number {
   ...
   * 💽 디스크 패턴 생성
   */
  private generateDiskPattern(timePattern: number, modifier: Record<string, number>, index: number): number {
   ...
   * 🌐 네트워크 입력 패턴
   */
  private generateNetworkInPattern(timePattern: number, modifier: Record<string, number>, index: number): number {
   ...
   * 🌐 네트워크 출력 패턴
   */
  private generateNetworkOutPattern(timePattern: number, modifier: Record<string, number>, index: number): number {
   ...
   * ⏱️ 응답시간 패턴
   */
  private generateResponseTimePattern(timePattern: number, modifier: Record<string, number>, index: number): number {
   ...
   * 🔗 활성 연결 패턴
   */
  private generateConnectionsPattern(timePattern: number, modifier: Record<string, number>, index: number): number {
  ...
  private generateLogEntry(timestamp: Date): DemoLogEntry {
    ...
    // 시나리오에 따른 로그 레벨 가중치
    let levelWeights: Record<string, number> = { INFO: 70, WARN: 20, ERROR: 8, CRITICAL: 2 };
    ...
    const messageData = scenarioMessages[level][Math.floor(Math.random() * scenarioMessages[level].length)];
  ...
  private getScenarioLogMessages() {
    const baseMessages = {
      INFO: [
        { message: 'HTTP 요청 처리 완료', category: 'application' as const, severity: 1 },
        { message: '일일 백업 작업 시작', category: 'system' as const, severity: 2 },
        { message: '사용자 인증 성공', category: 'security' as const, severity: 1 },
        { message: '캐시 히트 - 응답 최적화', category: 'performance' as const, severity: 1 }
      ],
      WARN: [
        { message: 'CPU 사용률 70% 초과', category: 'performance' as const, severity: 4 },
        { message: '메모리 사용량 증가 감지', category: 'system' as const, severity: 5 },
        { message: '응답 시간 지연 발생', category: 'performance' as const, severity: 4 },
        { message: '연결 풀 80% 사용 중', category: 'network' as const, severity: 5 }
      ],
      ERROR: [
        { message: '데이터베이스 연결 타임아웃', category: 'system' as const, severity: 7 },
        { message: 'API 요청 처리 실패', category: 'application' as const, severity: 6 },
        { message: '메모리 할당 오류', category: 'system' as const, severity: 8 },
        { message: '네트워크 연결 끊어짐', category: 'network' as const, severity: 7 }
      ],
      CRITICAL: [
        { message: '서비스 응답 불가', category: 'system' as const, severity: 10 },
        { message: 'DDoS 공격 의심 트래픽 감지', category: 'security' as const, severity: 9 },
        { message: '메모리 누수로 인한 서비스 불안정', category: 'system' as const, severity: 9 },
        { message: '디스크 공간 부족 - 긴급 조치 필요', category: 'system' as const, severity: 10 }
      ]
    };
    ...
    if (this.currentScenario === 'ddos') {
      baseMessages.ERROR.push(
        { message: '비정상적인 트래픽 패턴 감지', category: 'security' as any, severity: 8 },
        { message: '연결 수 임계치 초과', category: 'network' as const, severity: 7 }
      );
      baseMessages.CRITICAL.push(
        { message: '보안 위협 레벨 상승', category: 'security' as any, severity: 9 }
      );
    }
  ...
  private getRandomSource(): string {
    const sources = [
      ...
      'database-primary', 'database-replica', 'cache-server',
      'load-balancer', 'worker-node-01', 'worker-node-02',
      'monitoring-service', 'backup-service', 'auth-service'
    ];
   ...
   * 📊 로그 메타데이터 생성
   */
  private generateLogMetadata(level: DemoLogEntry['level'], category: DemoLogEntry['category']): Record<string, any> {
   ...
   * 📈 현재 시나리오 정보 반환
   */
  public getCurrentScenarioInfo(): ScenarioConfig & { currentScenario: DemoScenario } {
    return {
