...
export class BaselineManager {
    ...
    private generateCPUBaseline(serverType: string): any[] {
        ...
        const profile = baselineProfiles[serverType as keyof typeof baselineProfiles] || baselineProfiles.web;
        const hourlyData = [];

        for (let hour = 0; hour < 24; hour++) {
            // 시간대별 부하 패턴 (오전 9시-오후 6시가 피크)
            const timeMultiplier = this.getTimeMultiplier(hour);
            const cpuValue = profile.base + (profile.peak - profile.base) * timeMultiplier;
    ...
    private generateMemoryBaseline(serverType: string): any[] {
        ...
        for (let hour = 0; hour < 24; hour++) {
            const timeMultiplier = this.getTimeMultiplier(hour);
            const memoryValue = profile.base + (profile.peak - profile.base) * timeMultiplier;
            const variance = (Math.random() - 0.5) * profile.variance;
    ...
    private generateNetworkBaseline(serverType: string): any[] {
        ...
        for (let hour = 0; hour < 24; hour++) {
            const timeMultiplier = this.getTimeMultiplier(hour);
            const networkInValue = profile.inBase + (profile.inPeak - profile.inBase) * timeMultiplier;
            const networkOutValue = profile.outBase + (profile.outPeak - profile.outBase) * timeMultiplier;
    ...
    private generateDiskBaseline(serverType: string): any[] {
        ...
        for (let hour = 0; hour < 24; hour++) {
            const timeMultiplier = this.getTimeMultiplier(hour);
            const diskValue = profile.base + (profile.peak - profile.base) * timeMultiplier;
            const variance = (Math.random() - 0.5) * profile.variance;
    ...
    public getBaselineStats() {
        return {
            ...
            serverTypes: Array.from(new Set(
                Array.from(this.serverBaselines.values()).map(b => b.type)
            )),
            locations: Array.from(new Set(
                Array.from(this.serverBaselines.values()).map(b => b.location)
            )),
