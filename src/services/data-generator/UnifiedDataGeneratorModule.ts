 ...
 * - AdvancedServerDataGenerator: 고급 메트릭
 * - RealisticDataGenerator: 시연용 시나리오
 * 
 * 🎯 특징:
 * - 공통 Redis 연결 풀
 * - 공통 타이머 관리  
...
import { ServerInstance, ServerCluster, ApplicationMetrics } from '@/types/data-generator';
...
export interface UnifiedGeneratorConfig {
    ...
    realisticConfig?: {
        scenario: 'normal' | 'spike' | 'memory_leak' | 'ddos' | 'performance_degradation';
    };
}
...
class RealDataStrategy implements DataGeneratorStrategy {
    ...
    private initializeServers(): void {
        for (let i = 1; i <= this.config.maxServers; i++) {
            ...
            const server: ServerInstance = {
                ...
                status: Math.random() > 0.1 ? 'running' : Math.random() > 0.5 ? 'warning' : 'error',
                specs: {
                    cpu: {
    ...
    async generateData(): Promise<ServerInstance[]> {
        ...
        for (const [id, server] of this.servers) {
            ...
            server.metrics.memory = Math.max(10, Math.min(90, server.metrics.memory + (Math.random() - 0.5) * 8));
            server.metrics.disk = Math.max(20, Math.min(85, server.metrics.disk + (Math.random() - 0.5) * 3));
...
class OptimizedDataStrategy implements DataGeneratorStrategy {
    ...
    async initialize(): Promise<void> {
        console.log('🔧 Optimized Data Strategy 초기화 (기존 OptimizedDataGenerator 재사용)...');

        // 기존 OptimizedDataGenerator 설정 적용
        this.optimizedGenerator.updateConfig({
            usePregenerated: true,
            realTimeVariationIntensity: this.config.optimizedConfig?.realTimeVariationIntensity || 0.15,
    ...
    async start(): Promise<void> {
        ...
        console.log('🚀 Optimized Data Strategy 시작됨 (기존 OptimizedDataGenerator 활용)');
    }
    ...
    getStatus(): any {
        ...
        return {
            ...
            isRunning: this.isRunning,
            ...optimizedStatus,
            reuseExistingCode: '✅ 기존 OptimizedDataGenerator 완전 재사용 (중복 제거)',
        };
    }
...
class AdvancedDataStrategy implements DataGeneratorStrategy {
    ...
    private initializeAdvancedServers(): void {
        const regions = this.config.advancedConfig?.regions || ['us-east-1', 'us-west-2', 'eu-west-1'];
        const serverTypes = this.config.advancedConfig?.serverTypes || ['K8s', 'Host', 'Cloud', 'Container'];
        ...
        for (let i = 0; i < this.config.maxServers; i++) {
            const server = {
                ...
                resources: {
                    ...
                    memory: {
                        total: [8, 16, 32, 64][Math.floor(Math.random() * 4)] * 1024 * 1024 * 1024,
                        type: 'DDR4',
                    },
                },
            };
    ...
    async generateData(): Promise<any[]> {
        return this.servers.map(server => ({
            ...
            metrics: {
                ...
                memory: {
                    used: Math.floor(server.resources.memory.total * (0.2 + Math.random() * 0.6)),
                    available: Math.floor(server.resources.memory.total * (0.4 + Math.random() * 0.4)),
                },
                timestamp: new Date(),
            },
        }));
...
class RealisticDataStrategy implements DataGeneratorStrategy {
    ...
    async start(): Promise<void> {
        this.isRunning = true;
        console.log(`✅ Realistic Data Strategy 시작됨 (시나리오: ${this.scenario})`);
    }
...
export class UnifiedDataGeneratorModule {
    ...
    private loadConfiguration(): UnifiedGeneratorConfig {
        return {
            ...
            realConfig: {
                serverArchitecture: (process.env.SERVER_ARCHITECTURE as any) || 'load-balanced',
            },
            optimizedConfig: {
                usePregenerated: process.env.USE_PREGENERATED !== 'false',
                realTimeVariationIntensity: parseFloat(process.env.VARIATION_INTENSITY || '0.15'),
            },
            advancedConfig: {
                regions: (process.env.REGIONS || 'us-east-1,us-west-2,eu-west-1').split(','),
                serverTypes: (process.env.SERVER_TYPES || 'K8s,Host,Cloud,Container').split(','),
            },
            realisticConfig: {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            console.log(`✅ UnifiedDataGeneratorModule 초기화 완료 (전략: ${this.config.strategy})`);
        } catch (error) {
    ...
    private async initializeRedis(): Promise<void> {
        try {
            ...
            const redisHost = process.env.REDIS_HOST || 'charming-condor-46598.upstash.io';
            const redisPort = parseInt(process.env.REDIS_PORT || '6379');
            const redisPassword = process.env.REDIS_PASSWORD || process.env.KV_REST_API_TOKEN;
