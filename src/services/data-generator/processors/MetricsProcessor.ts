...
export class MetricsProcessor {
    ...
    public static processServerMetrics(
        ...
        const effectiveIntensity = scenarioContext.isAffectedServer ? scenarioContext.intensity : 1.0;
        ...
        let processedMetrics = {
            ...
            network: {
                in: Math.max(
                    0,
                    rawMetrics.network.in + (Math.random() - 0.5) * 50 * effectiveIntensity
                ),
                out: Math.max(
                    0,
                    rawMetrics.network.out + (Math.random() - 0.5) * 30 * effectiveIntensity
                ),
            },
        };
        ...
        if (scenarioContext.isAffectedServer) {
            processedMetrics = this.applyScenarioAdjustments(processedMetrics, scenarioContext);
        }

        // 🎯 4단계: 유의미한 변화 감지
        const hasSignificantChange = this.detectSignificantChange(server.metrics, processedMetrics);
     ...
     * 🎯 유의미한 변화 감지 (10% 이상 변화)
     */
    private static detectSignificantChange(originalMetrics: any, newMetrics: any): boolean {
     ...
     * 🎯 메트릭 기반 서버 상태 결정
     */
    public static determineServerStatus(metrics: any): 'running' | 'warning' | 'error' {
    ...
    public static calculateHealthScore(metrics: any): number {
        ...
        const healthScore = (cpuHealth * 0.4) + (memoryHealth * 0.35) + (diskHealth * 0.25);
     ...
     * 🏥 클러스터 건강 상태 계산
     */
    public static calculateClusterHealth(servers: ServerInstance[]): 'healthy' | 'warning' | 'critical' {
     ...
     * 🏥 애플리케이션 건강 상태 계산
     */
    public static calculateApplicationHealth(servers: ServerInstance[]): 'healthy' | 'warning' | 'critical' {
        if (servers.length === 0) return 'critical';
