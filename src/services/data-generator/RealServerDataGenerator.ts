...
export class GCPRealServerDataGenerator {
  ...
  async generateServers(): Promise<ServerInstance[]> {
    ...
    try {
      // 🧪 테스트 환경: 목업 데이터 생성 (외부 통신 방지)
      if (env.IS_TEST) {
        console.log('🧪 테스트 환경: 목업 서버 데이터 생성');
      ...
      // 🔧 개발 환경: 목업 데이터 생성 (fallback)
      if (env.IS_DEVELOPMENT) {
        console.log('🔧 개발 환경: 목업 서버 데이터 생성');
        return this.generateMockServers(this.config.limit);
      }

      // ⚠️ 알 수 없는 환경 - 안전한 fallback 제공
      console.warn('⚠️ 알 수 없는 환경에서 실행됨, 목업 데이터로 fallback');
      return this.generateMockServers(this.config.limit);
    } catch (error) {
  ...
  private async fetchFromGCP(limit: number): Promise<Response> {
    try {
      ...
      const response = await fetch(
        `/api/gcp/server-data?sessionId=${this.config.sessionId}&limit=${limit}`
      );
  ...
  private groupMetricsByServer(metrics: any[]): Map<string, any[]> {
    ...
    for (const metric of metrics) {
      const serverId =
        metric.serverId ||
        `gcp-server-${Math.random().toString(36).substr(2, 9)}`;
  ...
  async getDashboardSummary(): Promise<any> {
    ...
    try {
      ...
      // 에러 서버인지 확인
      const isErrorState = servers.some(
        server =>
  ...
  async getAllClusters(): Promise<any[]> {
    try {
      ...
      for (const cluster of clusters.values()) {
        const criticalCount = cluster.servers.filter(
    ...
    try {
      ...
      for (const type of appTypes) {
        ...
        const app = {
          ...
          healthyServers: typeServers.filter(s => s.status === 'healthy')
            .length,
          warningServers: typeServers.filter(s => s.status === 'warning')
            .length,
          criticalServers: typeServers.filter(s => s.status === 'critical')
            .length,
 ...
 * 🔧 GCP 직접 연동 팩토리 함수
 */
export function createServerDataGenerator(
  config?: GCPServerConfig
