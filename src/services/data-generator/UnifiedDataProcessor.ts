...
export class UnifiedDataProcessor {
    ...
    public async processData(
        ...
        try {
            ...
            const fallbackData = this.cacheManager.get(cacheKey) || await this.getEmptyProcessedData(purpose);
    ...
    private async applyAIProcessing(
        ...
        for (const serverInstance of baseData.rawServers) {
            try {
                const optimizedMetric = await this.createAIOptimizedMetric(serverInstance, options);
    ...
    private async createAIOptimizedMetric(
        ...
        const baseMetrics = await this.createStandardMetricsFromServerInstance(serverInstance);

        // 정규화된 메트릭 계산
        const normalizedMetrics = this.normalizeMetrics(baseMetrics, options.normalizationMode || 'minmax');

        // 컨텍스트 정보 추론
        const context = this.inferServerContext(serverInstance);

        // AI 특성 계산
        const aiFeatures = await this.calculateAIFeatures(baseMetrics, normalizedMetrics);
    ...
    private normalizeMetrics(
        ...
        const networkBytes = metrics.metrics.network.bytesReceived + metrics.metrics.network.bytesSent;
    ...
    private inferServerContext(serverInstance: ServerInstance) {
        // 서버 이름과 메트릭을 기반으로 역할 추론
        const hostname = serverInstance.hostname.toLowerCase();

        let serverRole: 'web' | 'api' | 'database' | 'cache' | 'queue' = 'web';
        if (hostname.includes('db') || hostname.includes('database')) {
            serverRole = 'database';
        } else if (hostname.includes('api') || hostname.includes('service')) {
            serverRole = 'api';
        } else if (hostname.includes('cache') || hostname.includes('redis')) {
            serverRole = 'cache';
        } else if (hostname.includes('queue') || hostname.includes('worker')) {
            serverRole = 'queue';
        }
    ...
    private async calculateAIFeatures(
        ...
        normalizedMetrics: { cpu: number; memory: number; disk: number; network: number; overall: number }
    ) {
        // 이상 점수 계산 (간단한 임계값 기반)
        let anomalyScore = 0;
     ...
     * 🔖 패턴 시그니처 생성
     */
    private generatePatternSignature(normalizedMetrics: { cpu: number; memory: number; disk: number; network: number }): string {
        // 간단한 해시 기반 패턴 시그니처
        const values = [
    ...
    private calculateMonitoringStats(servers: Server[]) {
        ...
        servers.forEach(server => {
            ...
            totalNetwork += (server.networkIn || 0) + (server.networkOut || 0);
        });
    ...
    private calculateAIAggregatedStats(metrics: AIOptimizedMetrics[]) {
        ...
        let totalNormalizedCpu = 0, totalNormalizedMemory = 0;
        let totalNormalizedDisk = 0, totalNormalizedNetwork = 0;
        let totalHealthScore = 0, anomalyCount = 0;
    ...
    private analyzeAITrends(metrics: AIOptimizedMetrics[]) {
        ...
        const avgCpu = metrics.reduce((sum, m) => sum + m.normalizedMetrics.cpu, 0) / metrics.length;
        const avgMemory = metrics.reduce((sum, m) => sum + m.normalizedMetrics.memory, 0) / metrics.length;
        const avgDisk = metrics.reduce((sum, m) => sum + m.normalizedMetrics.disk, 0) / metrics.length;
        const avgNetwork = metrics.reduce((sum, m) => sum + m.normalizedMetrics.network, 0) / metrics.length;
        const avgOverall = metrics.reduce((sum, m) => sum + m.normalizedMetrics.overall, 0) / metrics.length;

        return {
            cpuTrend: avgCpu > 0.7 ? 'increasing' : avgCpu < 0.3 ? 'decreasing' : 'stable' as const,
            memoryTrend: avgMemory > 0.7 ? 'increasing' : avgMemory < 0.3 ? 'decreasing' : 'stable' as const,
            diskTrend: avgDisk > 0.7 ? 'increasing' : avgDisk < 0.3 ? 'decreasing' : 'stable' as const,
            networkTrend: avgNetwork > 0.7 ? 'increasing' : avgNetwork < 0.3 ? 'decreasing' : 'stable' as const,
            overallTrend: avgOverall > 0.7 ? 'degrading' : avgOverall < 0.3 ? 'improving' : 'stable' as const
        };
    }
    ...
    private async generateAIInsights(metrics: AIOptimizedMetrics[]) {
        ...
        if (criticalServers.length > 0) {
            recommendations.push(`${criticalServers.length}개 서버가 위험 상태입니다. 즉시 점검이 필요합니다.`);
        }
        if (anomalousServers.length > 0) {
            recommendations.push(`${anomalousServers.length}개 서버에서 이상 패턴이 감지되었습니다.`);
        }
        ...
        const predictedIssues = metrics
            ...
            .map(m => ({
                serverId: m.serverId,
                issueType: m.aiFeatures.riskLevel === 'high' ? 'resource_exhaustion' : 'performance_degradation',
    ...
    // ... 기타 유틸리티 메서드들
    private createStandardMetricsFromServerInstance(serverInstance: ServerInstance): StandardServerMetrics {
        return {
            ...
            metrics: {
                cpu: {
                    ...
                    loadAverage: [serverInstance.cpu / 100, serverInstance.cpu / 100, serverInstance.cpu / 100],
                    cores: 4 // 기본값
                },
                memory: {
                    total: 8 * 1024 * 1024 * 1024, // 8GB 기본값
                    used: (serverInstance.memory / 100) * (8 * 1024 * 1024 * 1024),
                    available: ((100 - serverInstance.memory) / 100) * (8 * 1024 * 1024 * 1024),
                    usage: serverInstance.memory
                },
                disk: {
                    total: 100 * 1024 * 1024 * 1024, // 100GB 기본값
                    used: (serverInstance.disk / 100) * (100 * 1024 * 1024 * 1024),
                    available: ((100 - serverInstance.disk) / 100) * (100 * 1024 * 1024 * 1024),
                    usage: serverInstance.disk,
                    iops: { read: 100, write: 50 }
                },
                network: {
                    bytesReceived: (serverInstance.networkIn || 0) * 1024 * 1024,
                    bytesSent: (serverInstance.networkOut || 0) * 1024 * 1024,
    ...
    private mapServerStatus(status: string): 'online' | 'warning' | 'critical' | 'offline' {
    ...
    private generateCacheKey(purpose: string, options: ProcessingOptions): string {
    ...
    private calculateDataQuality(data: UnifiedProcessedData): number {
        ...
        const hasProcessedData = data.monitoring?.servers.length || data.ai?.metrics.length || 0;
    ...
    private async getEmptyProcessedData(purpose: string): Promise<UnifiedProcessedData> {
        return {
            rawServers: [],
            timestamp: new Date().toISOString(),
            source: 'unified-processor',
            monitoring: purpose === 'monitoring' || purpose === 'both' ? {
                servers: [],
                stats: {
                    total: 0, healthy: 0, warning: 0, critical: 0, offline: 0,
                    averageCpu: 0, averageMemory: 0, averageDisk: 0, averageNetwork: 0
                }
            } : undefined,
            ai: purpose === 'ai' || purpose === 'both' ? {
                metrics: [],
                aggregatedStats: {
                    totalServers: 0, avgNormalizedCpu: 0, avgNormalizedMemory: 0,
                    avgNormalizedDisk: 0, avgNormalizedNetwork: 0, overallHealthScore: 0,
                    anomalyCount: 0, riskDistribution: { low: 0, medium: 0, high: 0, critical: 0 }
                },
                trends: {
                    cpuTrend: 'stable', memoryTrend: 'stable', diskTrend: 'stable',
                    networkTrend: 'stable', overallTrend: 'stable'
                },
                insights: {
                    criticalServers: [], anomalousServers: [], recommendations: [], predictedIssues: []
                }
            } : undefined,
            metadata: { processingTime: 0, cacheHit: false, dataQuality: 0, completeness: 0 }
        };
    }
