...
export class GCPSessionManager {
  ...
  async startSession(userId: string): Promise<string> {
    ...
    console.log(
      `🚀 세션 시작: ${sessionId} (사용자: ${userId}, 만료: ${new Date(endTime).toLocaleString()})`
    );
  ...
  async stopSession(sessionId: string): Promise<void> {
    ...
    console.log(
      `🛑 세션 정지: ${sessionId} (생성된 메트릭: ${session.metricsGenerated}개)`
    );
  }
  ...
  async updateSessionActivity(
    ...
    if (session.metricsGenerated >= this.limits.maxMetricsPerSession) {
      console.log(
        `⏰ 세션 ${sessionId} 메트릭 생성 제한 도달 (${session.metricsGenerated}개)`
      );
  ...
  private async checkDailyLimit(userId: string): Promise<void> {
    ...
    if (userCount >= this.limits.maxDailySessions) {
      throw new Error(
        `일일 세션 생성 제한 초과 (${this.limits.maxDailySessions}개)`
      );
    }
  }
  ...
  private async checkConcurrentLimit(): Promise<void> {
    ...
    if (activeCount >= this.limits.maxConcurrentSessions) {
      throw new Error(
        `동시 세션 제한 초과 (${this.limits.maxConcurrentSessions}개)`
      );
    }
  }
  ...
  async healthCheck(): Promise<{
    ...
    else if (activeSessions > this.limits.maxConcurrentSessions * 0.8)
      status = 'warning';
