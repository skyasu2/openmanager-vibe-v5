...
export class GCPServerDataGenerator {
  ...
  constructor(
    firestore: GCPFirestoreClient,
    cloudStorage: GCPCloudStorageClient
  ) {
  ...
  async generateRealtimeMetrics(
    ...
    if (elapsed > 20 * 60 * 1000) {
      console.log(
        `⏰ 세션 ${sessionId} 시간 만료 (${Math.round(elapsed / 1000)}초)`
      );
    ...
    const timeMultiplier =
      options.timeMultiplier || this.getTimeMultiplier(timestamp.getHours());

    console.log(
      `🎭 시나리오: ${scenario.type} (부하: ${scenario.loadMultiplier}x, 이상: ${scenario.anomalyChance})`
    );
    ...
    console.log(
      `✅ 메트릭 생성 완료: ${metrics.length}개 서버, ${Date.now() - startTime}ms`
    );
    return metrics;
  }
  ...
  private async generateServerMetrics(
    ...
    const baseline =
      (server as any).baseline_metrics || this.getBaselineMetrics(server.type);
    ...
    // 인프라 메트릭 생성
    const infrastructureMetrics = this.generateInfrastructureMetrics(
      server,
      finalMultiplier,
      scenario
    );
    ...
    if (scenario.type !== 'normal') {
      (metric as ExtendedTimeSeriesMetrics).anomalies = this.detectAnomalies(
        metric,
        scenario
      );
    }
  ...
  private generateSystemMetrics(
    ...
    return {
      ...
      memory: {
        ...
        available: Math.max(
          0,
          server.specs.memory.total - Math.floor(memoryUsed)
        ),
      ...
      disk: {
        io: {
          read: Math.max(
            0,
            baseline.disk.io.read * multiplier + (Math.random() - 0.5) * 50
          ),
          write: Math.max(
            0,
            baseline.disk.io.write * multiplier + (Math.random() - 0.5) * 25
          ),
        },
        throughput: {
        ...
        utilization: Math.max(
          0,
          Math.min(100, baseline.disk.utilization + (Math.random() - 0.5) * 10)
        ),
        queue: Math.floor(
          Math.random() * (scenario.type === 'critical' ? 10 : 3)
        ),
      },
      network: {
        io: {
          rx: Math.max(
            0,
            baseline.network.io.rx * multiplier + (Math.random() - 0.5) * 100000
          ),
          tx: Math.max(
            0,
            baseline.network.io.tx * multiplier + (Math.random() - 0.5) * 50000
          ),
        },
        packets: {
  ...
  private generateApplicationMetrics(
    ...
    return {
      ...
      database: {
        ...
        deadlocks:
          scenario.type === 'critical' ? Math.floor(Math.random() * 3) : 0,
      },
      cache: {
   ...
   * 📊 Vercel 연동 API
   */
  async fetchMetricsForVercel(
    sessionId: string,
    limit: number = 10
  ): Promise<GCPDataResponse> {
    try {
      const sessionInfo = await this.getSessionInfo(sessionId);
      if (!sessionInfo) {
        return {
  ...
  private async loadBaselineDataset(): Promise<void> {
    ...
    try {
      const file = this.cloudStorage
        .bucket('openmanager-baseline-data')
        .file('baseline-dataset.json');
  ...
  private async saveBaselineToFirestore(
  ...
  private async saveMetricsToFirestore(
    sessionId: string,
  ...
  private getRandomProcessName(serverType: string): string {
    ...
    const processes = processMap[serverType] || [
      'systemd',
      'sshd',
      'cron',
      'docker',
  ...
  private detectAnomalies(
    ...
    if (
      metric.system.memory.used /
        (metric.system.memory.used + metric.system.memory.available) >
      0.85
    ) {
      anomalies.push({
        type: 'high_memory',
   ...
   */
  private async addMetricsToBatch(
    sessionId: string,
    metrics: TimeSeriesMetrics[],
    scenario: ScenarioContext
  ): Promise<void> {
  ...
  async flushBatchToCloudStorage(sessionId: string): Promise<void> {
    ...
    try {
      ...
      const file = this.cloudStorage
        .bucket('openmanager-metrics-backup')
        .file(fileName);
      await file.save(JSON.stringify(batchData, null, 2));

      console.log(
        `💾 배치 데이터 Cloud Storage 저장: ${buffer.length}개 메트릭`
      );
  ...
  private async checkDailyBackup(): Promise<void> {
    ...
    try {
      ...
      const statsFile = this.cloudStorage
        .bucket('openmanager-daily-stats')
        .file(`stats-${new Date().toISOString().split('T')[0]}.json`);
  ...
  private async generateDailyStats(): Promise<any> {
    try {
      ...
      return {
        ...
        totalMetrics: sessionsSnapshot.docs.reduce(
          (sum, doc) => sum + (doc.data().metricsCount || 0),
          0
        ),
