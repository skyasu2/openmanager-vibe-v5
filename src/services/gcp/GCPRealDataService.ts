...
export class GCPRealDataService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `GCP 서비스 초기화 실패: ${error instanceof Error ? error.message : 'Unknown'}`
      );
    }
  }
    ...
    try {
      if (!env.IS_VERCEL) {
        ...
        return this.createErrorResponse(
          '로컬 환경에서는 GCP 실제 데이터를 사용할 수 없습니다'
        );
      }
  ...
  private createErrorResponse(errorMessage: string): GCPRealDataResponse {
    const errorServers: GCPServerMetrics[] = STATIC_ERROR_SERVERS.map(
      server => ({
        id: server.id,
        name: `🚨 ${server.name}`,
        type: 'ERROR',
    ...
    try {
      ...
      if (!hasGCPConfig) {
        return {
          success: false,
          error:
            'GCP 설정이 누락되었습니다 (GOOGLE_CLOUD_PROJECT, GOOGLE_APPLICATION_CREDENTIALS)',
        };
      }
