...
export class TDDGCPDataGenerator {
    ...
    private activeSessions: Map<string, { startTime: Date; autoStopTimer?: NodeJS.Timeout }> = new Map();
    ...
    async flushBatchToCloudStorage(sessionId: string): Promise<void> {
        ...
        await this.cloudStorage.file().save(JSON.stringify({
            sessionId,
            metrics: buffer,
     ...
     */
    generateScenarioMetrics(scenario: 'normal' | 'warning' | 'critical'): ServerMetric[] {
        ...
        return this.baselineDataset.servers.map(server => {
            ...
            switch (scenario) {
                case 'critical':
                    ...
                    memoryUsage = Math.random() > 0.1 ? 95 + Math.random() * 5 : memoryUsage;
                    break;

                case 'warning':
                    // 경고 시나리오: 70% 확률로 중간 사용률
                    cpuUsage = Math.random() > 0.3 ? 80 + Math.random() * 15 : cpuUsage;
                    memoryUsage = Math.random() > 0.3 ? 85 + Math.random() * 12 : memoryUsage;
                    break;
     ...
     * 세션 정지
     */
    async stopSession(sessionId: string, isAutoStop: boolean = false): Promise<void> {
     ...
     * 시간 경과 시뮬레이션 (테스트용)
     */
    async simulateTimeElapse(sessionId: string, milliseconds: number): Promise<void> {
        const session = this.activeSessions.get(sessionId);
        if (!session) return;
    ...
    async generateHistoricalPattern(
        ...
        while (currentDate <= end) {
            ...
            const dayData: HistoricalDataPoint = {
                ...
                metrics: {
                    averageCpuUsage: this.generateHistoricalMetric(baseLoad, variability, 100),
                    averageMemoryUsage: this.generateHistoricalMetric(baseLoad, variability, 100),
                    totalRequests: Math.floor(this.generateHistoricalMetric(baseLoad, variability, 10000)),
                    averageResponseTime: this.generateHistoricalMetric(0.5, 0.3, 200) + 50 // 50-250ms
                }
            };
     ...
     * 히스토리컬 메트릭 생성 헬퍼
     */
    private generateHistoricalMetric(baseLoad: number, variability: number, maxValue: number): number {
