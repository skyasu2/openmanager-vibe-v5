...
export class EnvAutoRecoveryService {
    ...
    private constructor() {
        ...
        console.log(`🔧 EnvAutoRecoveryService 초기화 ${this.isMockMode ? '(목업 모드)' : '(실제 모드)'}`);
    }
    ...
    validateEnvironment(): EnvValidationResult {
        ...
        // URL 형식 검증
        const urlVars = ['NEXT_PUBLIC_SUPABASE_URL', 'SUPABASE_URL', 'UPSTASH_REDIS_REST_URL', 'RENDER_MCP_SERVER_URL'];
        ...
        const criticalMissing = missing.filter(key => this.envConfig.critical.includes(key));
        const importantMissing = missing.filter(key => this.envConfig.important.includes(key));
     ...
     * 🔐 암호화된 환경변수 복구 시도
     */
    private async tryEncryptedRecovery(missingVars: string[]): Promise<EnvRecoveryResult> {
        try {
     ...
     * 💾 백업 파일에서 복구 시도
     */
    private async tryBackupRecovery(missingVars: string[]): Promise<EnvRecoveryResult> {
        try {
     ...
     * 🎯 기본값으로 복구 시도
     */
    private async tryDefaultRecovery(missingVars: string[]): Promise<EnvRecoveryResult> {
        try {
            const recovered: string[] = [];
    ...
    private detectExecutionContext(): void {
        ...
        // 목업 모드 결정
        this.isMockMode = this.isHealthCheckContext ||
            this.isTestContext ||
            process.env.FORCE_MOCK_REDIS === 'true';
    ...
    start(): void {
        ...
        console.log(`✅ EnvAutoRecoveryService 시작됨 (${this.CHECK_INTERVAL / 1000}초 간격)`);
    }
        ...
        try {
            ...
            if (missing.length > 0) {
                ...
                if (recoveryResult.success) {
                    console.log(`✅ 자동 복구 성공: ${recoveryResult.recovered.join(', ')}`);
                } else {
    ...
    private async attemptRecovery(missingVars: string[]): Promise<{
        ...
        try {
            ...
            return {
                ...
                recovered,
                method: 'defaults',
                message: recovered.length > 0 ? '기본값으로 복구 완료' : '복구 가능한 변수 없음'
            };
