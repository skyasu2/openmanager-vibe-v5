...
export class BrowserNotificationService {
  ...
  private async initialize(): Promise<void> {
    ...
    // 브라우저 환경에서만 permission 확인
    if (typeof window !== 'undefined' && 'Notification' in window) {
      this.permission = Notification.permission;
    }

    // 서비스 워커에서 알림 액션 처리 (SSR 호환)
    ...
    console.log(
      `🌐 브라우저 알림 서비스 초기화 완료 (권한: ${this.permission})`
    );
  }
  ...
  async sendServerAlert(
    ...
    if (metrics) {
      ...
      if (metrics.memory !== undefined)
        metricStrings.push(`메모리: ${metrics.memory}%`);
      if (metrics.disk !== undefined)
        metricStrings.push(`디스크: ${metrics.disk}%`);
