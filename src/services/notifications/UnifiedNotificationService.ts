...
export class UnifiedNotificationService {
    ...
    try {
      ...
      if (this.shouldSendToChannel('browser', notification)) {
        channelPromises.push(
          this.sendBrowserNotification(notification, result)
        );
      }
      ...
      if (this.shouldSendToChannel('console', notification)) {
        channelPromises.push(
          this.sendConsoleNotification(notification, result)
        );
      }
      ...
      // 성공 여부 결정
      result.success = Object.values(result.channels).some(
        channel => channel.sent
      );

      console.log(
        `✅ 통합 알림 처리 완료: ${notification.title} (${result.success ? '성공' : '실패'})`
      );
    } catch (error) {
  ...
  async info(
    ...
    const id =
      options.id ||
      `info-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    return this.sendNotification({
      id,
      type: 'custom',
      severity: 'info',
  ...
  async success(
    ...
    const id =
      options.id ||
      `success-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    return this.sendNotification({
      id,
      type: 'custom',
      severity: 'info',
  ...
  private async preprocessNotification(
    ...
    // 2. 시스템 시작 조용 모드
    if (
      this.settings.filters.systemStartupQuietMode &&
      notification.severity === 'info' &&
      (notification.message.includes('초기화') ||
        notification.message.includes('시작'))
    ) {
      return false;
    }
    ...
    // 3. 중복 방지
    if (
      this.settings.filters.enableDeduplication &&
      notification.deduplicationKey
    ) {
      const lastSent = this.recentNotifications.get(
        notification.deduplicationKey
      );
      const cooldownMs =
        notification.cooldownMs || this.settings.filters.defaultCooldown;
    ...
    // 4. 비율 제한
    const now = new Date();
    this.rateLimitBuffer = this.rateLimitBuffer.filter(
      time => now.getTime() - time.getTime() < 60000 // 1분 이내
    );

    if (
      this.rateLimitBuffer.length >=
      this.settings.filters.maxNotificationsPerMinute
    ) {
  ...
  private async sendBrowserNotification(
    ...
    try {
      const sent = await this.browserService.sendNotification({
        ...
        requireInteraction:
          notification.actionRequired || notification.severity === 'critical',
