...
export class SmartNotificationRouter {
    ...
    async sendAlert(
        ...
        console.log(`🔔 통합 알림 처리: ${alert.title} (심각도: ${alert.severity})`);
        ...
        console.log(`✅ 통합 알림 처리 완료: ${this.getSuccessCount(result)}개 채널 성공`);
        return result;
    }
    ...
    private selectActiveChannels(
        ...
        return {
            ...
            toast: preferences.channels.toast &&
                this.checkSeverityFilter(alert.severity, preferences.severityFilter.toast),

            // 브라우저 알림: 조용한 시간에는 심각한 알림만
            browser: preferences.channels.browser &&
                this.checkSeverityFilter(alert.severity, preferences.severityFilter.browser) &&
                (!isQuietTime || alert.severity === 'critical'),

            // 슬랙 알림: 조용한 시간에는 심각한 알림만  
            slack: preferences.channels.slack &&
                this.checkSeverityFilter(alert.severity, preferences.severityFilter.slack) &&
                (!isQuietTime || alert.severity === 'critical'),
    ...
    private async sendToastNotification(
        ...
        try {
            ...
            result.channels.toast.error = error instanceof Error ? error.message : 'Toast 전송 실패';
    ...
    private async sendBrowserNotification(
        ...
        try {
            ...
            result.channels.browser.error = error instanceof Error ? error.message : '브라우저 알림 실패';
    ...
    private async sendSlackNotification(
        ...
        try {
            ...
            result.channels.slack.error = error instanceof Error ? error.message : 'Slack 전송 실패';
    ...
    private async saveToDatabaseLog(
        ...
        try {
            ...
            result.channels.database.error = error instanceof Error ? error.message : 'DB 저장 실패';
     ...
     * ⏰ 쿨다운 체크
     */
    private shouldSendAlert(alert: UnifiedAlert, preferences: NotificationPreferences): boolean {
     ...
     * 📝 쿨다운 기록 업데이트
     */
    private updateCooldown(alert: UnifiedAlert, preferences: NotificationPreferences): void {
    ...
    private checkSeverityFilter(
        ...
        switch (filterLevel) {
            ...
            case 'warning': return alertSeverity === 'warning' || alertSeverity === 'critical';
    ...
    private getSuccessCount(result: NotificationResult): number {
        return Object.values(result.channels).filter(channel => channel.sent).length;
    }

    /**
     * 🛠️ 유틸리티 메서드들
     */
    private getPrimaryMetric(metrics: NonNullable<UnifiedAlert['metrics']>): string {
    ...
    private getPrimaryMetricValue(metrics: NonNullable<UnifiedAlert['metrics']>): number {
        return metrics.cpu || metrics.memory || metrics.disk || metrics.responseTime || 0;
    }

    private getThresholdForMetric(metrics: NonNullable<UnifiedAlert['metrics']>): number {
        // 기본 임계값들
