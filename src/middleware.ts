...
export function middleware(request: NextRequest) {
  ...
  if (isProduction) {
    ...
    if (isBlocked) {
      ...
      console.warn(`🚨 Security Alert: Blocked access to ${pathname} in production`, {
        ip: request.ip,
  ...
  if (pathname.startsWith('/api/dev/')) {
    ...
    if (!isDevelopment || !isLocalhost) {
      return NextResponse.json(
        {
          error: 'Forbidden',
          message: 'Development APIs are only available in local development environment',
  ...
  // 🚧 테스트 파일 접근 로깅 (개발 환경)
  if (isDevelopment && (pathname.startsWith('/tests/') || pathname.includes('test-'))) {
...
export const config = {
  matcher: [
     ...
     * - favicon.ico
     */
    '/((?!_next/static|_next/image|favicon.ico|.*\\.(?:css|js|jpg|jpeg|png|gif|svg|ico|woff|woff2)$).*)',
  ],
};