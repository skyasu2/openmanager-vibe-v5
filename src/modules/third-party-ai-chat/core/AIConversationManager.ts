...
export class AIConversationManager {
    ...
    private initializeProviders() {
        ...
        this.providers.set('google', {
            ...
            apiKey: process.env.GOOGLE_AI_API_KEY || 'AIzaSyABFUHbGGtjs6S_y756H4SYJmFNuNoo3fY',
     ...
     * 새로운 대화 세션 시작
     */
    async startConversation(providerName: string, title?: string): Promise<string> {
        ...
        if (!provider.enabled) {
            throw new Error(`❌ ${provider.name}이 비활성화되어 있습니다. API 키를 확인해주세요.`);
        }

        const sessionId = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        const session: ConversationSession = {
            id: sessionId,
            title: title || `${provider.name} 대화 - ${new Date().toLocaleString('ko-KR')}`,
     ...
     * AI와 메시지 주고받기
     */
    async sendMessage(message: string, sessionId?: string): Promise<ConversationMessage> {
        const targetSessionId = sessionId || this.currentSessionId;
        if (!targetSessionId) {
            throw new Error('❌ 활성 대화 세션이 없습니다. 먼저 대화를 시작해주세요.');
        }
        ...
        try {
            ...
            console.log(`💬 [${provider.name}] 응답 완료 (${processingTime}ms, ${aiResponse.tokensUsed} 토큰)`);
     ...
     * AI 제공자별 API 호출
     */
    private async callAIProvider(provider: AIProvider, messages: ConversationMessage[]): Promise<{
        content: string;
        tokensUsed: number;
        confidence: number;
    }> {
     ...
     * Google AI 호출
     */
    private async callGoogleAI(provider: AIProvider, messages: ConversationMessage[]): Promise<{
        content: string;
        tokensUsed: number;
        confidence: number;
    }> {
        ...
        const content = data.candidates?.[0]?.content?.parts?.[0]?.text || '응답을 받을 수 없습니다.';
     ...
     * OpenAI 호출
     */
    private async callOpenAI(provider: AIProvider, messages: ConversationMessage[]): Promise<{
        content: string;
        tokensUsed: number;
        confidence: number;
    }> {
        ...
        const data = await response.json();
        const content = data.choices?.[0]?.message?.content || '응답을 받을 수 없습니다.';
     ...
     * Anthropic 호출
     */
    private async callAnthropic(provider: AIProvider, messages: ConversationMessage[]): Promise<{
        content: string;
        tokensUsed: number;
        confidence: number;
    }> {
     ...
     * Cohere 호출
     */
    private async callCohere(provider: AIProvider, messages: ConversationMessage[]): Promise<{
        content: string;
        tokensUsed: number;
        confidence: number;
    }> {
     ...
     * 대화 기록 내보내기
     */
    exportConversation(sessionId: string, format: 'json' | 'text' = 'json'): string {
