...
export class FailureAnalyzer {
   ...
   * 낮은 신뢰도 응답 분석
   */
  async analyzeLowConfidenceResponses(
    ...
    const cutoffDate = new Date(
      Date.now() - this.config.analysisTimeWindow * 24 * 60 * 60 * 1000
    );
    ...
    const averageConfidence =
      recentInteractions.reduce((sum, i) => sum + i.confidence, 0) /
      totalInteractions;
    ...
    console.log(
      `🔍 [FailureAnalyzer] 낮은 신뢰도 분석 완료 (${processingTime}ms):`,
      {
        totalInteractions,
        lowConfidenceCount,
   ...
   * 부정적 피드백 패턴 분석
   */
  async analyzeNegativeFeedbackPatterns(
    interactions: UserInteractionLog[]
    ...
    const improvementOpportunities =
      this.identifyImprovementOpportunities(negativeInteractions);
   ...
   * 미처리 질문 유형 식별
   */
  async identifyUnhandledQuestionTypes(
    interactions: UserInteractionLog[]
  ): Promise<QuestionType[]> {
    // 낮은 신뢰도 또는 부정적 피드백을 받은 질문들
    const problematicInteractions = interactions.filter(
      i =>
        i.confidence < this.config.lowConfidenceThreshold ||
        i.userFeedback === 'not_helpful' ||
        i.userFeedback === 'incorrect'
    );

    // 질문 그룹핑
    const questionGroups = this.groupSimilarQuestions(
      problematicInteractions.map(i => i.query)
    );
    ...
    interactions.forEach(interaction => {
      ...
      if (existing) {
        ...
        patternMap.set(pattern, {
          ...
          suggestedImprovement: this.generateImprovementSuggestion(
            pattern,
            interaction
          ),
  ...
  private extractPatternFromQuery(query: string): string {
    ...
    const foundServerKeywords = serverKeywords.filter(keyword =>
      normalizedQuery.includes(keyword)
    );
    const foundActionKeywords = actionKeywords.filter(keyword =>
      normalizedQuery.includes(keyword)
    );

    // 패턴 생성
    const serverPart =
      foundServerKeywords.length > 0 ? foundServerKeywords[0] : 'general';
    const actionPart =
      foundActionKeywords.length > 0 ? foundActionKeywords[0] : 'query';
  ...
  private groupSimilarQuestions(questions: string[]): QuestionGroup[] {
    ...
    questions.forEach(question => {
      ...
      if (groups.has(pattern)) {
        ...
        group.frequency++;
        group.commonKeywords = this.mergeKeywords(
  ...
  private categorizePattern(pattern: string): string {
    ...
    if (pattern.includes('에러') || pattern.includes('장애'))
      return '장애 대응';
    return '일반 질의';
  }
  ...
  private calculateEstimatedImpact(
    negativeInteractions: UserInteractionLog[],
    allInteractions: UserInteractionLog[]
  ): number {
  ...
  private generateSuggestedPatterns(group: QuestionGroup): string[] {
    ...
    // 카테고리 기반 패턴
    patterns.push(
      `${group.suggestedCategory.replace(/\s+/g, '_').toLowerCase()}_.*`
    );
  ...
  private generateRecommendations(
    ...
    if (patterns.length > 0) {
      recommendations.push(
        `${patterns.length}개의 실패 패턴이 발견되었습니다. 우선순위에 따라 개선이 필요합니다.`
      );
    }

    const lowConfidenceRate =
      interactions.length > 0
        ? (interactions.filter(i => i.confidence < 0.6).length /
            interactions.length) *
          100
        : 0;
    ...
    if (lowConfidenceRate > 30) {
      recommendations.push(
        '낮은 신뢰도 응답이 30% 이상입니다. 패턴 매칭 알고리즘 개선이 필요합니다.'
      );
    }

    if (patterns.some(p => p.frequency > 10)) {
      recommendations.push(
        '빈번한 실패 패턴이 있습니다. 해당 패턴에 대한 전용 응답 템플릿 추가를 고려하세요.'
      );
    }
