...
export class AIResponseService {
  ...
  private categoryKeywords: CategoryKeywords = {
    monitoring: ['상태', '모니터링', '서버', '시스템', '헬스체크', 'monitoring', 'status', 'health'],
    analysis: ['분석', '통계', '리포트', '데이터', 'analysis', 'report', 'analytics', 'metrics'],
    prediction: ['예측', '미래', '전망', '예상', 'prediction', 'forecast', 'future', 'trend'],
    incident: ['장애', '문제', '오류', '에러', '알림', 'incident', 'error', 'issue', 'alert', 'problem']
  };
  ...
  async callActualAIFunction(
    ...
    try {
      ...
      return {
        success: false,
        answer: '죄송합니다. AI 기능 처리 중 오류가 발생했습니다. 다시 시도해 주세요.',
        error: error.message,
      };
    }
  }
  ...
  private async handleMonitoringQuery(question: string, sessionId: string) {
    ...
    const template: AIResponseTemplate = {
      ...
      analysis: '현재 모든 서버가 정상적으로 운영되고 있으며, CPU 사용률 평균 45%, 메모리 사용률 평균 67% 수준입니다.',
  ...
  private async handleAnalysisQuery(question: string, sessionId: string) {
    ...
    const template: AIResponseTemplate = {
      ...
      analysis: '최근 7일간의 성능 데이터를 분석한 결과, 전체적으로 안정적인 트렌드를 보이고 있습니다. 피크 시간대는 오후 2-4시이며, 이 시간에 CPU 사용률이 약 15% 증가합니다.',
      conclusion: '현재 시스템 성능은 최적화된 상태이며, 향후 3개월간 현재 용량으로 충분할 것으로 예상됩니다.',
      recommendations: [
        '피크 시간대 리소스 모니터링 강화',
        '캐싱 전략 검토',
        '자동 스케일링 옵션 고려'
  ...
  private async handlePredictionQuery(question: string, sessionId: string) {
    ...
    const template: AIResponseTemplate = {
      ...
      analysis: '향후 72시간 동안의 시스템 부하를 예측한 결과, 평균 부하는 현재 수준을 유지할 것으로 예상됩니다. 다만, 금요일 오후에 약간의 트래픽 증가가 예상됩니다.',
      conclusion: '전반적으로 안정적인 운영이 예상되며, 특별한 대응이 필요한 상황은 없을 것으로 판단됩니다.',
      recommendations: [
        '금요일 오후 모니터링 강화',
        '예방적 스케일링 준비',
        '백업 시스템 대기 상태 유지'
  ...
  private async handleIncidentQuery(question: string, sessionId: string) {
    ...
    const template: AIResponseTemplate = {
      ...
      analysis: '현재 활성화된 장애나 경고 상황은 발견되지 않았습니다. 모든 시스템이 정상적으로 작동하고 있으며, 네트워크 연결도 안정적입니다.',
      conclusion: '시스템 상태가 양호하여 즉각적인 조치가 필요한 상황은 없습니다.',
      recommendations: [
        '정기적인 헬스체크 지속',
        '로그 모니터링 강화',
        '장애 대응 절차 숙지'
  ...
  private async handleGeneralQuery(question: string, sessionId: string) {
    ...
    const template: AIResponseTemplate = {
      ...
      analysis: 'OpenManager Vibe 시스템과 관련된 다양한 기능과 정보를 제공할 수 있습니다. 구체적인 요청사항이 있으시면 더 자세한 도움을 드릴 수 있습니다.',
      conclusion: '추가 질문이나 구체적인 요청사항이 있으시면 언제든 말씀해 주세요.',
      recommendations: [
        '구체적인 질문으로 재질의',
        '시스템 기능 탐색',
        '도움말 문서 참조'
