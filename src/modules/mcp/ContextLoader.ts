...
export class ContextLoader {
  ...
  async loadMergedContext(clientId?: string): Promise<MergedContext> {
    // 캐시 확인
    if (this.cachedContext && this.lastLoadTime && 
    ...
    try {
      ...
      let mergedKnowledge = '';
      const mergedPatterns: any = { intentPatterns: {}, responseTemplates: {}, keywordExtraction: {}, validationRules: {} };
      ...
      if (baseContext) {
        mergedKnowledge += baseContext.documents.markdown['core-knowledge'] || '';
      ...
      if (clientId) {
        ...
        if (customContext) {
          ...
          console.log(`✅ [ContextLoader] Custom 컨텍스트 로드 완료 (${clientId})`);
        }
      }

      // 템플릿 및 인텐트 매핑 추출
      this.extractTemplatesAndIntents(mergedPatterns, mergedTemplates, mergedIntentMappings);
      ...
      console.log(`✅ [ContextLoader] 컨텍스트 병합 완료: ${sources.join(' → ')}`);
   ...
   * 특정 번들 로드
   */
  private async loadContextBundle(bundleType: 'base' | 'advanced' | 'custom', clientId?: string): Promise<ContextBundle | null> {
    try {
      const bundlePath = clientId && bundleType === 'custom' 
        ? path.join(this.documentsPath, bundleType, clientId)
        : path.join(this.documentsPath, bundleType);

      if (!fs.existsSync(bundlePath)) {
        console.log(`📁 [ContextLoader] 번들 경로가 존재하지 않음: ${bundlePath}`);
        return null;
      }
      ...
      // Markdown 파일 로드
      const markdownFiles = fs.readdirSync(bundlePath).filter(file => file.endsWith('.md'));
      ...
      // JSON 패턴 파일 로드
      const jsonFiles = fs.readdirSync(bundlePath).filter(file => file.endsWith('.json'));
      ...
      console.error(`❌ [ContextLoader] 번들 로드 실패 (${bundleType}):`, error);
      return null;
    }
  }
   ...
   * 템플릿 및 인텐트 매핑 추출
   */
  private extractTemplatesAndIntents(patterns: any, templates: Record<string, string>, intents: Record<string, string>): void {
    // 응답 템플릿 추출
    if (patterns.responseTemplates) {
      for (const [key, template] of Object.entries(patterns.responseTemplates)) {
   ...
   * 컨텍스트 번들 업로드 (관리자 전용)
   */
  async uploadContextBundle(bundleType: 'base' | 'advanced' | 'custom', bundleData: any, clientId?: string): Promise<boolean> {
    try {
      ...
      // 새 번들 파일 저장
      for (const [fileName, content] of Object.entries(bundleData.documents.markdown)) {
        const filePath = path.join(targetPath, `${fileName}.md`);
        fs.writeFileSync(filePath, content as string, 'utf-8');
      }

      for (const [fileName, content] of Object.entries(bundleData.documents.patterns)) {
      ...
      console.log(`✅ [ContextLoader] 컨텍스트 번들 업로드 완료: ${bundleType}${clientId ? `-${clientId}` : ''}`);
      return true;
  ...
  async getAvailableBundles(): Promise<{
    ...
    try {
      ...
      if (fs.existsSync(basePath)) {
        result.base = fs.readdirSync(basePath).filter(file => 
      ...
      if (fs.existsSync(advancedPath)) {
        result.advanced = fs.readdirSync(advancedPath).filter(file => 
          file.endsWith('.md') || file.endsWith('.json')
        );
      }
      ...
      if (fs.existsSync(customPath)) {
        const clientDirs = fs.readdirSync(customPath).filter(item => 
        ...
        for (const clientId of clientDirs) {
          ...
          result.custom[clientId] = fs.readdirSync(clientPath).filter(file => 
            file.endsWith('.md') || file.endsWith('.json')
          );
        }
      }
