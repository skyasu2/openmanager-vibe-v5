...
export class RealisticPatternEngine {
  ...
  generateRealisticMetric(
    metricType: 'cpu_usage' | 'memory_usage' | 'disk_usage' | 'network_in' | 'network_out' | 'response_time',
    serverType: string,
    timestamp: Date,
    previousMetrics?: any
  ): number {
    const profile = this.serverProfiles.get(serverType) || this.serverProfiles.get('web')!;
    ...
    // 버스트 이벤트 체크
    const burstMultiplier = this.checkBurstEvent(profile) ? profile.peak_multiplier : 1.0;
    
    // 기본 계산
    let value = baseValue * timeMultiplier * seasonalMultiplier * burstMultiplier;
    ...
    if (previousMetrics) {
      switch (metricType) {
        ...
        case 'response_time':
          if (previousMetrics.cpu_usage) {
            const responseBase = serverType === 'cache' ? 5 : serverType === 'database' ? 50 : 100;
            const cpuImpact = (previousMetrics.cpu_usage - 30) * Math.abs(profile.correlation.cpu_response_time) * 20;
    ...
    const randomVariation = (Math.random() - 0.5) * volatilityRange * (1 - stabilityFactor);
    ...
    } else if (serverType === 'kubernetes' && metricType === 'cpu_usage') {
   ...
   * 📊 전체 패턴 요약 정보
   */
  getPatternSummary(serverType: string, timestamp: Date): {
