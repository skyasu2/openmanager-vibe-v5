...
export interface ServerBaselineData {
  ...
  role: 'web' | 'api' | 'database' | 'cache' | 'worker' | 'k8s-worker' | 'k8s-control' | 'storage' | 'load-balancer' | 'backup';
...
export class BaselineOptimizer {
  ...
  private getServerBaseLoad(role: string, status: string): BaselineConfig {
    ...
    // 상태별 조정
    const statusMultiplier = status === 'critical' ? 0.3 : status === 'warning' ? 0.7 : 1.0;
  ...
  private updateBaselinePattern(baseline: ServerBaselineData): ServerBaselineData {
    // 기존 패턴에 약간의 변화를 주어 업데이트
    const updatedPattern = baseline.daily_pattern.map(point => ({
      ...point,
      pattern_multiplier: point.pattern_multiplier * (0.95 + Math.random() * 0.1) // ±5% 변화
    }));
  ...
  getBaselineStats() {
    return {
      ...
      oldestBaseline: Math.min(...Array.from(this.baselineStorage.values()).map(b => b.last_generated)),
