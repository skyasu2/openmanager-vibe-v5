...
export class QueryComplexityAnalyzer {
  ...
  private static readonly KOREAN_KEYWORDS = [
    '분석', '비교', '예측', '추천', '설명', '원인', '해결', '최적화',
    '검토', '진단', '평가', '조사', '연구', '탐색', '발견', '식별',
  ];
  ...
  private static readonly TECHNICAL_KEYWORDS = [
    'analyze', 'compare', 'predict', 'recommend', 'explain', 'cause', 'solve', 'optimize',
    'review', 'diagnose', 'evaluate', 'investigate', 'research', 'explore', 'discover', 'identify',
    'performance', 'memory', 'cpu', 'network', 'error', 'bug', 'issue', 'problem',
  ];
  ...
  private static readonly MULTI_STEP_INDICATORS = [
    '그리고', '그다음', '이후', '다음으로', '그런데', '또한', '뿐만아니라',
    'and then', 'after that', 'next', 'also', 'furthermore', 'moreover', 'additionally',
    '1.', '2.', '3.', 'step', 'first', 'second', 'finally',
  ];
  
  /**
   * 🎯 Main Analysis Method
   */
  static analyze(query: string, options: AnalysisOptions = {}): QueryComplexity {
    const { hasContext, requiresRAG, requiresML, requiresNLP, customKeywords } = options;
    ...
    const totalScore = lengthScore + keywordScore + contextScore + multiStepScore;
    ...
    const providerTime = this.calculateProviderTime(requiresRAG, requiresML, requiresNLP);
   ...
   */
  private static calculateKeywordScore(query: string, customKeywords?: string[]): number {
    const lowerQuery = query.toLowerCase();
    let keywordCount = 0;
    
    // Check Korean keywords
    for (const keyword of this.KOREAN_KEYWORDS) {
      if (query.includes(keyword)) keywordCount++;
    }
    
    // Check technical keywords
    for (const keyword of this.TECHNICAL_KEYWORDS) {
  ...
  private static calculateMultiStepScore(query: string): number {
    ...
    for (const indicator of this.MULTI_STEP_INDICATORS) {
  ...
  static detectProviderRequirements(query: string): {
    ...
    const requiresNLP = 
      /[\uac00-\ud7af]/.test(query) && query.length > 20; // Korean text with significant length
   ...
   */
  static smartAnalyze(query: string, options: Omit<AnalysisOptions, 'requiresRAG' | 'requiresML' | 'requiresNLP'> = {}): QueryComplexity {
  ...
  static getTimeout(query: string, hasContext: boolean = false): number {
    const complexity = this.smartAnalyze(query, { hasContext });
 ...
 */
export function calculateDynamicTimeout(query: string, hasContext: boolean = false): number {
 ...
 */
export function analyzeQueryComplexity(query: string, options?: AnalysisOptions): QueryComplexity {
