 ...
 *
 * OpenManager v5.44.3 - 데이터 보존 스케줄러 (2025-01-04 18:10 KST)
...
class DataRetentionScheduler {
    ...
    private constructor() {
        ...
        this.startScheduler();
        console.log('🗂️ DataRetentionScheduler 초기화 완료 (2025-01-04 18:10 KST)');
    }
    ...
    private async cleanupMetrics(policy: RetentionPolicy): Promise<{ itemsRemoved: number; sizeFreed: number }> {
        ...
        if (typeof window !== 'undefined' && window.localStorage) {
            const keys = Object.keys(localStorage).filter(key =>
                key.includes('metrics') || key.includes('server-data')
            );
            ...
            for (const key of keys) {
                try {
                    ...
                    if (data) {
                        ...
                        if (parsed.timestamp && Date.now() - parsed.timestamp > policy.maxAge) {
    ...
    private async cleanupAlerts(policy: RetentionPolicy): Promise<{ itemsRemoved: number; sizeFreed: number }> {
        ...
        if (typeof window !== 'undefined' && window.localStorage) {
            const keys = Object.keys(localStorage).filter(key =>
                key.includes('alert') || key.includes('notification')
            );
    ...
    private async cleanupConnections(policy: RetentionPolicy): Promise<{ itemsRemoved: number; sizeFreed: number }> {
        ...
        if (typeof window !== 'undefined' && window.localStorage) {
            const keys = Object.keys(localStorage).filter(key =>
                key.includes('connection') || key.includes('session')
            );
            ...
            for (const key of keys) {
                try {
                    ...
                    if (data) {
                        const parsed = JSON.parse(data);
                        if (parsed.lastActivity && Date.now() - parsed.lastActivity > policy.maxAge) {
                            localStorage.removeItem(key);
                            itemsRemoved++;
                            sizeFreed += data.length;
                        }
    ...
    private async cleanupCache(policy: RetentionPolicy): Promise<{ itemsRemoved: number; sizeFreed: number }> {
        ...
        if (typeof window !== 'undefined' && window.localStorage) {
            const keys = Object.keys(localStorage).filter(key =>
                key.includes('cache') || key.includes('temp') || key.includes('buffer')
            );
            ...
            for (const key of keys) {
                try {
                    ...
                    if (data) {
                        const parsed = JSON.parse(data);
                        if (parsed.cacheTime && Date.now() - parsed.cacheTime > policy.maxAge) {
                            localStorage.removeItem(key);
                            itemsRemoved++;
                            sizeFreed += data.length;
                        }
     ...
     */
    private async cleanupSSE(policy: RetentionPolicy): Promise<{ itemsRemoved: number; sizeFreed: number }> {
        let itemsRemoved = 0;
        let sizeFreed = 0;

        if (typeof window !== 'undefined' && window.localStorage) {
            const keys = Object.keys(localStorage).filter(key =>
                key.includes('sse') || key.includes('eventsource') || key.includes('realtime')
            );
     ...
     * 🔧 일반 데이터 정리
     */
    private async cleanupGeneric(policy: RetentionPolicy): Promise<{ itemsRemoved: number; sizeFreed: number }> {
    ...
    private updateMemoryUsage(): void {
        if (typeof window !== 'undefined' && 'performance' in window && 'memory' in (performance as any)) {
            ...
            this.stats.memoryUsageMB = Math.round(memory.usedJSHeapSize / 1024 / 1024);
        }
    }
    ...
    private updateActivePolicies(): void {
        this.stats.activePolicies = Array.from(this.policies.values()).filter(p => p.enabled).length;
    }
