...
export class AgentProgressTracker {
  ...
  displayCurrentStatus(): string {
    ...
    const activeTasks = Array.from(this.tasks.values())
      .filter(task => ['starting', 'in_progress', 'checkpoint'].includes(task.status));
    ...
    if (activeTasks.length > 0) {
      ...
      activeTasks.forEach((task, index) => {
        ...
        const duration = task.startTime ? this.formatDuration(new Date().getTime() - task.startTime.getTime()) : '0초';
        
        output.push(`${index + 1}. ${config.color}${config.icon} ${task.agentType}${RESET_COLOR} ${createProgressBar(task.progress.percentage)}`);
        output.push(`   - 상태: ${task.progress.currentStep}`);
        output.push(`   - 시작: ${duration} 전`);
        
        if (task.progress.estimatedTimeLeft) {
          output.push(`   - 예상 완료: ${this.formatDuration(task.progress.estimatedTimeLeft * 1000)} 후`);
        }
        output.push('');
      });
    }
    ...
    const completedTasks = Array.from(this.tasks.values())
      .filter(task => ['completed', 'failed', 'timeout'].includes(task.status));
    ...
    if (completedTasks.length > 0 && this.displayMode === 'detailed') {
      ...
      completedTasks.forEach(task => {
        ...
        const duration = task.startTime && task.endTime 
          ? this.formatDuration(task.endTime.getTime() - task.startTime.getTime())
          : 'N/A';
  ...
  getParallelDashboard(): string {
    const output: string[] = [];
    const groups = Array.from(this.parallelGroups.values())
    ...
    groups.forEach(group => {
      ...
      group.tasks.forEach(task => {
        const config = STATUS_CONFIG[task.status];
        const progress = task.status === 'in_progress' ? ` - ${task.progress.percentage}%` : '';
  ...
  private startUpdateLoop(): void {
    ...
    this.updateInterval = setInterval(() => {
      ...
      const hasActiveTasks = Array.from(this.tasks.values())
        .some(task => ['starting', 'in_progress'].includes(task.status));
