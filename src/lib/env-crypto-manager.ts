...
export class EnvironmentCryptoManager {
    ...
    async unlockEnvironmentVars(teamPassword: string): Promise<{
        ...
        try {
            ...
            if (!environmentData) {
                return {
                    success: false,
                    error: '암호화된 환경변수 설정을 찾을 수 없습니다. 관리자에게 문의하세요.',
                };
            }
            ...
            // 4. 모든 환경변수 복호화
            const decryptedCount = await this.decryptAllVariables(environmentData.variables, teamPassword);
    ...
    private async loadFromSupabase(): Promise<EnvironmentData | null> {
        ...
        try {
            ...
            if (error) {
                console.warn('⚠️ Supabase에서 환경변수를 찾을 수 없습니다:', error.message);
                return null;
            }
     ...
     * 🔓 단일 값 복호화
     */
    private decryptValue(encryptedData: EncryptedEnvVar, password: string): string {
        try {
