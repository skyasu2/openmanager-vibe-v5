...
import { enhancedCryptoManager, type EncryptedEnvConfig } from '@/lib/crypto/EnhancedEnvCryptoManager';
...
class EncryptedEnvLoader {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      if (!masterPassword) {
        console.log('⚠️ ENV_MASTER_PASSWORD가 설정되지 않음 - 일반 환경변수 사용');
        return false;
      }
  ...
  private async loadEncryptedConfig(): Promise<EncryptedEnvConfig | null> {
    try {
      ...
      return configModule.encryptedEnvConfig;
    } catch (error) {
      // 파일이 없거나 import 실패
      return null;
    }
...
export const secureEnv = {
  ...
  GOOGLE_AI_MODEL: () => encryptedEnvLoader.getEnv('GOOGLE_AI_MODEL') || 'gemini-1.5-flash',
  ...
  // Supabase
  NEXT_PUBLIC_SUPABASE_URL: () => encryptedEnvLoader.getEnv('NEXT_PUBLIC_SUPABASE_URL'),
  NEXT_PUBLIC_SUPABASE_ANON_KEY: () => encryptedEnvLoader.getEnv('NEXT_PUBLIC_SUPABASE_ANON_KEY'),
  SUPABASE_SERVICE_ROLE_KEY: () => encryptedEnvLoader.getEnv('SUPABASE_SERVICE_ROLE_KEY'),
  
  // Redis
  UPSTASH_REDIS_REST_URL: () => encryptedEnvLoader.getEnv('UPSTASH_REDIS_REST_URL'),
  UPSTASH_REDIS_REST_TOKEN: () => encryptedEnvLoader.getEnv('UPSTASH_REDIS_REST_TOKEN'),
 ...
 * (서버 사이드에서만 실행)
 */
if (typeof window === 'undefined' && process.env.AUTO_LOAD_ENCRYPTED_ENV !== 'false') {
