...
class FeatureExtractor {
    ...
    static extractQueryFeatures(queryData: any): number[] {
        ...
        // 카테고리 특성 (키워드 기반)
        const categories = [
            'status', 'performance', 'error', 'log', 'system',
            'ai', 'engine', 'database', 'network', 'memory',
            'cpu', 'disk', 'alert', 'monitoring', 'analysis'
        ];
...
export class LightweightMLEngine {
    ...
    async trainLinearRegression(
        ...
        for (let iteration = 0; iteration < this.config.maxIterations; iteration++) {
            ...
            // 가중치 업데이트 (정규화 포함)
            for (let j = 0; j < featureCount; j++) {
                weights[j] -= this.config.learningRate * (
            ...
            if (avgError < this.config.convergenceThreshold) {
                console.log(`✅ 모델 수렴: ${iteration}회 반복, 오차: ${avgError.toFixed(6)}`);
                break;
            }
        }

        // 정확도 계산
        const predictions = features.map(f => MathUtils.dotProduct(f, weights) + bias);
        const mse = labels.reduce((sum, label, i) =>
            sum + Math.pow(label - predictions[i], 2), 0) / labels.length;
        ...
        console.log(`📊 선형 회귀 모델 '${modelName}' 훈련 완료 - 정확도: ${(accuracy * 100).toFixed(2)}%`);
    }
    ...
    async trainLogisticRegression(
        ...
        console.log(`🎯 로지스틱 회귀 모델 '${modelName}' 훈련 완료 - 정확도: ${(accuracy * 100).toFixed(2)}%`);
    }
    ...
    async trainKMeansClustering(
        ...
        console.log(`🔍 K-Means 클러스터링 모델 '${modelName}' 훈련 완료 - ${iterations}회 반복`);
    }
    ...
    // 예측 수행
    async predict(modelName: string, features: number[]): Promise<PredictionResult | null> {
        ...
        switch (model.type) {
            case 'regression':
                prediction = MathUtils.dotProduct(features, model.weights!) + (model.bias || 0);
                confidence = Math.min(1, model.accuracy || 0.5);
                explanation = `선형 회귀 예측값: ${prediction.toFixed(2)}`;
                break;

            case 'classification':
                const z = MathUtils.dotProduct(features, model.weights!) + (model.bias || 0);
    ...
    async learnFromIncidentReports(incidentReports: any[]): Promise<void> {
        console.log(`📚 자동 장애보고서 학습 시작: ${incidentReports.length}개 보고서`);
        ...
        // 여러 모델 훈련
        await this.trainLinearRegression('incident-severity-predictor', trainingData);
    ...
    async learnFromQueryLogs(queryLogs: any[]): Promise<void> {
        ...
        await this.trainLinearRegression('query-response-time-predictor', responseTimeData);
    ...
    async learnFromPerformanceData(performanceData: any[]): Promise<void> {
        ...
        await this.trainAnomalyDetection('performance-anomaly-detector', trainingData);
