/**
 * 🚀 Supabase Vector RAG Engine v2.0
...
export class SupabaseRAGEngine {
    ...
    private createSupabaseClient() {
        ...
        if (!supabaseUrl || !supabaseKey) {
            ...
            // 암호화된 환경변수에서 복원
            supabaseUrl = process.env.ENCRYPTED_SUPABASE_URL || 'https://vnswjnltnhpsueosfhmw.supabase.co';
            supabaseKey = process.env.ENCRYPTED_SUPABASE_KEY ||
        ...
        if (!supabaseUrl || !supabaseKey) {
            throw new Error('❌ Supabase 환경변수 2회 점검 실패 - Vercel 환경변수 설정을 확인해주세요');
        }
    ...
    private async _performInitialization(): Promise<void> {
        try {
            ...
            if (countError) {
                console.warn('⚠️ 테이블 접근 오류, 자동 생성 시도:', countError.message);
    ...
    private async performConnectionCheck(): Promise<void> {
        ...
        try {
            ...
            if (firstCheck && firstCheck.code !== '42P01') { // 테이블 없음 오류가 아닌 경우
                console.warn('⚠️ 1차 연결 점검 실패:', firstCheck.message);

                // 2차 점검: 다른 방식으로 연결 확인
                const { error: secondCheck } = await this.supabase
                    .rpc('version'); // PostgreSQL 버전 확인
    ...
    private async loadAndVectorizeCommands(): Promise<void> {
        try {
            ...
            const sampleCommands = [
                {
                    id: 'linux-top-001',
                    content: 'top 명령어는 실시간으로 실행 중인 프로세스를 모니터링하는 도구입니다. CPU 사용률, 메모리 사용률, 프로세스 상태 등을 확인할 수 있습니다.',
                    metadata: {
                ...
                {
                    id: 'k8s-pod-001',
                    content: 'kubectl get pods 명령어로 Kubernetes 클러스터의 모든 Pod 상태를 확인할 수 있습니다. CrashLoopBackOff 상태는 Pod가 계속 재시작되는 문제를 나타냅니다.',
                    metadata: {
                ...
                },
                {
                    ...
                    content: 'MySQL 데이터베이스 연결 문제 해결 방법: 1) 서비스 상태 확인, 2) 포트 접근성 확인, 3) 사용자 권한 확인, 4) 방화벽 설정 확인',
                    metadata: {
            ...
            console.log(`📚 ${sampleCommands.length}개 샘플 문서 벡터화 시작 (로컬 임베딩)...`);
    ...
    async searchSimilar(
        ...
        try {
            ...
            try {
                const { data: rpcResults, error: rpcError } = await this.supabase
                ...
                if (rpcError) {
                    ...
                    searchResults = await this.fallbackSearch(query, queryEmbedding, options);
                } else {
