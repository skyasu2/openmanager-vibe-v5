...
export class AdvancedContextManager {
    ...
    try {
      ...
      for (const docPath of documentPaths) {
        try {
          ...
          contextCache.categories.set(category, (contextCache.categories.get(category) || 0) + 1);
          
          console.log(`✅ [AdvancedContext] 문서 처리 완료: ${embedding.title}`);
        } catch (error) {
          console.error(`❌ [AdvancedContext] 문서 처리 실패: ${docPath}`, error);
        }
      }
      ...
      console.log(`🎉 [AdvancedContext] 문서 임베딩 완료: ${contextCache.totalDocuments}개 문서`);
    } catch (error) {
   ...
   * 📊 카테고리 결정
   */
  private determineCategory(filePath: string, content: string): DocumentEmbedding['metadata']['category'] {
    ...
    if (content.includes('FAQ') || content.includes('Q&A') || content.includes('질문')) {
      return 'faq';
    }
  ...
  private calculateImportance(filePath: string, content: string): number {
    ...
    if (filePath.includes('IMPORTANT') || filePath.includes('CRITICAL')) importance += 2;
    if (filePath.includes('archive') || filePath.includes('backup')) importance -= 2;
    
    // 내용 기반 가중치
    const contentLower = content.toLowerCase();
    if (contentLower.includes('중요') || contentLower.includes('critical')) importance += 1;
    if (contentLower.includes('deprecated') || contentLower.includes('삭제')) importance -= 1;
   ...
   * ❓ FAQ 생성
   */
  private async generateFAQs(contextCache: AdvancedContextCache): Promise<void> {
    ...
    const logDocs = Array.from(contextCache.documents.values())
      .filter(doc => doc.metadata.category === 'log');
    ...
    console.log(`✅ [AdvancedContext] ${contextCache.faqs.size}개 FAQ 생성 완료`);
  }
  ...
  private extractFAQsFromDocument(doc: DocumentEmbedding): FAQEntry[] {
    ...
    // Q&A 패턴 찾기
    const qaPattern = /(?:Q:|질문:|Question:)\s*(.+?)\s*(?:A:|답변:|Answer:)\s*(.+?)(?=\n\n|\nQ:|$)/g;
    let match;
    ...
    const errorPatterns = [
      {
        pattern: /Error: (.+)/gi,
        category: 'error',
        generateAnswer: (error: string) => `'${error}' 에러가 발생했습니다. 로그를 확인하고 관련 서비스를 재시작해보세요.`
      },
      {
        pattern: /WARNING: (.+)/gi,
        category: 'warning',
        generateAnswer: (warning: string) => `'${warning}' 경고가 발생했습니다. 시스템 상태를 모니터링하세요.`
      }
    ];
   ...
   * 🔍 검색 인덱스 구축
   */
  private async buildSearchIndex(contextCache: AdvancedContextCache): Promise<void> {
    ...
    console.log(`🔍 [AdvancedContext] ${contextCache.searchIndex.size}개 키워드 인덱싱 완료`);
  }

  /**
   * 🔎 문서 검색
   */
  async searchDocuments(query: string, limit: number = 10): Promise<DocumentEmbedding[]> {
    const contextCache = await this.loadContextCache();
   ...
   * 💾 컨텍스트 캐시 저장
   */
  private async saveContextCache(contextCache: AdvancedContextCache): Promise<void> {
    try {
      const serializable = {
  ...
  async getStatistics(): Promise<{
    ...
    return {
      ...
      lastIndexed: contextCache.lastIndexed ? new Date(contextCache.lastIndexed) : null,
