...
export const serverCommandsMap: Record<string, ServerCommands> = {
  ...
  'db-main-01': {
    ...
    commands: {
      ...
      advanced: [
        {
          command: 'sudo -u postgres psql -c "SELECT pg_database_size(\'dbname\');"',
        ...
        {
          command: 'sudo -u postgres psql -c "SELECT * FROM pg_stat_user_tables;"',
      ...
      troubleshooting: [
        ...
        {
          command: 'sudo -u postgres psql -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state = \'idle\' AND state_change < current_timestamp - interval \'1 hour\';"',
  ...
  'db-repl-01': {
    ...
    commands: {
      basic: [
        ...
        {
          command: 'sudo -u postgres psql -c "SELECT pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn();"',
      ...
      advanced: [
        {
          command: 'sudo -u postgres psql -c "SELECT pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn());"',
          description: '복제 지연 바이트 수',
          category: 'monitoring',
          riskLevel: 'safe'
        },
        {
          command: 'pg_basebackup -h master_host -D /backup -U replicator -v -P -W',
      ...
      troubleshooting: [
        {
          command: 'tail -100 /var/log/postgresql/postgresql-14-main.log | grep -i "conflict"',
...
export function recommendCommands(
  ...
  switch (scenario) {
    case 'cpu_high':
      commands = [
        ...serverCommands.commands.basic.filter(cmd => 
          cmd.category === 'monitoring' || cmd.category === 'process'
        ),
        ...serverCommands.commands.troubleshooting.filter(cmd =>
          cmd.category === 'process'
        )
      ];
      break;

    case 'memory_leak':
      commands = serverCommands.commands.basic.filter(cmd =>
    ...
    case 'disk_full':
      commands = [
        ...
        ...serverCommands.commands.troubleshooting.filter(cmd => cmd.category === 'disk')
      ];
      break;
    ...
    case 'service_down':
      commands = [
        ...
        ...serverCommands.commands.advanced.filter(cmd => 
...
export function translateCommand(command: string, fromOS: string, toOS: string): string | null {
  const translations: Record<string, Record<string, string>> = {
    ...
    'free -m': {
      'windows': 'Get-WmiObject Win32_OperatingSystem | Select-Object TotalVisibleMemorySize,FreePhysicalMemory'
    },
    'netstat -tuln': {
