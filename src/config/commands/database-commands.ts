  ...
  'db-repl-01': {
    ...
    commands: {
      ...
      advanced: [
        ...
        {
          command: 'sudo -u postgres psql -c "SELECT * FROM pg_replication_slots;"',
      ...
      troubleshooting: [
        {
          command: 'sudo -u postgres psql -c "SELECT now() - pg_last_xact_replay_timestamp() AS replication_lag;"',
          description: '복제 지연 시간 확인',
          category: 'monitoring',
          riskLevel: 'safe'
        },
        {
          command: 'tail -n 100 /var/log/postgresql/postgresql-14-main.log | grep -E "(FATAL|ERROR)"',
          description: 'PostgreSQL 에러 로그 필터링',
          category: 'monitoring',
          riskLevel: 'safe'
        },
        {
          command: 'pg_basebackup -h master_host -D /var/lib/postgresql/14/main -U replicator -v -P -W',
