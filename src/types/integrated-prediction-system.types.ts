...
export interface IIntegratedPredictionSystem {
    ...
    addDataPoint(serverId: string, dataPoint: MetricDataPoint): Promise<PredictionResult | null>;
    predictFailure(serverId: string): Promise<PredictionResult | null>;
    calculateAccuracy(serverId?: string): Promise<{ overall: number; byServer: { [key: string]: number } }>;
    ...
    // 새로운 통합 기능
    performIntegratedAnalysis(serverId: string): Promise<IntegratedAnalysisResult>;
...
export interface PredictionEvent {
    type: 'prediction_generated' | 'incident_detected' | 'anomaly_found' | 'system_health_changed';
    serverId: string;
    timestamp: Date;
    data: any;
    severity: 'info' | 'warning' | 'critical';
...
// 유틸리티 타입
export type PredictionSystemStatus = 'initializing' | 'running' | 'paused' | 'error' | 'maintenance';
