...
export interface MetricsResponse extends GCPDataGeneratorResponse<ServerMetric[]> {
    count: number;
    scenario?: string;
}

export interface HistoricalResponse extends GCPDataGeneratorResponse<HistoricalDataPoint[]> {
    period: string;
    count: number;
}
...
export interface GCPEvent {
    type: 'session_start' | 'session_stop' | 'metrics_generated' | 'error' | 'limit_exceeded';
    timestamp: number;
    sessionId?: string;
    userId?: string;
    data: any;
