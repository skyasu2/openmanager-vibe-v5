...
const PRESET_QUESTIONS: PresetQuestion[] = [
    { id: '1', text: '현재 서버 상태는 어떤가요?', category: '상태 확인', icon: Cpu, color: 'bg-blue-500' },
    { id: '2', text: 'CPU 사용률이 높은 서버를 찾아주세요', category: '성능 분석', icon: Zap, color: 'bg-red-500' },
    { id: '3', text: '메모리 부족 경고가 있나요?', category: '리소스 모니터링', icon: Brain, color: 'bg-yellow-500' },
    { id: '4', text: '네트워크 지연이 발생하고 있나요?', category: '네트워크 진단', icon: Globe, color: 'bg-green-500' },
    { id: '5', text: '최근 에러 로그를 분석해주세요', category: '로그 분석', icon: FileText, color: 'bg-purple-500' },
    { id: '6', text: '시스템 최적화 방안을 제안해주세요', category: '최적화', icon: Sparkles, color: 'bg-pink-500' },
    { id: '7', text: '디스크 사용량이 임계치에 도달했나요?', category: '스토리지', icon: HardDrive, color: 'bg-indigo-500' },
    { id: '8', text: '데이터베이스 연결 상태를 확인해주세요', category: '데이터베이스', icon: Database, color: 'bg-teal-500' },
];
        ...
        <div className={`p-4 bg-white border-t border-gray-200 ${className}`}>
            ...
            <div className='flex items-center justify-between mb-3'>
                ...
                <div className='flex items-center space-x-2'>
                    ...
                    <span className='text-xs text-gray-500' data-testid='preset-page-indicator'>
                        {currentPage + 1}/{totalPages}
                    </span>
                    <button
                        onClick={goToNextPresets}
                        disabled={(currentPage + 1) * PRESETS_PER_PAGE >= PRESET_QUESTIONS.length}
                        className='p-1 rounded hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors'
            ...
            {/* 질문 그리드 */}
            <div className='relative h-32 overflow-hidden' data-testid='preset-questions-grid'>
                <motion.div
                    key={currentPage}
                    ...
                    {currentPresets.map((question, index) => (
                        ...
                        >
                            <div className='flex items-center space-x-1 mb-1'>
                                <div className={`w-4 h-4 ${question.color} rounded flex items-center justify-center`}>
