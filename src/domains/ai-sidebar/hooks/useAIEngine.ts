...
export function useAIEngine(initialEngine: AIMode = 'LOCAL'): UseAIEngineReturn {
  ...
  // AI 모드 변경 핸들러
  const handleModeChange = useCallback(async (newMode: AIMode): Promise<ChatMessage | null> => {
    try {
