...
export default function VirtualServersPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      // 서버 목록 조회
      const serversResponse = await fetch('/api/virtual-servers?action=servers');
      ...
      // AI 분석 결과 조회
      const aiResponse = await fetch('/api/ai-agent/virtual-analysis?action=system-overview');
      if (aiResponse.ok) {
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{systemStatus.totalServers}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{systemStatus.healthyServers}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-600">{systemStatus.warningServers}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">{systemStatus.criticalServers}</p>
                </div>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              ...
              disabled={actionLoading === 'start' || generationStatus?.isRunning}
              variant="outline"
            >
            ...
            <Button
              ...
              disabled={actionLoading === 'stop' || !generationStatus?.isRunning}
              variant="outline"
            >
            ...
            >
              ...
              {actionLoading === 'full-setup' ? '설정 중...' : '전체 시스템 설정'}
            </Button>
          </div>
          ...
          {generationStatus && (
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between text-sm">
                ...
                <Badge variant={generationStatus.isRunning ? 'default' : 'secondary'}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">{server.metrics.cpu_usage.toFixed(1)}%</span>
                    </div>
                    <Progress value={server.metrics.cpu_usage} className="h-2" />
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">{server.metrics.memory_usage.toFixed(1)}%</span>
                    </div>
                    <Progress value={server.metrics.memory_usage} className="h-2" />
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">{server.metrics.disk_usage.toFixed(1)}%</span>
                    </div>
                    <Progress value={server.metrics.disk_usage} className="h-2" />
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{server.metrics.response_time.toFixed(0)}ms</span>
                  </div>

                  {/* 성능 점수 */}
                  <div className="flex items-center justify-between text-sm">
                    <span>성능 점수</span>
                    <Badge variant={server.trends.performance_score > 80 ? 'default' : 
                                  server.trends.performance_score > 60 ? 'secondary' : 'destructive'}>
                  ...
                  {server.metrics.alerts.length > 0 && (
                    <div className="mt-3">
                      <Alert>
                        ...
                        <AlertTitle>알림 ({server.metrics.alerts.length}개)</AlertTitle>
                        <AlertDescription>
                          {server.metrics.alerts[0]}
                          {server.metrics.alerts.length > 1 && ` 외 ${server.metrics.alerts.length - 1}개`}
                        </AlertDescription>
                      </Alert>
                    </div>
                  )}
          ...
          {servers.length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">서버가 없습니다</h3>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>현재</span>
                      <span className="font-medium">{systemStatus.averageCpu.toFixed(1)}%</span>
                    </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>현재</span>
                      <span className="font-medium">{systemStatus.averageMemory.toFixed(1)}%</span>
                    </div>
                    <Progress value={systemStatus.averageMemory} className="h-3" />
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          {aiAnalysis ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span>전체 상태</span>
                    <Badge variant={aiAnalysis.summary.status === 'healthy' ? 'default' : 
                                  aiAnalysis.summary.status === 'warning' ? 'secondary' : 'destructive'}>
                      {aiAnalysis.summary.status}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>성능 점수</span>
                    <span className="font-medium">{aiAnalysis.summary.performanceScore}점</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>총 이슈</span>
                    <span className="font-medium">{aiAnalysis.summary.totalIssues}개</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>심각 이슈</span>
                    <span className="font-medium text-red-600">{aiAnalysis.summary.criticalIssues}개</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {aiAnalysis.summary.recommendations.map((rec: string, index: number) => (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">AI 분석 데이터 없음</h3>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">모니터링 대시보드</h3>
                <p className="text-gray-600">
