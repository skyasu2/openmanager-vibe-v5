...
export default function PatternDemoPage() {
  ...
  const getFallbackLevelText = (level: number) => {
    switch (level) {
      case 1: return 'Primary (고신뢰도)';
      case 2: return 'Secondary (중신뢰도)';
      case 3: return 'Fallback (저신뢰도)';
        ...
        <CardContent className="space-y-4">
          <div className="flex space-x-2">
            <input
              ...
              onKeyPress={(e: React.KeyboardEvent<HTMLInputElement>) => e.key === 'Enter' && handleQuery()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  <p className="text-sm font-medium text-gray-600">매칭된 패턴:</p>
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">응답 단계:</p>
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">문서 출처:</p>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {result.metaData.queryAnalysis.keywords.map((keyword, index) => (
                ...
                <div>
                  ...
                  <Badge variant="outline">{result.metaData.queryAnalysis.intent}</Badge>
                </div>

                <div>
                  <p className="text-sm font-medium text-gray-600">복잡도:</p>
                  <Badge className={getComplexityColor(result.metaData.queryAnalysis.complexity)}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  {Object.entries(result.dynamicMetrics).map(([key, value]) => {
                    ...
                    const getMetricColor = (val: number) => {
                      if (val >= 85) return 'text-red-600 bg-red-50 border-red-200';
                      if (val >= 70) return 'text-yellow-600 bg-yellow-50 border-yellow-200';
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-2">응답 생성 정보:</p>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span>헬스 데이터 통합:</span>
                      <Badge 
                        variant={result.metaData.responseGeneration.healthDataIntegrated ? "default" : "secondary"}
                        className="text-xs"
                      >
                        {result.metaData.responseGeneration.healthDataIntegrated ? '✅ 통합됨' : '❌ 미통합'}
                      </Badge>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-2">시스템 정보:</p>
