...
export async function POST() {
  ...
  if (isBuildTime) {
    return NextResponse.json(
      {
        success: false,
        error: 'Not available during build time',
  ...
  try {
    ...
    try {
      ...
      if (error) {
        ...
        console.log(
          `✅ Supabase 시딩 완료: ${data?.length || seedData.length}개 서버`
        );
        result = {
          mode: 'supabase',
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        kubernetesNodes: ENTERPRISE_SERVERS.filter(s => s.id.includes('k8s-'))
          .length,
        onPremiseServers: ENTERPRISE_SERVERS.filter(s => !s.id.includes('k8s-'))
          .length,
        criticalServers: ENTERPRISE_SERVERS.filter(s => s.status === 'error')
          .length,
        warningServers: ENTERPRISE_SERVERS.filter(s => s.status === 'warning')
          .length,
        healthyServers: ENTERPRISE_SERVERS.filter(s => s.status === 'online')
          .length,
      },
    });
  } catch (error) {
...
function generateMockIP(serverId: string): string {
  ...
  if (serverId.includes('k8s-master')) subnet = '10.10';
    ...
    serverId.includes('storage-') ||
    serverId.includes('file-') ||
    serverId.includes('backup-')
  )
    subnet = '10.50';
  else if (
    serverId.includes('monitor-') ||
    serverId.includes('log-') ||
    serverId.includes('proxy-') ||
    serverId.includes('dns-')
  )
    subnet = '10.60';
