...
const GCP_FIRESTORE_DATABASE =
  process.env.GCP_FIRESTORE_DATABASE || '(default)';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const gcpResponse = await fetchGCPMetrics(
      sessionId,
      limit,
      serverId || undefined
    );
...
function generateServerMetric(
  ...
  const cpuUsage = Math.max(
    0,
    Math.min(100, baseCpu * multiplier * (1 + variation()))
  );
  const memoryUsed = Math.max(0, baseMemory * multiplier * (1 + variation()));
  const diskUtil = Math.max(
    0,
    Math.min(100, baseDisk * multiplier * (1 + variation()))
  );
