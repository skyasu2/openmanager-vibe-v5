...
export async function POST(request: NextRequest) {
  try {
    ...
    // 질의 분석 및 적절한 엔진 선택
    const selectedEngine = analyzeQueryAndSelectEngine(query);
    ...
    console.log(`✅ AI 질의 완료: ${processingTime}ms (신뢰도: ${(result.confidence * 100).toFixed(1)}%)`);
    ...
    return NextResponse.json({
      ...
      data: {
        query: '',
        answer: '죄송합니다. 현재 AI 서비스에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.',
        processing_time: 0,
        confidence: 0
      }
    }, { status: 500 });
 ...
 * 질의 분석 및 엔진 선택
 */
function analyzeQueryAndSelectEngine(query: string): string {
  const lowerQuery = query.toLowerCase();

  // 시간 관련 질의
  if (lowerQuery.includes('시간') || lowerQuery.includes('몇시') || lowerQuery.includes('언제')) {
    return 'korean';
  }

  // 서버/시스템 관련 질의
  if (lowerQuery.includes('서버') || lowerQuery.includes('시스템') || lowerQuery.includes('모니터링')) {
    return 'enhanced';
  }

  // 장애/문제 관련 질의
  if (lowerQuery.includes('장애') || lowerQuery.includes('오류') || lowerQuery.includes('문제')) {
    return 'anomaly';
  }

  // 예측 관련 질의
  if (lowerQuery.includes('예측') || lowerQuery.includes('미래') || lowerQuery.includes('전망')) {
    return 'prediction';
  }
...
function detectIntent(query: string): string {
  ...
  if (query.includes('장애') || query.includes('오류')) return 'troubleshooting';
  return 'general';
}
