  ...
  try {
    ...
    return createSuccessResponse({
      ...
      analysis: {
        ...
        costImpact: decision.costImpact.savings > 0 ? 
...
async function updateScalingPolicyHandler(request: NextRequest) {
  try {
    ...
    if (policy) {
      // 정책 검증
      if (policy.minServers && (policy.minServers < 1 || policy.minServers > 50)) {
        return createErrorResponse('최소 서버 수는 1-50 사이여야 합니다', 'VALIDATION_ERROR');
      }
      
      if (policy.maxServers && (policy.maxServers < 1 || policy.maxServers > 100)) {
        return createErrorResponse('최대 서버 수는 1-100 사이여야 합니다', 'VALIDATION_ERROR');
      }

      if (policy.minServers && policy.maxServers && policy.minServers >= policy.maxServers) {
        return createErrorResponse('최소 서버 수는 최대 서버 수보다 작아야 합니다', 'VALIDATION_ERROR');
      }
    ...
    if (rules && Array.isArray(rules)) {
      ...
      for (const rule of rules) {
        if (!rule.id || !rule.name || !rule.metric) {
          return createErrorResponse('규칙에는 id, name, metric이 필요합니다', 'VALIDATION_ERROR');
        }
        
        if (!['cpu', 'memory', 'disk', 'response_time', 'composite'].includes(rule.metric)) {
          return createErrorResponse('지원하지 않는 메트릭입니다', 'VALIDATION_ERROR');
        }

        if (rule.weight < 0 || rule.weight > 1) {
          return createErrorResponse('가중치는 0-1 사이여야 합니다', 'VALIDATION_ERROR');
        }
      }
...
async function getScalingHistoryHandler(request: NextRequest) {
  try {
    ...
    if (limit < 1 || limit > 100) {
      return createErrorResponse('limit은 1-100 사이여야 합니다', 'VALIDATION_ERROR');
    }
    ...
    const scaleDownActions = history.filter(h => h.action === 'scale_down').length;
...
async function simulateScalingHandler(request: NextRequest) {
  try {
    ...
    // 시뮬레이션을 위한 서버 데이터 수정
    if (targetCpu !== null || targetMemory !== null || targetResponseTime !== null) {
      servers = servers.map(server => ({
        ...server,
        cpu_usage: targetCpu !== null ? targetCpu : server.cpu_usage,
        memory_usage: targetMemory !== null ? targetMemory : server.memory_usage,
        response_time: targetResponseTime !== null ? targetResponseTime : server.response_time
      }));
    }
    ...
    return createSuccessResponse({
      simulation: {
        ...
        simulatedMetrics: {
          avgCpu: targetCpu || servers.reduce((sum, s) => sum + s.cpu_usage, 0) / servers.length,
          avgMemory: targetMemory || servers.reduce((sum, s) => sum + s.memory_usage, 0) / servers.length,
          avgResponseTime: targetResponseTime || servers.reduce((sum, s) => sum + s.response_time, 0) / servers.length
        }
      },
      decision,
      analysis: {
