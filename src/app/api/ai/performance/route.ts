  ...
  try {
    ...
    return NextResponse.json({
      ...
      analysis: {
        ...
        recommendations: generateRecommendations(stats.metrics, stats.optimization),
      }
    }, { headers: corsHeaders });
...
async function runComparisonBenchmark(queries: string[], iterations: number) {
  ...
  // 통계 계산
  const originalAvg = results.originalEngine.totalTime / results.originalEngine.responses.length;
  const optimizedAvg = results.optimizedEngine.totalTime / results.optimizedEngine.responses.length;
  const improvement = ((originalAvg - optimizedAvg) / originalAvg * 100);

  const cacheHits = results.optimizedEngine.responses.filter(r => r.cached).length;
  const cacheHitRate = (cacheHits / results.optimizedEngine.responses.length * 100);
  ...
  return NextResponse.json({
    ...
    results: {
      originalEngine: {
        ...
        successRate: Math.round(
          ...
          results.originalEngine.responses.length * 100
        ),
      ...
      optimizedEngine: {
        ...
        successRate: Math.round(
          ...
          results.optimizedEngine.responses.length * 100
        ),
    ...
    analysis: {
      ...
      timeSaved: Math.round(results.originalEngine.totalTime - results.optimizedEngine.totalTime),
      performanceBetter: improvement > 0,
      cacheEffectiveness: cacheHitRate > 30 ? 'high' : cacheHitRate > 10 ? 'medium' : 'low'
    }
  }, { headers: corsHeaders });
}
...
async function runLoadBenchmark(queries: string[], iterations: number) {
  ...
  return NextResponse.json({
    ...
    analysis: {
      performanceGrade: throughput > 10 ? 'excellent' : throughput > 5 ? 'good' : throughput > 2 ? 'fair' : 'poor',
      bottlenecks: avgResponseTime > 2000 ? ['response_time'] : [],
      scalability: successRate > 95 ? 'high' : successRate > 80 ? 'medium' : 'low'
    }
  }, { headers: corsHeaders });
}
...
function generateRecommendations(metrics: any, optimization: any): string[] {
  ...
  if (metrics.avgResponseTime > 2000) {
    recommendations.push('응답 시간이 느립니다. 벡터 인덱스 최적화를 고려하세요.');
  }

  if (metrics.cacheHitRate < 0.3) {
    recommendations.push('캐시 적중률이 낮습니다. 캐시 전략을 aggressive로 변경하세요.');
  }

  if (!optimization.warmupCompleted) {
    recommendations.push('워밍업이 완료되지 않았습니다. 시스템 시작 시 워밍업을 활성화하세요.');
  }

  if (optimization.preloadedEmbeddings < 5) {
    recommendations.push('예열된 임베딩이 부족합니다. 자주 사용되는 쿼리 패턴을 추가하세요.');
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push('현재 성능이 양호합니다. 지속적인 모니터링을 권장합니다.');
  }
