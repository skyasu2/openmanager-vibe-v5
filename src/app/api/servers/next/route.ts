...
async function handleGET(request: NextRequest) {
  try {
    ...
    // 🚀 서버 통계 조회
    if (action === 'stats') {
      const stats = generatedServers.reduce((acc, server) => {
        acc.total++;
  ...
  try {
    ...
    const generateServices = (status: string) => {
      ...
      return baseServices.map(service => ({
        ...service,
        status: (status === 'offline' || (status === 'warning' && Math.random() < 0.3)) 
          ? 'stopped' as const 
          : 'running' as const
      }));
    };
    
    const serverStatus = generateServerStatus() as 'online' | 'warning' | 'offline';
    ...
    const newServer = {
      ...
      environment: 'production',
      location: ['Seoul DC1', 'Tokyo DC2', 'Singapore DC3', 'US-East DC4'][serverCount % 4],
      provider: 'onpremise',
      status: serverStatus,
      cpu: metrics.cpu,
      memory: metrics.memory,
      disk: metrics.disk,
      uptime: serverStatus === 'offline' ? '0d 0h 0m' : 
