...
interface ServerFileSchema {
  servers?: Record<string, RawServerData | undefined>;
}

interface BaselineMetricRange {
  average?: number;
...
interface ServerBaseline {
  normalRanges?: {
    cpu?: BaselineMetricRange;
    memory?: BaselineMetricRange;
    disk?: BaselineMetricRange;
    network?: BaselineMetricRange;
  };
...
interface BaselinesFileSchema {
  serverBaselines?: Record<string, ServerBaseline | undefined>;
  globalBaselines?: {
    correlationPatterns?: Record<string, CorrelationData | undefined>;
    predictionModels?: Record<string, PredictionModelConfig | undefined>;
  };
}
  ...
  static async analyzeTimeline(
    ...
    try {
      ...
      const currentDataPath = path.join(
        process.cwd(),
        'public/ai-server-data/realtime/current.json'
      );
      const currentData = JSON.parse(
        await fs.readFile(currentDataPath, 'utf-8')
      );

      // 히스토리컬 인시던트 데이터 로드
      const incidentPath = path.join(
        process.cwd(),
        'public/ai-server-data/historical/incidents/sample-incident-001.json'
      );
      ...
      try {
        ...
        servers = Object.entries(currentData.servers).map(([id, data]) => {
          ...
          return {
            ...
            metrics: {
              cpu:
                typeof validatedData.cpu === 'number' ? validatedData.cpu : 50,
              memory:
                typeof validatedData.memory === 'number'
                  ? validatedData.memory
                  : 50,
              disk:
                typeof validatedData.disk === 'number'
                  ? validatedData.disk
                  : 50,
              network:
                typeof validatedData.network === 'number'
                  ? validatedData.network
                  : 50,
            },
            currentEvents: Array.isArray(validatedData.events)
              ? validatedData.events
              : [],
            currentTrend:
              typeof validatedData.trend === 'string'
                ? validatedData.trend
                : 'stable',
            status:
              typeof validatedData.status === 'string'
      ...
      const recommendations =
        findings.length > 0
          ...
          : ['현재 안정적인 상태 유지', '정기적인 성능 점검 계속 진행'];
  ...
  static async analyzeRootCause(query: string) {
    try {
      ...
      const baselinesPath = path.join(
        process.cwd(),
        'public/ai-server-data/ai-features/baselines.json'
      );
      const baselinesData = JSON.parse(
        await fs.readFile(baselinesPath, 'utf-8')
      ) as BaselinesFileSchema;

      const findings: Finding[] = [];
      const serverBaselines = baselinesData.serverBaselines ?? {};
      const correlationPatterns =
        baselinesData.globalBaselines?.correlationPatterns ?? {};

      // 서버 데이터 변환
      const servers = Object.entries(currentData.servers ?? {}).map(
        ([id, data]) => {
          const safeData = data ?? {};
          return {
            id,
            ...
            metrics: {
              cpu: safeData.cpu,
              memory: safeData.memory,
              disk: safeData.disk,
              network: safeData.network,
            },
            currentEvents: safeData.events || [],
            currentTrend: safeData.trend || 'stable',
          };
        }
      );
      ...
      if (/cpu|CPU|프로세서/i.test(query)) {
        const highCpuServers = servers.filter(
          (server) => (server.metrics.cpu ?? 0) > 70
        );

        highCpuServers.forEach((server) => {
          const baseline = serverBaselines[server.id];
          const baselineCpu = baseline?.normalRanges?.cpu?.average;
          if (typeof baselineCpu === 'number') {
            const deviation = (server.metrics.cpu ?? 0) - baselineCpu;

            findings.push({
              type: 'insight' as const,
              severity:
                deviation > 30 ? ('critical' as const) : ('high' as const),
      ...
      if (/메모리|memory|RAM/i.test(query)) {
        const highMemoryServers = servers.filter(
          (server) => (server.metrics.memory ?? 0) > 80
        );

        highMemoryServers.forEach((server) => {
          const baseline = serverBaselines[server.id];
          const memoryBaseline = baseline?.normalRanges?.memory?.average;
          if (typeof memoryBaseline === 'number') {
            findings.push({
              type: 'warning' as const,
              severity: 'high' as const,
      ...
      Object.entries(correlationPatterns).forEach(([key, correlation]) => {
        ...
        findings.push({
          type: 'correlation' as const,
          severity: 'medium' as const,
          message: `상관관계 분석: ${correlation.description ?? key} (상관계수: ${correlationValue})`,
          evidence: {
            pattern: key,
            correlation: correlationValue,
            description: correlation.description ?? key,
      ...
      return {
        ...
        relatedData: {
          analyzedCorrelations: Object.keys(correlationPatterns).length,
  ...
  static async analyzePrediction(query: string) {
    try {
      ...
      const predictionModels =
        baselinesData.globalBaselines?.predictionModels ?? {};
      ...
      // 메모리 누수 예측
      const memoryLeakModel = predictionModels.memoryLeakDetection;
      if (memoryLeakModel) {
        ...
        serversAtRisk.forEach((server) => {
          findings.push({
            ...
            confidence: (memoryLeakModel.confidence ?? 75) / 100,
          });
        });
      }

      // 트래픽 급증 예측
      const trafficSpikeModel = predictionModels.trafficSpikePredictor;
      if (trafficSpikeModel) {
        const webServers = servers.filter((server) => server.type === 'web');

        webServers.forEach((server) => {
          if (
            (server.metrics.cpu ?? 0) > 60 &&
            (server.metrics.network ?? 0) > 70
          ) {
            findings.push({
              type: 'prediction' as const,
              severity: 'medium' as const,
              ...
              evidence: {
                ...
                timeWindow: trafficSpikeModel.timeWindow,
                model: 'trafficSpikePredictor',
              },
              confidence: (trafficSpikeModel.confidence ?? 65) / 100,
            });
          }
        });
      }
      ...
      if (/내일|tomorrow|다음|next/i.test(query)) {
        ...
        const dayOfWeek = tomorrow.toLocaleDateString('ko-KR', {
          weekday: 'long',
        });
  ...
  static async analyzeComparison(query: string) {
    try {
      ...
      servers.forEach((server) => {
        const baseline = serverBaselines[server.id];
        const cpuBaseline = baseline?.normalRanges?.cpu?.average;
        const memoryBaseline = baseline?.normalRanges?.memory?.average;
        if (
          typeof cpuBaseline === 'number' &&
          typeof memoryBaseline === 'number'
        ) {
          const cpuDiff = (server.metrics.cpu ?? 0) - cpuBaseline;
          const memoryDiff = (server.metrics.memory ?? 0) - memoryBaseline;
      ...
      const avgMemory =
        servers.length > 0
          ? servers.reduce(
              (sum: number, s) => sum + (s.metrics.memory ?? 0),
              0
            ) / servers.length
          : 0;

      const outliers = servers.filter(
        (server) =>
      ...
      if (/주|week|이번|지난/i.test(query)) {
        const seasonalPatterns =
          serverBaselines['api-server-1']?.seasonalPatterns;
        if (seasonalPatterns) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { query, analysisType, timeRange } = requestData;
    ...
    switch (analysisType) {
      case 'timeline':
        results = await TimelineAnalysisEngine.analyzeTimeline(
          query,
          timeRange
        );
    ...
    if (
      results instanceof NextResponse ||
      !results ||
    ...
    const confidenceLevel =
      safeResults.findings.length > 0
        ? safeResults.findings.reduce(
            (sum, finding) => sum + finding.confidence,
            0
          ) / safeResults.findings.length
        : 0.5;
    ...
    console.log(
      `✅ [AI-ANALYSIS] 분석 완료 (${executionTime}ms, 신뢰도: ${(confidenceLevel * 100).toFixed(1)}%)`
    );
