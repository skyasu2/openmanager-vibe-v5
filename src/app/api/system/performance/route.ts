...
async function getPerformanceMetricsHandler(request: NextRequest) {
  try {
    ...
    return createSuccessResponse({
      ...
      analysis: {
        memoryStatus: currentMetrics.memoryUsage.usagePercent > 80 ? 'high' : 
                    currentMetrics.memoryUsage.usagePercent > 60 ? 'medium' : 'low',
        redisStatus: currentMetrics.redisMetrics?.connected ? 'connected' : 'disconnected',
...
async function runLoadTestHandler(request: NextRequest) {
  try {
    ...
    // 테스트 설정 검증
    if (duration > 300) {
      return createErrorResponse('테스트 지속 시간은 최대 300초입니다', 'VALIDATION_ERROR');
    }
    
    if (concurrency > 50) {
      return createErrorResponse('동시 접속 수는 최대 50개입니다', 'VALIDATION_ERROR');
    }

    if (requestsPerSecond > 100) {
      return createErrorResponse('초당 요청 수는 최대 100개입니다', 'VALIDATION_ERROR');
    }
...
async function performOptimizationHandler(request: NextRequest) {
  try {
    ...
    // 최적화 실행
    const optimizationResult = await performanceTester.performAutoOptimization();
    ...
    // 개선 효과 계산
    const improvement = {
      memoryReduction: currentMetrics.memoryUsage.usagePercent - afterMetrics.memoryUsage.usagePercent,
...
async function stopLoadTestHandler(request: NextRequest) {
  try {
    ...
    if (!performanceTester.isTestRunning()) {
      return createErrorResponse('실행 중인 부하 테스트가 없습니다', 'NOT_FOUND');
    }
