...
async function handleThinkingStream(request: NextRequest, thinkingSessionId: string | null) {
        ...
        if (session.sessionId === thinkingSessionId) {
          try {
            ...
            if (session.status === 'completed' || session.status === 'error') {
              setTimeout(() => {
                controller.enqueue(`data: ${JSON.stringify({ type: 'stream_end' })}\n\n`);
                controller.close();
              }, 1000);
            }
          } catch (error) {
...
async function handleGetSession(thinkingSessionId: string | null) {
  ...
  // 보안을 위해 민감한 정보 제거
  const protectedSession = thinkingProcessor.getProtectedThinkingData(thinkingSessionId);
...
async function handleGetActiveSessions() {
  ...
  // 보안을 위해 민감한 정보 제거
  const protectedSessions = activeSessions.map(session => 
...
async function handleStartThinking(data: any) {
  ...
  const thinkingSessionId = thinkingProcessor.startThinking(queryId, query, mode);
