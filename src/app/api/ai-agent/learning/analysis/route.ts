  ...
  try {
    ...
    switch (action) {
      ...
      case 'compare-patterns': {
        ...
        const comparison = await analysisService.comparePatterns(oldPatternId, newPatternId);
  ...
  try {
    ...
    switch (action) {
      case 'approve-suggestion': {
        ...
        const success = await analysisService.approvePatternSuggestion(suggestionId);
        
        return NextResponse.json({
          success,
          message: success ? '패턴 제안이 승인되었습니다.' : '패턴 제안 승인에 실패했습니다.'
        });
      }
      ...
      case 'reject-suggestion': {
        ...
        const success = await analysisService.rejectPatternSuggestion(suggestionId, reason);
        
        return NextResponse.json({
          success,
          message: success ? '패턴 제안이 거부되었습니다.' : '패턴 제안 거부에 실패했습니다.'
        });
      }
      ...
      case 'start-test': {
        ...
        if (!testResult) {
          return NextResponse.json({
            success: false,
            error: 'A/B 테스트 시작에 실패했습니다. (최대 동시 테스트 수 초과 가능)'
          }, { status: 500 });
        }
      ...
      case 'auto-approve-high-confidence': {
        ...
        const results = await Promise.all(
          highConfidenceSuggestions.map(s => analysisService.approvePatternSuggestion(s.id))
        );
