...
// 실제 AI 엔진 사고 과정 생성
async function generateRealThinkingProcess(query: string, serverData: any[], context?: any): Promise<ThinkingStep[]> {
...
// 패턴 매칭
async function matchPatterns(query: string, serverData: any[]): Promise<string> {
...
// 추론 및 결론 도출
async function generateReasoning(query: string, serverData: any[]): Promise<string> {
...
function extractKeywords(query: string): string[] {
  return query.toLowerCase()
    ...
    .filter(word => !['은', '는', '이', '가', '을', '를', '의', '에', '와', '과', '해', '줘', '어떤', '어때'].includes(word))
    .slice(0, 5);
}
...
function classifyIntent(query: string): string {
  ...
  if (lowerQuery.includes('분석') || lowerQuery.includes('확인')) return 'general_analysis';
...
function extractEntities(query: string): string[] {
  ...
  if (query.includes('메모리') || query.includes('memory')) entities.push('Memory');
...
function calculateCpuStats(serverData: any[]) {
  ...
  const variance = cpuValues.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / cpuValues.length;
...
function identifyPatterns(query: string, serverData: any[]) {
  ...
  if (serverData.length > 0) {
    ...
    if (highCpuServers.length > 0) {
      patterns.push({ name: `고부하 CPU 클러스터 (${highCpuServers.length}대)`, confidence: '94.2%' });
    }

    const memoryPattern = serverData.filter(s => (s.memory || s.metrics?.memory || 0) > 80);
    if (memoryPattern.length > 0) {
      patterns.push({ name: `메모리 압박 패턴 (${memoryPattern.length}대)`, confidence: '88.7%' });
    }
  }
...
function detectAnomalies(serverData: any[]) {
  ...
  serverData.forEach((server, index) => {
    ...
    if (cpu > 95) {
      anomalies.push({ type: 'CPU Spike', description: `서버 ${index + 1} CPU 과부하`, severity: 'HIGH' });
    }
    
    if (memory > 95) {
      anomalies.push({ type: 'Memory Leak', description: `서버 ${index + 1} 메모리 누수 의심`, severity: 'HIGH' });
    }
  });

  if (anomalies.length === 0) {
    anomalies.push({ type: 'Normal', description: '이상 징후 없음', severity: 'LOW' });
  }
...
function assessRisks(serverData: any[]) {
  ...
  const stability = critical === 0 && warning < 2 ? '우수' : warning < 5 ? '양호' : '주의';
  ...
  return {
    ...
    stability,
    riskScore1h,
    prediction30m: critical > 0 ? '악화 예상' : warning > 2 ? '현상 유지' : '개선 예상',
    checkInterval: critical > 0 ? 5 : warning > 0 ? 15 : 30,
    summary: critical > 0 ? '즉시 대응이 필요한 상황입니다.' : 
             warning > 2 ? '지속적인 모니터링이 필요합니다.' : 
...
function generateRecommendations(query: string, serverData: any[]) {
  ...
  if (critical > 0) {
    recommendations.push({ action: '위험 서버 즉시 점검 및 복구', priority: 'HIGH' });
  }
  
  if (warning > 2) {
    recommendations.push({ action: '경고 서버 모니터링 강화', priority: 'MEDIUM' });
  }
...
function determineTone(query: string): string {
  if (query.includes('급해') || query.includes('빨리')) return '긴급하고 직접적인 톤';
  if (query.includes('분석') || query.includes('보고서')) return '전문적이고 상세한 톤';
