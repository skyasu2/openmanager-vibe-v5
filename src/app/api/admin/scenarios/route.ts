...
// 실행 가능한 시나리오 템플릿
const SCENARIO_TEMPLATES: Omit<Scenario, 'id' | 'status' | 'createdAt' | 'lastRun' | 'runCount' | 'results'>[] = [
  {
    name: '일반 운영 패턴',
    type: 'normal',
  ...
  try {
    ...
    try {
      ...
      scenarios = SCENARIO_TEMPLATES.map((template, index) => ({
        ...
        createdAt: new Date(Date.now() - Math.random() * 86400000 * 7).toISOString(), // 최근 7일
        lastRun: Math.random() > 0.5 ? new Date(Date.now() - Math.random() * 3600000 * 24).toISOString() : undefined,
    ...
    const estimatedCompletions = runningScenarios.map(scenario => ({
      id: scenario.id,
      name: scenario.name,
      estimatedCompletion: new Date(Date.now() + scenario.parameters.duration * 60000).toISOString()
    }));
    ...
    return NextResponse.json({
      success: false,
      error: '시나리오 조회 실패',
      message: error instanceof Error ? error.message : '시나리오 데이터를 불러올 수 없습니다.',
...
export async function PUT(request: NextRequest) {
  try {
        ...
        setTimeout(async () => {
          try {
            ...
            if (updatedScenarios) {
              ...
              const runningScenario = scenarioList.find(s => s.id === scenarioId);
              if (runningScenario && runningScenario.status === 'running') {
                ...
                await redis.setex('scenarios:list', 300, JSON.stringify(scenarioList));
    ...
    return NextResponse.json({
      ...
      estimatedCompletion: scenario.status === 'running' ? 
        new Date(Date.now() + scenario.parameters.duration * 60000).toISOString() : null
    });
    ...
    return NextResponse.json({
      success: false,
      error: '시나리오 실행 실패',
      message: error instanceof Error ? error.message : '시나리오 실행 중 오류가 발생했습니다.'
    }, { status: 500 });
  }
} 