...
export async function GET(request: NextRequest) {
  try {
    ...
    const mockAnalyses: AnalysisResult[] = [
      {
        ...
        type: 'pattern',
        title: '서버 부하 패턴 분석',
        description: '주중 오후 2-4시 사이 CPU 사용률이 평균 15% 증가하는 패턴을 감지했습니다.',
        confidence: 92,
        impact: 'medium',
        timestamp: new Date(),
        status: 'completed',
      ...
      },
      {
        ...
        type: 'anomaly',
        title: '메모리 사용량 이상 감지',
        description: 'api-server-03에서 지난 24시간 동안 비정상적인 메모리 증가 패턴이 발견되었습니다.',
        confidence: 87,
        impact: 'high',
      ...
      },
      {
        ...
        type: 'prediction',
        title: '디스크 용량 예측',
        description: '현재 증가율을 기준으로 db-server-01의 디스크 사용량이 7일 내 80%에 도달할 것으로 예상됩니다.',
        confidence: 78,
        impact: 'critical',
      ...
      },
      {
        ...
        type: 'optimization',
        title: '리소스 최적화 제안',
        description: '현재 서버 구성에서 30% 비용 절감이 가능한 최적화 방안을 발견했습니다.',
        confidence: 85,
        impact: 'medium',
        ...
        data: {
          potentialSavings: 30,
          optimizationAreas: ['cpu_rightsizing', 'storage_optimization', 'network_efficiency'],
    ...
    if (type) {
      filteredAnalyses = mockAnalyses.filter(analysis => analysis.type === type);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 입력 검증
    if (!type || !['pattern', 'anomaly', 'prediction', 'optimization'].includes(type)) {
      return NextResponse.json({
        success: false,
        error: 'Invalid analysis type',
        message: 'Type must be one of: pattern, anomaly, prediction, optimization'
      }, { status: 400 });
    }
