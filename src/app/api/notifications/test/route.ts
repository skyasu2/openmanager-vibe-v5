...
export async function POST(request: NextRequest) {
  try {
    ...
    const testResult = {
      ...
      channels: {
        slack: channel === 'all' || channel === 'slack' ? 'test_sent' : 'not_tested',
        discord: channel === 'all' || channel === 'discord' ? 'test_sent' : 'not_tested',
        email: channel === 'all' || channel === 'email' ? 'test_sent' : 'not_tested'
      }
    };
