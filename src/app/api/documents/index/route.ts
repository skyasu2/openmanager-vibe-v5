  ...
  try {
    ...
    try {
      ...
      documents['cpu-optimization'] = {
        content: 'CPU 사용률이 높을 때는 프로세스를 확인하고 불필요한 서비스를 중지하세요. top 명령어로 CPU 사용량이 높은 프로세스를 찾을 수 있습니다.',
      ...
      documents['memory-management'] = {
        content: '메모리 사용률이 높을 때는 메모리 누수를 확인하고 캐시를 정리하세요. free -h 명령어로 메모리 상태를 확인할 수 있습니다.',
      ...
      documents['disk-cleanup'] = {
        content: '디스크 공간이 부족할 때는 로그 파일을 정리하고 임시 파일을 삭제하세요. df -h로 디스크 사용량을 확인할 수 있습니다.',
      ...
      documents['network-troubleshooting'] = {
        content: '네트워크 문제가 발생했을 때는 ping과 traceroute로 연결 상태를 확인하세요. netstat으로 포트 상태를 점검할 수 있습니다.',
    ...
    console.log(`✅ 문서 인덱스 생성 완료: ${Object.keys(documents).length}개 문서`);
...
function extractKeywords(text: string): string[] {
  ...
  const technical = text.match(/\b(?:CPU|API|DB|RAM|SSD|HTTP|JSON|서버|모니터링|성능|메모리|디스크|네트워크)\b/gi) || [];
...
function categorizeDocument(filename: string, content: string): string {
  ...
  if (lower.includes('performance') || lower.includes('성능') || lower.includes('cpu') || lower.includes('memory')) {
    return 'performance';
  }
  if (lower.includes('security') || lower.includes('보안') || lower.includes('auth')) {
    return 'security';
  }
  if (lower.includes('network') || lower.includes('네트워크') || lower.includes('connection')) {
    return 'network';
  }
  if (lower.includes('storage') || lower.includes('disk') || lower.includes('디스크')) {
    return 'storage';
  }
  if (lower.includes('deployment') || lower.includes('배포') || lower.includes('setup')) {
    return 'deployment';
  }
  if (lower.includes('development') || lower.includes('개발') || lower.includes('guide')) {
    return 'development';
  }
