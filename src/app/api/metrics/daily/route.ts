...
export async function GET(request: NextRequest) {
  try {
    ...
    const resolutionMinutes = parseInt(searchParams.get('resolution') || '1') as 1 | 2 | 5;
    ...
    if (interpolate && metrics.length > 1) {
      ...
      const interpolationOptions: Partial<InterpolationOptions> = {
        resolutionMinutes: [1, 2, 5].includes(resolutionMinutes) ? resolutionMinutes : 1,
        noiseLevel: Math.max(0, Math.min(1, noiseLevel)),
        preserveOriginal,
        smoothingFactor: 0.1
      };
      
      const interpolatedMetrics = interpolateMetricsByServer(metrics, interpolationOptions);
      
      // 상태별 재필터링 (보간 후)
      if (status && ['healthy', 'warning', 'critical'].includes(status)) {
        finalMetrics = interpolatedMetrics.filter(metric => metric.status === status);
      } else {
      ...
      interpolationQuality = validateInterpolationQuality(sampleOriginal, sampleInterpolated);
      
      console.log(`✅ 보간 완료: ${metrics.length} → ${interpolatedMetrics.length}개 (${interpolationQuality.qualityScore}점)`);
    }
    ...
    const stats = {
      ...
      timeRange: finalMetrics.length > 0 ? {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'generate') {
      ...
      return NextResponse.json({
        success: true,
        message: '메트릭 데이터 생성이 시작되었습니다. npm run generate:metrics 명령어를 직접 실행하세요.',
        timestamp: new Date().toISOString()
      });
    }
