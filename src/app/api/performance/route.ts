...
export async function POST(request: NextRequest) {
    try {
        ...
        const { engine, mode, responseTime, success, confidence, fallbacksUsed, queryType, errorType } = body;

        // 필수 필드 검증
        if (!engine || !mode || responseTime === undefined || success === undefined) {
            return NextResponse.json(
                {
                    success: false,
                    error: '필수 필드가 누락되었습니다. (engine, mode, responseTime, success)'
                },
                { status: 400 }
            );
        }
