  ...
  try {
    ...
    switch (responseType) {
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            summary: {
              ...
              healthyServers: operationalServers.filter(s => s.status === 'healthy').length,
              warningServers: operationalServers.filter(s => s.status === 'warning').length,
              criticalServers: operationalServers.filter(s => s.status === 'critical').length
            },
            metadata: {
              isDynamic: true,
              scalingMode: 'auto',
      ...
      case 'integrated':
        ...
        const integratedMetrics = advancedSimulationEngine.getIntegratedAIMetrics();
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'AI 분리 시뮬레이션 조회 실패'
    }, { status: 500 });
  }
}
  ...
  try {
    ...
    switch (action) {
      case 'trigger_scaling':
        // 강제 스케일링 시뮬레이션
        const scalingEvent = await scalingSimulationEngine.simulateScaling();
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            operationalServers: scalingSimulationEngine.getOperationalServers().length,
            aiTargetsUnchanged: advancedSimulationEngine.getAnalysisTargets().length
          }
        });
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : '스케일링 액션 실행 실패'
    }, { status: 500 });
  }
}
  ...
  try {
    ...
    switch (testType) {
      case 'scaling_stress_test':
        ...
        // 10번의 스케일링 시뮬레이션
        for (let i = 0; i < 10; i++) {
          const event = await scalingSimulationEngine.simulateScaling();
          if (event) {
            results.push({
              iteration: i + 1,
              event: event.type,
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            results,
            aiTargetsStable: advancedSimulationEngine.getAnalysisTargets().length,
            operationalServersAfter: scalingSimulationEngine.getOperationalServers().length,
      ...
      case 'ai_consistency_test':
        ...
        // 스케일링 시뮬레이션 실행
        await scalingSimulationEngine.simulateScaling();
        await scalingSimulationEngine.simulateScaling();
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            operationalCount: scalingSimulationEngine.getOperationalServers().length,
