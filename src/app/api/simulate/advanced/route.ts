      ...
      case 'health':
        responseData = {
          ...
          healthDetails: {
            ...
            riskAnalysis: {
              highRiskServers: servers.filter(s => s.cascade_risk > 60).map(s => ({
                id: s.id,
                role: s.role,
                cascade_risk: s.cascade_risk,
                reason: s.active_scenarios.length > 0 ? '활성 장애 시나리오' : '구조적 위험'
              })),
              dependencyIssues: servers.filter(s => s.dependency_health < 70).map(s => ({
                id: s.id,
                role: s.role,
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    switch (action) {
      case 'start':
        if (!advancedSimulationEngine.isRunning()) {
