...
import { globalQueryCache, globalMetricsCache, globalMonitoringCache } from '@/lib/cache/AICache';
    ...
    return NextResponse.json({
      ...
      components: {
        ...
        caching: {
          status: 'healthy',
          stats: cacheStats,
          totalEntries: Object.values(cacheStats).reduce((sum, cache) => sum + cache.totalEntries, 0),
          averageHitRate: Object.values(cacheStats).reduce((sum, cache) => sum + cache.hitRate, 0) / 3
        },
        apis: {
          status: 'operational',
...
async function getSystemStats() {
  try {
    ...
    const totalRequests = Object.values(cacheStats).reduce(
      (sum, cache) => sum + cache.totalHits + cache.totalMisses, 0
    );

    const totalHits = Object.values(cacheStats).reduce(
      (sum, cache) => sum + cache.totalHits, 0
    );
    ...
    return NextResponse.json({
      statistics: {
        requests: {
          total: totalRequests,
          cached: totalHits,
          cache_hit_rate: totalRequests > 0 ? (totalHits / totalRequests * 100).toFixed(2) + '%' : '0%'
        },
        caching: cacheStats,
        memory: {
