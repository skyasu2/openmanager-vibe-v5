...
const demoScenarios = {
  normal: {
    name: "정상 운영 상태",
    description: "일반적인 업무시간 중 안정적인 서버 운영 상태를 시뮬레이션합니다.",
    icon: "🟢",
    features: ["안정적인 메트릭", "낮은 에러율", "예측 가능한 패턴"]
  },
  spike: {
    name: "갑작스런 트래픽 증가", 
    description: "마케팅 캠페인이나 바이럴 컨텐츠로 인한 급격한 사용자 증가를 시뮬레이션합니다.",
    icon: "📈",
    features: ["트래픽 스파이크", "CPU/메모리 부하", "응답시간 증가"]
  },
  memory_leak: {
    name: "메모리 누수 발생",
    description: "애플리케이션 버그로 인한 점진적인 메모리 증가와 성능 저하를 시뮬레이션합니다.",
    icon: "🔄",
    features: ["점진적 메모리 증가", "성능 저하", "시스템 불안정"]
  },
  ddos: {
    name: "DDoS 공격 시뮬레이션",
    description: "분산 서비스 거부 공격으로 인한 시스템 부하와 보안 위협을 시뮬레이션합니다.",
    icon: "🛡️",
    features: ["네트워크 부하", "보안 경고", "서비스 불가"]
  },
  performance_degradation: {
    name: "점진적 성능 저하",
    description: "디스크 공간 부족이나 데이터베이스 성능 저하로 인한 시스템 악화를 시뮬레이션합니다.",
    icon: "📉",
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 메트릭 데이터 생성
    const metrics: ServerMetrics[] = demoGenerator.generateTimeSeriesData(count);
    
    // 로그 데이터 생성 (선택적)
    const logs: DemoLogEntry[] = includeLog ? demoGenerator.generateLogEntries(Math.min(count, 30)) : [];
      ...
      default:
        return NextResponse.json({
          ...
          endpoints: {
            ...
            'GET /api/v1/demo/scenarios?action=info&scenario=<name>': '시나리오 상세 정보',
            'GET /api/v1/demo/scenarios?action=preview': '모든 시나리오 미리보기'
          },
          usage: {
            generateData: 'POST { "scenario": "ddos", "count": 50, "includeLog": true }',
...
function calculateKeyIndicators(metrics: ServerMetrics[]) {
  ...
  return {
    avgCPU: Math.round((cpuValues.reduce((sum, val) => sum + val, 0) / cpuValues.length) * 100) / 100,
    avgMemory: Math.round((memoryValues.reduce((sum, val) => sum + val, 0) / memoryValues.length) * 100) / 100,
    avgResponseTime: Math.round((responseTimeValues.reduce((sum, val) => sum + val, 0) / responseTimeValues.length) * 100) / 100,
