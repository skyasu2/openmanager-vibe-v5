...
import { RealisticDataGenerator, DemoScenario, ServerMetrics, DemoLogEntry } from '@/services/data-generator/RealisticDataGenerator';

// 🤖 AI 학습용 데이터 생성기
const aiTrainingGenerator = new RealisticDataGenerator();

// 🎯 AI 학습 타입 정의
type TrainingType = 'classification' | 'anomaly_detection' | 'prediction' | 'clustering' | 'mixed';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🤖 AI 학습 데이터셋 생성: ${trainingType}, 시나리오당 ${samplesPerScenario}개 샘플`);

    // 시나리오별 데이터 생성
    const scenarios: DemoScenario[] = ['normal', 'spike', 'memory_leak', 'ddos', 'performance_degradation'];
    ...
    for (const scenario of scenarios) {
      ...
      const scenarioMetrics = aiTrainingGenerator.generateTimeSeriesData(samplesPerScenario);
      const scenarioLogs = aiTrainingGenerator.generateLogEntries(Math.floor(samplesPerScenario / 3));
    ...
    trainingData.metadata.datasetInfo.totalSamples = trainingData.features.length;
    trainingData.metadata.datasetInfo.featureCount = trainingData.features[0]?.length || 0;
    ...
    // 데이터 정규화
    const normalizedData = normalizeData(trainingData.features, normalizationMode);
    
    // Train/Test 분할
    const splitData = splitTrainTest([...normalizedData], [...trainingData.labels], testSplit);
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
        ...
        return NextResponse.json({
          trainingTypes: {
            classification: {
              ...
              features: ['cpu', 'memory', 'disk', 'network', 'responseTime', 'errorRate'],
              labels: ['normal', 'spike', 'memory_leak', 'ddos', 'performance_degradation'],
              useCase: '실시간 서버 상태 분류'
            },
            anomaly_detection: {
              description: '이상 탐지 모델 학습용 데이터',
              features: ['cpu', 'memory', 'disk', 'network', 'responseTime', 'patterns'],
              labels: [0, 1], // 0: normal, 1: anomaly
              useCase: '비정상 서버 동작 탐지'
            },
            prediction: {
              description: '시계열 예측 모델 학습용 데이터',
              features: ['sequence_data', 'temporal_patterns', 'seasonal_effects'],
      ...
      case 'sample':
        // 샘플 데이터 생성
        const sampleType = url.searchParams.get('type') as TrainingType || 'classification';
        const sampleSize = parseInt(url.searchParams.get('size') || '10');
        
        aiTrainingGenerator.setScenario('normal');
        const sampleMetrics = aiTrainingGenerator.generateTimeSeriesData(sampleSize);
        const processed = processDataForTraining(sampleMetrics, [], 'normal', sampleType);
      ...
      default:
        return NextResponse.json({
          ...
          supportedTypes: ['classification', 'anomaly_detection', 'prediction', 'clustering', 'mixed'],
          endpoints: {
            'POST /api/v1/demo/ai-training': '학습 데이터셋 생성',
            'GET /api/v1/demo/ai-training?action=templates': '학습 타입 템플릿',
            'GET /api/v1/demo/ai-training?action=sample&type=<type>&size=<n>': '샘플 데이터'
          },
          usage: {
            generateDataset: 'POST { "trainingType": "classification", "samplesPerScenario": 100 }',
...
function getFeatureNames(trainingType: TrainingType): string[] {
  switch (trainingType) {
    case 'classification':
      return ['cpu', 'memory', 'disk', 'networkIn', 'networkOut', 'responseTime', 'errorRate', 'throughput'];
    case 'anomaly_detection':
      return ['cpu', 'memory', 'disk', 'responseTime', 'errorRate', 'anomalyScore'];
    case 'prediction':
      return ['cpu', 'memory', 'responseTime', 'activeConnections', 'hourSin', 'hourCos'];
    case 'clustering':
...
function normalizeData(features: number[][], mode: string): number[][] {
  ...
  for (let i = 0; i < featureCount; i++) {
    ...
    for (let j = 0; j < features.length; j++) {
      ...
      normalized[j][i] = mode === 'standard' ? (features[j][i] - mean) / (std || 1) : features[j][i];
    }
  }
 ...
 * ✂️ Train/Test 분할
 */
function splitTrainTest(features: number[][], labels: (string | number)[], testSplit: number) {
 ...
 * 📊 클래스 분포 계산
 */
function getClassDistribution(labels: (string | number)[]): Record<string, number> {
 ...
 * ✅ 데이터 품질 평가
 */
function assessDataQuality(features: number[][], labels: (string | number)[]): Record<string, any> {
  const totalSamples = features.length;
 ...
 * 🎯 전체 품질 점수
 */
function calculateOverallQuality(features: number[][], labels: (string | number)[]): number {
