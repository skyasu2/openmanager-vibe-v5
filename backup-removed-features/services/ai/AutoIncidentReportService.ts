...
export class AutoIncidentReportService {
     ...
     */
    async generateIncidentReportFromQuery(query: string): Promise<{ success: boolean, report?: IncidentReport, error?: string }> {
        try {
     ...
     */
    async generateAutomaticIncidentReport(): Promise<{ success: boolean, report?: IncidentReport, error?: string }> {
        try {
            ...
            if (!this.isIncidentDetected(anomalies, systemData, metricsData)) {
                return { success: false, error: '현재 장애 상황이 감지되지 않았습니다.' };
            }

            // 자동 보고서 생성
            const report = this.generateReport({
                query: '자동 장애 감지',
                queryAnalysis: { scope: 'automatic', timeRange: '1h', severity: 'auto-detected' },
                systemData,
                logData,
                metricsData,
...
생성 일시: ${KoreanTimeUtil.logTimestamp()}
=================================================================
`;
     ...
     */
    async createDownloadUrl(reportId: string): Promise<{ success: boolean, downloadUrl?: string, filename?: string, error?: string }> {
        try {
    ...
    private async detectAnomalies(): Promise<any[]> {
        // 자동 이상 징후 감지
        return [
            { type: 'cpu_spike', severity: 'high', timestamp: new Date().toISOString() },
            { type: 'memory_leak', severity: 'medium', timestamp: new Date().toISOString() }
        ];
    }

    private isIncidentDetected(anomalies: any[], systemData: any, metricsData: any): boolean {
    ...
    private generateReport(data: any): IncidentReport {
        ...
        return {
            ...
            duration: data.triggeredBy === 'automatic_detection' ? undefined : this.calculateDuration(startTime),
