 ...
 *
 * Features:
 * - 3-Tier 처리 전략 (Core → Enhanced → Google_Extended)
 * - 실시간 컴포넌트 상태 모니터링
 * - Multi-AI 사고 과정 추적
 * - 자동 폴백 및 복구
 * - Google AI 할당량 관리
...
export class GracefulDegradationManager {
  ...
  determineProcessingStrategy(systemHealth: SystemHealth): ProcessingStrategy {
    ...
    if (hasBeta && hasEnhanced) {
      return {
        tier: 'google_extended',
        availableEngines: availableComponents,
        usageReason: this.evaluateGoogleExtendedUsageNeed(),
  ...
  private evaluateGoogleExtendedUsageNeed(): string {
    const usageRate =
      this.resourceManager.dailyQuota.googleAIUsed /
      this.resourceManager.dailyQuota.googleAILimit;
