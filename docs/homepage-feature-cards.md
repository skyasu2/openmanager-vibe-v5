...
#### 🛠️ 기술 스택
- **OpenAI·Claude·Gemini(@ai-sdk)** 모델을 자동 선택해 분석
- **MCP Orchestrator**가 `statistical_analysis`와 `anomaly_detection`을 조합
- **Scikit‑learn**과 **Transformers.js** 연동
- **Supabase(Postgres)**와 **Upstash Redis** 실시간 데이터 활용
...
#### 🛠️ 기술 스택
- **prom-client**로 Prometheus 호환 메트릭을 실시간 생성
- **node-cron 기반 TimerManager** 최적화로 CPU 사용량 최소화
- **Upstash Redis** 캐시와 **delta-compression**으로 65% 압축률 달성
- **Express** 서버의 **/api/data-generator** 엔드포인트 제공
...
#### 🛠️ 기술 스택
- **Next.js 15**와 **React 19** 기반 최신 프론트엔드
- **Zustand**와 **@tanstack/react-query**로 상태와 캐시 관리
...
- **GitHub Copilot**과 **auto-doc-generator.js** 활용
