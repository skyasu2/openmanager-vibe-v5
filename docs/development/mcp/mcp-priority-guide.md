...
| ------------------- | -------------------- | ---------------------------------------------------------- | ------------- | ---------- |
| **코드 검색**       | Serena               | 심볼/구조 분석, 코드 패턴 찾기 시                          | Grep, Find    | 3-5배 빠름 |
| **파일 작업**       | filesystem           | 파일 읽기/쓰기, 디렉토리 탐색 시                           | Read, Write   | 표준화     |
...
#### 2️⃣ @filesystem - 파일 작업 전문

```bash
# ❌ Before: 모든 MCP 활성화
"src 디렉토리의 모든 파일 목록을 보여줘"
# 토큰: ~145 토큰

# ✅ After: filesystem만 활성화
@filesystem "src 디렉토리의 모든 파일 목록을 보여줘"
# 토큰: ~118 토큰 (19% 절약!)
# filesystem 도구만 로드: read_text_file, list_directory, search_files, write_file 등
```

#### 3️⃣ @context7 - 라이브러리 문서 조회
...
#### 8️⃣ @sequential-thinking - 복잡한 분석
...
```bash
# 📝 예시: 코드 검색 + 파일 읽기
@serena @filesystem "LoginClient.tsx의 심볼을 분석하고 전체 파일 내용을 읽어줘"
# 토큰: ~130 토큰 (2개 서버만 활성화, 13% 절약)

# 📚 예시: 코드 검색 + 라이브러리 문서 조회
