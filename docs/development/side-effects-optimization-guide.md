...
} from './config/performance/timeout-optimization.js';
...
import {
  globalMCPManager,
  MCPCircuitBreaker,
} from './config/mcp/stability-config.js';
...
import {
  DEVELOPMENT_CONFIG,
  WSL_CONFIG,
} from './config/environments/development.js';
...
3. **MCP Circuit Breaker 활성화**
   ...
   const mcpResult = await globalMCPManager.connectToMCP(
     serverId,
     connectionFn
   );
   ```
