...
class MCPLocalManager {
  ...
  validateConfig() {
    try {
      ...
      console.log(
        `📦 등록된 MCP 서버: ${Object.keys(config.mcpServers).length}개`
      );
      return true;
    } catch (error) {
  ...
  startServer() {
    ...
    server.unref();
    console.log(
      `✅ MCP 서버가 백그라운드에서 시작되었습니다 (포트: ${this.port})`
    );
  ...
  listMCPServers() {
    try {
      ...
      if (config.settings?.typescript) {
        ...
        console.log(
          `   UI 라이브러리: ${config.settings.typescript.uiLibrary}`
        );
  ...
  showHelp() {
    ...
    console.log(
      'npm run cursor:mcp           - Cursor MCP 설정 검증 및 가이드'
    );
    console.log('npm run mcp:help             - 도움말');
    console.log('\n🚀 TypeScript 개발 전용 명령어:');
    console.log(
      'node development/scripts/mcp-local-manager.js list      - MCP 서버 목록'
    );
    console.log(
      'node development/scripts/mcp-local-manager.js magic     - Magic MCP 설정 가이드'
    );
  }
}
