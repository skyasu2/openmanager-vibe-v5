...
사용법:
  node development/scripts/mcp-profile-manager.js [명령] [프로필]
...
function listProfiles() {
  ...
  Object.entries(profiles).forEach(([name, configPath]) => {
    const fullPath = path.resolve(
      path.join(__dirname, '..', 'mcp-config', configPath)
    );
    ...
    if (exists) {
      try {
        ...
        console.log(
          `     └─ ${serverCount}개 서버, 환경: ${config.environment || 'unknown'}`
        );
      } catch (e) {
  ...
  try {
    ...
    if (config.mcpServers) {
      ...
      Object.entries(config.mcpServers).forEach(([name, server]) => {
        ...
        console.log(
          `  ${status} ${name} - ${server.description || 'No description'}`
        );
      });
    }
    console.log();
