...
class MCPSystemTester {
  ...
  private async testSystemHealth(): Promise<void> {
    ...
    // 기본 헬스 체크
    suite.tests.push(
      await this.runTest('기본 헬스 체크', async () => {
    ...
    // MCP 상태 체크
    suite.tests.push(
      await this.runTest('MCP 상태 체크', async () => {
    ...
    // 통합 AI 시스템 상태
    suite.tests.push(
      await this.runTest('통합 AI 시스템 상태', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/ai/unified?action=health`
        );
        const data = await response.json();
  ...
  private async testUnifiedAI(): Promise<void> {
    ...
    // 기본 질의 테스트
    suite.tests.push(
      await this.runTest('기본 질의 처리', async () => {
    ...
    // 한국어 NLP 테스트
    suite.tests.push(
      await this.runTest('한국어 NLP 분석', async () => {
        const response = await fetch(`${this.baseUrl}/api/ai/unified`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            question:
              '서버에 문제가 있는 것 같아요. CPU 사용률이 너무 높습니다.',
    ...
    // 시스템 초기화 테스트
    suite.tests.push(
      await this.runTest('시스템 초기화', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/ai/unified?action=initialize`,
          {
            method: 'PUT',
          }
        );
  ...
  private async testMCPStatus(): Promise<void> {
    ...
    // 전체 상태 조회
    suite.tests.push(
      await this.runTest('전체 상태 조회', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/mcp-status?view=overview`
        );
        const data = await response.json();
    ...
    // 컴포넌트 상세 조회
    suite.tests.push(
      await this.runTest('컴포넌트 상세 조회', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/mcp-status?view=components`
        );
        const data = await response.json();
    ...
    // 성능 메트릭 조회
    suite.tests.push(
      await this.runTest('성능 메트릭 조회', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/mcp-status?view=performance`
        );
        const data = await response.json();
    ...
    // 시스템 핑 테스트
    suite.tests.push(
      await this.runTest('시스템 핑', async () => {
        const response = await fetch(`${this.baseUrl}/api/system/mcp-status`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  private async testFastAPIIntegration(): Promise<void> {
    ...
    // FastAPI 연결 테스트
    suite.tests.push(
      await this.runTest('FastAPI 연결 상태', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/mcp-status?view=components`
        );
        const data = await response.json();
    ...
    // Python 웜업 테스트
    suite.tests.push(
      await this.runTest('Python 엔진 웜업', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/python-warmup`,
          {
            method: 'POST',
          }
        );
  ...
  private async testKeepAliveSystem(): Promise<void> {
    ...
    // Keep-Alive 상태 확인
    suite.tests.push(
      await this.runTest('Keep-Alive 상태', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/mcp-status?view=components`
        );
        const data = await response.json();
  ...
  private async testContextManagers(): Promise<void> {
    ...
    // 컨텍스트 상태 확인
    suite.tests.push(
      await this.runTest('컨텍스트 상태', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/system/mcp-status?view=components`
        );
        const data = await response.json();
  ...
  private async testErrorHandling(): Promise<void> {
    ...
    // 잘못된 요청 테스트
    suite.tests.push(
      await this.runTest('잘못된 질의 요청', async () => {
        const response = await fetch(`${this.baseUrl}/api/ai/unified`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    // 존재하지 않는 엔드포인트
    suite.tests.push(
      await this.runTest('존재하지 않는 엔드포인트', async () => {
        const response = await fetch(
          `${this.baseUrl}/api/non-existent-endpoint`
        );
  ...
  private async testPerformance(): Promise<void> {
    ...
    // 동시 요청 테스트
    suite.tests.push(
      await this.runTest('동시 요청 처리 (5개)', async () => {
        ...
        const avgTime =
          results.reduce(
            (sum, r) => sum + (r.metadata?.processingTime || 0),
            0
          ) / results.length;
    ...
    // 메모리 사용량 테스트
    suite.tests.push(
      await this.runTest('메모리 사용량 확인', async () => {
  ...
  private printSummary(): void {
    ...
    this.results.forEach(suite => {
      ...
      console.log(`\n${status} ${suite.name}`);
      console.log(
        `   성공률: ${suite.successRate}% (${suite.tests.filter(t => t.success).length}/${suite.tests.length})`
      );
    ...
    if (overallSuccessRate < 100) {
      ...
      if (overallSuccessRate < 70) {
        console.log(
          '   - 시스템에 심각한 문제가 있습니다. 즉시 점검이 필요합니다.'
        );
      } else {
        console.log(
          '   - 일부 기능에 문제가 있습니다. 실패한 테스트를 확인해주세요.'
        );
      }
