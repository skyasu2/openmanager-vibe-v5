...
const MCP_TOOLS = [
  {
    ...
    setup: async () => {
      ...
      console.log(
        '📝 21st.dev에서 API 키를 발급받으세요: https://21st.dev/api-access'
      );
  ...
  },
  {
    ...
    setup: async () => {
      ...
      console.log(
        '🔗 Chrome 확장 프로그램을 설치하세요: https://github.com/AgentDeskAI/browser-tools-mcp'
      );
      console.log(
        '⚙️  브라우저 디버깅을 위해 개발자 도구에서 BrowserToolsMCP 패널을 확인하세요'
      );
    },
  },
  {
    ...
    setup: async () => {
      ...
      console.log(
        '📦 이제 Cursor에서 다른 MCP 서버들을 쉽게 설치할 수 있습니다'
      );
    },
  },
];
...
async function checkMcpConfig() {
  ...
  try {
    ...
    console.log(
      `📊 설정된 MCP 서버 개수: ${Object.keys(config.mcpServers || {}).length}`
    );
    return true;
  } catch (error) {
