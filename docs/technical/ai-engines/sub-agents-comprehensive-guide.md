...
서브 에이전트는 특정 작업을 전문적으로 수행하는 AI 역할 정의입니다. 각 에이전트는:
- 전문 영역: 특정 기술 도메인에 특화
- MCP 통합: 관련 MCP 서버를 활용하여 실제 작업 수행
- 협업 가능: 다른 에이전트와 연동하여 복잡한 작업 처리
...
| 에이전트 | 역할 | 주요 MCP | 테스트 상태 |
|---------|------|----------|------------|
| central-supervisor | 마스터 오케스트레이터 | 모든 MCP | ⚪ 미테스트 |
...
| codex-cli-partner | Codex 알고리즘 최적화 | filesystem, memory | ⚪ 미테스트 |
