...
일반적인 AES 암호화는 [Web Crypto API 공식 가이드](https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API) 및 [PBKDF2 보안 가이드](https://owasp.org/www-community/controls/Using_PBKDF2)를 참조하세요.

**OpenManager VIBE v5 특화 암호화**:

- **AES-256-GCM**: 포트폴리오용 기본 보안 (OWASP 권장)
- **Vercel Edge 최적화**: 런타임 복호화 성능 개선
- **무료 티어 호환**: 메모리 사용량 최소화
- **WSL 환경 지원**: Windows/Linux 교차 호환성

핵심 구현은 `src/lib/crypto/` 디렉토리를 참조하세요.
...
암호화 키 생성 및 관리는 [NIST 암호화 가이드](https://csrc.nist.gov/projects/cryptographic-standards-and-guidelines)를 따릅니다.
...
# 프로젝트 전용 암호화 키 설정
# (32자 이상, 특수문자 포함)
echo "OpenManager-Vibe-v5-Secret-2025!" > .env.key
