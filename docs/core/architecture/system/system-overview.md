...
| **Analyst Agent** | Gemini Pro | 패턴 분석, 이상 탐지 | `analyzePattern` |
| **Reporter Agent** | Llama 70b | 리포트 생성, RAG | `searchKnowledgeBase` |
...
│                     VERCEL (Frontend/BFF)                        │
│  ┌─────────────┐    ┌────────────────┐    ┌─────────────────┐  │
│  │ AIWorkspace │───▶│ useChat (SDK)  │───▶│ /api/ai/        │  │
│  │ 컴포넌트     │    │ DefaultTransport│   │ supervisor      │  │
│  └─────────────┘    └────────────────┘    └────────┬────────┘  │
└────────────────────────────────────────────────────┼────────────┘
