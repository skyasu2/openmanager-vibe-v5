...
이 파일은 v5.64.3 이전의 모든 변경 기록을 포함합니다.
...
- **Agent Evolution Manager 서브 에이전트 추가**:
  - 10번째 서브 에이전트로 에이전트 진화 관리자 추가
  - 다른 모든 에이전트의 성능을 자동으로 모니터링하고 개선하는 메타 에이전트
  - 백그라운드 24/7 작동으로 지속적인 시스템 개선
  - 반복 실패 패턴, 성능 저하, 새로운 기술 트렌드 감지 시 자동 활성화
  - A/B 테스트를 통한 개선안 검증 및 안전한 자동화 구현

### 🔧 Maintenance

- **agent-evolution-manager.md 파일 표준화**:
  - 다른 서브 에이전트와 동일한 양식으로 수정
  - 한국어 400자 이내 description 작성
  - recommended_mcp 섹션 추가 (primary/secondary 분류)
  - 이모지 추가 (🧬)로 일관성 유지

### 📝 Documentation

- **서브 에이전트 문서 업데이트**:
  - CLAUDE.md: 9개에서 10개 서브 에이전트로 업데이트
  - docs/sub-agents-mcp-mapping-guide.md: Agent Evolution Manager 추가
  - MCP 사용 빈도 매트릭스에 새로운 에이전트 추가
  - 서브 에이전트 사용 예시에 에이전트 진화 관리 예제 추가

### 📊 Summary

- **에이전트 생태계 강화**: 자가 진화 시스템으로 지속적인 품질 향상
- **자동화된 성능 개선**: 수동 개입 없이 에이전트들의 성능 최적화
- **안전한 변경 관리**: 백업, 롤백, 점진적 개선으로 안정성 보장

## [5.63.61] - 2025-07-26

### 🎯 Features

- **서브 에이전트 MCP 매핑 시스템 구축**:
  - 9개 서브 에이전트 모두 정상 동작 확인
  - 각 에이전트별 최적 MCP 서버 조합 정의
  - Primary/Secondary MCP 분류로 우선순위 명확화
  - 작업 특성에 맞는 MCP 추천 시스템 구현

### 🔧 Maintenance

- **서브 에이전트 설정 파일 개선**:
  - 모든 에이전트 파일에 `recommended_mcp` 섹션 추가
  - 주요(primary) 및 보조(secondary) MCP 서버 명시
  - 각 MCP의 용도를 주석으로 설명

### 📝 Documentation

- **`docs/sub-agents-mcp-mapping-guide.md` 생성**:
  - 서브 에이전트별 상세 MCP 매핑 가이드
  - MCP 사용 빈도 매트릭스 제공
  - 실제 사용 예시 코드 포함
  - 성능 최적화 팁 제공

- **CLAUDE.md 업데이트**:
  - Sub Agents 섹션에 MCP 매핑 정보 추가
  - 에이전트별 추천 MCP 서버 목록화
  - 실제 Task 호출 예시 추가
  - 상세 가이드 문서 링크 제공
...
- **MCP 사용 최적화**: 각 서브 에이전트가 작업에 필요한 최소한의 MCP만 사용하도록 최적화
- **명확한 가이드라인**: 어떤 작업에 어떤 에이전트와 MCP 조합을 사용해야 하는지 명확화
- **성능 향상**: 불필요한 MCP 로드를 줄여 컨텍스트 효율성 향상

## [5.63.60] - 2025-07-26

### 🔧 Maintenance

- **서브 에이전트 시스템 정리 및 최적화**:
  - planner-spec (Sub Agent Orchestrator) 삭제 - 현재 9개 서브 에이전트로 운영
  - 삭제된 에이전트 관련 문서 참조 정리 완료
  - gemini-cli-collaborator.md의 불필요한 "color: blue" 라인 제거

- **서브 에이전트 및 MCP 서버 점검**:
  - 9개 서브 에이전트 모두 정상 작동 확인
  - 9개 MCP 서버 모두 정상 작동 확인 (GitHub 토큰 이슈 없음)
  - Supabase RLS 보안 이슈 발견 - 7개 테이블에 RLS 비활성화 상태

### 📝 Documentation

- **CLAUDE.md 대폭 간소화 (26KB → 3.6KB)**:
  - Claude Code 베스트 프랙티스 적용 (50KB 이하 권장)
  - 목차 20개 → 핵심 섹션만 유지
  - 중복 내용 제거 및 간결한 bullet points 사용
  - "think hard" 모드 섹션 간소화
  - 긴 서술형 설명을 핵심 정보 위주로 압축
  - 가독성과 실용성 크게 향상
...
- **서브 에이전트 파일명 표준화**:
  - 모든 서브 에이전트 파일명을 `이모지-에이전트이름.md` 형식으로 통일
  - 시각적 구분을 위한 이모지 추가로 가독성 향상
  - 🎨 UX, 🎭 오케스트레이터, 📚 문서, 🔍 보안, 🗄️ DB, 🚨 DevOps, 🛠️ MCP, 🤖 AI, 🧪 QA
  - 웹 검색을 통한 Claude Code 베스트 프랙티스 반영
  - 모든 description을 400자 이내로 간결하게 최적화

### 📝 Documentation

- **CLAUDE.md 서브 에이전트 파일 구조 섹션 추가**:
  - 새로운 파일명 형식과 구조를 명확하게 문서화
  - 각 서브 에이전트의 역할을 한 줄로 요약 제공
...
- **Claude Code 서브 에이전트 설정 간략화**
  - planner-spec: 요구사항을 구조화된 작업 계획으로 변환하는 전문 에이전트
  - issue-summary: 서비스 상태 자동 점검 및 이슈 요약 에이전트
  - mcp-server-admin: MCP 서버 관리 전문가 에이전트
  - 모든 서브 에이전트 설명을 한국어로 통일
  - 장황한 설명을 핵심 기능 위주로 간략화
  - 기존 에이전트와 기능 중복 없음 확인
  - CLAUDE.md에 전체 서브 에이전트 목록 업데이트

## [5.63.20] - 2025-07-25

### 💄 UI/UX 개선

- **메인 페이지 시스템 시작 버튼 카운트다운 효과 개선**
  - 깜빡임(animate-pulse) 효과를 진행바로 변경
  - 3초 동안 부드럽게 채워지는 진행바 애니메이션 구현
  - 빛나는 효과(shimmer) 추가로 시각적 피드백 강화
  - 카운트다운 중 호버 효과 비활성화로 안정성 향상
  - 기존 UX/UI 유지하면서 더 부드러운 사용자 경험 제공
...
- **대시보드 서버 카드 표시 문제 수정**
  - 서버가 있을 때 서버 카드가 표시되지 않던 문제 해결
  - 조건부 렌더링 로직 수정 (반대로 구현되어 있던 문제)
  - 서버가 있을 때: InfrastructureOverviewPage + ServerDashboard 모두 표시
  - 서버가 없을 때: 빈 상태 메시지 표시

### 🚀 Improved

- **대시보드 레이아웃 개선**
  - 인프라 전체 현황과 서버 카드를 함께 표시
  - 더 나은 사용자 경험을 위한 레이아웃 구성
  - Mock 데이터 시스템 정상 작동 확인
...
- **OAuth 콜백 PKCE 에러 해결**
  - `AuthApiError: invalid request: both auth code and code verifier should be non-empty` 오류 수정
  - 미들웨어와 클라이언트 사이드의 중복 PKCE 처리 제거
  - 미들웨어가 PKCE 플로우를 전담하도록 구조 개선

### 🔄 Refactored

- **`/auth/callback` 페이지 간소화**
  - 클라이언트 사이드 `exchangeCodeForSession` 호출 제거
  - 미들웨어의 `updateSession`이 모든 PKCE 처리 담당
  - 세션 확인 로직만 남겨 성능 및 안정성 향상

### 🚀 Improved

- **미들웨어 OAuth 처리 최적화**
  - `/auth/callback` 경로에서 PKCE 자동 처리
  - Phase 3 최적화 지원: 성공 시 바로 `/main`으로 리다이렉트
  - 디버그 로그 추가로 문제 추적 용이
...
- GitHub OAuth 로그인 최적화 Phase 2 & 3 적용
  - Vercel 환경: 14.5초 → 2초 미만 (86% 개선)
  - 로컬 환경: 8.5초 → 0.5초 미만 (88% 개선)
- 이벤트 기반 세션 감지로 즉각적인 인증 처리
- 스마트 쿠키 폴링으로 불필요한 대기 제거
- 조건부 세션 새로고침으로 중복 호출 방지
- success 페이지 건너뛰기 옵션 추가 (Phase 3)
- 상세 성능 메트릭 측정 기능 추가

### 🔧 기술적 개선

- `success/page.tsx`: 성능 측정 도구 및 최적화 로직 구현
- `callback/page.tsx`: 다이렉트 리다이렉트 모드 추가
- 개발 환경에서 성능 메트릭 UI 표시 기능
...
- **클라이언트 사이드 OAuth 처리로 변경**
  - `/auth/callback` 페이지에서 직접 `exchangeCodeForSession` 호출
  - 미들웨어의 OAuth 처리 로직 완전 제거
  - Vercel Edge Runtime 환경에서의 호환성 문제 해결
- **명시적 OAuth 코드 교환**
  - URL에서 OAuth 코드 파라미터 확인
  - 클라이언트에서 직접 세션 생성
  - 에러 처리 로직 강화
...
- **Vercel 환경에서 GitHub OAuth 인증 후 리다이렉트 실패 문제 해결**
  - `/auth/success` 페이지: Vercel 환경 감지 및 더 긴 대기 시간 적용
  - 세션 확인 재시도 로직 강화 (최대 7회, Vercel에서는 더 긴 대기)
  - 라우터 캐시 갱신 여러 번 수행으로 세션 인식 향상
  - 최종 세션 확인 단계 추가로 안정성 강화
- **미들웨어 Vercel 환경 최적화**
  - Vercel 환경 자동 감지 (hostname, 환경변수 기반)
  - 환경별 차별화된 재시도 로직 (Vercel: 8회, 로컬: 5회)
  - 세션 새로고침 시점 최적화 및 Vercel 전용 추가 시도
- **디버깅 API 엔드포인트 개선**
  - `/api/auth/debug`: 세션 상태 상세 정보 제공
  - 클라이언트/미들웨어 세션 비교 분석
  - 쿠키 정보 및 환경 설정 통합 진단
  - POST 메서드로 세션 새로고침 테스트 기능 추가

#### Improved

- 재시도 횟수 UI 표시로 사용자 경험 개선
- 로그 메시지 개선으로 문제 추적 용이성 향상
...
- **GitHub OAuth 로그인 후 리다이렉트 루프 문제 해결**
  - 인증 완료 후 `/main` 대신 `/login` 페이지로 돌아가는 문제 수정
  - OAuth 콜백과 미들웨어 간 세션 동기화 타이밍 이슈 해결
- **미들웨어 세션 체크 로직 강화**
  - OAuth 콜백 후 더 많은 재시도 (2→5회) 및 긴 대기시간 (500ms→1000ms) 적용
  - referer 헤더 기반 OAuth 상태 감지로 더 정확한 세션 체크
  - 세션 새로고침 추가 시도로 세션 안정성 향상
- **인증 성공 페이지 세션 안정화 개선**
  - 세션 안정화 대기시간 증가 (1.5초→2.5초)
  - 쿠키 동기화 추가 대기 (2초) 및 상태 로깅 강화
  - 완전한 페이지 새로고침을 통한 확실한 리다이렉트 보장
