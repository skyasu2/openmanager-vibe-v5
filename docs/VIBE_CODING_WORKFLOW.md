...
- **Cursor AI + Claude 4 Sonnet**
  - 실시간 코드 생성 및 자동완성
  - 전체 프로젝트 컨텍스트 인식
  - 타이핑과 동시에 지능형 코드 제안
  - 리팩토링 및 최적화 자동 수행
  - **긴 문맥 이해력과 구조적 코드 분석**에서 가장 뛰어난 성능

#### **🔧 Cursor MCP Tools 통합 활용**

- **filesystem**: 전체 코드 구조 파악, .env/README.md 등 설정 파일 분석
- **duckduckgo-search**: 외부 라이브러리 문서, 에러 해결 방법 즉시 검색
- **sequential-thinking**: 복잡한 다단계 로직에서 일관된 추론 흐름 유지
...
**Claude 4 Sonnet을 메인으로 하되, 작업 특성에 따라 최적 AI 선택:**

| AI 도구            | 활용 영역                       | 선택 이유                         |
| ------------------ | ------------------------------- | --------------------------------- |
| **GPT-4-turbo**    | 빠른 코드 생성, 아이디어 실험   | 응답 속도 우수, 프롬프트 다양성   |
| **Gemini 1.5 Pro** | 대규모 문서 처리, 멀티모달 작업 | 백그라운드 자동화, 다중 파일 처리 |

#### **Google Jules (Gemini 1.5 Pro 기반)**
...
병행 AI 도구들: 5%
  - GPT-4-turbo: 빠른 아이디어 실험
  - Gemini 1.5 Pro: 대규모 문서 처리
  - Google Jules + OpenAI Codex: 백그라운드 최적화
```
