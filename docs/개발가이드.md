...
**v5.44.4부터 적용되는 엄격한 사용 정책:**

#### ✅ **Google AI 사용 허용 영역**

- **자연어 질의**: `/api/ai/unified-query` 엔드포인트
- **AI 채팅**: 사용자와의 대화형 인터페이스
- **텍스트 생성**: 사용자 요청에 따른 콘텐츠 생성

#### ❌ **Google AI 사용 금지 영역**

- **모니터링 시스템**: `IntelligentMonitoringService`, `AnomalyDetectionService`
- **자동장애 분석**: `AutoReportService`, `AutoIncidentReportSystem`
- **시스템 분석**: 서버 메트릭, 로그 분석, 성능 모니터링
- **백그라운드 작업**: 자동화된 시스템 작업, 스케줄링된 작업
- **데이터 처리**: 실시간 데이터 분석, 메트릭 수집

### 🛠️ **개발 시 AI 엔진 선택 가이드**

```typescript
// ✅ 자연어 질의 - Google AI 사용 허용
...
export class NaturalLanguageService {
  async processUserQuery(query: string) {
    ...
    return await router.processQuery({ query, mode: 'AUTO' });
  }
}

// ❌ 모니터링 기능 - Google AI 사용 금지
import { GoogleAIService } from '@/services/ai/GoogleAIService'; // ❌ 금지!
...
export class MonitoringService {
  async analyzeSystemMetrics(metrics: any[]) {
    // 로컬 AI 엔진 또는 한국어 AI 엔진 사용
    return await koreanAIEngine.processQuery(analysisQuery);
  }
}
```
...
| 우선순위 | 엔진             | 사용 목적                | 비고                |
| -------- | ---------------- | ------------------------ | ------------------- |
| 1순위    | Korean AI Engine | 한국어 처리, 시스템 분석 | 오프라인 지원       |
| 2순위    | Local AI Engine  | 메트릭 분석, 패턴 감지   | 빠른 응답           |
| 3순위    | Google AI        | 자연어 질의만            | 비용 및 할당량 고려 |

### ⚠️ **주의사항**

1. **새로운 기능 개발 시** Google AI 사용 전 반드시 정책 확인
2. **코드 리뷰 시** Google AI 사용이 적절한지 검토
3. **테스트 시** 로컬 AI 엔진으로도 정상 작동하는지 확인
4. **문서 작성 시** 사용된 AI 엔진 명시
