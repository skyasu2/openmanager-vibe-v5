...
const TEST_CASES = {
  koreanNLP: [
    ...
    },
    {
      ...
      request: {
        query:
          'db-master 서버의 메모리가 90% 넘었고 어제부터 문제가 발생했습니다',
        mode: 'detailed',
      },
      expected: {
        success: true,
...
async function checkHealth(name, url) {
  ...
  try {
    ...
    if (data.status === 'healthy') {
      ...
      console.log(
        chalk.gray(`   버전: ${data.version}, 메모리: ${data.memory}`)
      );
      if (data.features) {
...
async function runTests(functionName, endpoint, testCases) {
  ...
  for (const testCase of testCases) {
    ...
    if (!response.success) {
      console.log(
        chalk.red(`   ❌ 실패: ${response.error || '알 수 없는 오류'}`)
      );
      continue;
    }
    ...
    if (functionName === 'Korean NLP') {
      ...
      if (metadata && metadata.analysis) {
        if (
          ...
          console.log(
            chalk.red(
              `   의도 불일치: ${metadata.analysis.intent} (예상: ${testCase.expected.intent})`
            )
          );
        }

        if (
          testCase.expected.sentiment &&
          metadata.analysis.sentiment !== testCase.expected.sentiment
        ) {
          testPassed = false;
          console.log(
            chalk.red(
              `   감정 불일치: ${metadata.analysis.sentiment} (예상: ${testCase.expected.sentiment})`
            )
          );
        }
        ...
        if (testCase.expected.entities) {
          ...
          const hasAllEntities = testCase.expected.entities.every(expected =>
            entityTexts.some(entity => entity.includes(expected))
          );
          if (!hasAllEntities) {
            testPassed = false;
            console.log(
              chalk.red(`   엔티티 누락: ${JSON.stringify(entities)}`)
            );
          }
        }
      }
    ...
    if (functionName === 'Basic ML') {
      ...
      if (
        ...
        console.log(
          chalk.red(
            `   분류 불일치: ${metadata.classification} (예상: ${testCase.expected.classification})`
          )
        );
      }
      ...
      if (testCase.expected.hasPredictions && !metadata.predictions) {
        ...
        testCase.expected.trend &&
        metadata.predictions.trend !== testCase.expected.trend
      ) {
        testPassed = false;
        console.log(
          chalk.red(
            `   트렌드 불일치: ${metadata.predictions.trend} (예상: ${testCase.expected.trend})`
          )
        );
      }
    ...
    if (testPassed) {
      ...
      console.log(
        chalk.gray(`   신뢰도: ${(response.confidence * 100).toFixed(1)}%`)
      );
      console.log(
        chalk.gray(`   응답: ${response.response.substring(0, 100)}...`)
      );
      passedTests++;
    } else {
...
async function main() {
  ...
  // 헬스 체크
  console.log(chalk.blue('\n1️⃣  헬스 체크'));
  const koreanHealthOk = await checkHealth(
    'Korean NLP Python',
    ENDPOINTS.koreanNLPHealth
  );
  const mlHealthOk = await checkHealth(
    'Basic ML Python',
    ENDPOINTS.basicMLHealth
  );
  ...
  console.log(chalk.yellow('\n📈 테스트 성공률:'));
  console.log(
    `   Korean NLP: ${koreanResults.successRate}% (${koreanResults.passed}/${koreanResults.total})`
  );
  console.log(
    `   Basic ML: ${mlResults.successRate}% (${mlResults.passed}/${mlResults.total})`
  );
