...
class MCPHealthChecker {
  constructor() {
    ...
    this.inactiveConfigPath = path.join(
      process.cwd(),
      'mcp-configs-inactive.json'
    );
  }
  ...
  listInactiveServers() {
    try {
      const config = JSON.parse(
        fs.readFileSync(this.inactiveConfigPath, 'utf8')
      );
      console.log('🔴 **비활성 MCP 서버 목록**');
      console.log('================================');

      Object.entries(config.inactive_mcpServers).forEach(
        ([name, server], index) => {
          console.log(`${index + 1}. **${name}**`);
          console.log(`   📝 설명: ${server.description}`);
          console.log(`   ⚠️  비활성 이유: ${server.reason}`);
          console.log(
            `   🔧 명령어: ${server.command} ${server.args.join(' ')}`
          );
          console.log('');
        }
      );
  ...
  activateServer(serverName) {
    try {
      const inactiveConfig = JSON.parse(
        fs.readFileSync(this.inactiveConfigPath, 'utf8')
      );
      ...
      fs.writeFileSync(
        this.inactiveConfigPath,
        JSON.stringify(inactiveConfig, null, 2)
      );
  ...
  generateStatusReport() {
    ...
    console.log(
      `📝 총 서버: ${activeServers.length + inactiveServers.length}개`
    );
    console.log('');
  ...
  switch (args[0]) {
    ...
    case 'activate':
      if (args[1]) {
        ...
        console.log(
          '사용법: node development/scripts/mcp-health-check.js activate [서버이름]'
        );
      }
      break;
    case 'deactivate':
      if (args[1]) {
        ...
        console.log(
          '사용법: node development/scripts/mcp-health-check.js deactivate [서버이름] [이유]'
        );
      }
      break;
    default:
      ...
      console.log('사용법:');
      console.log(
        '  node development/scripts/mcp-health-check.js status     # 전체 상태 확인'
      );
      console.log(
        '  node development/scripts/mcp-health-check.js activate [서버이름]   # 서버 활성화'
      );
      console.log(
        '  node development/scripts/mcp-health-check.js deactivate [서버이름] # 서버 비활성화'
      );
      console.log('');
      console.log('예시:');
