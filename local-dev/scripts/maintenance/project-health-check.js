...
function analyzeProjectStructure() {
    ...
    // 루트 구조 분석
    results.root = {
        configFiles: countFiles(ROOT_DIR, ['.json', '.js', '.ts', '.yaml', '.yml'], true),
        sourceFiles: countFiles(path.join(ROOT_DIR, 'src'), ['.ts', '.tsx', '.js', '.jsx'])
    };
...
function analyzeGitActivity() {
    ...
    try {
        // 최근 7일간 커밋 수
        const recentCommits = execSync('git log --since="7 days ago" --oneline', { encoding: 'utf8' });
        const commitCount = recentCommits.trim().split('\n').filter(line => line.length > 0).length;
...
function calculateHealthScore(analysis) {
    ...
    // 성능 점수
    if (analysis.performance.buildTime && analysis.performance.buildTime > 120000) { // 2분 초과
        score -= 15;
...
function printReport(analysis, health, recommendations) {
    ...
    // 전체 건강도 점수
    const scoreColor = health.score >= 80 ? '🟢' : health.score >= 60 ? '🟡' : '🔴';
    console.log(`\n${scoreColor} 전체 건강도: ${health.score}/100점`);

    // 구조 분석 결과
    console.log('\n📊 프로젝트 구조 분석:');
    console.log(`  📁 개발 스크립트: ${analysis.structure.development.scripts?.count || 0}개`);
    console.log(`  📚 개발 문서: ${analysis.structure.development.docs?.count || 0}개`);
    console.log(`  ⚙️  설정 파일: ${analysis.structure.development.config?.count || 0}개`);
    console.log(`  🚀 인프라 문서: ${analysis.structure.infra.docs?.count || 0}개`);
    ...
    console.log(`  🔧 DevDependencies: ${analysis.dependencies.devDependencies}개`);
    console.log(`  📜 NPM Scripts: ${analysis.dependencies.scripts}개`);

    // 성능 메트릭
    console.log('\n⚡ 성능 메트릭:');
    console.log(`  🏗️  빌드 시간: ${analysis.performance.buildTime ? (analysis.performance.buildTime / 1000).toFixed(1) + 's' : 'N/A'}`);
    console.log(`  🧪 테스트 시간: ${analysis.performance.testTime ? (analysis.performance.testTime / 1000).toFixed(1) + 's' : 'N/A'}`);
    console.log(`  🔍 린트 시간: ${analysis.performance.lintTime ? (analysis.performance.lintTime / 1000).toFixed(1) + 's' : 'N/A'}`);
