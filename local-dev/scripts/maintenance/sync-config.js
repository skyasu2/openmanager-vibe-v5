...
function copyFile(source, target) {
    try {
        ...
        console.log(`✅ 복사됨: ${path.basename(source)} → ${path.relative(ROOT_DIR, target)}`);
        return true;
    } catch (error) {
        console.error(`❌ 복사 실패: ${path.basename(source)} → ${path.relative(ROOT_DIR, target)}: ${error.message}`);
        return false;
    }
}
