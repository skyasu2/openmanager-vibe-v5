    ...
    try {
        ...
        for (const password of teamPasswords) {
            try {
                ...
                if (Object.keys(tempRecovered).length === Object.keys(ENCRYPTED_ENV_CONFIG.variables).length) {
                    recoveredVars = tempRecovered;
                    console.log(`✅ 비밀번호 "${password.substring(0, 3)}***"로 모든 환경변수 복호화 성공!`);
                    break;
                }
        ...
        if (Object.keys(recoveredVars).length === 0) {
            console.warn('⚠️ 복호화된 환경변수가 없습니다. Vercel에 설정된 환경변수에 의존합니다.');
            return;
        }
        ...
        console.log(`✅ [TSX] 복호화 완료! ${Object.keys(recoveredVars).length}개의 환경변수를 .env.production 파일에 저장했습니다.`);
    } catch (error) {
        console.error('❌ [TSX] 빌드용 환경변수 복호화 중 심각한 오류 발생:', error);
        process.exit(1);
    }
}
