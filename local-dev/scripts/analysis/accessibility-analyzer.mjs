...
class AccessibilityAnalyzer {
  ...
  checkAriaAttributes(content, filePath) {
    ...
    const ariaPatterns = [
      ...
      {
        pattern: /aria-labelledby=["'][\s]*["']/g,
        issue: 'empty-aria-labelledby',
      },
      {
        pattern: /aria-describedby=["'][\s]*["']/g,
        issue: 'empty-aria-describedby',
      },
    ];
  ...
  async checkScreenReaderAttributes(filePath) {
    try {
      ...
      if (hasInteractiveElements) {
        const hasAriaSupport = srAttributes.some(attr =>
  ...
  generateReport() {
    ...
    // 심각도별 분류
    const errorCount = this.results.issues.filter(
      i => i.severity === 'error'
    ).length;
    const warningCount = this.results.issues.filter(
      i => i.severity === 'warning'
    ).length;
    const infoCount = this.results.issues.filter(
      i => i.severity === 'info'
    ).length;
    ...
    // WCAG 준수 상태
    const wcagAA = this.results.wcagViolations.filter(
      v => v.level === 'AA'
    ).length;
    const wcagA = this.results.wcagViolations.filter(
      v => v.level === 'A'
    ).length;
