...
class APIOptimizer {
  ...
  async analyzeRoute(routePath, usedEndpoints) {
    try {
      ...
      const endpoint =
        '/' + relativePath.replace('/route.ts', '').replace(/\\/g, '/');
  ...
  isTestOrDemo(endpoint, content) {
    ...
    return (
      patterns.some(pattern => pattern.test(endpoint)) ||
  ...
  isDevelopmentOnly(endpoint, content) {
    ...
    return devPatterns.some(
      pattern => pattern.test(endpoint) || pattern.test(content)
    );
  }
  ...
  isEmptySkeleton(content) {
    ...
    // 매우 짧고 기본 응답만
    if (lines < 15) {
      const hasBasicResponse =
        /return\s+NextResponse\.json\(\s*\{\s*[^}]*\}\s*\)/.test(content);
      const hasNoBusinessLogic =
        !/import.*Service|supabase|database|redis|fetch|axios/.test(content);
  ...
  checkIfAPIIsUsed(endpoint, usedEndpoints) {
    ...
    for (const used of usedEndpoints) {
      if (
        used.includes(endpoint.replace(/\/\[.*?\]/g, '')) ||
  ...
  generateReport() {
    ...
    console.log(
      `💰 절약 가능 크기: ${(this.results.totalSavings / 1024).toFixed(2)} KB`
    );
    console.log();
    ...
    if (this.results.totalSavings >= 436 * 1024) {
      ...
      console.log(
        `📈 목표 달성률: ${((this.results.totalSavings / (436 * 1024)) * 100).toFixed(1)}%`
      );
    }
    ...
    sorted.forEach((api, index) => {
      console.log(
        `${String(index + 1).padStart(2)}. ${api.endpoint.padEnd(35)} ${String(api.size).padStart(5)}B - ${api.reasons.join(', ')}`
      );
    });
  ...
  async confirmOptimization() {
    ...
    console.log(
      `   - ${(this.results.totalSavings / 1024).toFixed(2)} KB 절약`
    );
  ...
  async createBackup() {
    ...
    const backupInfo = {
      ...
      originalSize:
        ...
        (this.results.analyzed - this.results.toRemove.length) * 1000, // 추정
