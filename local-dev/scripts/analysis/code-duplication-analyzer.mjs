...
class CodeDuplicationAnalyzer {
    ...
    analyzeImportPatterns(content, filePath) {
        ...
        imports.forEach(importStatement => {
            ...
            const existing = this.results.commonPatterns.find(p => p.pattern === pattern);
            if (existing) {
                existing.count++;
    ...
    isMeaningfulCode(code) {
        ...
        return meaningfulPatterns.some(pattern => pattern.test(code)) &&
            code.length > 50 && // 최소 길이
            !code.includes('console.log') && // 디버그 코드 제외
            !/^\s*[\{\}]\s*$/.test(code); // 빈 블록 제외
    }
    ...
    findDuplicateBlocks() {
        ...
        // 크기별 정렬
        this.results.duplicateGroups.sort((a, b) => b.savingsPotential - a.savingsPotential);
    }
    ...
    generateReport() {
        ...
        console.log(`💰 예상 절약: ${(this.results.potentialSavings / 1024).toFixed(2)} KB`);
        console.log();

        console.log('🎯 리팩토링 우선순위 TOP 17:');
        this.results.refactoringOpportunities.forEach((opp, index) => {
            console.log(`${String(index + 1).padStart(2)}. [${opp.effort.padEnd(6)}] ${opp.description}`);
            console.log(`    💰 절약: ${(opp.savingsPotential / 1024).toFixed(2)}KB | 📁 파일: ${opp.files.length}개`);
        ...
        this.results.commonPatterns.slice(0, 10).forEach((pattern, index) => {
            console.log(`${String(index + 1).padStart(2)}. ${pattern.pattern} (${pattern.count}회 사용)`);
        });
