...
class AccessibilityFixer {
  ...
  async fixFormLabelsInFile(relativePath) {
    try {
      ...
      modifiedContent = modifiedContent.replace(
        ...
        (match, before, after) => {
          if (
            match.includes('type="text"') ||
            match.includes('type="search"') ||
            match.includes('type="email"')
          ) {
  ...
  async fixPageStructureInFile(relativePath) {
    try {
      ...
      if (
        modifiedContent.includes('return (') &&
        !modifiedContent.includes('<main')
      ) {
        modifiedContent = modifiedContent.replace(
          /return \(\s*(<div[^>]*>)/,
          'return (\n    <main>\n      $1'
