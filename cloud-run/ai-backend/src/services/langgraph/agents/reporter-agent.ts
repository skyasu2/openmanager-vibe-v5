/**
 * Reporter Agent
 * 인시던트 리포트 및 RAG 기반 솔루션 검색 에이전트 (Cloud Run Standalone)
 *
 * 역할:
 * - 장애 원인 분석 (RCA)
...
/**
 * Mock 지식베이스 (Supabase 연결 실패 시 폴백)
 */
function getMockKnowledgeBase(query: string): {
  success: boolean;
  ...
  const mockKnowledge = [
    {
      keyword: 'cpu',
      content:
        'CPU 과부하 발생 시: 1) 높은 CPU 사용 프로세스 확인 (top, htop), 2) 불필요한 프로세스 종료, 3) 로드 밸런싱 검토',
      similarity: 0.92,
    },
    {
      keyword: '메모리',
      content:
        '메모리 부족 해결: 1) 메모리 캐시 정리 (sync; echo 3 > /proc/sys/vm/drop_caches), 2) OOM killer 로그 확인, 3) 스왑 공간 추가 검토',
      similarity: 0.88,
    },
    {
      keyword: '서버',
      content:
        '서버 장애 일반 절차: 1) 로그 확인 (/var/log/syslog), 2) 서비스 상태 점검, 3) 네트워크 연결 확인, 4) 디스크 공간 확인',
      similarity: 0.85,
    },
    {
      keyword: '재시작',
      content:
        '서비스 재시작 절차: 1) systemctl status [service] 로 상태 확인, 2) systemctl restart [service]로 재시작, 3) 로그 확인으로 성공 여부 검증',
      similarity: 0.9,
    },
    {
      keyword: '디스크',
      content:
        '디스크 공간 부족: 1) df -h로 사용량 확인, 2) du -sh /*로 큰 디렉토리 찾기, 3) 불필요한 로그/캐시 정리, 4) 디스크 추가 검토',
      similarity: 0.87,
    },
  ];

  const matchedResults = mockKnowledge.filter((k) => q.includes(k.keyword));
  const results =
    matchedResults.length > 0 ? matchedResults : mockKnowledge.slice(0, 3);
