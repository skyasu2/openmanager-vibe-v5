...
class LoadTester {
  ...
  async testSystemStart(userId = 1) {
    ...
    try {
      ...
      const totalTime = statusResult.responseTime + startResult.responseTime + healthResult.responseTime;
      ...
      const result = {
        ...
        steps: {
          status: { time: statusResult.responseTime, code: statusResult.statusCode },
          start: { time: startResult.responseTime, code: startResult.statusCode },
          health: { time: healthResult.responseTime, code: healthResult.statusCode }
        },
        details: {
          statusResponse: statusResult.data ? JSON.parse(statusResult.data) : null,
  ...
  async testDataGenerator(userId = 1, pattern = 'normal') {
    ...
    try {
      // 1. 기본 메트릭 데이터 조회
      const metricsResult = await this.makeRequest('/api/data-generator?type=metrics&count=10');
      
      // 2. 서버 데이터 조회 (다양한 타입 테스트)
      const typeMap = { 'normal': 'servers', 'high-load': 'logs', 'maintenance': 'traffic' };
      const dataType = typeMap[pattern] || 'metrics';
      const typeResult = await this.makeRequest(`/api/data-generator?type=${dataType}&count=5`);

      // 3. 대시보드 데이터 확인
      const dashboardResult = await this.makeRequest('/api/dashboard?limit=5');

      const totalTime = metricsResult.responseTime + typeResult.responseTime + dashboardResult.responseTime;
      ...
      const result = {
        ...
        pattern,
        timestamp: new Date().toISOString(),
        success: metricsResult.statusCode === 200 && typeResult.statusCode === 200,
        totalResponseTime: totalTime,
        steps: {
          metrics: { time: metricsResult.responseTime, code: metricsResult.statusCode },
          type: { time: typeResult.responseTime, code: typeResult.statusCode },
          dashboard: { time: dashboardResult.responseTime, code: dashboardResult.statusCode }
        },
        details: {
          metricsResponse: metricsResult.data ? JSON.parse(metricsResult.data) : null,
          typeResponse: typeResult.data ? JSON.parse(typeResult.data) : null,
          dashboardResponse: dashboardResult.data ? JSON.parse(dashboardResult.data) : null,
          dataType: dataType
        }
      };
  ...
  generateReport() {
    ...
    const report = {
      summary: {
        totalTests: this.results.systemStart.length + this.results.dataGenerator.length,
        successfulTests: systemStartSuccess.length + dataGenSuccess.length,
        failedTests: this.results.systemStart.filter(r => !r.success).length + 
                     this.results.dataGenerator.filter(r => !r.success).length,
        errors: totalErrors,
        successRate: ((systemStartSuccess.length + dataGenSuccess.length) / 
                     (this.results.systemStart.length + this.results.dataGenerator.length) * 100).toFixed(2)
      },
      systemStart: {
  ...
  printReport() {
    ...
    console.log(`   성공: ${report.summary.successfulTests}개 (${report.summary.successRate}%)`);
    ...
    console.log(`   성공률: ${(report.systemStart.successful / report.systemStart.totalTests * 100).toFixed(1)}%`);
    console.log(`   응답시간: 최소 ${report.systemStart.responseTimeStats.min}ms, 평균 ${report.systemStart.responseTimeStats.avg}ms, 최대 ${report.systemStart.responseTimeStats.max}ms`);
    
    console.log('\n📊 데이터 생성 기능:');
    console.log(`   테스트 수: ${report.dataGenerator.totalTests}개`);
    console.log(`   성공률: ${(report.dataGenerator.successful / report.dataGenerator.totalTests * 100).toFixed(1)}%`);
    console.log(`   응답시간: 최소 ${report.dataGenerator.responseTimeStats.min}ms, 평균 ${report.dataGenerator.responseTimeStats.avg}ms, 최대 ${report.dataGenerator.responseTimeStats.max}ms`);
