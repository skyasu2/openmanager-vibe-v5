...
export async function GET(request: NextRequest) {
  try {
    ...
    const recommendedAdjustment =
      systemLoad > 80 ? 'increase' : systemLoad < 30 ? 'decrease' : 'maintain';
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const cachedThresholds = await redis.get('alert:thresholds');
      currentThresholds = cachedThresholds
        ? JSON.parse(cachedThresholds)
        : {
            cpu: 80,
            memory: 85,
            disk: 90,
    ...
    if (
      updatedThresholds.responseTime < 100 ||
      updatedThresholds.responseTime > 30000
    ) {
