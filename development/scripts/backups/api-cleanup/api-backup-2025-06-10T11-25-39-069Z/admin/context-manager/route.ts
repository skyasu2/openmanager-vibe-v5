  ...
  try {
    ...
    switch (action) {
      ...
      case 'pending-suggestions':
        // 관리자 승인 대기 중인 제안들
        const pendingSuggestions = contextUpdateEngine
          .getPendingUpdates()
      ...
      case 'approved-suggestions':
        // 승인된 제안들
        const approvedSuggestions = contextUpdateEngine
          .getAppliedUpdates()
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            patternsCount: Object.keys(
              mergedContext.patterns.intentPatterns || {}
            ).length,
            templatesCount: Object.keys(mergedContext.templates).length,
            intentMappingsCount: Object.keys(mergedContext.intentMappings)
              .length,
          },
        });
  ...
  try {
    ...
    switch (action) {
      ...
      case 'reject-suggestion':
        ...
        const rejected = await contextUpdateEngine.adminRejectUpdate(
          rejectId,
          reason
        );
      ...
      case 'export-approved-bundle':
        ...
        const exportedBundle =
          await contextUpdateEngine.exportApprovedUpdatesToBundle(
            exportBundleType,
            exportClientId
          );
  ...
  try {
    ...
    switch (action) {
      case 'cancel-suggestion':
        ...
        const cancelled = await contextUpdateEngine.adminRejectUpdate(
          id,
          '관리자에 의한 취소'
        );
