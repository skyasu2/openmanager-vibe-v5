  ...
  try {
    ...
    for (let i = 0; i < Math.min(dataPoints, 100); i++) {
      ...
      streamData.push({
        ...
        sequenceNumber: i + 1,
        events: eventMode
          ? generateStreamEvents(scenario as DemoScenario, i)
          : [],
      });
    }
    ...
    const streamStats = {
      ...
      performance: {
        avgCPU:
          streamData.reduce((sum, d) => sum + d.cpu, 0) / streamData.length,
        avgMemory:
          streamData.reduce((sum, d) => sum + d.memory, 0) / streamData.length,
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'list':
        const activeStreamsList = Array.from(activeStreams.entries()).map(
          ([id, info]) => ({
            streamId: id,
      ...
      default:
        return NextResponse.json({
          ...
          endpoints: {
            ...
            'GET /api/v1/demo/stream?action=status&streamId=<id>':
              '스트림 상태',
          ...
          usage: {
            startStream:
              'POST { "scenario": "ddos", "interval": 1000, "duration": 60000 }',
