...
async function generateInteractions(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const userRating = success
      ? Math.floor(Math.random() * 3) + 3
      : Math.floor(Math.random() * 2) + 1; // 성공시 3-5, 실패시 1-2

    const timestamp =
      Date.now() - Math.floor(Math.random() * 7 * 24 * 60 * 60 * 1000); // 최근 7일 내
    ...
    const interactionId = await aiDatabase.saveInteraction({
      ...
      triggers:
        mode === 'advanced'
          ? ['complex_query', 'analysis_required']
          : ['simple_query'],
...
function generateSampleResponse(
  ...
  const modeResponses =
    responses[mode as keyof typeof responses] || responses.basic;
