...
const getAIAgent = async () => {
  const { aiAgentEngine } = await import(
    '../../../modules/ai-agent/core/AIAgentEngine'
  );
...
const createErrorResponse = (error: any, processingTime: number) => {
  ...
  if (error instanceof SyntaxError) {
    ...
    if (error.message.includes('timeout')) {
      errorMessage =
        'AI 처리가 시간 초과되었습니다. 잠시 후 다시 시도해주세요.';
      statusCode = 408;
      ...
      case 'status':
        try {
          ...
          const enhancedStatus = {
            ...status,
            mode: status.isInitialized ? 'active' : 'standby',
            standbyInfo: !status.isInitialized
              ? {
                  message: '백엔드 AI 엔진이 스탠바이 상태입니다',
                  canActivate: true,
                  requirements: ['프론트엔드 UI에서 활성화 요청'],
                  estimatedActivationTime: '3-5초',
                }
              : null,
          };

          return NextResponse.json({ success: true, data: enhancedStatus });
        } catch (error) {
          console.warn(
            'AI Agent status check failed - returning standby mode:',
            error
          );
          // 🔄 오류 시에도 standby 모드로 응답
          return NextResponse.json({
            success: true,
            data: {
              isInitialized: false,
              ...
              standbyInfo: {
                message: '백엔드 AI 엔진이 오류 복구 스탠바이 상태입니다',
                canActivate: true,
                requirements: ['시스템 재시작 또는 수동 활성화'],
                estimatedActivationTime: '5-10초',
              },
            },
          });
        }
