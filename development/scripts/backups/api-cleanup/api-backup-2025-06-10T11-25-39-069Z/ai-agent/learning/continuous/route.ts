  ...
  try {
    ...
    switch (action) {
      ...
      case 'integrated-report': {
        // 통합 보고서 생성
        const report =
          await continuousLearningService.generateIntegratedReport();
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action parameter. Available actions: system-status, integrated-report, report-history, health-check',
          },
          { status: 400 }
        );
    }
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action parameter. Available actions: initialize, start-all, stop-all, auto-optimize, update-config, force-report',
          },
          { status: 400 }
        );
    }
      ...
      case 'reset-system': {
        ...
        if (process.env.NODE_ENV === 'development') {
          const continuousLearningService =
            ContinuousLearningService.getInstance();
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action parameter. Available actions: reset-system, clear-reports',
          },
          { status: 400 }
        );
    }
