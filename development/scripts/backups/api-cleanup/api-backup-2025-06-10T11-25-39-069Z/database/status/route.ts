...
async function databaseStatusHandler(request: NextRequest) {
  ...
  try {
    ...
    // 4. 전체 상태 요약
    const healthySystems = databaseStatuses.filter(
    ...
    const avgResponseTime =
      databaseStatuses.reduce(
        (sum, db) => sum + db.performance.responseTime,
        0
      ) / totalSystems;
    const totalOps = databaseStatuses.reduce(
      (sum, db) => sum + db.performance.operationsPerSecond,
      0
    );
    const avgErrorRate =
      databaseStatuses.reduce((sum, db) => sum + db.performance.errorRate, 0) /
      totalSystems;
