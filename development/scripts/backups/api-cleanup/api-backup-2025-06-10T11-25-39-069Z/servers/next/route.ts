...
async function handlePOST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            '서버 생성기가 아직 초기화되지 않았거나 서버가 생성되지 않았습니다.',
        },
        { status: 404 }
      );
    }
    ...
    const serverResponse = {
      ...
      specs: {
        ...
        memory_gb: Math.round(
          currentServer.specs.memory.total / (1024 * 1024 * 1024)
        ),
        disk_gb: Math.round(
          currentServer.specs.disk.total / (1024 * 1024 * 1024)
        ),
      },
      os:
        currentServer.specs.cpu.architecture === 'arm64'
    ...
    console.log(
      `📊 서버 ${currentServerIndex}/${allServers.length} 생성 - ${serverResponse.name} (${serverResponse.status})`
    );
    ...
    return NextResponse.json({
      ...
      isComplete,
      progress,
      nextServerType: isComplete
        ? null
        : allServers[currentServerIndex % allServers.length]?.type || null,
      message: isComplete
        ? '모든 서버 생성이 완료되었습니다.'
