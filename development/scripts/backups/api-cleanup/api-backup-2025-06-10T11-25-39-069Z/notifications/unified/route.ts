    ...
    const unifiedStatus = {
      ...
      statistics: {
        ...
        supportedFeatures: Object.keys(browserStatus.features).filter(
          key =>
            browserStatus.features[key as keyof typeof browserStatus.features]
        ).length,
      },
    };
...
async function sendUnifiedNotificationHandler(request: NextRequest) {
  try {
    ...
    const { alert, preferences, testMode = false } = body;
                ...
                case 'memory':
                  if (unifiedAlert.metrics?.memory) {
                    success = await slackNotificationService.sendMemoryAlert({
                      ...
                      heapUsed: Math.round(
                        unifiedAlert.metrics.memory * 1024 * 1024
                      ),
    ...
    tasks.push(
      (async () => {
        try {
          ...
          console.log(
            `💾 DB 로그 저장: ${unifiedAlert.id} - ${unifiedAlert.title}`
          );
          ...
          results.channels.database.error =
    ...
    const successCount = Object.values(results.channels).filter(
...
async function updateUnifiedSettingsHandler(request: NextRequest) {
  try {
    ...
    if (slackConfig && slackConfig.webhookUrl) {
      try {
        if (
          !slackConfig.webhookUrl.startsWith(
        ...
        updateResults.errors.push(
          `슬랙 설정 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
        );
      }
    }
    ...
    if (preferences) {
      try {
        ...
        updateResults.errors.push(
          `설정 검증 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
        );
      }
    }
