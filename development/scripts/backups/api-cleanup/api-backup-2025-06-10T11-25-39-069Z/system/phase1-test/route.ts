...
export async function GET(request: NextRequest) {
  try {
    ...
    const overallStatus =
      activeModules === allModules.length ? 'healthy' : 'degraded';
  ...
  try {
      ...
      try {
        ...
        testResults.push({
          ...
          tests: {
            ...
            cleanupExecution: cleanupResults.every(
    ...
    if (testType === 'full') {
      try {
        ...
        hub.registerConnection(integrationConnectionId, null, {
          testMode: true,
        });
    ...
    console.log(
      `🧪 Phase 1 통합 테스트 완료: ${successfulTests}/${testResults.length} 성공`
    );
