...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `📊 시계열 메트릭 조회: format=${format}, timeRange=${timeRange}`
    );

    // 단일 서버 조회
    if (serverId) {
      const result = await redisTimeSeriesService.queryMetrics(
        serverId,
        timeRange,
        metrics
      );
...
function formatResponse(result: any, format: string) {
  switch (format) {
    case 'chart':
      // 차트용 데이터 구조
      return {
        labels: result.points.map((p: any) =>
 ...
 * 📊 다중 서버 응답 포맷팅
 */
function formatMultiServerResponse(
  ...
  switch (format) {
    ...
    case 'stats':
      ...
      return Object.entries(results).map(([serverId, points]) => ({
        ...
        avg_value:
          points.length > 0
            ? points.reduce((sum, p) => sum + p.metrics.cpu_usage, 0) /
              points.length
            : 0,
      }));
