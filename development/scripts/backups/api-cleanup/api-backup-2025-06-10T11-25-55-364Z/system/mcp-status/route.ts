...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    switch (section) {
      ...
      case 'components':
        // 개별 컴포넌트 상세 상태
        const [systemHealthDetail, keepAliveStatus, fastApiStatus] =
        ...
        return NextResponse.json({
          ...
          data: {
            unifiedAI: {
              status:
                systemHealthDetail.status === 'fulfilled'
                ...
                keepAliveStatus.status === 'fulfilled'
                  ? {
                      ...
                      consecutiveSuccesses:
                        keepAliveStatus.value.consecutiveSuccesses,
                      uptime:
                        Math.round(keepAliveStatus.value.uptimeHours * 100) /
                        100,
                      lastPing: keepAliveStatus.value.lastPing,
                      averageResponseTime: Math.round(
                        keepAliveStatus.value.averageResponseTime
                      ),
                    }
                  : null,
            },
          ...
          data: {
            ...
            contexts: {
              ...
              advanced: {
                ...
                documentsCount:
                  perfData.components.contexts.advanced.documentsCount,
              },
              custom: {
      ...
      case 'diagnostic':
        ...
        if (!fastApiData.isConnected) {
          ...
          diagnostics.fastapi.recommendations.push(
            'FastAPI 서버 상태를 확인하세요'
          );
        }
        ...
        if (!keepAliveData.isActive) {
          ...
          diagnostics.keepAlive.recommendations.push(
            'Keep-Alive 시스템을 시작하세요'
          );
