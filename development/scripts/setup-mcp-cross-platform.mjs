...
function createMCPConfig() {
    ...
    const config = {
        "mcpServers": {
            "filesystem": {
                "command": "npx",
                "args": ["-y", "@modelcontextprotocol/server-filesystem", "/d:/cursor/openmanager-vibe-v5"]
            },
            "memory": {
                "command": "npx",
...
function validateMCPSetup() {
    ...
    try {
        ...
        colorLog('blue', `📊 설정된 서버 수: ${Object.keys(config.mcpServers || {}).length}`);
        return true;
    } catch (error) {
...
async function main() {
    ...
    // 4. 로컬 MCP 서버 시작
    const serverResult = executeCommand('npm run mcp:local:start', '로컬 MCP 서버 시작', { silent: true });
    if (serverResult.success) {
        colorLog('green', '✅ 로컬 MCP 서버 시작 완료');
    } else {
        colorLog('yellow', '⚠️ 로컬 MCP 서버 시작 실패 (백그라운드에서 실행 중일 수 있음)');
    }
