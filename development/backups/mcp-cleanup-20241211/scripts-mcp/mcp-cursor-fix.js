...
function fixGlobalConfig() {
  ...
  try {
    ...
    if (!fs.existsSync(globalMcpPath)) {
      ...
      fs.writeFileSync(globalMcpPath, JSON.stringify({ mcpServers: {} }, null, 2));
...
function cleanupConflictingConfigs() {
  ...
  conflictingPaths.forEach(filePath => {
    if (fs.existsSync(filePath)) {
      const backupPath = path.join(backupsDir, `${path.basename(filePath)}.backup`);
      try {
        fs.renameSync(filePath, backupPath);
        console.log(`✅ 백업 완료: ${path.basename(filePath)} → backups/${path.basename(backupPath)}`);
      } catch (error) {
        console.log(`⚠️  백업 실패: ${path.basename(filePath)} - ${error.message}`);
      }
    }
  });
}
  ...
  if (!fs.existsSync(projectMcpPath)) {
    ...
    const defaultConfig = {
      "mcpServers": {
        ...
        "filesystem": {
          "command": "npx",
          "args": ["-y", "@modelcontextprotocol/server-filesystem", process.cwd().replace(/\\/g, '/')],
...
function validateProjectConfig() {
  try {
    ...
    console.log(`✅ 프로젝트 MCP 설정 검증 완료 (${Object.keys(config.mcpServers).length}개 서버)`);
    return true;
  } catch (error) {
...
module.exports = { fixGlobalConfig, cleanupConflictingConfigs, ensureProjectConfig, validateProjectConfig, showServerStatus, showNextSteps }; 