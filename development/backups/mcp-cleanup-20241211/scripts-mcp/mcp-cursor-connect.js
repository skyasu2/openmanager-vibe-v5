...
function checkGlobalConfig() {
    ...
    if (!fs.existsSync(GLOBAL_CONFIG_PATH)) {
        ...
        fs.writeFileSync(GLOBAL_CONFIG_PATH, JSON.stringify({ mcpServers: {} }, null, 2));
    } else {
        try {
...
async function startLocalServer() {
    ...
    try {
        ...
        try {
            const response = execSync('curl -s http://localhost:3100/health', { timeout: 5000 });
...
async function quickTest() {
    ...
    // 로컬 서버 테스트
    try {
        const response = execSync('curl -s http://localhost:3100/health', { timeout: 3000 });
    ...
    try {
        const config = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
        const enabledCount = Object.values(config.mcpServers || {})
