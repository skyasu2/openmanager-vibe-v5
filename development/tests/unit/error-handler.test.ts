...
describe('Error Handler', () => {
  ...
  describe('Error classification helpers', () => {
    ...
    it('isTypeSafetyError는 타입 안전성 에러를 감지한다', () => {
      expect(isTypeSafetyError('Cannot read properties of undefined')).toBe(true);
  ...
  describe('withErrorRecovery', () => {
    ...
    it('shouldRetry 조건을 확인한다', async () => {
      ...
      const failingOperation = vi.fn().mockRejectedValue(new Error('Should not retry'));
  ...
  describe('createErrorBoundaryInfo', () => {
    it('에러 바운더리 정보를 생성한다', () => {
      ...
      expect(boundaryInfo).toHaveProperty('errorInfo');
    ...
    it('errorInfo 없이도 동작한다', () => {
      ...
      expect(boundaryInfo.errorInfo).toBeUndefined();
    });
  });
}); 