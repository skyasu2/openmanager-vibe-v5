...
async function testAIEnginePriority() {
    ...
    const testQueries = [
        ...
        {
            name: '복잡한 자연어 테스트',
            query: '서버 성능 최적화를 위한 종합적인 전략을 세우고 단계별 실행 계획을 만들어주세요.',
    ...
    for (const test of testQueries) {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.log(`   ${isCorrectEngine ? '🎯 예상대로 처리됨!' : '⚠️ 예상과 다른 엔진이 처리함'}\n`);
    ...
    results.forEach(result => {
        ...
        console.log(`${status} ${result.test}: ${result.expected} → ${result.actual} ${details}`);
    });
    ...
    try {
        ...
        if (quotaResponse.ok) {
            ...
            if (quota) {
                console.log('\n📋 Google AI 할당량 상태:');
                console.log(`   사용: ${quota.used}/${quota.limit}회 (${quota.percentage}%)`);
