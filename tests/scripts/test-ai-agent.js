...
async function runSingleTest(testCase) {
  ...
  try {
    const response = await makeRequest(
      `${TEST_CONFIG.baseUrl}/api/ai/enhanced`,
      {
    ...
    const keywordScore =
      (foundKeywords.length / testCase.expectedKeywords.length) * 100;

    // 결과 출력
    console.log(`✅ 응답 시간: ${responseTime}ms`);
    console.log(`📊 신뢰도: ${result.confidence}%`);
    console.log(
      `🎯 키워드 매칭: ${foundKeywords.length}/${testCase.expectedKeywords.length} (${keywordScore.toFixed(1)}%)`
    );
    ...
    // 성공 기준 판정
    const isSuccess =
      keywordScore >= 50 && result.confidence >= 70 && responseTime < 10000;
...
async function runAllTests() {
  ...
  console.log(
    `📈 전체 성공률: ${successCount}/${totalCount} (${successRate.toFixed(1)}%)`
  );
  ...
  for (const [category, stats] of Object.entries(categoryResults)) {
    ...
    console.log(
      `  ${category}: ${stats.success}/${stats.total} (${rate.toFixed(1)}%)`
    );
  }
