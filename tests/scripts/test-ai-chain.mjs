...
const testCases = [
  ...
  },
  {
    ...
    data: {
      question:
        '서버 모니터링 데이터를 분석하여 향후 24시간 내 발생 가능한 문제점을 예측해주세요',
      userId: 'test-user-3',
    },
  },
  {
...
async function runTest(testCase, index) {
  ...
  if (testCase.data) {
    console.log(
      colors.yellow(`   데이터: ${JSON.stringify(testCase.data, null, 2)}`)
    );
  }
  ...
  try {
    ...
    if (result.result) {
      ...
      console.log(
        colors.blue(`   처리시간: ${result.result.processingTime}ms`)
      );
    ...
    if (result.aiChain) {
      ...
      console.log(
        colors.blue(
          `   사용 가능한 엔진: ${Object.keys(result.aiChain.engines).join(', ')}`
        )
      );
    }
