...
FAILURE_SCENARIOS.forEach((scenario, idx) => {
  ...
  console.log(
    `      시간대: ${scenario.timeRange[0]}시 ~ ${scenario.timeRange[1]}시`
  );
  console.log(`      영향받는 서버: ${scenario.affectedServers.length}개`);
  const criticalCount = scenario.affectedServers.filter(
    (s) => s.status === 'critical'
  ).length;
  const warningCount = scenario.affectedServers.filter(
    (s) => s.status === 'warning'
  ).length;
  console.log(
    `      - Critical: ${criticalCount}개, Warning: ${warningCount}개`
  );
});
console.log('');
...
console.log(
  `   ✅ 시계열 데이터 서버 수: ${Object.keys(mockData.timeSeries).length}개`
);
...
const statusCounts = mockData.servers.reduce(
  (acc, server) => {
...
console.log(
  `   ${isDataPointsCorrect ? '✅' : '❌'} 데이터 포인트 수 (288개 예상)`
);
console.log(`   ${isServerCountCorrect ? '✅' : '❌'} 서버 수 (15개 예상)`);
console.log(
  `   ${isTotalPointsCorrect ? '✅' : '❌'} 총 포인트 수 (4,320개 예상)`
);
