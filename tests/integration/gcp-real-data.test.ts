...
describe('🌐 GCP 실제 데이터 서비스 테스트', () => {
  ...
  test('📊 GCP 실제 서버 메트릭 조회 테스트', async () => {
    ...
    // 응답 구조 검증 (실제 타입 정의에 맞게 수정)
    expect(response).toHaveProperty('success');
    expect(response.success).toBe(true);

    if (response.data && Array.isArray(response.data)) {
      const healthyCount = response.data.filter(
        s => s.status === 'healthy'
      ).length;
      const warningCount = response.data.filter(
        s => s.status === 'warning'
      ).length;
      const criticalCount = response.data.filter(
        s => s.status === 'critical'
      ).length;
  ...
  test('🔄 GCP 메트릭 캐싱 테스트', async () => {
    ...
    console.log(
      `📊 GCP 메트릭 조회 성능: 첫 호출 ${duration1}ms, 캐시 호출 ${duration2}ms`
    );
  });
  ...
  test('⚡ GCP 서버 상태 분석 테스트', async () => {
    ...
    if (response.success && response.data.length > 0) {
      ...
      console.log(
        `📊 GCP 서버 상태 분석: 정상 ${healthyCount}개, 경고 ${warningCount}개, 위험 ${criticalCount}개`
      );
    }
  });
