    ...
    if (response.ok) {
      try {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          if (jsonData.data) {
            console.log(
              `   📊 Data Keys: ${Object.keys(jsonData.data).join(', ')}`
            );
          }
...
function analyzeResults(results) {
  ...
  console.log(
    `✅ 성공: ${successful.length}개 (${Math.round((successful.length / results.length) * 100)}%)`
  );
  console.log(
    `❌ 실패: ${failed.length}개 (${Math.round((failed.length / results.length) * 100)}%)`
  );

  // 평균 응답 시간
  const avgResponseTime =
    results.reduce((sum, r) => sum + (r.result.responseTime || 0), 0) /
    results.length;
  ...
  const dataGeneratorResult = results.find(
    r => r.endpoint === '/api/data-generator/unified?action=status'
  );
  ...
  if (failed.length === 0) {
    ...
    console.log(
      '   📊 데이터 흐름: 서버 데이터 생성기 → API → AI 에이전트 완전 연결'
    );
