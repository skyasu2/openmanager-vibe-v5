...
class AISidebarRealtimeTest {
  ...
  async testAIInsights() {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        console.log(
          `   예측 정확도: ${data.data?.predictions?.accuracy || 'N/A'}%`
        );
        console.log(`   이상 탐지: ${data.data?.anomalies?.count || 0}개`);
        console.log(
          `   추천사항: ${data.data?.recommendations?.length || 0}개`
        );
  ...
  async testServerDataGenerator() {
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/api/servers/realtime?type=servers`
      );
      const data = await response.json();
      ...
      if (response.ok && data.success) {
        ...
        console.log(
          `   실시간 데이터: ${data.realtime ? '활성화' : '비활성화'}`
        );
  ...
  async testRealTimeUpdates() {
    ...
    try {
      ...
      const cpuChanged =
        Math.abs(server1_before.metrics.cpu - server1_after.metrics.cpu) > 0.1;
      const memoryChanged =
        Math.abs(server1_before.metrics.memory - server1_after.metrics.memory) >
        0.1;
      ...
      console.log(
        `   CPU: ${server1_before.metrics.cpu.toFixed(1)}% → ${server1_after.metrics.cpu.toFixed(1)}%`
      );
      console.log(
        `   메모리: ${server1_before.metrics.memory.toFixed(1)}% → ${server1_after.metrics.memory.toFixed(1)}%`
      );
    ...
    for (const question of testQuestions) {
      ...
      try {
        ...
        if (response.ok && data.response) {
          ...
          console.log(
            `   신뢰도: ${data.confidence || 'N/A'}, 응답시간: ${data.responseTime || 'N/A'}ms`
          );
        } else {
