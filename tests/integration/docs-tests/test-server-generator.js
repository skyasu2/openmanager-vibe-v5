...
async function testServerDataGenerator() {
  ...
  try {
    ...
    const response = await fetch(
      'http://localhost:3000/api/servers/realtime?type=servers'
    );
    ...
    servers.forEach((server, index) => {
      ...
      validationResults.environments.push(
        validEnvironments.includes(server.environment)
      );
      ...
      validationResults.healthScores.push(
        healthScore >= 60 && healthScore <= 100
      );
      ...
      if (index < 5) {
        ...
        console.log(
          `   상태: ${server.status}, 건강도: ${Math.round(healthScore)}%`
        );
        console.log(
          `   CPU: ${cpuCores}코어, 메모리: ${Math.round(memoryGB)}GB`
        );
        console.log('');
      }
    });
    ...
    console.log(
      `   서버 개수 (20개): ${validationResults.serverCount ? '✅' : '❌'}`
    );
    console.log(
      `   서버 타입: ${validationResults.serverTypes.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   서버 이름 형식: ${validationResults.serverNames.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   역할: ${validationResults.roles.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   환경: ${validationResults.environments.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   위치: ${validationResults.locations.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   상태: ${validationResults.statuses.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   CPU 코어 수: ${validationResults.cpuCores.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   메모리 범위: ${validationResults.memoryRanges.every(v => v) ? '✅' : '❌'}`
    );
    console.log(
      `   건강도 범위: ${validationResults.healthScores.every(v => v) ? '✅' : '❌'}`
    );
    ...
    setTimeout(async () => {
      try {
        ...
        if (result2.success) {
          ...
          const cpuChanged =
            Math.abs(server1_before.metrics.cpu - server1_after.metrics.cpu) >
            0.1;
          const memoryChanged =
            Math.abs(
              server1_before.metrics.memory - server1_after.metrics.memory
            ) > 0.1;

          console.log(
            `   실시간 업데이트: ${cpuChanged || memoryChanged ? '✅' : '❌'}`
          );
          console.log(
            `   CPU 변화: ${server1_before.metrics.cpu.toFixed(1)}% → ${server1_after.metrics.cpu.toFixed(1)}%`
          );
          console.log(
            `   메모리 변화: ${server1_before.metrics.memory.toFixed(1)}% → ${server1_after.metrics.memory.toFixed(1)}%`
          );
        }
      } catch (error) {
