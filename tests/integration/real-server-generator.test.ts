...
describe('RealServerDataGenerator initialize without Redis', () => {
  ...
  it('initialize 호출 시 생성 루프가 시작된다', async () => {
    // 초기화 전 상태 확인
    const initialServers = await realServerDataGenerator.getAllServers();

    await realServerDataGenerator.initialize();

    // 초기화 후 서버가 생성되었는지 확인
    await vi.advanceTimersByTimeAsync(100);
    const afterServers = await realServerDataGenerator.getAllServers();
    ...
    const finalServers = await realServerDataGenerator.getAllServers();
