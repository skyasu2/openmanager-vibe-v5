...
vi.mock('@/lib/environment/auto-decrypt-env', () => ({
  checkEnvironmentStatus: vi.fn().mockResolvedValue({
    initialized: true,
    valid: true,
    missing: [],
    message: '🧪 Mock checkEnvironmentStatus 호출됨 - valid: true 반환',
  }),
}));
...
describe('On-Demand Health Check', () => {
  describe('Health API', () => {
    ...
    it('정상 상태에서 헬스체크를 수행해야 함', async () => {
      ...
      try {
        ...
        // 정상 상태에서는 200 또는 503(warning) 둘 다 허용
  ...
  describe('Manual Health Check', () => {
    it('사용자가 수동으로 헬스체크를 요청할 수 있어야 함', async () => {
      ...
      // 두 요청 모두 응답해야 함 (크론 없이 온디맨드)
      // 상태는 healthy 또는 warning 모두 허용 (환경변수 상태에 따라)
      ...
      expect(['healthy', 'warning']).toContain(data1.status);
      expect(['healthy', 'warning']).toContain(data2.status);
    });
  });
});
