...
describe('AI Learning System - Basic Tests', () => {
    ...
    describe('해결방안 데이터베이스', () => {
        ...
        it('추천 해결방안을 가져올 수 있다', async () => {
            const recommendations = await solutionDatabase.getRecommendedSolutions('cpu_overload', 3);
    ...
    describe('AI 학습 기능', () => {
        ...
        it('해결방안 효과성을 업데이트할 수 있다', async () => {
            ...
            // 효과성 업데이트
            const updateResult = await solutionDatabase.updateSolutionEffectiveness(solutionId, 0.9);
    ...
    describe('에러 처리', () => {
        it('잘못된 입력에 대해 적절히 처리한다', async () => {
            ...
            // 존재하지 않는 해결방안 업데이트
            const invalidUpdate = await solutionDatabase.updateSolutionEffectiveness('invalid-id', 0.5);
