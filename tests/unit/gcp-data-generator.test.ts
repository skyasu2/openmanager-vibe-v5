...
import {
    TDDGCPDataGenerator
} from '@/services/gcp/TDDGCPDataGenerator';
...
describe('🟢 TDD Green Phase: GCP 데이터 생성기 최소 구현', () => {
    ...
    describe('📊 요구사항 1: 10개 서버 기본 데이터셋 생성', () => {
        ...
        test('🟢 PASS: 각 서버는 타입별 특성화된 스펙을 가져야 함', async () => {
            ...
            expect(dbServer!.specs.memory.total).toBeGreaterThan(16 * 1024 * 1024 * 1024);
        });
    });
    ...
    describe('⚡ 요구사항 2: 실시간 메트릭 생성 (30초 간격)', () => {
        test('🟢 PASS: 10개 서버의 실시간 메트릭을 생성해야 함', async () => {
            ...
            const metrics = await generator.generateRealtimeMetrics('test-session-001');
            ...
            metrics.forEach(metric => {
                ...
                expect(metric.applicationMetrics.requestCount).toBeGreaterThanOrEqual(0);
...
describe('🔴 TDD Red Phase: 아직 구현되지 않은 기능들', () => {
    test('🔴 FAIL: 시나리오별 메트릭 변화 (미구현)', () => {
        ...
        expect(normalMetrics.every(metric =>
        ...
        expect(criticalMetrics.filter(metric =>
        ...
        expect(warningMetrics.filter(metric =>
