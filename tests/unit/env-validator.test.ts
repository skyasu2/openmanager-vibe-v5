...
describe('Environment Validator', () => {
  ...
  describe('validateRuntimeEnvironment', () => {
    ...
    it('잘못된 URL 형식을 감지한다', () => {
      ...
      expect(result.errors).toContain(
        'UPSTASH_REDIS_REST_URL은 유효한 URL이어야 합니다'
      );
    });
  ...
  describe('requireEnvironmentVariables', () => {
    it('빌드 시에는 항상 성공한다', () => {
      ...
      const middleware = requireEnvironmentVariables([
    ...
    it('필요한 환경변수가 없을 때 실패한다', () => {
      ...
      const middleware = requireEnvironmentVariables([
        'MISSING_VAR' as keyof typeof env,
      ]);
      const result = middleware();
    ...
    it('여러 환경변수를 검증한다', () => {
      ...
      const middleware = requireEnvironmentVariables([
        'VAR1' as keyof typeof env,
        'VAR2' as keyof typeof env,
      ]);
      const result = middleware();
