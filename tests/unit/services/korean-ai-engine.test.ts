...
describe('🇰🇷 KoreanAIEngine 통합 테스트', () => {
    ...
    describe('🎯 추가 기능 테스트', () => {
        it('의도별 추가 팁을 제공해야 함', async () => {
            ...
            expect(analysisResult.additionalInfo.tips).toEqual(
                expect.arrayContaining([
                    expect.stringMatching(/상세 분석|분석해줘/)
                ])
            );
        });
        ...
        it('위험 상태에서 특별 알림 팁을 제공해야 함', async () => {
            ...
            expect(result.additionalInfo.tips).toEqual(
                expect.arrayContaining([
                    expect.stringMatching(/즉시 조치|담당자|알림/)
                ])
            );
        });
    });
    ...
    describe('🚨 에러 처리 및 폴백 테스트', () => {
        ...
        it('처리 중 에러 발생 시 폴백 응답을 제공해야 함', async () => {
            ...
            vi.spyOn(errorKoreanAI as any, 'analyzeServerMetrics')
            ...
            expect(result.fallbackResponse).toMatch(/죄송합니다|문제가 발생|다시 시도/);
    ...
    describe('🌐 다양한 한국어 표현 테스트', () => {
        it('다양한 한국어 표현을 이해해야 함', async () => {
            ...
            for (const query of variations) {
                ...
                expect(['조회', '분석', '제어', '최적화', '모니터링', '기타']).toContain(result.understanding.intent);
            }
        });
        ...
        it('존댓말과 반말을 모두 처리해야 함', async () => {
            ...
            const politeResult = await koreanAI.processQuery(politeQuery, mockServerData);
            const casualResult = await koreanAI.processQuery(casualQuery, mockServerData);
