...
describe('🌐 GCP 실제 데이터 생성기 테스트', () => {
  ...
  test('🔍 데이터 품질 검증', async () => {
    if (isVercelEnvironment) {
      ...
      if (response.data.length > 0) {
        ...
        expect(server.type).toMatch(
          /^(compute-engine|gke-node|cloud-sql|cloud-run)$/
        );
        ...
        console.log(
          `✅ GCP 실제 데이터 품질 검증 완료: ${response.data.length}개 서버`
        );
      }
    } else {
      // 로컬: 목업 데이터 품질 검증
  ...
  test('📈 실시간 메트릭 변동 테스트', async () => {
    if (isVercelEnvironment) {
      ...
      if (response1.data.length > 0 && response2.data.length > 0) {
        ...
        expect(
          Math.abs(server1.metrics.cpu.usage - server2.metrics.cpu.usage)
        ).toBeLessThan(50);

        console.log(
          `📈 GCP 실시간 메트릭 변동 확인: CPU ${server1.metrics.cpu.usage}% → ${server2.metrics.cpu.usage}%`
        );
      }
    } else {
  ...
  test('🚨 장애 상황 시뮬레이션', async () => {
    if (isVercelEnvironment) {
      ...
      if (response.success && response.data.length > 0) {
        const criticalServers = response.data.filter(
          s => s.status === 'critical'
        );
        const warningServers = response.data.filter(
          s => s.status === 'warning'
        );
        ...
        console.log(
          `🚨 장애 상황 분석: 위험 ${criticalServers.length}개, 경고 ${warningServers.length}개`
        );
      }
    } else {
      // 로컬: 목업 데이터로 장애 시뮬레이션
  ...
  test('💾 데이터 캐싱 효율성 테스트', async () => {
    if (isVercelEnvironment) {
      ...
      console.log(
        `💾 GCP 캐싱 효율성: ${cacheEfficiency}% 성능 향상 (${duration1}ms → ${duration2}ms)`
      );
    } else {
