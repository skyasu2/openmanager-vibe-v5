...
import { UnifiedEnvCryptoManager, type EncryptedData } from '../../../src/lib/crypto/UnifiedEnvCryptoManager';
...
describe('UnifiedEnvCryptoManager', () => {
    ...
    describe('복호화 기능', () => {
        ...
        it('잘못된 비밀번호로 복호화 시 오류를 발생시켜야 함', async () => {
            ...
            await expect(cryptoManager.decrypt(encrypted, 'wrong-password')).rejects.toThrow();
        });
        ...
        it('잘못된 암호화 데이터로 복호화 시 오류를 발생시켜야 함', async () => {
            ...
            await expect(cryptoManager.decrypt(invalidData, testPassword)).rejects.toThrow();
        });
    });
    ...
    describe('성능 테스트', () => {
        it('10회 암복호화가 합리적인 시간 내에 완료되어야 함', async () => {
            ...
            for (let i = 0; i < 10; i++) {
                const encrypted = await cryptoManager.encrypt(`test-value-${i}`, testPassword);
                const decrypted = await cryptoManager.decrypt(encrypted, testPassword);
