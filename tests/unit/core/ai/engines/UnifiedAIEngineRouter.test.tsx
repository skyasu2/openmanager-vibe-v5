...
describe('UnifiedAIEngineRouter 통합 테스트', () => {
    ...
    describe('🎯 모드별 처리', () => {
        ...
        test('LOCAL 모드 처리', async () => {
            const result = await router.processQuery({ ...mockRequest, mode: 'LOCAL' });
            expect(result.mode).toBe('LOCAL');
            expect(result.success).toBeDefined();
        });

        test('GOOGLE_AI 모드 처리', async () => {
            const result = await router.processQuery({ ...mockRequest, mode: 'GOOGLE_AI' });
            expect(result.mode).toBe('GOOGLE_AI');
            expect(result.success).toBeDefined();
        });

        test('AUTO 모드 처리', async () => {
            const result = await router.processQuery({ ...mockRequest, mode: 'AUTO' });
