...
describe('🟢 TDD Green Phase: SupabaseTimeSeriesManager 기본 구현', () => {
    ...
    test('🟢 PASS: 시계열 데이터를 조회할 수 있어야 함', async () => {
        ...
        mockBuilder.eq = vi.fn().mockResolvedValue({ data: mockRecords, error: null });
    ...
    test('🟢 PASS: 세션별 메트릭 히스토리를 조회할 수 있어야 함', async () => {
        ...
        // Mock 체인 설정
        const mockBuilder = createMockQueryBuilder();
        mockBuilder.order = vi.fn().mockResolvedValue({ data: mockRecords, error: null });
        mockSupabase.from = vi.fn().mockReturnValue(mockBuilder);
    ...
    test('🟢 PASS: TTL 기반 만료 데이터를 정리할 수 있어야 함', async () => {
        const mockBuilder = createMockQueryBuilder();
        mockBuilder.lte = vi.fn().mockResolvedValue({ data: [1, 2, 3], error: null });
        mockSupabase.from = vi.fn().mockReturnValue(mockBuilder);
    ...
    test('🟢 PASS: 빈 세션에 대해 에러를 발생시켜야 함', async () => {
        ...
        await expect(manager.calculateSessionAggregates(sessionId))
    ...
    test('🟢 PASS: 시계열 데이터 조회 시 에러 처리', async () => {
        ...
        await expect(manager.queryTimeSeriesData({ sessionId }))
            .rejects.toThrow('시계열 데이터 조회 실패: Database connection failed');
    });
});
...
describe('🔄 TDD Refactor Phase: 고급 기능 및 최적화', () => {
    ...
    test('🔄 REFACTOR: 대용량 데이터 아카이빙', async () => {
        ...
        mockSelectBuilder.lte = vi.fn().mockResolvedValue({ data: mockArchiveData, error: null });

        // Delete Mock  
        const mockDeleteBuilder = createMockQueryBuilder();
        mockDeleteBuilder.lte = vi.fn().mockResolvedValue({ data: mockArchiveData, error: null });
