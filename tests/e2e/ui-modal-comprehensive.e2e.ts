...
test.describe('🎭 UI 모달 전수 동작 확인 테스트', () => {
  ...
  test('통합 설정 모달 전체 동작 테스트', async ({ page }) => {
    await test.step('프로필 드롭다운에서 통합 설정 열기', async () => {
      ...
      // 통합 설정 메뉴 클릭
      const unifiedSettingsItem = page.locator(
        '[data-testid="unified-settings-item"]'
      );
      await unifiedSettingsItem.click();

      // 통합 설정 모달 확인
      const unifiedModal = page.locator(
    ...
    await test.step('통합 설정 모달 컨텐츠 확인', async () => {
      ...
      const securityContent = page.locator(
        '[data-testid="security-settings-content"]'
      );
  ...
  test('모달 스택(중첩) 테스트', async ({ page }) => {
    await test.step('모달 위에 모달 열기 테스트', async () => {
      ...
      if (await helpButton.isVisible()) {
        ...
        // 맨 위 모달 닫기
        const helpCloseButton = page.locator(
  ...
  test('모달 접근성 테스트', async ({ page }) => {
    await test.step('키보드 네비게이션 테스트', async () => {
      ...
      const firstFocusableElement = page
        .locator(
          '[data-testid="ai-agent-modal"] button, [data-testid="ai-agent-modal"] input'
        )
        .first();
