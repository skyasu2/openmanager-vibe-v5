...
async function testDashboardElements(page) {
  ...
  await page.screenshot({
    path: 'test-results/dashboard-after-system-start.png',
    fullPage: true,
  });
...
test.describe('서버 모니터링 대시보드 심화 테스트', () => {
  ...
  test('시스템 시작 후 대시보드 접근 테스트', async ({ page }) => {
    ...
    if (
      (await systemStartButton.count()) > 0 &&
      ...
      if (
        currentUrl.includes('/system-boot') ||
      ...
      console.log(
        'ℹ️ 시스템 시작 버튼이 비활성화되어 있음 - 대시보드 직접 접근 테스트'
      );
      await page.goto('/dashboard');
      await page.waitForLoadState('networkidle');
      await testDashboardElements(page);
  ...
  test('대시보드 UI 요소 상세 검증', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/dashboard-ui-detailed.png',
      fullPage: true,
    });
      ...
      if (count > 0) {
        ...
        if (await firstElement.isVisible()) {
          ...
          await firstElement.screenshot({
            path: `test-results/monitoring-element-${selector.replace(/[^a-zA-Z0-9]/g, '_')}.png`,
          });
        } else {
  ...
  test('실시간 데이터 업데이트 모니터링', async ({ page }) => {
    ...
    if (initialCount > 0) {
      ...
      console.log(
        `🔄 변경된 메트릭 수: ${changedCount}/${initialValues.length}`
      );
      ...
      if (changedCount > 0) {
        ...
        console.log(
          'ℹ️ 실시간 데이터 업데이트가 감지되지 않음 (Mock 데이터일 수 있음)'
        );
      }
    } else {
  ...
  test('대시보드 인터랙션 테스트', async ({ page }) => {
    ...
    const clickableElements = page.locator(
      'button, [role="button"], .clickable, [data-testid*="card"]'
    );
    ...
    if (clickableCount > 0) {
      for (let i = 0; i < Math.min(clickableCount, 3); i++) {
        ...
        if ((await element.isVisible()) && (await element.isEnabled())) {
          ...
          // 클릭 전 스크린샷
          await page.screenshot({
            path: `test-results/before-click-${i + 1}.png`,
          });
          ...
          // 클릭 후 스크린샷
          await page.screenshot({
            path: `test-results/after-click-${i + 1}.png`,
          });
  ...
  test('다양한 해상도에서 대시보드 레이아웃 테스트', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      // 스크린샷 캡처 (fullPage 제거로 타임아웃 방지)
      await page.screenshot({
        path: `test-results/dashboard-${viewport.name.toLowerCase()}.png`,
      });

      // 기본 요소들이 여전히 표시되는지 확인
      const essentialElements = page.locator(
        'header, main, .dashboard, [role="main"]'
      );
