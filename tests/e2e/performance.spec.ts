    ...
    test('리소스 로딩 성능 분석', async ({ page }) => {
      ...
      Object.entries(resourcesByType).forEach(
        ([type, resources]: [string, any[]]) => {
          ...
          console.log(
            `   ${type}: ${resources.length}개, 평균 ${avgDuration.toFixed(2)}ms, 총 ${(totalSize / 1024).toFixed(2)}KB`
          );
        }
      );

      const slowResources = performanceEntries.filter(
        (entry) => entry.duration > 3000
      );
    ...
    test('JavaScript 실행 성능 측정', async ({ page }) => {
      ...
      if (jsMetrics.longTasks.length > 0) {
        const avgLongTaskDuration =
          jsMetrics.longTasks.reduce(
            (sum: number, task: any) => sum + task.duration,
            0
          ) / jsMetrics.longTasks.length;
        console.log(
          `   평균 Long Task 지속시간: ${avgLongTaskDuration.toFixed(2)}ms`
        );
    ...
    test('스크롤 성능 측정', async ({ page }) => {
      ...
      if (scrollMetrics && scrollMetrics.totalFrames > 0) {
        const frameDropPercentage =
          (scrollMetrics.frameDrops / scrollMetrics.totalFrames) * 100;
