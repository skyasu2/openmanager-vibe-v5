...
test.describe('프론트엔드 성능 종합 검증', () => {
  ...
  test('Core Web Vitals 및 성능 메트릭 측정', async ({ page }) => {
    ...
    console.log('\n📊 === Core Web Vitals 결과 ===');
    console.log(
      `🎯 LCP (Largest Contentful Paint): ${metrics.lcp}ms ${metrics.lcp <= 2500 ? '✅' : '⚠️'}`
    );
    console.log(
      `🚀 FCP (First Contentful Paint): ${metrics.fcp}ms ${metrics.fcp <= 1800 ? '✅' : '⚠️'}`
    );
    console.log(
      `📐 CLS (Cumulative Layout Shift): ${metrics.cls.toFixed(3)} ${metrics.cls <= 0.1 ? '✅' : '⚠️'}`
    );
    console.log(
      `⚡ TTFB (Time to First Byte): ${metrics.ttfb}ms ${metrics.ttfb <= 800 ? '✅' : '⚠️'}`
    );
    ...
    if (metrics.memory) {
      ...
      console.log(
        `📊 메모리 사용률: ${Math.round((metrics.memory.usedJSHeapSize / metrics.memory.totalJSHeapSize) * 100)}%`
      );
    }
    ...
    const networkStats = await page.evaluate(() => {
      ...
      resources.forEach((resource) => {
        ...
        if (resource.name.includes('.js')) networkStats.js++;
          ...
          (resource as any).initiatorType === 'fetch'
        )
          networkStats.xhr++;
    ...
    console.log(
      `📊 성능 점수: ${passedTests}/${totalTests} (${scorePercentage}%)`
    );
    console.log(
      `${scorePercentage >= 80 ? '🟢' : scorePercentage >= 60 ? '🟡' : '🔴'} 전체적인 성능 평가: ${
        scorePercentage >= 80
          ? '우수'
          : scorePercentage >= 60
            ? '보통'
  ...
  test('반응형 및 뷰포트 성능 확인', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      console.log(
        `  📏 가로 스크롤: ${isResponsive.hasOverflowX ? '⚠️ 있음' : '✅ 없음'}`
      );
      console.log(
        `  📏 세로 스크롤: ${isResponsive.hasVerticalScroll ? '✅ 있음' : '✅ 없음'}`
      );
      console.log(
        `  📐 콘텐츠 크기: ${isResponsive.bodyWidth}x${isResponsive.bodyHeight}px`
      );

      // 가로 스크롤이 있으면 반응형 디자인에 문제가 있을 수 있음
      if (isResponsive.hasOverflowX) {
        console.log(
          `  ⚠️ 주의: 가로 스크롤이 발생했습니다 (${isResponsive.bodyWidth}px > ${isResponsive.viewportWidth}px)`
        );
      }
    }
