...
import { GET as getThresholds, POST as updateThresholds } from '@/app/api/admin/thresholds/route';
import { GET as getDashboardConfig, POST as updateDashboardConfig } from '@/app/api/admin/dashboard-config/route';
import { GET as getBackupStatus, POST as manageBackup } from '@/app/api/admin/backup-status/route';
...
describe('🔐 Admin API 인증 테스트', () => {
  ...
  describe('인증 없이 접근 시 401 반환', () => {
    it('GET /api/admin/thresholds - 인증 헤더 없이 접근 시 401', async () => {
      const request = new NextRequest('http://localhost:3000/api/admin/thresholds');
    ...
    it.skip('GET /api/admin/dashboard-config - 인증 헤더 없이 접근 시 401 (현재 GET은 인증 불필요)', async () => {
      const request = new NextRequest('http://localhost:3000/api/admin/dashboard-config');
    ...
    it('GET /api/admin/backup-status - 인증 헤더 없이 접근 시 401', async () => {
      const request = new NextRequest('http://localhost:3000/api/admin/backup-status');
