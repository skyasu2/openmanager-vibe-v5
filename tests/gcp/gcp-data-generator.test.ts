...
describe('🌐 GCP 서버 데이터 생성기', () => {
  ...
  describe('⚡ 실시간 메트릭 생성', () => {
    test('TimeSeriesMetrics 구조가 기존 AI 엔진 호환성을 유지해야 함', async () => {
      ...
      metrics.forEach(metric => {
        ...
        expect(metric.system.network.connections.active).toBeGreaterThanOrEqual(
          0
        );
  ...
  describe('🔗 Vercel 연동', () => {
    ...
    test('세션이 없을 때 적절한 에러를 반환해야 함', async () => {
      ...
      // When
      const response = await generator.fetchMetricsForVercel(
        invalidSessionId,
        10
      );
  ...
  describe('💰 무료 티어 사용량 제한', () => {
    test('일일 세션 생성 제한이 적용되어야 함', async () => {
      ...
      // 제한 초과 시도
      await expect(sessionManager.startSession(userId)).rejects.toThrow(
        '일일 세션 생성 제한 초과'
      );
    });
  ...
  describe('🔧 확장성 및 유지보수성', () => {
    ...
    test('서버 타입 추가가 쉬워야 함', async () => {
      ...
      // Then
      const elasticsearchServer = dataset.servers.find(
        s => s.type === 'elasticsearch'
      );
      expect(elasticsearchServer).toBeDefined();
      expect(elasticsearchServer!.specs.memory.total).toBe(
        32 * 1024 * 1024 * 1024
      );
    });
  });
