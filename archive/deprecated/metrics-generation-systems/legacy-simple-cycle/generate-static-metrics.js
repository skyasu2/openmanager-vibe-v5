...
// --- Utils ---
function generate24HourData(serverId) {
  const data = [];
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    for (let min = 0; min < 60; min += 5) {
      ...
      data.push({
        ...
        disk: Math.max(0, Math.min(100, base.disk + ((hour * 0.5) % 20))), // Disk grows slowly then resets
        network: Math.max(
          0,
...
console.log('Generating 24h static metrics data (JS)...');
