  ...
  duplicateAnalysis.forEach(({ name, files }) => {
    ...
    files.forEach((file, index) => {
      if (fs.existsSync(file)) {
        ...
        const hasNamedExport = /export\s+(?:const|function|class)\s+\w+/.test(
          content
        );
        ...
        console.log(
          `      📦 Export: ${hasDefaultExport ? 'Default' : ''} ${hasNamedExport ? 'Named' : ''}`
        );
...
function analyzeUnusedAPIRoutes() {
  ...
  // 사용되지 않는 API 라우트
  const unusedAPIs = apiRoutes.filter(
    route => usageCount[`/api${route.path}`] === 0
  );
...
function analyzeUnusedUtilities() {
  ...
  const utilDirs = ['src/lib', 'src/utils', 'src/services', 'src/hooks'];
  ...
  utilDirs.forEach(dir => {
    ...
    files.forEach(file => {
      ...
      srcFiles.forEach(srcFile => {
        ...
        try {
          ...
          const patterns = [
            ...
            new RegExp(
              `import\\s+.*from\\s+['"\`].*${relativePath.replace(/\\/g, '/')}`,
              'g'
            ),
      ...
      console.log(
        `   ${isUnused ? '❌' : '✅'} ${relativePath} (${(stats.size / 1024).toFixed(1)}KB, 사용: ${usageFound}회)`
      );
    });
