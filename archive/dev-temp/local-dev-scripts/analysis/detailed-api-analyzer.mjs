...
class DetailedAPIAnalyzer {
  ...
  isTestOrDemo(endpoint, content) {
    const testPatterns = [/test/i, /demo/i, /example/i, /sample/i, /mock/i];
  ...
  generateDetailedReport() {
    ...
    console.log('🎯 주요 발견사항:');
    console.log(
      `🔹 436B 동일 크기 API: ${this.results.identical436B.length}개`
    );
    ...
    const uniqueCandidates = allCandidates
      .filter(
        (api, index, arr) => arr.findIndex(a => a.path === api.path) === index
      )
    ...
    uniqueCandidates.forEach((api, index) => {
      console.log(
        `${String(index + 1).padStart(2)}. ${api.endpoint.padEnd(40)} (${String(api.size).padStart(4)}B) - ${api.issues.join(', ')}`
      );
    });

    const totalSavings = uniqueCandidates.reduce(
      (sum, api) => sum + api.size,
      0
    );
    console.log();
    ...
    if (totalSavings >= 436 * 1024) {
      ...
      console.log(
        `📈 목표 대비: ${((totalSavings / (436 * 1024)) * 100).toFixed(1)}%`
      );
      console.log(
        '💡 추가 최적화가 필요합니다. 중복 로직 통합을 고려해보세요.'
      );
    }
    ...
    console.log('\n📈 상세 통계:');
    console.log(
      `평균 API 크기: ${(this.results.totalSize / this.results.total).toFixed(0)}B`
    );
    console.log(
      `가장 큰 API: ${Math.max(...this.results.verySmall.map(a => a.size))}B`
    );
    console.log(
      `가장 작은 API: ${Math.min(...this.results.verySmall.map(a => a.size))}B`
    );
  }
}
