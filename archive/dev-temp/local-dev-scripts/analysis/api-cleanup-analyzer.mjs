...
class APICleanupAnalyzer {
  ...
  generateReport() {
    ...
    cleanupCandidates.forEach((api, index) => {
      console.log(
        `${index + 1}. ${api.endpoint} (${api.size}B) - ${api.issues.join(', ')}`
      );
    });

    // 예상 절약량 계산
    const potentialSavings = cleanupCandidates.reduce(
    ...
    if (potentialSavings > 436 * 1024) {
      ...
      console.log(
        `⚠️  추가 분석 필요 (목표: 436KB, 현재: ${(potentialSavings / 1024).toFixed(2)}KB)`
      );
    }
  }
